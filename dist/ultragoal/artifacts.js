...
export function isUltragoalDone(plan) {
    ...
    if (plan.goals.length === 0)
        return true;
    if (plan.goals.some((goal) => goal.status === 'pending' ||
...
export function deriveGoalCandidates(brief) {
    ...
    const selected = objectives.length > 0
        ? objectives
...
function validateQualityGate(value) {
    ...
    if (!Array.isArray(verification.commands) ||
        ...
        verification.commands.some((command) => typeof command !== 'string' || command.trim() === '')) {
...
export async function checkpointUltragoal(cwd, options) {
    ...
    if (options.status === 'blocked') {
        ...
        const snapshot = options.claudeGoal === undefined
            ? null
        ...
        if (normalizeObjective(snapshot.objective) ===
    ...
    if (options.status === 'complete') {
        ...
        if (!reconciliation.ok) {
            ...
            if (completedTaskScopedAggregateSnapshot) {
                ...
                const taskScopedRequirement = aggregateMode &&
                    snapshot?.status === 'complete' &&
                    Boolean(snapshot.objective)
    ...
    const qualityGate = options.status === 'complete' &&
...
export async function recordFinalReviewBlockers(cwd, options) {
    ...
    const expectedObjective = expectedClaudeObjective(plan, goal);
    const aggregateMode = claudeGoalMode(plan) === 'aggregate';
    const reconciliation = reconcileClaudeGoalSnapshot(options.claudeGoal === undefined
        ? null
        : parseClaudeGoalSnapshot(options.claudeGoal), {
        expectedObjective,
