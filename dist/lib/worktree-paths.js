    ...
    if (omcStateDir) {
        try {
            ...
            for (const entry of entries) {
                ...
                if (name.startsWith('sibling-retrofit-warned-') &&
                    name.endsWith('.json')) {
                    try {
...
export function getOmcRoot(worktreeRoot) {
    ...
    if (customDir) {
        ...
        if (!dualDirWarnings.has(warningKey) &&
...
export function isPathUnderOmc(absolutePath, worktreeRoot) {
    ...
    return (normalizedPath.startsWith(normalizedOmc + sep) ||
 ...
 * fallback.
 */
export function findGitRootByFsWalk(startDir) {
    try {
...
export function resolveToWorktreeRoot(directory) {
    ...
    if (directory) {
        ...
        if (fsWalkRoot) {
            console.error('[worktree] git resolution failed; recovered repo root via filesystem walk-up', {
                directory: resolved,
    ...
    return (resolveRoot(process.cwd()) ||
