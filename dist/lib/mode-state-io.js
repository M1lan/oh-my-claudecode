...
function readEmergencyJournal(path) {
    try {
        ...
        if (journal.version !== 1 ||
            ...
            return null;
        const complete = typeof journal.originalDigest === 'string' &&
            (journal.intent === 'clear' ||
                (journal.intent === 'publish' &&
                    typeof journal.intendedDigest === 'string'));
...
function recoverDeadEmergencyStateFile(filePath, authorizeState) {
    ...
    if (journal.phase === 'preparing') {
        ...
        journal.phase = 'prepared';
        return (writeEmergencyJournal(journalPath, journal) &&
...
export function emergencyMutateStateFileIf(filePath, predicate, transform, recoveryOptions) {
    ...
    try {
        ...
        if (!owns() ||
            !captureAndUnlinkPrimary(filePath, quarantinePath, journal.originalDigest)) {
            removeOwnedEmergencyArtifacts(journalPath, journal, true);
            return false;
        }
        journal.phase = 'quarantined';
    ...
    if (sessionId) {
        for (const legacyPath of getLegacyStateCandidates(mode, baseDir)) {
            ...
            try {
                ...
                const result = clearStateFileLockedIf(legacyPath, (current) => canClearStateForSession(current, sessionId) &&
