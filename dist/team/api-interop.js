...
function parseTaskDelegationPlan(value) {
    ...
    if ('max_parallel_subtasks' in raw) {
        if (!isFiniteInteger(raw.max_parallel_subtasks) ||
    ...
    if ('spawn_before_serial_search_threshold' in raw) {
        if (!isFiniteInteger(raw.spawn_before_serial_search_threshold) ||
    ...
    if ('subtask_candidates' in raw) {
        if (!Array.isArray(raw.subtask_candidates) ||
...
function readTeamStateRootFromEnv(env = process.env) {
    const candidate = typeof env.OMC_TEAM_STATE_ROOT === 'string' &&
...
export function resolveTeamApiCliCommand(env = process.env) {
    const hasOmcContext = (typeof env.OMC_TEAM_WORKER === 'string' &&
        env.OMC_TEAM_WORKER.trim() !== '') ||
        (typeof env.OMC_TEAM_STATE_ROOT === 'string' &&
            env.OMC_TEAM_STATE_ROOT.trim() !== '');
    if (hasOmcContext)
        return 'omc team api';
    const hasOmxContext = (typeof env.OMX_TEAM_WORKER === 'string' &&
...
async function executeTeamCleanupViaRuntime(teamName, cwd) {
    ...
    if (isLegacyRuntimeConfig(config)) {
        ...
        const sessionName = typeof legacyConfig.tmuxSession === 'string' &&
            legacyConfig.tmuxSession.trim() !== ''
            ? legacyConfig.tmuxSession.trim()
            : `omc-team-${teamName}`;
        const leaderPaneId = typeof legacyConfig.leaderPaneId === 'string' &&
...
export function resolveTeamApiOperation(name) {
    ...
    return TEAM_API_OPERATIONS.includes(normalized)
        ? normalized
        : null;
}
    ...
    try {
        ...
        switch (operation) {
            ...
            case 'claim-task': {
                ...
                if (rawExpectedVersion !== undefined &&
            ...
            case 'transition-task-status': {
                ...
                const result = await teamTransitionTaskStatus(teamName, taskId, from, to, claimToken, cwd, {
                    result: typeof transitionResult === 'string'
                        ? transitionResult
                        : undefined,
            ...
            case 'update-worker-heartbeat': {
                ...
                await teamUpdateWorkerHeartbeat(teamName, worker, {
                    pid,
