...
export async function startMergeOrchestrator(config) {
    ...
    const service = config.serviceGeneration === undefined ||
    ...
    function persistState() {
        const payload = {
            lastShas: Object.fromEntries(Array.from(workers.values()).map((w) => [
                w.workerName,
                w.lastObservedSha,
            ])),
            ...(service ? { service } : {}),
    ...
    async function attemptMergeForWorker(entry) {
        await mutex(async () => {
            ...
            if (conflicts.length > 0) {
                ...
                try {
                    mergeBaseSha = execFileSync('git', ['merge-base', config.leaderBranch, entry.workerBranch], {
                        cwd: mergerPath,
                        encoding: 'utf-8',
                        stdio: 'pipe',
                        windowsHide: true,
            ...
            if (!result.success) {
                ...
                const message = formatMergeConflictForLeader({
                    ...
                    workerBranch: entry.workerBranch,
                    leaderBranch: config.leaderBranch,
                    conflictingFiles: result.conflicts.length > 0
                        ? result.conflicts
