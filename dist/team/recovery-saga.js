...
export async function runRecoverySaga(input, deps) {
    ...
    if (persisted.manifestSync !== 'synced') {
        return finalize({
            ...failure(input, 'config_commit_failed', 'Replacement config committed but manifest projection requires repair.'),
            outcome: 'commit_unknown',
        }, tasks.length ? 'reserved' : 'none', tasks.length ? 'pending' : 'not_started');
    }
    ...
    let continuations = [];
    if (tasks.length) {
        const adopted = await deps.adoptAll(input, {
            recoveryId: input.recoveryId,
            requestId: input.requestId,
