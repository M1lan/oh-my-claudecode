...
function isWorkerInfo(value) {
    ...
    return ((value.role === undefined || typeof value.role === 'string') &&
        ...
        (value.worker_cli === undefined ||
            ['claude', 'codex', 'gemini', 'cursor', 'grok', 'antigravity'].includes(value.worker_cli)) &&
        ...
        (value.operational_state === undefined ||
            ['starting', 'active', 'dead', 'stopped'].includes(value.operational_state)) &&
...
function isScaleUpAttempt(value) {
    return (isRecord(value) &&
        ...
        ['reserved', 'effects', 'failed'].includes(value.phase) &&
        isSafeCounter(value.pid) &&
        value.pid > 0 &&
        isNonEmptyString(value.process_started_at) &&
        isSafeCounter(value.state_revision) &&
...
function isScaleDownAttempt(value) {
    return (isRecord(value) &&
        isNonEmptyString(value.operation_id) &&
        ['draining', 'effects', 'failed'].includes(value.phase) &&
        isSafeCounter(value.pid) &&
        value.pid > 0 &&
        isNonEmptyString(value.process_started_at) &&
        Array.isArray(value.workers) &&
        value.workers.every((worker) => isRecord(worker) &&
...
function isTeamConfig(value, requireRevision, expectedTeamName) {
    ...
    if (!requireRevision && Object.hasOwn(value, 'state_revision'))
        return false;
    return ((value.lifecycle_state === undefined ||
        ['active', 'shutting_down', 'stopped'].includes(value.lifecycle_state)) &&
...
function isOptionalWorkspaceShape(value) {
    return ((value.leader_cwd === undefined || typeof value.leader_cwd === 'string') &&
        ...
        (value.workspace_mode === undefined ||
            ['single', 'worktree'].includes(value.workspace_mode)) &&
        (value.worktree_mode === undefined ||
            ['disabled', 'detached', 'named'].includes(value.worktree_mode)) &&
        (value.lifecycle_profile === undefined ||
            ['default', 'linked_ralph'].includes(value.lifecycle_profile)));
}
function isOptionalPaneShape(value) {
    return ((value.leader_pane_id === undefined ||
...
async function assertPersistedConfigPathBinding(teamName, cwd, includeManifestWhenAbsent = false) {
    ...
    if (manifestState.kind === 'value' &&
        !validateLegacyTeamConfig(configFromManifest(normalizeTeamManifest(manifestState.value)), teamName)) {
        throw new Error('invalid_persisted_state');
    }
}
export async function readTeamConfig(teamName, cwd) {
    ...
    if (manifestState.kind === 'invalid')
        throw new Error('invalid_persisted_state');
    const manifest = manifestState.kind === 'value'
...
export async function readTeamPhaseState(teamName, cwd) {
    ...
    try {
        ...
        return {
            ...
            max_fix_attempts: typeof parsed.max_fix_attempts === 'number'
                ? parsed.max_fix_attempts
                : 3,
            current_fix_attempt: typeof parsed.current_fix_attempt === 'number'
...
export function diffSnapshots(prev, current) {
    ...
    for (const [workerName, currentAlive] of Object.entries(current.workerAliveByName)) {
        ...
        const currentLiveness = current.workerLivenessByName?.[workerName] ??
