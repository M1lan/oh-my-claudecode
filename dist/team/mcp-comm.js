...
import { invokeDirectMailboxEffect, verifyTeamTargetOwnership, } from './rmux-session.js';
...
function mailboxResultMatchesAttempt(read, params) {
    if (read.kind !== 'valid' && read.kind !== 'replay_suppressed')
        return false;
    return (read.message.message_id === params.messageId &&
...
async function readMailboxMarkerState(params, deps) {
    try {
        ...
        const dispatchMatches = dispatch.kind === 'valid' &&
        ...
        return {
            ...
            dispatchMarked: dispatchMatches &&
...
async function writeAndVerifyMailboxMarkers(params, deps) {
    ...
    if (!before.mailboxMarked) {
        writes.push(deps
            .markMailbox(params.teamName, params.recipient, params.messageId, params.cwd)
            .catch(() => false));
    }
...
export async function runMailboxNotificationAttempt(params, overrides = {}) {
    if (!isExactText(params.teamName) ||
...
export async function queueInboxInstruction(params) {
    ...
    }, params.triggerMessage, { request: queued.request })).catch((error) => ({
        ok: false,
...
export async function queueDirectMailboxMessage(params) {
    ...
    }, params.triggerMessage, { request: queued.request, message_id: message.message_id })).catch((error) => ({
        ok: false,
        transport: fallbackTransportForPreference(params.transportPreference),
        reason: notifyExceptionReason(error),
    }));
...
export async function queueBroadcastMailboxMessage(params) {
    ...
    for (const recipient of persistedRecipients) {
        ...
        }, recipient.triggerMessage, { request: queued.request, message_id: recipient.message.message_id })).catch((error) => ({
            ok: false,
            transport: fallbackTransportForPreference(params.transportPreference),
            reason: notifyExceptionReason(error),
        }));
        ...
        if (isConfirmedNotification(outcome)) {
            ...
            await markDispatchRequestNotified(params.teamName, queued.request.request_id, {
                message_id: recipient.message.message_id,
