...
import { sanitizeName, getWorkerLiveness, killWorkerPanes, buildWorkerStartCommand, waitForPaneReady, } from './rmux-session.js';
...
export async function scaleUpOwned(teamName, count, agentType, tasks, cwd, env = process.env) {
    ...
    return await withScalingLock(sanitized, leaderCwd, async () => {
        ...
        if (config.lifecycle_state === 'shutting_down' ||
        ...
        const rollbackScaleUp = async (error, paneId, orphanFailure) => {
            ...
            if (reservedWorkerNames.size > 0) {
                ...
                if (persisted &&
                    ...
                    for (const row of reservedRows) {
                        ...
                        const launchMatches = JSON.stringify(row.launch_descriptor) ===
            ...
            if (cleanupFailures.length > 0) {
                ...
                await writeAtomic(evidencePath, JSON.stringify({
                    schema_version: 1,
                    team_name: sanitized,
                    error,
        ...
        for (let i = 0; i < count; i++) {
            ...
            let skipped = 0;
            while (config.workers.some((w) => w.name === `worker-${nextIndex}`) &&
                skipped < maxSkip) {
                nextIndex++;
                skipped++;
            }
            ...
            try {
                ...
                try {
                    ...
                    const primaryReason = primaryError instanceof Error
                ...
                config = {
                    ...config,
                    workers: config.workers.map((candidate, index) => index === reservedIndex ? workerInfo : candidate),
...
export async function scaleDownOwned(teamName, cwd, options = {}, env = process.env) {
    ...
    return await withScalingLock(sanitized, leaderCwd, async () => {
        ...
        const identitiesMatch = (workers, expected) => JSON.stringify(workers.map(workerIdentity)) ===
            ...
            config = await withProcessIdentityFileLock(lifecycleLock, async () => {
                ...
                const next = {
                    ...
                    active_scale_down: {
                        ...
                        pid: process.pid,
                        process_started_at: processStartedAt,
                        workers: selected.map(workerIdentity),
                        state_revision: nextRevision,
                        created_at: now,
                        updated_at: now,
                    },
        ...
        const reserveEffects = async () => withProcessIdentityFileLock(lifecycleLock, async () => {
            ...
            if (!current ||
                ...
                current.config.active_recovery ||
                scaleUpAttempt(current.config) ||
                !identitiesMatch(selectedNames
                    .map((name) => current.config.workers.find((worker) => worker.name === name))
                    .filter(Boolean), reservation.workers))
                return false;
            const nextRevision = current.stateRevision + 1;
            const next = {
                ...current.config,
        ...
        const unaddressableWorkers = targetWorkers
            .filter((worker) => typeof worker.pane_id !== 'string' ||
        ...
        const liveness = await Promise.all(targetWorkers.map(async (w) => w.pane_id
