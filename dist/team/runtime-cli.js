...
export function classifyAllDeadRecoveryEvidence(refresh, workers, hasOutstanding) {
    if (!hasOutstanding)
        return 'clear';
    if (!refresh.allWorkerPaneIdsKnown ||
...
export async function updateAllDeadRecoveryGrace(teamName, cwd, evidence, nowMs = Date.now()) {
    for (let attempt = 0; attempt < 3; attempt++) {
        ...
        if ((evidence === 'alive' || evidence === 'clear') &&
...
function canonicalRecoveryAdmissionEntryId(name, path) {
    ...
    if (name !== `${requestId}.pending.json` ||
...
export function hasPendingRecoveryAdmissionBeforeDeadline(teamName, cwd, deadlineAt) {
    ...
    for (const name of names) {
        ...
        try {
            ...
            if (!reservation)
                throw new Error('invalid_persisted_state');
            if (reservation.team_name !== teamName ||
                reservation.workspace_hash !== workspaceHash ||
...
export async function runPersistentRecoveryOwnerLoop(input, options = {}) {
    ...
    while (options.shouldContinue?.(iteration) ?? true) {
        ...
        if (afterOwner?.epoch !== fence.epoch ||
            afterOwner?.nonce !== fence.nonce ||
            (input.bootstrap &&
                (afterOwner?.pid !== input.bootstrap.pid ||
                    afterOwner?.process_started_at !== input.bootstrap.processStartedAt ||
...
async function main() {
        ...
        while (pollActive) {
            ...
            const leaderGuidance = deriveTeamLeaderGuidance({
                ...
                workers: {
                    ...
                    idle: snap.workers.filter((worker) => worker.alive &&
                        (worker.status.state === 'idle' ||
                            worker.status.state === 'done')).length,
            ...
            const v2Observed = snap.tasks.pending +
    ...
    while (pollActive) {
        ...
        const allWorkersDead = runtime.workerPaneIds.length > 0 &&
...
export async function runRecoveryOwnerFromEnvironment() {
    ...
    if (!Number.isSafeInteger(expectedEpoch) ||
        expectedEpoch < 1 ||
