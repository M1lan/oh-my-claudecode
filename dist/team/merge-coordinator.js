...
export function checkMergeConflicts(workerBranch, baseBranch, repoRoot) {
    ...
    try {
        execFileSync('git', ['merge-tree', '--write-tree', baseBranch, workerBranch], {
            cwd: repoRoot,
            encoding: 'utf-8',
    ...
    const mergeBase = execFileSync('git', ['merge-base', baseBranch, workerBranch], {
        cwd: repoRoot,
        encoding: 'utf-8',
        stdio: ['pipe', 'pipe', 'pipe'],
        windowsHide: true,
    }).trim();
    const baseDiff = execFileSync('git', ['diff', '--name-only', mergeBase, baseBranch], {
        cwd: repoRoot,
        encoding: 'utf-8',
        stdio: ['pipe', 'pipe', 'pipe'],
        windowsHide: true,
    }).trim();
    const workerDiff = execFileSync('git', ['diff', '--name-only', mergeBase, workerBranch], {
        cwd: repoRoot,
        encoding: 'utf-8',
        stdio: ['pipe', 'pipe', 'pipe'],
        windowsHide: true,
