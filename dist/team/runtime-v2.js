...
import { createTeamSession, spawnWorkerInPane, sendToWorker, killTeamSession, waitForPaneReady, paneHasActiveTask, paneLooksReady, applyMainVerticalLayout, getWorkerLiveness, captureTeamPane, sendTeamPaneKey, splitTeamWorkerPane, splitTeamWorkerPaneWithEvidence, } from './rmux-session.js';
...
export async function reconcileCommittedTeamServices(config, cwd) {
    ...
    if (descriptor.cadence_policy !== 'worker-auto-commit-v1' ||
    ...
    try {
        ...
        for (const context of expectedContexts) {
            ...
            registerTeamCadence(config.name, context, installedCadence.method === 'fallback-poll'
        ...
        return cadenceRemovalsConverged &&
            ...
            [...expectedWorkers].every((workerName) => local.registeredWorkers.has(workerName))
            ? 'synced'
            : 'repair_required';
    }
    catch {
...
export function resolveTaskAssignment(task, resolvedRouting, roleRoutingConfig, resolvedBinaryPaths, fallbackAgent) {
    ...
    if (!hasExplicitRole && !hasConfigForRole) {
        if (fallbackAgent === 'cursor' &&
            !CURSOR_EXECUTOR_TEAM_ROLES.includes(canonical)) {
...
async function spawnV2Worker(opts) {
    ...
    const dispatchOutcome = await queueInboxInstruction({
        ...
        fallbackAllowed: DEFAULT_TEAM_TRANSPORT_POLICY.dispatch_mode ===
...
function persistRecoveryFinal(input, recoveryId, result) {
    ...
    writeRecoveryFinal(input.cwd, {
        ...
        adoption: succeeded && result.requeuedTaskIds.length > 0
            ? 'adopted'
            : 'not_started',
...
export async function finalizeRecoveryOwnerResult(input, recoveryId, result, deps = {
    ...
    if (transientFailure) {
        ...
        if (pending?.config.active_recovery?.recovery_id === recoveryId) {
            const phase = result.outcome === 'recovered' &&
                result.activation === 'services_pending'
                ? 'services_pending'
    ...
    if (terminal &&
        ...
        if (!saved || !published) {
            return {
                ...recoveryError(input, recoveryId, 'stale_state_revision', 'Recovery reached a terminal state, but config cleanup could not be verified.'),
                outcome: 'commit_unknown',
            };
        }
        return published;
    }
    const withLock = deps.withConfigLock ??
        (async (_teamName, _cwd, fn) => fn());
    return withLock(input.teamName, input.cwd, async () => {
        const verified = await deps.readRevisionedConfig(input.teamName, input.cwd);
        const expectedPhase = result.outcome === 'recovered' || result.outcome === 'already_running'
            ? 'adopted'
            : 'failed';
        const verifiedLast = verified?.config.last_recovery;
        if (verified &&
        ...
        return {
            ...recoveryError(input, recoveryId, 'stale_state_revision', 'Recovery terminal state is no longer the active or last revision-checked attempt.'),
            outcome: 'commit_unknown',
        };
    });
}
async function finalizeBoundRecoveryOwnerTerminal(input, recoveryId, result) {
    try {
        ...
        if (active?.request_id === input.requestId &&
            active.recovery_id === recoveryId &&
            active.worker_name === input.workerName) {
    ...
    return {
        ...recoveryError(input, recoveryId, 'stale_state_revision', 'Recovery terminal cleanup could not prove the exact active attempt.'),
        outcome: 'commit_unknown',
    };
}
...
async function hasBootstrapRecoveryEvidence(teamName, cwd, input, waitOptions = {}) {
    ...
    try {
        ...
        if (intent.request_id !== input.requestId ||
...
async function isExactDeadOrphanBootstrapCandidate(teamName, cwd, input, config, orphan) {
    ...
    for (;;) {
        ...
        else if (candidate.request_id !== input.requestId ||
            candidate.recovery_id !== bootstrap.recoveryId ||
        ...
        const candidatePredecessor = candidate.predecessor_epoch === 0
            ? null
            : {
...
function isExactRecoverySidecar(value, task, input, active, replacementGeneration, adoptionToken) {
    ...
    if (!sidecar ||
        ...
        return false;
    const sameReservation = persisted.recovery_id === sidecar.recovery_id &&
    ...
    if (!sameReservation)
        return false;
    if ('adoption_token_hash' in persisted &&
...
async function hasBootstrapActiveRecoveryEvidence(teamName, cwd, input, config) {
    ...
    const replacementGeneration = worker?.recovery_id === active.recovery_id &&
    ...
    const untouchedClaims = tasks.filter((task) => task.status === 'in_progress' &&
        task.owner === input.workerName &&
...
async function ensureRecoveryOwner(teamName, cwd, input, waitOptions) {
    ...
    if (bootstrap) {
        if (bootstrap.expectedEpoch !== bootstrap.predecessorEpoch + 1 ||
        ...
        const alreadyPublished = predecessor?.epoch === bootstrap.expectedEpoch &&
            predecessor.pid === bootstrap.pid &&
            predecessor.process_started_at === bootstrap.processStartedAt &&
            predecessor.nonce === bootstrap.nonce;
        exactDeadOrphan =
            !alreadyPublished &&
                (await isExactDeadOrphanBootstrapCandidate(teamName, cwd, input, current.config, predecessor));
        if (alreadyPublished) {
            ...
            const retryFromNoOwner = bootstrap.predecessorEpoch === 0 &&
            ...
            if (owner.epoch !== bootstrap.expectedEpoch ||
                owner.pid !== bootstrap.pid ||
                owner.process_started_at !== bootstrap.processStartedAt ||
                owner.nonce !== bootstrap.nonce) {
                throw new Error('runtime_owner_bootstrap_fence_lost');
    ...
    for (let bindAttempt = 0; bindAttempt < 3 &&
        (current.config.runtime_owner_epoch?.epoch !== owner.epoch ||
            current.config.runtime_owner_epoch?.nonce !== owner.nonce); bindAttempt++) {
        if (current.config.runtime_owner_epoch &&
            (current.config.runtime_owner_epoch.epoch !== owner.epoch ||
                current.config.runtime_owner_epoch.nonce !== owner.nonce) &&
            !(bootstrap &&
                exactDeadOrphan &&
                (await isExactDeadOrphanBootstrapCandidate(teamName, cwd, input, current.config, bootstrapPredecessor)))) {
            throw new Error('runtime_owner_bootstrap_rebind_rejected');
        }
        if (bootstrap &&
            current.config.active_recovery &&
...
export async function executeRecoverDeadWorkerV2Owner(input) {
    ...
    try {
        ...
        if (beforeOwner?.config.active_scale_down ||
        ...
        if (owner.config.lifecycle_state === 'shutting_down' ||
        ...
        const replacementGeneration = existingAttempt &&
            worker.recovery_id === recoveryId &&
        ...
        const deps = {
            ...
            spawnGatedPane: async (sagaInput) => {
                ...
                if (!split.paneId) {
                    await recordUnaddressableRecoveryPaneFailure(input, sagaInput.recoveryId, paneAttemptId, split.commandSucceeded
                        ? 'unaddressable_spawned_pane'
                        : 'split_command_uncertain', split);
                    return { ok: false, error: 'spawn_failed' };
                }
            ...
            persistActive: async (sagaInput, paneId) => {
                ...
                if (!(await saveTeamConfigAtRevision(next, current.stateRevision, input.cwd)))
                    throw new Error('stale_state_revision');
                const manifestSync = await resolveCommittedRecoveryManifestSync(() => readTeamManifest(input.teamName, input.cwd), {
                    workerName: sagaInput.workerName,
                    paneId,
                    paneAttemptId: pending.paneAttemptId,
                    recoveryId: sagaInput.recoveryId,
            ...
            writeRun: async (sagaInput, paneAttemptId, continuations) => {
                ...
                const instruction = continuations.length > 0
                    ? continuations
                        .map((continuation) => renderRecoveryContinuationInstruction({
                        teamName: input.teamName,
                        workerName: sagaInput.workerName,
...
export async function startTeamV2(config) {
    ...
    for (let i = 0; i < workerNames.length; i++) {
        ...
        const outputFile = taskIndex !== undefined &&
            assignment.role &&
            ...
            : undefined;
        const outputContract = outputFile && assignment.role
...
export async function monitorTeamV2(teamName, cwd) {
    ...
    for (const { worker: w, alive, liveness, status, heartbeat, paneCapture, } of workerSignals) {
        ...
        const expectedTaskId = status.current_task_id ??
            outstandingTask?.id ??
            w.assigned_tasks[0] ??
            '';
    ...
    const allTasksTerminal = taskCounts.pending === 0 &&
    ...
    await emitMonitorDerivedEvents(sanitized, allTasks, workers.map((w) => ({
        name: w.name,
        alive: w.alive,
        liveness: w.liveness,
        status: w.status,
    })), previousSnapshot, cwd);
...
export async function shutdownTeamV2(teamName, cwd, options = {}) {
    ...
    const revalidateShutdownFence = async () => withProcessIdentityFileLock(lifecycleLock, async () => {
        ...
        if (!current ||
            !['shutting_down', 'stopped'].includes(current.config.lifecycle_state ?? '') ||
            current.config.active_recovery ||
            current.config.active_scale_up) {
    ...
    try {
        const { killWorkerPanes, killTeamSession, resolveSplitPaneWorkerPaneIds, getWorkerLiveness, } = await import('./rmux-session.js');
