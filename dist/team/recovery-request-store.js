...
export function reserveRecoveryRequest(cwd, requestId, payload, recoveryId = randomUUID()) {
    ...
    try {
        return {
            kind: 'created',
            reservation: publishImmutable(reservationPath(cwd, requestId), reservation),
        };
    }
    catch (error) {
...
export function aliasActiveRecoveryRequest(cwd, requestId, payload, active) {
    ...
    const payloadHash = canonicalRecoveryPayloadHash(payload);
    if (active.operation !== payload.operation ||
...
export function writeRecoveryPhase(cwd, phase) {
    ...
    return publishImmutable(join(phaseDirectory(cwd, phase.request_id), sequence), {
        ...phase,
        schema_version: 1,
        kind: 'phase',
...
function isValidRecoveryResult(value) {
    ...
    if (result.outcome === 'recovered' || result.outcome === 'already_running') {
        if (result.committed !== true ||
            ...
            !Object.values(result.continuationSequenceByTask).every((sequence) => typeof sequence === 'number' &&
    ...
    return ((result.outcome === 'failed' || result.outcome === 'commit_unknown') &&
        result.committed === false &&
        typeof result.error === 'string' &&
        RECOVERY_ERRORS.has(result.error) &&
...
export function isMatchingRecoveryFinal(outcome, expected = {}) {
    if (!outcome ||
        ...
        return false;
    const succeeded = outcome.result.outcome === 'recovered' ||
    ...
    return (outcome.continuation === 'none' &&
