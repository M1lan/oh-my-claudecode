...
import { rmuxExec, rmuxExecAsync, rmuxShell, rmuxCmdAsync, isTmuxCompatibleMultiplexerAvailable, } from '../cli/rmux-utils.js';
import { configureTmuxClipboardForSession, configureTmuxClipboardForSessionAsync, } from '../cli/rmux-clipboard.js';
...
function parseCmuxResourceIds(output, collectionName) {
    ...
    const entries = Array.isArray(parsed)
        ...
        : parsed &&
            ...
            Array.isArray(parsed[collectionName])
...
export async function verifyTeamTargetOwnership(target, dependencies = defaultMailboxTargetOwnershipDependencies) {
    ...
    try {
        const panes = parseCmuxResourceIds((await dependencies.cmuxExec([
            '--json',
            'list-panes',
            '--workspace',
            workspace,
    ...
    try {
        ...
        return notified
            ? {
                ...
                reason: target.recipientRole === 'leader'
...
async function verifyWorkerStartCommandSubmitted(paneId, startCmd, opts = {}) {
    ...
    const maxPollIntervalMs = Number.isFinite(opts.maxPollIntervalMs) && (opts.maxPollIntervalMs ?? 0) > 0
        ...
        : 500;
    let pollIntervalMs = Number.isFinite(opts.initialPollIntervalMs) &&
...
export function validateTmux(hasTmuxContext = false) {
    ...
    try {
        ...
        throw new Error('No tmux-compatible multiplexer is available. Install rmux (preferred, POSIX) or tmux:\n' +
...
export async function spawnWorkerInPane(sessionName, paneId, config) {
    ...
    try {
        ...
        const sendResult = await rmuxExecAsync(['send-keys', '-t', paneId, '-l', startCmd], { timeout: 5000 });
    ...
    try {
        const enterResult = await rmuxExecAsync(['send-keys', '-t', paneId, 'Enter'], { timeout: 5000 });
...
export async function waitForPaneReady(paneId, opts = {}) {
    ...
    console.warn(`[rmux-session] waitForPaneReady: pane ${paneId} timed out after ${timeoutMs}ms ` +
...
export async function sendToWorker(_sessionName, paneId, message) {
    if (message.length > 200) {
        console.warn(`[rmux-session] sendToWorker: message rejected (${message.length} chars exceeds 200 char limit)`);
        return false;
    }
    try {
...
export async function killTeamSession(sessionName, workerPaneIds, leaderPaneId, options = {}) {
    ...
    if (process.env.OMC_TEAM_ALLOW_KILL_CURRENT_SESSION !== '1' &&
        process.env.TMUX) {
        try {
...
//# sourceMappingURL=rmux-session.js.map