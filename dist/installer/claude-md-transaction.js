...
function importRanges(content) {
    ...
    for (let index = 0; index + 2 < lines.length; index += 1) {
        if (lines[index].text === CLAUDE_MD_IMPORT_START &&
...
function mergeForOverwrite(existing, canonical, version) {
    ...
    return {
        content: cleaned.content.length === 0
            ? managed
            : `${managed}\n<!-- User customizations -->\n${cleaned.content}`,
...
export function executeClaudeMdTransaction(request) {
    ...
    try {
        ...
        if (request.mode === 'local')
            ...
            const mainContent = mainIsAlreadyOwned && mainBytes !== undefined
                ? mainBytes
                : Buffer.from(`${preserve.content}${preserve.content.length ? '\n\n' : ''}${CLAUDE_MD_IMPORT_BLOCK}`, 'utf8');
            operations.push({
                path: companion,
                type: 'write',
        ...
        const states = new Map(effectiveOperations.map((operation) => [
            operation.path,
            {
                path: operation.path,
                existedBefore: operation.existedBefore,
