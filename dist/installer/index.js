...
for (const record of HISTORICAL_AGENT_OWNERSHIP) {
    if (!isValidHistoricalAgent(record))
        continue;
    const hashes = HISTORICAL_AGENT_HASHES_BY_FILENAME.get(record.filename) ??
        new Set();
...
function configureInstallerSettings(baseSettings, context) {
    let settings = { ...baseSettings };
    {
        ...
        const shouldConfigureSettingsHooks = (!context.runningAsPlugin || !!context.allowPluginHookRefresh) &&
            !pluginHandlesHooks;
...
function getActiveAgentFiles() {
    ...
    if (pluginRootResolution.mode === 'unknown' ||
    ...
    for (const root of roots) {
        if (pluginRootResolution.mode === 'plugin' &&
...
const REQUIRED_PLUGIN_COMMAND_FILES = ['commands/omc-setup.md'];
...
function validateDeclaredPluginSkills(root, manifest) {
    ...
    for (const declaredSkill of declaredSkills) {
        if (typeof declaredSkill !== 'string' ||
...
function isCacheInstalledPluginRoot(root) {
    ...
    return (canonicalRoot === canonicalCacheBase ||
...
export function syncInstalledPluginPayload() {
    const targetRoots = getInstalledOmcPluginRoots().filter((root) => existsSync(root) && isCacheInstalledPluginRoot(root));
    if (targetRoots.length === 0) {
...
export function hasPluginProvidedAgentFiles() {
    return getInstalledOmcPluginRoots().some((pluginRoot) => hasCompletePluginPayload(pluginRoot) &&
    ...
    try {
        ...
        for (const candidate of [settings.enabledPlugins, settings.plugins]) {
            if (Array.isArray(candidate)) {
                if (candidate.some((plugin) => typeof plugin === 'string' &&
                    plugin.toLowerCase().includes('oh-my-claudecode'))) {
                    return true;
                }
            }
            else if (candidate && typeof candidate === 'object') {
                if (Object.entries(candidate).some(([pluginId, value]) => pluginId.toLowerCase().includes('oh-my-claudecode') &&
                    value !== false)) {
                    return true;
                }
            }
...
function syncBundledSkillDefinitions(log, options) {
    ...
    for (const entry of readdirSync(skillsDir, { withFileTypes: true })) {
        if (!entry.isDirectory())
            continue;
        if (SKININTHEGAMEBROS_ONLY_SKILLS.has(entry.name) &&
...
export function syncPersistedSetupVersion(options) {
    ...
    const isConfigured = typeof config.setupCompleted === 'string' ||
    ...
    const normalizedVersion = (() => {
        const candidate = detectedVersion && detectedVersion !== 'unknown'
            ? detectedVersion
            : VERSION;
...
export function mergeClaudeMd(existingContent, omcContent, version) {
    ...
    if (sourceMarkers.state === 'corrupt' ||
...
export function install(options = {}) {
    ...
    const shouldInstallBundledSkills = !pluginDirMode &&
    ...
    try {
        ...
        if ((!projectScoped || shouldInstallBundledSkills) &&
        ...
        if (!projectScoped) {
            ...
            log(transaction.operations.some((operation) => operation.type === 'write' &&
