...
export function getReplyConfig() {
    ...
    return {
        enabled: true,
        pollIntervalMs: parseIntSafe(process.env.OMC_REPLY_POLL_INTERVAL_MS) ??
            replyRaw?.pollIntervalMs ??
            3000,
        maxMessageLength: replyRaw?.maxMessageLength ?? 500,
        rateLimitPerMinute: parseIntSafe(process.env.OMC_REPLY_RATE_LIMIT) ??
            replyRaw?.rateLimitPerMinute ??
            10,
        includePrefix: process.env.OMC_REPLY_INCLUDE_PREFIX !== 'false' &&
