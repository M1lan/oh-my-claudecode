...
export function getCurrentTmuxSession() {
    ...
    try {
        ...
        if (paneId) {
            const lines = rmuxShell("list-panes -a -F '#{pane_id} #{session_name}'", {
                timeout: 3000,
        ...
        const sessionName = rmuxShell("display-message -p '#S'", {
            timeout: 3000,
            stdio: ['pipe', 'pipe', 'pipe'],
        }).trim();
...
export function getCurrentTmuxPaneId() {
    ...
    try {
        const paneId = rmuxShell("display-message -p '#{pane_id}'", {
            timeout: 3000,
            stdio: ['pipe', 'pipe', 'pipe'],
        }).trim();
