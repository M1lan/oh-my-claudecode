...
export function parseSkillPipelineMetadata(frontmatter) {
    ...
    const nextSkillArgs = stripOptionalQuotes(frontmatter['next-skill-args'] ?? '').trim() ||
        undefined;
    ...
    const handoffPolicy = stripOptionalQuotes(frontmatter['handoff-policy'] ?? '')
        .trim()
        .toLowerCase();
    const handoffRequiresApproval = handoffPolicy === 'approval-required' ||
