...
import { killWorkerPanes, killTeamSession, getWorkerLiveness, } from '../team/rmux-session.js';
...
function buildCliReplacement(toolName, args) {
    ...
    if (toolName === 'omc_run_team_wait') {
        const timeoutMs = typeof parsed.timeout_ms === 'number' &&
...
const startSchema = z.object({
    ...
    agentTypes: z
        .array(z.string())
        .describe('Agent type per worker: "claude", "codex", "gemini", or "antigravity"'),
    tasks: z
        .array(z.object({
    ...
    newWindow: z
        .boolean()
        .optional()
        .describe('Spawn workers in a dedicated tmux window instead of splitting the current window'),
});
...
const waitSchema = z.object({
    ...
    nudge_delay_ms: z
        .number()
        .optional()
        .describe('Milliseconds a pane must be idle before nudging (default: 30000)'),
    nudge_max_count: z
        .number()
        .optional()
    ...
    nudge_message: z
        .string()
        .optional()
        .describe('Message sent as nudge (default: "Continue working on your assigned task and report concrete progress (not ACK-only).")'),
});
...
if (process.env.OMC_TEAM_SERVER_DISABLE_AUTOSTART !== '1' &&
