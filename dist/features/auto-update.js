...
function pnpmExecOptions(verbose = false) {
    return {
        encoding: 'utf-8',
...
function assertSafePackageSpec(packageSpec) {
    if (!/^[A-Za-z0-9@._~+/-]+$/.test(packageSpec)) {
        throw new Error(`Unsafe package spec: ${packageSpec}`);
    }
}
function pnpmAddGlobalPackage(packageSpec, verbose = false) {
    assertSafePackageSpec(packageSpec);
    if (process.platform === 'win32') {
        execSync(`pnpm add -g ${packageSpec}`, pnpmExecOptions(verbose));
        return;
    }
    execFileSync('pnpm', ['add', '-g', packageSpec], pnpmExecOptions(verbose));
}
...
function detectClaudeCodeFromBinary(pnpmRoot) {
    try {
        ...
        if (!version && !binaryPath) {
            return {
                status: 'unknown',
                error: 'claude --version returned no parseable version and binary path could not be resolved',
            };
        }
        const normalizedBinaryPath = binaryPath?.replace(/\\/g, '/').toLowerCase();
        const normalizedPnpmRoot = pnpmRoot?.replace(/\\/g, '/').toLowerCase();
        const isGlobalBinary = Boolean(normalizedBinaryPath &&
            normalizedPnpmRoot &&
...
function restoreGlobalClaudeCodeIfNeeded(beforeUpdate, verbose = false) {
    ...
    if (verbose) {
        console.log(`[omc update] Restored global ${CLAUDE_CODE_PACKAGE}`);
    }
...
function syncMarketplaceClone(verbose = false) {
    ...
    let statusOutput = '';
    try {
        statusOutput = String(execFileSync('git', [
            '-C',
            marketplacePath,
            'status',
            '--porcelain',
    ...
    let behindCount = 0;
    try {
        const revListOutput = String(execFileSync('git', [
            '-C',
            marketplacePath,
            'rev-list',
            '--left-right',
    ...
    try {
        ...
        for (const [pluginId, entriesValue] of Object.entries(plugins)) {
            ...
            for (const entry of entriesValue) {
                ...
                const existingInstallPath = typeof pluginEntry.installPath === 'string'
...
export function syncPluginCache(verbose = false) {
    ...
    try {
        ...
        const version = typeof packageMetadata.version === 'string'
...
export function getInstalledVersion() {
    if (!existsSync(VERSION_FILE)) {
        ...
        try {
            // Check if we can find the package in the global store
...
export async function performUpdate(options) {
    ...
    try {
        ...
        try {
            ...
            throw new Error('Auto-update via pnpm failed. Please run manually:\n' +
