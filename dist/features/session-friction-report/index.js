...
function countToolUse(content) {
    if (!Array.isArray(content))
        return 0;
    return content.filter((block) => block &&
        typeof block === 'object' &&
        block.type === 'tool_use').length;
}
...
async function scanJsonlTarget(target, sessions, filters) {
    ...
    try {
        for await (const rawLine of reader) {
            ...
            const role = typeof record.message?.role ===
                'string'
                ? record.message.role
...
export async function generateSessionFrictionReport(rawOptions = {}) {
    ...
    const sessionReports = Array.from(sessions.values())
        .map((stats) => {
        if (stats.estimatedContextPercent === null &&
        ...
        .slice(0, limit);
    const criticalSignals = sessionReports.reduce((count, session) => count +
        session.signals.filter((signal) => signal.severity === 'critical').length, 0);
    const warningSignals = sessionReports.reduce((count, session) => count +
        session.signals.filter((signal) => signal.severity === 'warn').length, 0);
    return {
