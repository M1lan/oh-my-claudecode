...
export function isPaneAlive(paneId) {
    ...
    try {
        const result = rmuxExec(['display-message', '-t', paneId, '-p', '#{pane_dead}'], { stripTmux: true, stdio: 'pipe', timeout: 3000 });
...
export function capturePaneContent(paneId, lines = 15) {
    ...
    try {
        ...
        const result = rmuxExec(['capture-pane', '-t', paneId, '-p', '-S', `-${safeLines}`], {
            stripTmux: true,
            timeout: 5000,
        });
        return result;
...
export function analyzePaneContent(content) {
    ...
    const hasClaudeCode = hasClaudeText ||
...
//# sourceMappingURL=rmux-detector.js.map