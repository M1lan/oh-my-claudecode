...
export function getPaneHistorySize(paneId) {
    try {
        const raw = rmuxExec(['display-message', '-t', paneId, '-p', '#{pane_dead} #{history_size}'], { stripTmux: true, timeout: 3000 }).trim();
...
function capturePaneLines(paneId, lines) {
    try {
        ...
        return rmuxExec(['capture-pane', '-t', paneId, '-p', '-S', `-${safeLines}`], { stripTmux: true, timeout: 5000 });
    }
    catch {
        return '';
    }
