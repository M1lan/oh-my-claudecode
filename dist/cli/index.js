...
program
    ...
    .option('--yolo', 'Start both OMC and OMX in yolo mode (Claude --dangerously-skip-permissions, Codex --dangerously-bypass-approvals-and-sandbox)')
    .addHelpText('after', `
Requirements:
...
Examples:
  ...
  $ omc config --paths           Show config file locations
    ...
    if (options.validate) {
        ...
        if (config.mcpServers?.exa?.enabled &&
...
Examples:
  ...
  $ OMC_NOTIFY_PROFILE=work claude`)
    ...
    if (profiles[name]) {
        ...
        console.log(chalk.yellow(`Profile "${name}" not found.`));
        console.log(chalk.gray('Create it with: omc config-stop-callback <type> --profile ' +
            name +
            ' --enable ...'));
    }
});
...
doctorCmd
    .command('rmux')
    .description('Detect the rmux multiplexer and optionally install it (POSIX; non-fatal)')
    .option('--json', 'Output as JSON')
    .option('--install', 'Install rmux when missing (local source > cargo binstall > cargo; falls back to tmux)')
    .addHelpText('after', `
Examples:
