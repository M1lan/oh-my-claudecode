 ...
 */
export function resolveRmuxInvocation(env = process.env) {
    if (process.platform === 'win32')
        return null;
    const program = env.TMUX_PROGRAM;
    const tmux = env.TMUX;
    const looksLikeRmux = env.TERM_PROGRAM === 'rmux' ||
        (typeof program === 'string' && program.includes('rmux-shim'));
    if (!looksLikeRmux || !program || !tmux)
        return null;
...
function resolveRmuxBinaryPath() {
    if (process.platform === 'win32')
        return null;
    if (cachedRmuxBinaryPath !== undefined)
        return cachedRmuxBinaryPath;
    let resolved = null;
    try {
...
export function __resetRmuxBinaryPathCache() {
...
export function rmuxExec(args, opts) {
...
export async function rmuxExecAsync(args, opts) {
...
export function rmuxShell(command, opts) {
    const { stripTmux: _, ...execOpts } = opts ?? {};
...
export async function rmuxShellAsync(command, opts) {
    const { stripTmux: _, timeout, ...rest } = opts ?? {};
...
export function rmuxSpawn(args, opts) {
...
export async function rmuxCmdAsync(args, opts) {
 ...
 * tmux/psmux probe.
 */
export function isTmuxCompatibleMultiplexerAvailable() {
 ...
 */
export function buildRmuxSessionName(cwd) {
 ...
 */
export function sanitizeRmuxToken(value) {
    const cleaned = value
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '');
 ...
 */
export function parseRmuxPaneSnapshot(output) {
    return output
        .split('\n')
        .map((line) => line.trim())
        .filter(Boolean)
 ...
 */
export function killRmuxPane(paneId) {
...
//# sourceMappingURL=rmux-utils.js.map