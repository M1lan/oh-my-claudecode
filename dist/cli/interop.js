...
import { isTmuxAvailable, isClaudeAvailable, rmuxExec, buildRmuxShellCommandWithEnv, wrapWithLoginShell, killRmuxPane, } from './rmux-utils.js';
...
export function validateInteropRuntimeFlags(flags) {
    if (!flags.enabled && flags.mode !== 'off') {
        return {
            ok: false,
            reason: 'OMX_OMC_INTEROP_MODE must be "off" when OMX_OMC_INTEROP_ENABLED=0.',
        };
    }
 ...
 */
export function sendInteropCavemanActivation(paneId, activation = INTEROP_CAVEMAN_ACTIVATION) {
    try {
 ...
 */
export function launchInteropSession(cwd = process.cwd(), options = {}) {
    const yolo = Boolean(options.yolo);
    const flags = readInteropRuntimeFlags();
    const flagCheck = validateInteropRuntimeFlags(flags);
    console.log(`[interop] mode=${flags.mode}, enabled=${flags.enabled ? '1' : '0'}, tools=${flags.omcInteropToolsEnabled ? '1' : '0'}, failClosed=${flags.failClosed ? '1' : '0'}, yolo=${yolo ? '1' : '0'}`);
    if (yolo) {
        console.warn('[interop] --yolo: launching Claude with --dangerously-skip-permissions and Codex with --dangerously-bypass-approvals-and-sandbox.');
    }
    if (!flagCheck.ok) {
    ...
    try {
        if (hasCodex) {
            ...
            const codexCommand = wrapWithLoginShell(buildRmuxShellCommandWithEnv('codex', yolo ? [CODEX_YOLO_FLAG] : [], {
            ...
            if (yolo && codexPaneId)
                sendInteropCavemanActivation(codexPaneId);
    ...
    if (result.error) {
        ...
        if (codexPaneId)
            killRmuxPane(codexPaneId);
        console.error('Error launching claude:', result.error instanceof Error
