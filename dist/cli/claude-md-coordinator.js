...
const COMPILED_ENGINE_VERSION = typeof __OMC_COORDINATOR_ENGINE_VERSION__ === 'string'
    ? __OMC_COORDINATOR_ENGINE_VERSION__
    : '';
const COMPILED_SOURCE_SHA256 = typeof __OMC_COORDINATOR_SOURCE_SHA256__ === 'string'
...
export function runClaudeMdCoordinatorHandshake() {
    if (!COMPILED_ENGINE_VERSION || !COMPILED_SOURCE_SHA256) {
        return {
            exitCode: 2,
            response: coordinatorError(2, 'Coordinator build handshake is unavailable'),
        };
    }
    return {
        exitCode: 0,
...
export function runClaudeMdCoordinator(input) {
    try {
        ...
        const { mode } = input;
        if (input.schemaVersion !== CLAUDE_MD_COORDINATOR_SCHEMA_VERSION ||
        ...
        const sourceSha256 = createHash('sha256')
            .update(source.bytes)
            .digest('hex');
        if (sourceSha256 !== COMPILED_SOURCE_SHA256 ||
        ...
        return {
            exitCode: 3,
            response: coordinatorError(3, `Coordinator I/O validation failed: ${error instanceof Error ? error.message : String(error)}`),
        };
    }
}
function main() {
