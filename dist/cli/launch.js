...
import { resolveLaunchPolicy, buildRmuxSessionName, buildRmuxShellCommand, buildRmuxShellCommandWithEnv, isNativeWindowsShell, wrapWithLoginShell, isClaudeAvailable, isTmuxAvailable, quoteShellArg, rmuxExec, } from './rmux-utils.js';
import { configureTmuxClipboardForCurrentSession, configureTmuxClipboardForSession, } from './rmux-clipboard.js';
...
function hasOmcMarkers(path) {
    ...
    return (content.includes('<!-- OMC:START -->') &&
...
function hasLinkableCredentials(inspection) {
    ...
    const source = isJsonObject(inspection.parsed.claudeAiOauth)
        ...
        : inspection.parsed;
    return (typeof source.accessToken === 'string' &&
...
function accountsProvenSame(sourceClaudeJson, runtimeClaudeJson) {
    ...
    if (!sourceAccount || !runtimeAccount)
        return false;
    const sourceUuid = typeof sourceAccount.accountUuid === 'string'
        ? sourceAccount.accountUuid.trim()
        : '';
    const runtimeUuid = typeof runtimeAccount.accountUuid === 'string'
...
function refreshRuntimeClaudeJson(baseConfigDir, runtimeClaudeJsonPath, sourceClaudeJson = readJsonObject(join(dirname(baseConfigDir), '.claude.json'))) {
    ...
    let changed = false;
    if (sourceClaudeJson.hasCompletedOnboarding === true &&
...
function reconcileRuntimeCredentials(baseConfigDir, runtimeCredentialsPath, sourceClaudeJson, preservedRuntimeClaudeJson) {
    ...
    if (hasOwn(baseInspection.parsed, 'claudeAiOauth') ||
...
function runClaudeOutsideTmux(cwd, args, _sessionId, options = {}) {
    const forwardedEnv = Object.fromEntries(TMUX_ENV_FORWARD.map((name) => [name, process.env[name]]).filter(([, value]) => value !== undefined));
