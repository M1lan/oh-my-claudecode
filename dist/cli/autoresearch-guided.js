...
import { buildRmuxShellCommand, buildRmuxShellCommandWithEnv, isTmuxAvailable, quoteShellArg, rmuxExec, wrapWithLoginShell, } from './rmux-utils.js';
...
export async function runAutoresearchNoviceBridge(repoRoot, seedInputs = {}, io = createQuestionIO()) {
    ...
    try {
        while (true) {
            ...
            evaluatorCommand = await promptWithDefault(io, '\nEvaluator command (leave placeholder to refine further; must output {pass:boolean, score?:number} JSON before launch)', evaluatorCommand ||
...
export function spawnAutoresearchTmux(missionDir, slug) {
    ...
    rmuxExec(['new-session', '-d', '-s', sessionName, '-c', repoRoot, wrappedCommand], { stripTmux: true, stdio: 'ignore' });
    try {
...
export function spawnAutoresearchSetupTmux(repoRoot) {
    ...
    const claudeCommand = buildRmuxShellCommandWithEnv('claude', [CLAUDE_BYPASS_FLAG], { CODEX_HOME: codexHome });
    ...
    assertTmuxSessionAvailable(sessionName);
    if (paneId) {
        rmuxExec(['send-keys', '-t', paneId, '-l', buildAutoresearchSetupSlashCommand()], { stripTmux: true, stdio: 'ignore' });
