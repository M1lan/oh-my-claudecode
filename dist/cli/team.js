...
const VALID_CLI_AGENT_TYPES = new Set([
    'claude',
    'codex',
    'gemini',
    'cursor',
...
async function assertTeamSpawnAllowed(cwd, env = process.env) {
    ...
    for (const activeTeam of activeTeams) {
        ...
        if (governance.one_team_per_leader_session ??
...
function convergeWithResultArtifact(jobId, job, jobsDir) {
    try {
        ...
        if (artifactParsed?.status === 'completed' ||
...
export async function executeTeamApiOperation(operation, input, cwd = process.cwd()) {
    ...
    if (!canonicalOperation ||
        !SUPPORTED_API_OPERATIONS.has(canonicalOperation)) {
        return {
            ok: false,
            operation,
            error: {
    ...
    const normalizedInput = {
        ...
        ...(typeof input.claimToken === 'string' &&
            ...
            : {}),
        ...(typeof input.taskVersion === 'number' &&
            input.task_version === undefined
            ? { task_version: input.taskVersion }
            : {}),
        ...(typeof input.resumePayload !== 'undefined' &&
...
function parseStartArgs(args) {
    ...
    const resolvedTeamName = teamName && teamName.trim()
        ? teamName.trim()
