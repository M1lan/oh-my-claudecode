...
function isEnforceableNamedAutopilotState(state, directory, sessionId) {
    ...
    try {
        return (resolveToWorktreeRoot(state.project_path) ===
...
function isAuthenticatedAutopilotCancelSignal(signal, target) {
    if (signal.active !== true ||
        signal.mode !== 'autopilot' ||
        typeof signal.source !== 'string' ||
        signal.source.length === 0) {
        return false;
    }
    const now = Date.now();
    const requestedAt = typeof signal.requested_at === 'string'
        ? new Date(signal.requested_at).getTime()
        : NaN;
    const expiresAt = typeof signal.expires_at === 'string'
...
function isSessionCancelInProgress(directory, sessionId) {
    ...
    const validateSignal = (target) => {
        const locked = withStateFileMutationLock(cancelSignalPath, () => {
            ...
            const now = Date.now();
            const requestedAt = typeof raw.requested_at === 'string'
            ...
            if (!Number.isFinite(requestedAt) ||
                ...
                effectiveExpiry <= now) {
                if (Number.isFinite(effectiveExpiry) &&
                    effectiveExpiry <= now &&
                    existsSync(cancelSignalPath))
                    unlinkSync(cancelSignalPath);
                return false;
    ...
    const locked = withStateFileMutationLock(autopilotPath, () => {
        ...
        if (!current ||
            !isCurrentAutopilotTarget(current.state, directory, sessionId)) {
            return {
                autopilotCancellation: false,
                nonAutopilotCancellation: validateSignal(null),
            };
        }
        ...
        if (hasNamedWorkflowMarkers(current.state) &&
            !isEnforceableNamedAutopilotState(current.state, directory, sessionId)) {
            return {
                autopilotCancellation: false,
...
function hasPendingScheduledWakeup(directory, sessionId) {
    ...
    return readPendingWakeupState(directory, sessionId).some((state) => {
        ...
        const dueAt = parseTimestamp(state.due_at ??
            state.wakeup_at ??
            state.scheduled_for ??
            state.deadline_at ??
            state.expires_at);
...
function readTranscriptTailLines(transcriptPath) {
    ...
    try {
        if (statSync(transcriptPath).size > TRANSCRIPT_TAIL_BYTES &&
            lines.length > 0) {
            lines.shift();
        }
    }
            ...
            if (block?.type === 'tool_use' && block.id && block.name) {
                ...
                if (REVIEWER_COMMAND_TOOL_NAMES.has(block.name)) {
                    const command = block.input
                        ?.command;
                    if (isCodexReviewerCommand(command) &&
...
function isCriticalContextStop(stopContext) {
    ...
    return (estimateTranscriptContextPercent(transcriptPath) >=
    ...
    if (verificationState?.pending) {
        ...
        if (sessionId) {
            ...
            if (checkArchitectApprovalInTranscript(sessionId, verificationState)) {
                if (verificationState.verification_scope === 'story' &&
...
function getAutoresearchDeadlineMs(state) {
    if (typeof state.deadline_at === 'string' &&
    ...
    if (typeof state.max_runtime_ms === 'number' &&
...
async function checkAutoresearch(sessionId, directory, cancelInProgress) {
    ...
    const hasTimestampFields = Boolean(stateRecord &&
        ['updated_at', 'started_at'].some((key) => typeof stateRecord[key] === 'string' &&
...
async function checkRalplan(sessionId, directory, cancelInProgress) {
    ...
    const stateRecord = state;
    const hasTimestampFields = Boolean(stateRecord &&
        ['last_checked_at', 'updated_at', 'started_at'].some((key) => typeof stateRecord[key] === 'string' &&
            String(stateRecord[key]).length > 0));
...
async function checkUltrawork(sessionId, directory, _hasIncompleteTodos, cancelInProgress) {
    ...
    if (hardMax > 0 && state.reinforcement_count >= hardMax) {
        ...
        return {
            shouldBlock: true,
            message: '[ULTRAWORK - HARD LIMIT] Reached hard max iterations (' +
                hardMax +
...
async function resolvePersistentModeBlock(sessionId, directory, stopContext) {
    ...
    const skipHooks = (process.env.OMC_SKIP_HOOKS ?? '')
        ...
        .map((s) => s.trim())
        .filter(Boolean);
    if (skipHooks.includes('persistent-mode') ||
    ...
    const runAutopilotPriority = async () => {
        if (tombstonedWorkflowModes.has('autopilot') ||
            !(cancelCheck.enforceableAutopilot ||
