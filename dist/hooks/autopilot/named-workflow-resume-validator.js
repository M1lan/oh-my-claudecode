...
function validBoundary(value, sessionId, root) {
    ...
    try {
        ...
        if (stat.dev !== identity.device ||
            ...
            return false;
        return (hashTranscriptRange(opened.fd, 0, boundary.byteOffset) ===
...
function scanTranscriptJsonl(fd, start, end, sessionId, callback, hash) {
    ...
    const emitRecord = (crlf) => {
        const length = recordBytes -
            (crlf && recordBytes > 0 && record[recordBytes - 1] === 0x0d ? 1 : 0);
        const line = decodeRecord(length);
        if (line === null)
            return false;
        return (!callback ||
            callback(line, byteOffset, lineNumber, createHash('sha256').update(record.subarray(0, length)).digest('hex')));
    };
    for (let offset = start; offset < end;) {
        const maxRead = recordBytes >= MAX_JSONL_RECORD_BYTES
            ? 1
        ...
        for (let index = 0; index < count; index += 1) {
            ...
            else if ((recordBytes === MAX_JSONL_RECORD_BYTES && byte !== 0x0d) ||
...
function readStableTranscript(path, sessionId, root) {
    ...
    try {
        ...
        return {
            ...
            hashRange: (start, end) => start >= 0 && end >= start && end <= size
                ? hashTranscriptRange(fd, start, end)
                : null,
            scanJsonl: (start, end, callback) => start >= 0 && end >= start && end <= size
...
function authenticatedObservation(observation, sessionId, root) {
    ...
    try {
        ...
        const scanned = transcript.scanJsonl(Number(boundary.byteOffset), Number(stable.size), (line, byteOffset, lineNumber, recordContentSha256) => {
            if (byteOffset !== observation.byteOffset ||
