...
export function updateAutopilotStateIfCurrent(directory, observed, update, sessionId) {
    ...
    if (hasNamedWorkflowMarkers(observed)) {
        ...
        if (!namedWorkflowRuntimeSupported()) {
            ...
            return emergencyMutateStateFileIf(stateFile, (current) => canonicalStateJson(Object.fromEntries(Object.entries(current).filter(([key]) => key !== '_meta'))) === observedSnapshot, (current) => ({ ...current, ...update }))
                ? readAutopilotState(directory, sessionId)
                : null;
        }
    }
    let updated = null;
    const result = writeStateFileLockedIf(stateFile, (current) => sameAutopilotRun(current, observed) &&
        (!hasNamedWorkflowMarkers(observed) ||
            Boolean(validateNamedWorkflowState(current, sessionId))), (current) => {
...
export function updateAutopilotStateIfExact(directory, observed, update, sessionId, validateCurrent) {
    ...
    if (!namedWorkflowRuntimeSupported()) {
        return emergencyMutateStateFileIf(stateFile, (current) => current.workflowRunId === observed.workflowRunId &&
            ...
            validateCurrent(current), (current) => ({ ...current, ...update }))
            ? readAutopilotState(directory, sessionId)
            : null;
    }
    let updated = null;
    const result = writeStateFileLockedIf(stateFile, (current) => namedResumeIdentity(current) ===
        namedResumeIdentity(observed) &&
        validateCurrent(current), (current) => {
        const next = { ...current, ...update };
        updated = next;
        return next;
    });
