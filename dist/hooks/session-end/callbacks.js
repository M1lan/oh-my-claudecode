...
function stringList(value) {
    return Array.isArray(value)
        ? value.filter((item) => item === 'file' || item === 'telegram' || item === 'discord')
        : [];
}
            ...
            switch (action.name) {
                ...
                case 'notification': {
                    ...
                    const result = await notify('session-end', {
                        ...
                        profileName: typeof action.payload.profileName === 'string'
                ...
                // acceptance can precede an unavailable response.
                case 'openclaw-wake': {
                    if (action.payload.enabled !== true ||
                    ...
                    const result = await wakeOpenClaw('session-end', {
                        sessionId: context.sessionId,
                        projectPath: context.directory,
                        reason: typeof action.payload.reason === 'string'
