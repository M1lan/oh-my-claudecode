...
function openClawRoutingSnapshot() {
    ...
    return Object.fromEntries(values.flatMap(([property, environment]) => process.env[environment] === undefined
        ? []
...
const ACTIONS = [
    ...
    ['team-cleanup', 'required'],
    ['python-cleanup', 'required'],
    ['reply-cleanup', 'required'],
    ['callback', 'best-effort'],
    ['notification', 'best-effort'],
    ['openclaw', 'best-effort'],
];
const TEST_PRODUCER_GRACE_ENV = 'OMC_SESSION_END_TEST_PRODUCER_GRACE_MS';
const PRODUCER_GRACE_MS = process.env.NODE_ENV === 'test' &&
    /^\d+$/.test(process.env[TEST_PRODUCER_GRACE_ENV] ?? '')
    ? Math.max(1, Number(process.env[TEST_PRODUCER_GRACE_ENV]))
    : 30_000;
const REQUIRED_ACTION_EXECUTION_MS = ACTIONS.filter(([, actionClass]) => actionClass === 'required').length *
    9_000;
const BEST_EFFORT_ACTION_EXECUTION_MS = ACTIONS.filter(([, actionClass]) => actionClass === 'best-effort').length *
    2_000;
const REQUIRED_ACTION_MAX_ATTEMPTS = 3;
const LOCK_WAIT = new Int32Array(new SharedArrayBuffer(4));
const DISCOVERY_FILE = 'discovery.json';
export function sessionEndJobPath(directory, sessionId) {
    validateSessionId(sessionId);
    return path.join(getOmcRoot(directory), 'state', 'session-end-jobs', `${sessionId}.json`);
}
export function sessionEndJobsDirectory(directory) {
...
function initialDeadlines() {
    ...
    return {
        ...
        bestEffortDeadlineAt: new Date(startedAt +
            PRODUCER_GRACE_MS +
            REQUIRED_ACTION_EXECUTION_MS +
            BEST_EFFORT_ACTION_EXECUTION_MS).toISOString(),
    };
}
            ...
            try {
                ...
                if (parsed.processStartIdentity && process.platform === 'linux') {
                    try {
                        ...
                        if ((stat.substring(stat.lastIndexOf(')') + 2).split(' ')[19] ??
    ...
    for (let attempt = 0; attempt < 250; attempt++) {
        try {
            if (fs.existsSync(reclaim))
                throw Object.assign(new Error('reclaim-in-progress'), {
                    code: 'EEXIST',
                });
            ...
            try {
                ...
                processStartIdentity =
                    stat.substring(stat.lastIndexOf(')') + 2).split(' ')[19] ?? null;
            }
            catch {
                /* platform fallback retains nonce/timestamp bounded stale reclaim */
            }
            ...
            if (attempt > 20 && lockIsReclaimable(lock)) {
                ...
                try {
                    ...
                    /* another contender owns reclamation */
                }
                finally {
                    if (reclaimFd !== undefined) {
                        ...
                        try {
                            ...
                            /* next contender observes absence */
                        }
                    }
                }
            }
...
function discoveryPath(directory) {
    return path.join(sessionEndJobsDirectory(directory), DISCOVERY_FILE);
}
        ...
        if (parsed.version === 2 && Array.isArray(parsed.tickets))
            return {
                ...
                tickets: parsed.tickets
                    .filter((ticket) => !!ticket && typeof ticket.sessionId === 'string')
                    .map((ticket) => ({
                    sessionId: ticket.sessionId,
                    attempts: Number.isInteger(ticket.attempts) ? ticket.attempts : 0,
                    retryAt: typeof ticket.retryAt === 'string' ? ticket.retryAt : nowIso(),
...
function updateTicket(directory, sessionId, present) {
    ...
    withLock(file, () => {
        ...
        index.cursor =
            index.tickets.length === 0 ? 0 : index.cursor % index.tickets.length;
        atomicWriteJsonSync(file, index);
    });
}
function newAction(name, actionClass, payload) {
    return {
        class: actionClass,
        phase: actionClass === 'required' ? 'deferred-required' : 'deferred-best-effort',
        status: 'pending',
        attempts: 0,
        idempotencyKey: digest([name, payload]),
        payload,
        budgetMs: actionClass === 'required' ? 9_000 : 2_000,
    };
}
/** Terminality is solely a manifest property. Discovery tickets are deliberately excluded. */
export function isManifestTerminal(job) {
    return (['sealed', 'no-op'].includes(job.producers.core.state) &&
        ['sealed', 'no-op'].includes(job.producers.wiki.state) &&
        Object.values(job.actions).every((action) => action.status === 'completed' || action.status === 'expired') &&
        job.owner === null &&
        Object.values(job.actions).every((action) => !action.runner || action.runner.phase === 'terminal'));
}
export function readSessionEndJob(directory, sessionId) {
    try {
        ...
        const result = withLock(jobPath, () => {
            ...
            if (isManifestTerminal(next) && next.phase !== 'complete') {
                ...
                next.completion = {
                    ...
                    terminalRevision: next.revision,
                };
            }
...
function mutateLatest(directory, sessionId, mutate) {
    for (let i = 0; i < 8; i++) {
        ...
        const result = withLock(jobPath, () => {
            ...
            if (existing) {
                if (existing.phase === 'complete' ||
                    existing.producers.core.state !== 'absent')
                    return existing;
                const next = {
                    ...existing,
                    producers: {
            ...
            const now = nowIso();
            const actions = Object.fromEntries(ACTIONS.map(([name, klass]) => [
                name,
...
export function sealCoreManifest(directory, sessionId) {
    return mutateLatest(directory, sessionId, (job) => {
        if (job.phase !== 'complete' && job.producers.core.state !== 'sealed') {
            job.producers.core = {
                ...job.producers.core,
...
export function sealWikiManifest(directory, sessionId, payload) {
    ...
    try {
        const result = withLock(jobPath, () => {
            ...
            if (existing.phase === 'complete' ||
                ['sealed', 'no-op'].includes(existing.producers.wiki.state))
                return existing;
            const now = nowIso();
            ...
            next.producers.wiki = payload
                ...
                : { state: 'no-op', sealedAt: now, sealedBy: 'wiki-producer' };
            if (payload)
...
export function claimSessionEndJob(directory, sessionId, nonce, identity, deadlineAt) {
    const current = readSessionEndJob(directory, sessionId);
    if (!current)
        return null;
    const now = Date.now();
    return mutateSessionEndJob(directory, sessionId, current.revision, (job) => {
        if (job.phase === 'complete' || job.owner)
        ...
        job.owner = {
            ...
            claimedAt: nowIso(),
            heartbeatAt: nowIso(),
            leaseExpiresAt: new Date(Math.min(now + 750, deadlineAt)).toISOString(),
            runDeadlineAt: new Date(deadlineAt).toISOString(),
            leaseGeneration: 1,
            claimedFromRevision: job.revision,
        };
        job.phase = 'processing';
    });
}
export function renewSessionEndLease(directory, sessionId, nonce, generation, deadlineAt) {
    const current = readSessionEndJob(directory, sessionId);
    if (!current)
        return null;
    return mutateSessionEndJob(directory, sessionId, current.revision, (job) => {
        const owner = job.owner;
        if (!owner || owner.nonce !== nonce || owner.leaseGeneration !== generation)
            throw new Error('lease-conflict');
        owner.leaseGeneration++;
        owner.heartbeatAt = nowIso();
        owner.leaseExpiresAt = new Date(Math.min(Date.now() + 750, deadlineAt)).toISOString();
    });
}
/** Reaping is intentionally separate from a new claim. Caller must establish dead or PID-reused identity. */
export function reapStaleSessionEndOwner(directory, sessionId, expectedNonce, expectedGeneration, liveness) {
    const current = readSessionEndJob(directory, sessionId);
    if (!current || Date.now() < Date.parse(current.owner?.leaseExpiresAt ?? ''))
        return null;
    return mutateSessionEndJob(directory, sessionId, current.revision, (job) => {
        if (!job.owner ||
            job.owner.nonce !== expectedNonce ||
            job.owner.leaseGeneration !== expectedGeneration ||
            Date.now() < Date.parse(job.owner.leaseExpiresAt) ||
            (liveness !== 'dead' && liveness !== 'mismatch'))
            throw new Error('reap-conflict');
        if (Object.values(job.actions).some((action) => action.status === 'claimed' &&
            action.runner &&
            action.runner.phase !== 'terminal' &&
            Date.now() < Date.parse(action.runner.deadlineAt)))
            throw new Error('runner-still-bounded');
...
export function releaseSessionEndJob(directory, sessionId, nonce, generation) {
    const current = readSessionEndJob(directory, sessionId);
    if (!current)
        return null;
    return mutateSessionEndJob(directory, sessionId, current.revision, (job) => {
        if (!job.owner ||
            job.owner.nonce !== nonce ||
            job.owner.leaseGeneration !== generation)
...
export function updateSessionEndJob(directory, sessionId, expectedOwner, mutate) {
    const current = readSessionEndJob(directory, sessionId);
    if (!current)
        return null;
    return mutateSessionEndJob(directory, sessionId, current.revision, (job) => {
        if (job.owner?.nonce !== expectedOwner || job.phase === 'complete')
            throw new Error('owner-conflict');
        mutate(job);
    });
}
export function claimSessionEndAction(directory, sessionId, ownerNonce, name, deadlineAt) {
    return updateSessionEndJob(directory, sessionId, ownerNonce, (job) => {
        const action = job.actions[name];
        if (!action ||
            !['pending', 'retryable'].includes(action.status) ||
        ...
        action.runner = {
            ...
            runnerNonce: randomUUID(),
            phase: 'reserved',
            deadlineAt: new Date(deadlineAt).toISOString(),
        };
    });
}
export function markSessionEndActionRunner(directory, sessionId, ownerNonce, name, runnerNonce, phase) {
    return updateSessionEndJob(directory, sessionId, ownerNonce, (job) => {
        const action = job.actions[name];
        if (action.status !== 'claimed' ||
            action.runner?.runnerNonce !== runnerNonce)
            throw new Error('runner-phase-conflict');
        action.runner.phase = phase;
    });
}
export function finishSessionEndAction(directory, sessionId, ownerNonce, name, runnerNonce, completed, code) {
    return updateSessionEndJob(directory, sessionId, ownerNonce, (job) => {
        const action = job.actions[name];
        if (action.status !== 'claimed' ||
            action.claimantNonce !== ownerNonce ||
...
export function claimSessionEndDiscoveryTickets(directory, limit = 4, leaseMs = 15_000) {
    try {
        ...
        const claimed = withLock(file, () => {
            ...
            index.cursor = index.tickets.length
                ? (index.cursor + Math.max(1, claimed.length)) % index.tickets.length
                : 0;
            atomicWriteJsonSync(file, index);
            return claimed;
        });
...
export function releaseSessionEndDiscoveryTicket(directory, sessionId, nonce, spawned) {
    try {
        ...
        withLock(file, () => {
            const index = readIndex(file);
            const ticket = index.tickets.find((item) => item.sessionId === sessionId);
            if (!ticket || ticket.claimNonce !== nonce)
                return;
            ticket.claimNonce = undefined;
            ticket.leaseExpiresAt = undefined;
            if (!spawned) {
                ticket.attempts++;
                ticket.retryAt = new Date(Date.now() +
                    Math.min(30_000, 250 * 2 ** Math.min(ticket.attempts, 7))).toISOString();
            }
            else
                ticket.retryAt = nowIso();
            atomicWriteJsonSync(file, index);
        });
    }
    catch {
        /* recovery retries lease expiry */
    }
}
export function acknowledgeSessionEndDiscoveryTicket(directory, sessionId) {
    try {
        const file = discoveryPath(directory);
        withLock(file, () => {
            const index = readIndex(file);
        ...
        /* terminal ticket remains safely rediscoverable */
    }
}
/** Compatibility read-only page retained for callers that do not claim tickets. */
export function takeSessionEndDiscoveryPage(directory, limit = 4) {
    return claimSessionEndDiscoveryTickets(directory, limit).map((ticket) => ticket.sessionId);
}
...
export function recoverPreparedCoreProducer(directory, sessionId) {
    return mutateLatest(directory, sessionId, (job) => {
        if (!['prepared', 'sealed'].includes(job.producers.core.state) ||
...
export function failClosedMissingCoreProducer(directory, sessionId) {
    return mutateLatest(directory, sessionId, (job) => {
        if (job.producers.core.state !== 'absent' ||
            !['sealed', 'no-op'].includes(job.producers.wiki.state) ||
            Date.now() < Date.parse(job.producerGraceExpiresAt))
            return;
        job.producers.core = {
