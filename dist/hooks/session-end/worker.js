...
export function spawnSessionEndWorker(payload) {
    try {
        const child = spawn(process.execPath, [fileURLToPath(import.meta.url), WORKER_ARG, JSON.stringify(payload)], {
            detached: true,
            stdio: 'ignore',
...
export async function executeSessionEndAction(name, payload, deadlineAt) {
    ...
    if (name === 'foreground-cleanup')
        return legacy
            .runForegroundSessionEndCleanup(payload.directory, payload.sessionId, false)
            .then(() => undefined);
    ...
    if (name === 'team-cleanup') {
        ...
        const result = await legacy.cleanupSessionOwnedTeams(payload.directory, payload.sessionId, Array.isArray(names)
    ...
    if (name === 'callback')
        return legacy.runSessionEndCallbacks(payload.directory, payload.sessionId, readSessionEndJob(payload.directory, payload.sessionId)?.actions.callback
            .idempotencyKey, true);
...
function reschedulePendingWorker(payload, job) {
    ...
    const hasPendingAction = producersReady &&
...
export async function processSessionEndWorker(payload) {
    ...
    let producerReady = Boolean(admitted &&
    ...
    try {
        for (const name of Object.keys(claimed.actions)) {
            ...
            const heartbeatTimer = setInterval(() => {
                const renewedLease = renewSessionEndLease(payload.directory, payload.sessionId, nonce, generation, deadlineAt);
                if (!renewedLease?.owner)
                    leaseLost = true;
                else
                    generation = renewedLease.owner.leaseGeneration;
            }, 250);
            ...
            if (name === 'foreground-cleanup' && result.completed) {
                ...
                producerReady = Boolean(recovered &&
        ...
        reschedulePendingWorker(payload, terminalized ??
            released ??
