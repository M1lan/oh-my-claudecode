...
export async function cleanupSessionPython(directory, sessionId) {
    ...
    const transcriptPath = typeof manifest?.actions['python-cleanup'].payload.transcriptPath ===
        'string'
...
export async function runSessionEndCallbacks(directory, sessionId, idempotencyKey, strict = false) {
    ...
    const platforms = config && hasExplicitNotificationConfig(profileName)
    ...
    const outcome = await runSessionEndDeferredAction({
        ...
        idempotencyKey,
        payload: {
            skipPlatforms: platforms.length > 0
...
function buildDurableSessionEndPayload(directory, input, metrics) {
    ...
    return {
        ...
        notificationProfile: typeof process.env.OMC_NOTIFY_PROFILE === 'string'
