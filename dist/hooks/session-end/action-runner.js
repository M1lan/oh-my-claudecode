...
function runDirectory(context) {
    return path.join(getOmcRoot(context.directory), 'state', 'session-end-jobs', 'runs', context.job.jobId, context.actionName, String(context.action.attempts), context.runnerNonce);
}
function openClawRoutingEnvironment(payload) {
    ...
    return Object.fromEntries(values.flatMap(([property, environment]) => typeof snapshot[property] === 'string'
...
function runnerEnvironment(context) {
    ...
    const keys = context.actionName === 'callback' || context.actionName === 'notification'
    ...
    if (context.actionName !== 'openclaw')
        return exact;
    const enabled = context.action.payload.openClawEnabled === true
        ? { OMC_OPENCLAW: '1' }
        : {};
    return {
        ...exact,
...
export async function runSessionEndAction(context, _execute) {
    ...
    try {
        ...
        const child = spawn(process.execPath, [fileURLToPath(import.meta.url), RUNNER_ARG, JSON.stringify(childInput)], {
            detached: true,
            stdio: 'ignore',
            windowsHide: true,
        ...
        const terminate = async () => {
            ...
            await Promise.race([
                Promise.resolve(killProcessTree(child.pid, 'SIGKILL')).catch(() => false),
                postKillWait,
            ]);
