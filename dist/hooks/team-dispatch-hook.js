...
function defaultInjectTarget(request, config) {
    ...
    if (typeof request.worker_index === 'number' &&
...
async function defaultInjector(request, config, _cwd) {
    ...
    try {
        const inMode = await rmuxExecAsync(['display-message', '-t', paneTarget, '-p', '#{pane_in_mode}'], { timeout: 1000 });
    ...
    if (attemptCountAtStart >= 1) {
        try {
            const preCapture = await rmuxExecAsync(['capture-pane', '-t', paneTarget, '-p', '-S', '-8'], { timeout: 2000 });
    ...
    if (shouldTypePrompt) {
        ...
        await rmuxExecAsync(['send-keys', '-t', paneTarget, '-l', sanitizedMessage], { timeout: 3000 });
    }
    ...
    for (let round = 0; round < INJECT_VERIFY_ROUNDS; round++) {
        ...
        try {
            const narrowCap = await rmuxExecAsync(['capture-pane', '-t', paneTarget, '-p', '-S', '-8'], { timeout: 2000 });
            const wideCap = await rmuxExecAsync(['capture-pane', '-t', paneTarget, '-p'], { timeout: 2000 });
        ...
        try {
            ...
            const normalizedMessages = [...messagesById.values()].map((candidate) => ({
                ...
                ...(safeString(candidate.notified_at || candidate.notifiedAt)
                    ? {
                        notified_at: safeString(candidate.notified_at || candidate.notifiedAt),
                    }
                    : {}),
                ...(safeString(candidate.delivered_at || candidate.deliveredAt)
                    ? {
                        delivered_at: safeString(candidate.delivered_at || candidate.deliveredAt),
                    }
                    : {}),
            }));
        ...
        await withDispatchLock(teamDirPath, async () => {
            ...
            for (const request of requests) {
                ...
                if (issueCooldownMs > 0 && issueKey) {
                    ...
                    if (Number.isFinite(lastInjectedMs) &&
                        lastInjectedMs > 0 &&
                ...
                if (triggerCooldownMs > 0 && triggerKey) {
                    ...
                    const withinCooldown = Number.isFinite(parsed.at) &&
                        parsed.at > 0 &&
                        nowMs - parsed.at < triggerCooldownMs;
                    const sameRequestRetry = parsed.lastRequestId !== '' &&
