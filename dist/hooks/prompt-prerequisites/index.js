...
export function getPromptPrerequisiteConfig(config) {
    ...
    return {
        ...
        executionKeywords: dedupe(raw?.executionKeywords?.length
...
export function shouldEnforcePromptPrerequisites(keywords, parseResult, config) {
    ...
    return (parseResult.requiredToolCalls.length > 0 ||
...
export function activatePromptPrerequisiteState(directory, sessionId, executionKeywords, parseResult) {
    if (parseResult.requiredToolCalls.length === 0 &&
        parseResult.requiredFilePaths.length === 0) {
...
function matchesToolRequirement(toolName, requiredTool) {
    ...
    switch (requiredTool) {
        case 'notepad_read':
            return (normalizedTool === 'notepad_read' ||
                normalizedTool.endsWith('__notepad_read'));
        case 'project_memory_read':
            return (normalizedTool === 'project_memory_read' ||
...
export function recordPromptPrerequisiteProgress(directory, sessionId, toolName, toolInput) {
    ...
    for (const requiredTool of state.required_tool_calls) {
        if (!state.completed_tool_calls.includes(requiredTool) &&
    ...
    if (readPath) {
        for (const requiredPath of state.required_file_paths) {
            ...
            const relativeRead = isAbsolute(normalizedRead)
                ...
                : normalizedRead;
            if (!state.completed_file_paths.includes(requiredPath) &&
