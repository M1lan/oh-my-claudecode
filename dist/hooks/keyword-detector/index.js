...
const WORKFLOW_SLASH_PATTERN = new RegExp('^\\s*/(?:oh-my-claudecode:|omc:)?(' +
    CANONICAL_WORKFLOW_SLASH_SKILLS.map((skill) => skill.replace(/-/g, '\\-')).join('|') +
    ')(?=\\s|$|[?!.,;:])', 'i');
/**
...
function isRalphUltraworkMetaOrBanterContext(context, keywordText) {
    ...
    const currentKeywordAliases = normalizedKeyword === 'ralph' ||
...
function isAutopilotCreationAlias(keywordText) {
    ...
    return (/^(?:build|create|make)\s+me\b/.test(normalized) ||
        /^i\s+want\s+an?(?:\s+(?:app|feature|project|tool|plugin|website|api|server|cli|script|system|service|dashboard|bot|extension))?\s*$/.test(normalized));
}
...
function isInformationalKeywordContext(text, position, keywordLength, keywordText) {
    ...
    const hasCommandSeparatorInvocation = hasDirectInvocationPrefix(text, position) &&
        /^\s*[:：]/.test(text.slice(position + keywordLength));
    const hasActionableCommandSeparatorInvocation = hasCommandSeparatorInvocation &&
    ...
    if (keywordInsideQuotes &&
        QUESTION_FOLLOWUP_PATTERNS.some((pattern) => pattern.test(questionOutsideQuotes))) {
        return true;
    }
