...
async function readWorkerStatusSnapshot(stateDir, teamName, workerName, nowMs = Date.now()) {
    ...
    try {
        ...
        const updatedAt = parsed && typeof parsed.updated_at === 'string'
            ? parsed.updated_at
            : null;
        let fresh = false;
        if (updatedAt) {
...
async function readWorkerHeartbeatSnapshot(stateDir, teamName, workerName, nowMs = Date.now()) {
    ...
    try {
        ...
        const parsed = JSON.parse(raw);
        const lastTurnAt = parsed && typeof parsed.last_turn_at === 'string'
            ? parsed.last_turn_at
            : null;
...
export async function maybeNotifyLeaderWorkerIdle(params) {
    ...
    try {
        if (existsSync(statusPath)) {
            ...
            const updatedAtField = parsed && typeof parsed.updated_at === 'string'
                ? parsed.updated_at
                : null;
            if (updatedAtField) {
...
export async function maybeNotifyLeaderAllWorkersIdle(params) {
    ...
    const allIdle = snapshots.length > 0 &&
        snapshots.every(({ status, heartbeat }) => (status.state === 'idle' || status.state === 'done') &&
            status.fresh &&
            heartbeat.fresh);
    if (!allIdle)
        return;
