...
export function clearModeState(mode, cwd, sessionId, expectedState) {
    ...
    if (isSessionScopedClear && sessionId) {
        const sessionStateFile = resolveSessionStatePath(mode, sessionId, cwd);
        try {
            const result = clearStateFileLockedIf(sessionStateFile, (current) => canClearStateForSession(current, sessionId) &&
                (!expectedState ||
                    JSON.stringify(current) === JSON.stringify(expectedState)));
            if (result === 'failed' ||
                (result === 'skipped' && existsSync(sessionStateFile)))
    ...
    if (!isSessionScopedClear) {
        try {
            const result = clearStateFileLockedIf(stateFile, (current) => !expectedState ||
                JSON.stringify(current) === JSON.stringify(expectedState));
            if (result === 'failed' ||
                (result === 'skipped' && existsSync(stateFile)))
                throw new Error('state mutation lock unavailable');
        }
        catch (err) {
            if (err.code !== 'ENOENT') {
