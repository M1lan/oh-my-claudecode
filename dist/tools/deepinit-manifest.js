...
const deepinitManifestSchema = {
    action: z
        ...
        .describe('Action: diff (compare current filesystem to saved manifest — compares directory file lists, not file contents), ' +
        'save (write current filesystem state as manifest), ' +
        'check (return whether manifest exists and is valid)'),
    workingDirectory: z
        .string()
    ...
    mode: z
        ...
        .optional()
        .default('incremental')
        .describe('Only valid with action=diff. incremental (default) returns only changed dirs, full returns all dirs as added.'),
    dryRun: z
        .boolean()
        .optional()
        .default(false)
        .describe('Only valid with action=save. If true, return what would be saved without writing.'),
};
// =============================================================================
...
export function scanDirectories(projectRoot) {
    ...
    function walk(absDir, depth) {
        ...
        try {
            ...
            if (realDir !== realProjectRoot &&
                !realDir.startsWith(realProjectRoot + sep)) {
        ...
        for (const [path, entry] of Object.entries(current)) {
            ...
            if (!prev) {
                ...
                if (prevFiles.length !== currFiles.length ||
                    prevFiles.some((f, i) => f !== currFiles[i])) {
