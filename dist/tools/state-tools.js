...
const CONVERGED_STATE_PATH_MODES = new Set([
    'ralph',
    'ultrawork',
]);
...
function hasValidatedNamedWorkflowTuple(record) {
    ...
    return (typeof sessionId === 'string' &&
        validateNamedWorkflowStateStructure(record, sessionId) !== null);
}
...
function emergencyRecoveryOptionsForProject(mode, path, root) {
    if (mode !== 'autopilot' || !isSharedHomeAutopilotCandidate(path, root))
        return undefined;
    return {
        authorizeState: (state) => isStateCandidateForProject(mode, path, state, root),
    };
}
...
function discoverStatePaths(paths) {
    ...
    for (const path of paths) {
        ...
        discovered.push({
            ...
            workflowRunId: typeof state.workflowRunId === 'string'
                ? state.workflowRunId
                : undefined,
        });
    }
...
function isAutopilotRecoveryCandidateForProject(path, root) {
    ...
    let artifacts;
    try {
        artifacts = readdirSync(dirname(path)).filter((name) => name.startsWith(artifactPrefix) &&
...
function clearWorkingDirectoryLocalStateCandidates(mode, root, sessionId, discovered = discoverStatePaths(getWorkingDirectoryLocalStateClearCandidates(mode, root, sessionId))) {
    ...
    for (const candidate of discovered) {
        const result = clearDiscoveredStateCandidate(candidate, (current) => !sessionId ||
...
function clearLegacyStateCandidates(mode, root, sessionId, discovered = discoverStatePaths(getLegacyStateFileCandidates(mode, root))) {
    ...
    for (const candidate of discovered) {
        const result = clearDiscoveredStateCandidate(candidate, (current) => isStateCandidateForProject(mode, candidate.path, current, root) &&
            (!sessionId || canClearStateForSession(current, sessionId)), emergencyRecoveryOptionsForProject(mode, candidate.path, root));
        if (result === 'cleared')
            cleared++;
        else if (result === 'failed')
            hadFailure = true;
...
function clearSessionOwnedStateCandidates(mode, root, sessionId, discovered = findSessionOwnedStateCandidates(mode, sessionId, root)) {
    ...
    for (const candidate of discovered) {
        const result = clearDiscoveredStateCandidate(candidate, (current) => isStateCandidateForProject(mode, candidate.path, current, root) &&
            canClearStateForSession(current, sessionId), emergencyRecoveryOptionsForProject(mode, candidate.path, root));
        if (result === 'cleared')
            cleared++;
        else if (result === 'failed')
            hadFailure = true;
...
function clearCompletedSessionStateCandidates(mode, root, requesterSessionId, discovered = findCompletedSessionStateCandidates(mode, root, requesterSessionId)) {
    ...
    for (const candidate of discovered) {
        const result = clearDiscoveredStateCandidate(candidate, (current) => current.active === true &&
            Boolean(candidate.completionEvidencePath &&
                existsSync(candidate.completionEvidencePath)), emergencyRecoveryOptionsForProject(mode, candidate.path, root));
        if (result === 'cleared')
            cleared++;
        else if (result === 'failed')
            hadFailure = true;
...
export function redactAutopilotPublicState(state) {
    ...
    if (!hasValidatedNamedWorkflowTuple(record) ||
        !workflow ||
    ...
    const stages = Array.isArray(descriptor.stages) &&
...
export const stateReadTool = {
    ...
    schema: {
        ...
        session_id: z
            .string()
            .optional()
            .describe('Session ID for session-scoped state isolation. When provided, the tool operates only within that session. When omitted, the tool aggregates legacy state plus all session-scoped state (may include other sessions).'),
    },
    handler: async (args) => {
...
export const stateWriteTool = {
    ...
    schema: {
        ...
        state: z
            ...
            .optional()
            .describe('Additional custom state fields (merged with explicit parameters)'),
        workingDirectory: z
            .string()
            .optional()
            .describe('Working directory (defaults to cwd)'),
    ...
    handler: async (args) => {
        ...
        try {
            ...
            const requestedRunId = typeof builtState.workflowRunId === 'string'
                ? builtState.workflowRunId
                : undefined;
            const requestedStateDigest = typeof builtState.target_state_sha256 === 'string'
                        ...
                        const written = emergencyMutateStateFileIf(statePath, (current) => JSON.stringify(current) === snapshot &&
                            ...
                            (requestedStateDigest === undefined ||
                                createHash('sha256')
                                    ...
                                    .digest('hex') === requestedStateDigest), (current) => ({ ...current, active: false }));
                        if (!written)
...
function recoverAutopilotEmergencyTransactions(root, sessionId) {
    ...
    for (const path of broadPaths) {
        ...
        if (sessionId && !directSessionPaths.has(path)) {
            ...
            const journalOwner = typeof journal?.sessionOwner === 'string'
        ...
        try {
            ...
            if (mode === 'merge-readiness') {
                ...
                if (sessionId) {
                    ...
                    const callerSid = (process.env.CLAUDE_SESSION_ID &&
            ...
            if (sessionId) {
                ...
                const namedPrimaries = mode === 'autopilot'
                    ? operationCandidates.filter((candidate) => hasNamedWorkflowMarker(candidate.state))
                    : [];
                ...
                if (MODE_CONFIGS[mode]) {
                    ...
                    if (OWNER_SESSION_FALLBACK_MODES.has(mode) &&
                        ...
                        if (ownerSessionId) {
                            ...
                            const ownerDirectCandidate = ownerCandidates.find((candidate) => candidate.path ===
                                resolveSessionStatePath(mode, ownerSessionId, root)) ?? ownerCandidates[0];
                            const ownerNamedPrimary = mode === 'autopilot' &&
                    ...
                    const clearedStateOrArtifacts = directCleared +
                ...
                if (OWNER_SESSION_FALLBACK_MODES.has(mode) &&
                    ...
                    if (ownerSessionId) {
                        ...
                        const ownerCandidates = findSessionOwnedStateCandidates(mode, ownerSessionId, root);
                        if (mode === 'autopilot' &&
                            ownerCandidates.some((candidate) => hasNamedWorkflowMarker(candidate.state)) &&
                ...
                const clearedStateOrArtifacts = completedSessionCleanup.cleared +
                    ...
                    runtimeCleanup.cleared;
                const capturedCleanupIncomplete = operationCandidates.some((candidate) => existsSync(candidate.path));
                const hadFailure = capturedCleanupIncomplete ||
                    legacyCleanup.hadFailure ||
                    sessionCleanup.hadFailure ||
                    workingDirectoryLocalCleanup.hadFailure ||
                    convergedCleanup.hadFailure ||
            ...
            const broadNamedPrimaries = mode === 'autopilot'
                ? broadOperationCandidates.filter((candidate) => hasNamedWorkflowMarker(candidate.state))
                : [];
            ...
            if (broadLegacySignalCandidates.length > 0 ||
            ...
            clearedCount =
                broadCapturedCandidates.filter((candidate) => !existsSync(candidate.path)).length + runtimeCleanup.cleared;
...
export const stateListActiveTool = {
    ...
    schema: {
        ...
        session_id: z
            .string()
            .optional()
            .describe('Explicit session ID to scope the listing. Overrides OMC_SESSION_ID when provided.'),
        all: z
            .boolean()
            .optional()
            .describe('When true, list active modes across all sessions (legacy + every session-scoped dir). Overrides the default current-session scope.'),
    },
    handler: async (args) => {
        ...
        try {
            ...
            for (const mode of CONVERGED_STATE_PATH_MODES) {
                if (!legacyActiveModes.includes(mode) &&
    ...
    stateGetStatusTool,
    {
        ...
        schema: {
            ...
            baseRef: z
                ...
                .optional()
                .describe('Base ref to diff committed changes against (e.g. origin/dev, HEAD, HEAD~1, HEAD^). Defaults to the branch upstream / origin/HEAD.'),
        ...
        handler: async (args) => {
            try {
                ...
                const sessionId = (args.session_id && args.session_id.trim()) ||
    ...
    },
    {
        ...
        handler: async (args) => {
            try {
                ...
                const persistFailed = result === 'blocked' &&
    ...
    },
    {
        ...
        handler: async (args) => {
            try {
                ...
                const persistFailed = state?.result === 'blocked' &&
                    (state.validation_errors ?? []).some((e) => e.includes('persisted'));
                if (persistFailed) {
                    return {
                        content: [
