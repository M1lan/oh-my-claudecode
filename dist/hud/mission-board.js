...
function summarizeTask(task) {
    if (!task)
        return null;
    return compactText(task.result ||
        task.summary ||
        task.error ||
        task.subject ||
        task.description, 56);
...
function deriveSessionStatus(mission) {
    ...
    if (mission.taskCounts.blocked > 0 || mission.taskCounts.failed > 0)
        return 'blocked';
    if (mission.taskCounts.completed === mission.taskCounts.total &&
...
export function recordMissionAgentStart(directory, input, sessionId) {
    ...
    mission.timeline.push({
        ...
        kind: 'update',
        agent: agent.name,
        detail: compactText(input.taskDescription || `started ${agent.name}`, 72) ||
            `started ${agent.name}`,
...
function collectTeamMission(teamRoot, teamName, config) {
    ...
    for (const event of readJsonLinesSafe(join(teamRoot, 'events.jsonl'))) {
        ...
        if (event.type === 'task_completed' || event.type === 'task_failed') {
        ...
        else if (event.type === 'team_leader_nudge' ||
            ...
            timeline.push({
                ...
                kind: 'update',
                agent: event.worker || 'leader-fixed',
                detail: compactText(event.reason || event.type.replace(/_/g, ' '), 72) ||
                    event.type,
                sourceKey: `event:${event.event_id || event.type}`,
            });
        }
    ...
    const agents = workers.slice(0, config.maxAgentsPerMission).map((worker) => {
        ...
        const currentTask = (workerStatus?.current_task_id
        ...
        return {
            ...
            latestUpdate: compactText(workerStatus?.reason ||
                latestTimeline?.detail ||
                summarizeTask(currentTask), 64),
    ...
    const createdAt = teamConfig.created_at ||
