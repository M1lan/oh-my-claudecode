...
function writeCache(opts) {
    try {
        ...
        const cache = {
            ...
            rateLimitedCount: opts.rateLimitedCount && opts.rateLimitedCount > 0
...
function resolveScopedWeeklyLimits(limits, parseDate) {
    ...
    for (const bucket of byKey.values()) {
        ...
        if (isSonnetFamily) {
            ...
            const id = typeof bucket.modelId === 'string' && bucket.modelId.trim() !== ''
                ? bucket.modelId
                : lower;
            result.generic.push({
                id,
...
export function parseUsageResponse(response, options) {
    ...
    const hasUsableCreditExtraUsage = !isEnterpriseContext &&
        usedCredits != null &&
        extraCurrency === 'USD' &&
    ...
    if (extra != null) {
        ...
        else if (extra.used_credits != null &&
            currency === 'USD' &&
            !isEnterpriseContext &&
            extra.monthly_limit != null &&
            extra.monthly_limit > 0) {
...
export function parseMinimaxResponse(response) {
    ...
    if (response.base_resp?.status_code != null &&
