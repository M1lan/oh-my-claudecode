...
function releaseNextSteps(version: string): string {
  ...
  pnpm run build
  pnpm run plugin:shipping:verify
  pnpm run plugin:shipping:stage
  git add -- package.json pnpm-lock.yaml .claude-plugin/plugin.json .claude-plugin/marketplace.json docs/CLAUDE.md CHANGELOG.md README.md docs/REFERENCE.md .github/CLAUDE.md docs/ARCHITECTURE.md .github/release-body.md
