...
try {
  const runtimeOutput = run('pnpm run test:run -- src/autoresearch/__tests__/runtime.test.ts src/autoresearch/__tests__/runtime-parity-extra.test.ts');
  const cliOutput = run('pnpm test -- --run src/cli/__tests__/autoresearch.test.ts src/cli/__tests__/autoresearch-guided.test.ts');
  run('pnpm run build');
