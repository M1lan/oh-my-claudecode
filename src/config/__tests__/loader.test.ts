...
describe('team.roleRouting (Option E)', () => {
  ...
  it('accepts cursor as team defaultAgentType and executor roleRouting provider', () => {
    ...
    try {
      ...
      expect(config.team?.roleRouting?.executor).toEqual({
        provider: 'cursor',
      });
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
  ...
  it('rejects cursor for non-executor team roleRouting providers', () => {
    const tempDir = mkdtempSync(
      join(tmpdir(), 'omc-team-routing-cursor-reviewer-'),
    );
    try {
      const claudeDir = join(tempDir, '.claude');
      require('node:fs').mkdirSync(claudeDir, { recursive: true });
...
describe('loadConfig() — autopilot.workflows', () => {
  ...
  it.each([
    ...
    expect(loadConfig().autopilot?.workflows?.['plan-build']?.stages).toEqual(
      stages,
    );
  });
  ...
  it.each([
    [
      'stageModels',
      { version: 1, stages: ['ralplan', 'execution'], stageModels: {} },
      /project autopilot\.workflows\.plan-build\.stageModels/,
    ],
    [
      'wrong order',
      { version: 1, stages: ['execution', 'ralplan'] },
      /project autopilot\.workflows\.plan-build\.stages/,
    ],
    [
      'duplicate stage',
      { version: 1, stages: ['ralplan', 'execution', 'qa', 'qa'] },
      /project autopilot\.workflows\.plan-build\.stages/,
    ],
    [
      'missing version',
      { stages: ['ralplan', 'execution'] },
      /project autopilot\.workflows\.plan-build\.version/,
    ],
    [
      'comma-bearing composite stage',
      { version: 1, stages: ['ralplan', 'execution,qa'] },
      /project autopilot\.workflows\.plan-build\.stages/,
    ],
    [
      'nested stage array',
    ...
    writeProjectConfig(
      JSON.stringify({ autopilot: { workflows: { 'plan-build': profile } } }),
    );
  ...
  it.each([
    ...
    'ultragoal',
    'ultrapilot',
  ])('rejects reserved workflow name %s', (name) => {
    writeProjectConfig(
      JSON.stringify({
        autopilot: {
          workflows: {
            [name]: { version: 1, stages: ['ralplan', 'execution'] },
          },
        },
      }),
    );
    expect(() => loadConfig()).toThrow(
      new RegExp(`project autopilot\\.workflows\\.${name}: name .*reserved`),
    );
  });
  ...
  it('validates malformed user and project workflow blocks before composition', () => {
    writeUserConfig(
      JSON.stringify({
        autopilot: {
          workflows: { 'user-flow': { version: 1, stages: ['execution'] } },
        },
      }),
    );
    expect(() => loadConfig()).toThrow(
      /user autopilot\.workflows\.user-flow\.stages/,
    );
    ...
    expect(() => loadConfig()).toThrow(
      /project autopilot\.workflows\.same-flow\.stageModels/,
    );
  });
      ...
      JSON.stringify({
        autopilot: {
          workflows: {
            ...
            'project-flow': {
              version: 1,
              stages: ['ralplan', 'execution', 'ralph', 'qa'],
            },
          },
        },
      }),
  ...
  it('publishes the closed workflow schema', () => {
    ...
    expect(workflows.additionalProperties?.required).toEqual([
      'version',
      'stages',
    ]);
    expect(workflows.additionalProperties?.properties).not.toHaveProperty(
      'stageModels',
    );
  });
});
