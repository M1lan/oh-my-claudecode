...
describe('prompt projection fixtures', () => {
  it('golden managed block matches composed projection from canonical', () => {
    ...
    const pkgVer = JSON.parse(readFileSync(join(root, 'package.json'), 'utf8'))
      .version as string;
    ...
    const docsBody = readFileSync(
      join(root, 'tests/fixtures/prompt-projection/claude-body.normalized'),
      'utf8',
    );
  ...
  it('transaction fixture documents required phases', () => {
    ...
    expect(fixture.phases).toEqual(
      expect.arrayContaining(['validation', 'backup', 'mutation', 'rollback']),
    );
    expect(fixture.guards).toEqual(
      expect.arrayContaining(['exclusiveVerifiedBackup', 'atomicWrite']),
    );
  });
});
