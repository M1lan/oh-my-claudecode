...
describe('prompt projection parity #3705', () => {
  it('docs/CLAUDE.md is canonical and projections are deterministic', () => {
    ...
    const projections = composeAllClaudeProjections({
      canonicalDocsRaw: docsRaw,
      version: pkgVer,
    });
    ...
    const rev2 = canonicalSourceRevision(
      docsRaw.replace(
        `<!-- OMC:VERSION:${pkgVer} -->`,
        '<!-- OMC:VERSION:0.0.0 -->',
      ),
  ...
  it('manifest validates and covers claude projections', () => {
    ...
    expect(
      validateManifest({ ...manifest, sourceRevision: 'bad' }).length,
    ).toBeGreaterThan(0);
  });
  ...
  it('agent/command/skill projections have digests and are enumerable', () => {
    const agents = readdirSync(join(root, 'agents')).filter((f) =>
      f.endsWith('.md'),
    );
    const commands = readdirSync(join(root, 'commands')).filter((f) =>
      f.endsWith('.md'),
    );
    const skills = readdirSync(join(root, 'skills')).flatMap((d) => {
      try {
        return readdirSync(join(root, 'skills', d))
          .filter((f) => f === 'SKILL.md')
          .map(() => `skills/${d}/SKILL.md`);
      } catch {
        return [];
      }
    });
