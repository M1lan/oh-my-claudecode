...
import {
  ...
  decodeClaudeMdUtf8 as decodeClaudeMdUtf8PreservingBom,
  OMC_END_MARKER,
  OMC_START_MARKER,
...
interface PlannedOperation extends ClaudeMdOperation {
  bytes?: Buffer;
  tempPath?: string;
}
...
function publicOperation(operation: PlannedOperation): ClaudeMdOperation {
  return {
    path: operation.path,
    type: operation.type,
...
function failure(
  ...
  code: ClaudeMdTransactionExitCode,
  error: string,
  phase: ClaudeMdTransactionResult['failedPhase'],
  path?: string,
...
export function validateRootedRegularFile(
  root: string,
  path: string,
  allowAbsent = true,
  fs: ClaudeMdTransactionFs = defaultFs,
): string {
...
function importRanges(content: string): Array<{ start: number; end: number }> {
  ...
  for (let index = 0; index + 2 < lines.length; index += 1) {
    if (
      lines[index].text === CLAUDE_MD_IMPORT_START &&
      lines[index + 1].text === '@CLAUDE-omc.md' &&
      lines[index + 2].text === CLAUDE_MD_IMPORT_END
    ) {
...
function generatedHeaderRanges(
...
function cleanedExisting(content: string): {
  content: string;
  ...
  const imports = importRanges(content).filter((range) =>
    analysis.markers.outsideRanges.some(
...
export function executeClaudeMdTransaction(
  ...
  try {
    ...
    const companionBytes = fs.existsSync(companion)
      ? fs.readFileSync(companion)
      : undefined;
    const mainText = mainBytes ? decodeClaudeMdUtf8(mainBytes, main) : null;
    if (companionBytes) decodeClaudeMdUtf8(companionBytes, companion);
    const overwrite =
      request.mode === 'global-preserve'
        ? { content: '', ranges: [], variants: [] }
        : mergeForOverwrite(mainText, canonical, request.version);
    const preserve =
      mainText === null
        ? { content: '', ranges: [], variants: [] }
        : request.mode === 'global-preserve'
          ? cleanedExisting(mainText)
          : { content: '', ranges: [], variants: [] };
    if (
      request.mode !== 'local' &&
      companionBytes &&
      parseClaudeMdMarkers(decodeClaudeMdUtf8(companionBytes, companion))
        .state === 'corrupt'
    )
    ...
    if (request.mode === 'local')
      ...
      const mainIsAlreadyOwned =
        imports.length > 0 && preserve.ranges.length === imports.length;
      const mainContent =
        mainIsAlreadyOwned && mainBytes !== undefined
          ? mainBytes
          : Buffer.from(
    ...
    const effectiveOperations = operations.filter((operation) => {
      ...
      const existingBytes =
        operation.path === main ? mainBytes : companionBytes;
    ...
    const states = new Map<string, PreState>(
      effectiveOperations.map((operation) => [
        operation.path,
        {
          path: operation.path,
          existedBefore: operation.existedBefore,
          bytes: operation.path === main ? mainBytes : companionBytes,
        },
      ]),
    );
    const appliedMainCleanup = effectiveOperations.some(
      (operation) => operation.path === main,
    );
    const result: ClaudeMdTransactionResult = {
      ok: false,
      exitCode: 0,
      mode: request.mode,
      ...
      deletedPaths: [],
      mutatedPaths: [],
      removedRanges: appliedMainCleanup
        ? request.mode === 'global-preserve'
          ? preserve.ranges
          : overwrite.ranges
        : [],
      removedVariants: appliedMainCleanup
        ? request.mode === 'global-preserve'
          ? preserve.variants
          : overwrite.variants
        : [],
    ...
    try {
      ...
      result.failedPath = effectiveOperations.find(
        (operation) =>
          !result.completedOperations.some(
      ...
      for (const operation of [...result.completedOperations].reverse()) {
        ...
        try {
          if (state.existedBefore) {
            const rollbackOperation: PlannedOperation = {
              path: state.path,
              type: 'write',
              existedBefore: true,
              bytes: state.bytes,
