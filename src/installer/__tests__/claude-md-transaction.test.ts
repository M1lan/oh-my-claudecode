...
describe('CLAUDE.md transactions', () => {
  ...
  it('preserve writes the companion before the single owned import', () => {
    ...
    expect(result.ok).toBe(true);
    expect(readFileSync(join(root, 'CLAUDE-omc.md'), 'utf8')).toContain(
      '# canonical',
    );
    expect(readFileSync(join(root, 'CLAUDE.md'), 'utf8')).toContain(
      '@CLAUDE-omc.md',
  ...
  it.each(['local', 'global-overwrite', 'global-preserve'] as const)(
    'omits all operations and backups on an idempotent %s rerun',
    (mode) => {
      const { root, source } = fixture();
      ...
      const beforeCompanion = nodeFs.existsSync(companion)
        ? readFileSync(companion)
        : undefined;
  ...
  it('fails a changed root alias between forward operations and rolls back through the canonical root', () => {
    const { root, source } = fixture();
    const alternate = mkdtempSync(join(realpathSync(tmpdir()), 'omc-claude-md-transaction-alternate-'));
    const alias = `${root}-alias`;
  ...
  it('revalidates a changed root alias after the final local write and rolls back canonically', () => {
    const { root, source } = fixture();
    const alternate = mkdtempSync(join(realpathSync(tmpdir()), 'omc-claude-md-transaction-final-local-alternate-'));
    const alias = `${root}-alias`;
    roots.push(alternate, alias);
    symlinkSync(root, alias);
    const main = join(root, 'CLAUDE.md');
  ...
  it('revalidates a changed root alias after the final delete and rolls back canonically', () => {
    const { root, source } = fixture();
    const alternate = mkdtempSync(join(realpathSync(tmpdir()), 'omc-claude-md-transaction-final-delete-alternate-'));
    const alias = `${root}-alias`;
    roots.push(alternate, alias);
    symlinkSync(root, alias);
    const main = join(root, 'CLAUDE.md');
  ...
  it('preserves a leading UTF-8 BOM and does not classify the bytes as exact legacy content', () => {
    ...
    const guide = Buffer.from(
      (corpus.variants as Array<{ dataBase64: string }>)[0].dataBase64,
      'base64',
    );
  ...
  it('reports rollback failure with its phase and path', () => {
    ...
    expect(result).toMatchObject({
      ...
      failedPath: companion,
    });
    expect(result.rollback).toEqual([
      { path: companion, ok: false, error: 'rollback delete failed' },
    ]);
  });
...
describe('strict rooted path containment', () => {
  it.each([
    ...
    [
      'same-share UNC child',
      '\\\\server\\share\\root',
      '\\\\server\\share\\root\\child\\CLAUDE.md',
      true,
    ],
    [
      'share mismatch',
      '\\\\server\\share-a\\root',
      '\\\\server\\share-b\\root\\CLAUDE.md',
      false,
    ],
    [
      'server mismatch',
      '\\\\server-a\\share\\root',
      '\\\\server-b\\share\\root\\CLAUDE.md',
      false,
    ],
    [
      'local and UNC mix',
      'C:\\root',
      '\\\\server\\share\\root\\CLAUDE.md',
      false,
    ],
    ['traversal', 'C:\\root', 'C:\\root\\..\\outside\\CLAUDE.md', false],
    ['root equality', 'C:\\root', 'C:\\root', false],
    [
      'device namespace candidate',
      'C:\\root',
      '\\\\?\\C:\\root\\CLAUDE.md',
      false,
    ],
...
describe('CLAUDE.md coordinator protocol', () => {
  it('reports an unavailable ordinary-module handshake without exposing build internals', () => {
    ...
    expect(outcome).toMatchObject({
      ...
      response: {
        ok: false,
        error: 'Coordinator build handshake is unavailable',
  ...
  it('keeps normal stdin request validation separate from the handshake mode', () => {
    const outcome = runClaudeMdCoordinator({
      schemaVersion: CLAUDE_MD_COORDINATOR_SCHEMA_VERSION,
    });
    expect(outcome).toMatchObject({
      exitCode: 2,
