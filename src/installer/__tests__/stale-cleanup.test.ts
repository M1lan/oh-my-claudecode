...
import {
  cleanupStaleAgents,
  cleanupStaleSkills,
  prunePluginDuplicateSkills,
  prunePluginDuplicateAgents,
} from '../index.js';
...
function sameLengthByteDivergence(content: Buffer): Buffer {
  ...
  const offset =
    bodyStart >= 0 && bodyStart + 2 < changed.length ? bodyStart + 2 : 0;
  changed[offset] ^= 1;
  return changed;
}
...
function createPluginRoot(
  dir: string,
  agentContents: Record<string, Buffer>,
): void {
...
describe('cleanupStaleAgents', () => {
  ...
  it.each(['build-fixer.md', 'deep-executor.md', 'quality-reviewer.md'])(
    'removes exact release-authenticated stale agent bytes for %s',
    async (filename) => {
      vi.resetModules();
      const { cleanupStaleAgents: cleanup, AGENTS_DIR: agentsDir } =
  ...
  it('removes the v4.1.0 build-fixer bytes as stale history', async () => {
    ...
    mkdirSync(agentsDir, { recursive: true });
    writeFileSync(
      join(agentsDir, 'build-fixer.md'),
      historicalAgent('build-fixer.md', 'v4.1.0/build-fixer.md'),
    );
  ...
  it('fails closed when the plugin registry is malformed or the explicit plugin root is missing', async () => {
    ...
    try {
      ...
      writeFileSync(
        join(agentsDir, 'build-fixer.md'),
        historicalAgent('build-fixer.md'),
      );
  ...
  it('fails closed when explicit OMC and Claude plugin roots conflict', async () => {
    ...
    try {
      ...
      createPluginRoot(omcRoot, {
        'architect.md': historicalAgent('architect.md'),
      });
      createPluginRoot(claudeRoot, {
        'architect.md': Buffer.from('conflicting active architect\n'),
      });
  ...
  it('uses a valid explicit OMC plugin root to preserve active historical basenames', async () => {
    ...
    try {
      ...
      createPluginRoot(pluginRoot, {
        'build-fixer.md': historicalAgent('build-fixer.md'),
      });
  ...
  it('preserves a current-package basename when the explicit plugin witness omits it', async () => {
    ...
    try {
      ...
      createPluginRoot(pluginRoot, {
        'executor.md': Buffer.from('active executor\n'),
      });
      process.env.OMC_PLUGIN_ROOT = pluginRoot;
      delete process.env.CLAUDE_PLUGIN_ROOT;
      ...
      writeFileSync(
        join(agentsDir, 'architect.md'),
        historicalAgent('architect.md', 'v4.5.0/architect.md'),
      );
  ...
  it('preserves when a structurally complete explicit root has a lookalike manifest name', async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          name: 'oh-my-claudecode-lookalike',
          commands: 'commands',
          skills: ['skills/test'],
        }),
      );
  ...
  it('preserves when the installed registry has a structurally complete lookalike ID', async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          plugins: {
            'oh-my-claudecode-lookalike@omc': [{ installPath: pluginRoot }],
          },
        }),
      );
  ...
  it('preserves custom frontmatter and byte-divergent historical content', async () => {
    ...
    writeFileSync(
      join(agentsDir, 'build-fixer.md'),
      sameLengthByteDivergence(historicalAgent('build-fixer.md')),
    );
    expect(cleanup(log)).toEqual([]);
    expect(existsSync(join(agentsDir, 'build-fixer.md'))).toBe(true);
    ...
    writeFileSync(
      join(agentsDir, 'build-fixer.md'),
      Buffer.concat([historicalAgent('build-fixer.md'), Buffer.from('\n')]),
    );
    expect(cleanup(log)).toEqual([]);
    expect(existsSync(join(agentsDir, 'build-fixer.md'))).toBe(true);
  });
  ...
  it('preserves authenticated bytes changed during unlink revalidation', async () => {
    ...
    const divergent = sameLengthByteDivergence(
      historicalAgent('build-fixer.md'),
    );
...
describe('cleanupStaleSkills', () => {
  ...
  it('removes stale skills only when OMC ownership is explicitly marked', async () => {
    vi.resetModules();
    const { cleanupStaleSkills: cleanup, SKILLS_DIR: skillsDir } =
      await import('../index.js');
...
describe('prunePluginDuplicateSkills', () => {
  ...
  it('removes standalone skills that match plugin-provided skills when marked as OMC-owned', async () => {
    vi.resetModules();
    const { prunePluginDuplicateSkills: prune, SKILLS_DIR: skillsDir } =
      await import('../index.js');
  ...
  it('removes exact-match standalone alias duplicates like omc-plan while preserving alias lookup behavior', async () => {
    ...
    const packagePlanSkill = readFileSync(
      join(process.cwd(), 'skills', 'plan', 'SKILL.md'),
      'utf-8',
    );
...
describe('prunePluginDuplicateAgents', () => {
  ...
  it.each([
    ...
    'prunes current-name bytes from released history for %s',
    async (filename, fixturePath) => {
      vi.resetModules();
      const { prunePluginDuplicateAgents: prune, AGENTS_DIR: agentsDir } =
        await import('../index.js');
      ...
      writeFileSync(
        ...
        historicalAgent(filename, fixturePath),
      );
  ...
  it('fails closed when installed plugin agent witnesses disagree', async () => {
    ...
    try {
      ...
      createPluginRoot(firstRoot, {
        'architect.md': Buffer.from('first active architect\n'),
      });
      createPluginRoot(secondRoot, {
        'architect.md': Buffer.from('second active architect\n'),
      });
      mkdirSync(join(tempDir, 'plugins'), { recursive: true });
      writeFileSync(
        join(tempDir, 'plugins', 'installed_plugins.json'),
  ...
  it('preserves current-name custom frontmatter and byte-divergent historical content', async () => {
    ...
    writeFileSync(
      join(agentsDir, 'architect.md'),
      sameLengthByteDivergence(historicalAgent('architect.md')),
    );
    expect(prune(log)).toEqual([]);
    expect(existsSync(join(agentsDir, 'architect.md'))).toBe(true);
    ...
    writeFileSync(
      join(agentsDir, 'architect.md'),
      Buffer.concat([historicalAgent('architect.md'), Buffer.from('\n')]),
    );
    expect(prune(log)).toEqual([]);
    expect(existsSync(join(agentsDir, 'architect.md'))).toBe(true);
  });
