...
describe('keyword-detector packaged artifacts', () => {
  ...
  it('does not ship stale pipeline keyword handling in plugin scripts', () => {
    const pluginScript = readFileSync(
  ...
  it('keeps installer template and plugin script aligned for supported compatibility keywords', () => {
    ...
    for (const [prompt, expected] of [
      ...
      [
        'deep interview me about requirements',
        '[MAGIC KEYWORD: DEEP-INTERVIEW]',
      ],
      [
        'deslop this module with duplicate dead code',
        '[MAGIC KEYWORD: AI-SLOP-CLEANER]',
      ],
    ] as const) {
      const templateResult = JSON.stringify(
        runKeywordHook(templatePath, prompt),
      );
  ...
  it('emits compact skill invocation guidance instead of full SKILL.md bodies', () => {
    ...
    for (const scriptPath of [templatePath, pluginPath]) {
      const result = runKeywordHook(
        scriptPath,
        'ralph fix and code review this change',
      );
      const context = JSON.stringify(result);

      expect(context).toContain('[MAGIC KEYWORD: RALPH]');
      expect(context).toContain(
        'Preferred invocation: /oh-my-claudecode:ralph',
      );
  ...
  it('keeps multi-skill keyword payloads under a compact budget', () => {
    ...
    const result = runKeywordHook(
      pluginPath,
      'ralph this with ultrawork and plan this migration',
    );
    const context = JSON.stringify(result);
  ...
  it('only triggers ai-slop-cleaner for anti-slop cleanup/refactor prompts', () => {
    ...
    const positivePrompt =
      'cleanup this ai slop: remove dead code and duplicate wrappers';
    const negativePrompt = 'refactor auth to support SSO';

    const templatePositive = JSON.stringify(
      runKeywordHook(templatePath, positivePrompt),
    );
    const pluginPositive = JSON.stringify(
      runKeywordHook(pluginPath, positivePrompt),
    );
  ...
  it('does not auto-trigger team mode from keyword-detector artifacts', () => {
    ...
    const templateResult = runKeywordHook(
      templatePath,
      'team 3 agents fix lint',
    );
    ...
    try {
      for (const [scriptPath, statePath] of [
        ...
        execFileSync('node', [scriptPath], {
          ...
          input: JSON.stringify({
            prompt:
              'ralph fix the regression in src/hooks/bridge.ts after issue #1795',
            directory: tempDir,
            cwd: tempDir,
  ...
  it('preserves foreign shared-home state and every dead recovery generation during generic autopilot activation', () => {
    ...
    try {
      ...
      execFileSync('node', [templatePath], {
        ...
        input: JSON.stringify({
          prompt: 'autopilot fix the regression',
          directory: projectA,
          cwd: projectA,
      ...
      execFileSync('node', [templatePath], {
        ...
        input: JSON.stringify({
          prompt: 'autopilot fix another regression',
          directory: projectA,
          cwd: projectA,
          session_id: 'project-a-session-2',
        }),
        encoding: 'utf-8',
      });
...
이런대화가 한번이라면 몇번할수있을까 오픈라우터 20달러 결제기준 api로`,
      ...
      expect(runKeywordHook(templatePath, prompt)).toEqual({
        continue: true,
        suppressOutput: true,
      });
      expect(runKeywordHook(pluginPath, prompt)).toEqual({
        continue: true,
        suppressOutput: true,
      });
    }
  ...
  it('still triggers for explicit activation requests in bug-fix context', () => {
    ...
    const templateAutopilot = runKeywordHook(
      templatePath,
      'use autopilot to fix bug in payments',
    );
    const pluginAutopilot = runKeywordHook(
      pluginPath,
      'use autopilot to fix bug in payments',
    );
    expect(JSON.stringify(templateAutopilot)).toContain(
      '[MAGIC KEYWORD: AUTOPILOT]',
    );
    expect(JSON.stringify(pluginAutopilot)).toContain(
      '[MAGIC KEYWORD: AUTOPILOT]',
    );

    const templateRalph = runKeywordHook(
      templatePath,
      'run ralph on issue in parser module',
    );
    const pluginRalph = runKeywordHook(
      pluginPath,
      'run ralph on issue in parser module',
    );
    ...
    const templateAutopilotIssue = runKeywordHook(
      templatePath,
      'fix issue with autopilot in parser module',
    );
    const pluginAutopilotIssue = runKeywordHook(
      pluginPath,
      'fix issue with autopilot in parser module',
    );
    expect(JSON.stringify(templateAutopilotIssue)).toContain(
      '[MAGIC KEYWORD: AUTOPILOT]',
    );
    expect(JSON.stringify(pluginAutopilotIssue)).toContain(
      '[MAGIC KEYWORD: AUTOPILOT]',
    );

    const templateRalphProblem = runKeywordHook(
      templatePath,
      'run ralph on parser state issue',
    );
    const pluginRalphProblem = runKeywordHook(
      pluginPath,
      'run ralph on parser state issue',
    );
    expect(JSON.stringify(templateRalphProblem)).toContain(
      '[MAGIC KEYWORD: RALPH]',
    );
    expect(JSON.stringify(pluginRalphProblem)).toContain(
      '[MAGIC KEYWORD: RALPH]',
    );
  });
  ...
  it('disambiguates bare ralph when the official ralph-loop plugin is installed and enabled (#3668)', () => {
    ...
    const fakeHome = mkdtempSync(
      join(tmpdir(), 'keyword-hook-ralph-loop-home-'),
    );
    const projectDir = mkdtempSync(
      join(tmpdir(), 'keyword-hook-ralph-loop-project-'),
    );
    ...
    try {
      ...
      writeFileSync(
        join(officialRoot, 'commands', 'ralph-loop.md'),
        '# official ralph-loop\n',
      );
      mkdirSync(join(omcRoot, 'skills', 'ralph'), { recursive: true });
      writeFileSync(
        join(omcRoot, 'skills', 'ralph', 'SKILL.md'),
        '---\nname: ralph\n---\nOMC ralph\n',
      );
      ...
      writeRegistry({
        ...
        'oh-my-claudecode@omc': [
          { installPath: omcRoot, version: '4.15.4', enabled: true },
        ],
      });
      for (const scriptPath of [templatePath, pluginPath]) {
        const context = contextOf(
          runIn(scriptPath, `ralph-both-${basename(scriptPath)}`),
        );
        expect(context).toContain('[MAGIC KEYWORD: RALPH]');
        expect(context).toContain(
          'official Anthropic `ralph-loop` plugin is also installed',
        );
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(runIn(scriptPath, `ralph-array-${basename(scriptPath)}`)),
        ).toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(runIn(scriptPath, `ralph-absent-${basename(scriptPath)}`)),
        ).not.toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-disabled-${basename(scriptPath)}`),
          ),
        ).not.toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-noenable-${basename(scriptPath)}`),
          ),
        ).not.toContain('ralph-loop');
      }
      ...
      writeRegistry({
        'ralph-loop@claude-plugins-official': [
          {
            installPath: join(officialRoot, '..', '9.9.9'),
            version: '9.9.9',
            enabled: true,
          },
        ],
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(runIn(scriptPath, `ralph-missing-${basename(scriptPath)}`)),
        ).not.toContain('ralph-loop');
      }
      ...
      writeRegistry({
        'my-ralph-loop@community': [
          { installPath: officialRoot, version: '1.0.0', enabled: true },
        ],
        'oh-my-claudecode@omc': [
          { installPath: omcRoot, version: '4.15.4', enabled: true },
        ],
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-lookalike-${basename(scriptPath)}`),
          ),
        ).not.toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(runIn(scriptPath, `ralph-noreg-${basename(scriptPath)}`)),
        ).not.toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        const result = JSON.parse(
          execFileSync('node', [scriptPath], {
            ...
            input: JSON.stringify({
              prompt: 'autopilot build me a CLI',
              cwd: projectDir,
              directory: projectDir,
              session_id: `autopilot-${basename(scriptPath)}`,
            }),
            encoding: 'utf-8',
          }),
        ) as {
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        const result = JSON.parse(
          execFileSync('node', [scriptPath], {
            ...
            input: JSON.stringify({
              prompt: '/ralph-loop fix the parser',
              cwd: projectDir,
              directory: projectDir,
              session_id: `ralphloop-cmd-${basename(scriptPath)}`,
            }),
            encoding: 'utf-8',
          }),
        ) as {
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        const context = contextOf(
          runIn(
            scriptPath,
            `omcralph-${basename(scriptPath)}`,
            '/omc-ralph fix the parser',
          ),
        );
        expect(context).toContain('[MAGIC KEYWORD: RALPH]');
        expect(context).toContain(
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(runIn(scriptPath, `ralph-regflag-${basename(scriptPath)}`)),
        ).toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-nosettings-${basename(scriptPath)}`),
          ),
        ).not.toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(runIn(scriptPath, `ralph-legacy-${basename(scriptPath)}`)),
        ).toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-malformed-${basename(scriptPath)}`),
          ),
        ).not.toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        const result = runWithEnv(
          scriptPath,
          `ralph-homeroot-${basename(scriptPath)}`,
      ...
      writeRegistry({
        'ralph-loop@claude-plugins-official': officialEntry(true),
        'ralph-loop@community': [
          { installPath: officialRoot, version: '2.0.0', enabled: true },
        ],
        'oh-my-claudecode@omc': [
          { installPath: omcRoot, version: '4.15.4', enabled: true },
        ],
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(
              scriptPath,
              `ralph-community-same-name-${basename(scriptPath)}`,
            ),
          ),
        ).not.toContain('ralph-loop');
      }
      ...
      writeSettings({
        enabledPlugins: ['ralph-loop@community', 'other-plugin@foo'],
      });
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-community-array-${basename(scriptPath)}`),
          ),
        ).not.toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(runIn(scriptPath, `ralph-bare-id-${basename(scriptPath)}`)),
        ).not.toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-canonical-wins-${basename(scriptPath)}`),
          ),
        ).not.toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        const context = contextOf(
          runIn(
            scriptPath,
            `ralph-multi-${basename(scriptPath)}`,
            '/ralph ultrawork fix the parser',
          ),
        );
        expect(context).toContain(
          '[MAGIC KEYWORDS DETECTED: RALPH, ULTRAWORK]',
        );
        expect(context).toContain(
          'official Anthropic `ralph-loop` plugin is also installed',
        );
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        const context = contextOf(
          runIn(
            scriptPath,
            `nonralph-multi-${basename(scriptPath)}`,
            'autopilot and ultrawork this repo',
          ),
        );
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(
              scriptPath,
              `ralph-multi-disabled-${basename(scriptPath)}`,
              '/ralph ultrawork fix the parser',
            ),
          ),
        ).not.toContain('ralph-loop');
      ...
      const projectLocalSettingsPath = join(
        projectDir,
        '.claude',
        'settings.local.json',
      );
      ...
      writeProjectSettings(projectSettingsPath, {
        enabledPlugins: { 'ralph-loop@claude-plugins-official': false },
      });
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-proj-off-${basename(scriptPath)}`),
          ),
        ).not.toContain('ralph-loop');
        expect(
          contextOf(
            runIn(
              scriptPath,
              `ralph-proj-off-multi-${basename(scriptPath)}`,
              '/ralph ultrawork fix the parser',
            ),
          ),
        ).not.toContain('ralph-loop');
      ...
      writeProjectSettings(projectLocalSettingsPath, {
        enabledPlugins: { 'ralph-loop@claude-plugins-official': true },
      });
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-proj-local-on-${basename(scriptPath)}`),
          ),
        ).toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-proj-only-${basename(scriptPath)}`),
          ),
        ).toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-proj-silent-${basename(scriptPath)}`),
          ),
        ).toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        expect(
          contextOf(
            runIn(scriptPath, `ralph-proj-malformed-${basename(scriptPath)}`),
          ),
        ).not.toContain('ralph-loop');
      }
      ...
      for (const scriptPath of [templatePath, pluginPath]) {
        const context = contextOf(
          runIn(scriptPath, `ralph-proj-privacy-${basename(scriptPath)}`),
        );
        expect(context).toContain(
          'official Anthropic `ralph-loop` plugin is also installed',
        );
      ...
      //     directory.
      const sandboxCwd = mkdtempSync(
        join(tmpdir(), 'keyword-hook-ralph-loop-sandbox-'),
      );
      try {
        writeSettings({
          enabledPlugins: { 'ralph-loop@claude-plugins-official': true },
        ...
        for (const scriptPath of [templatePath, pluginPath]) {
          ...
          expect(
            contextOf(
              runFromSandbox(
                scriptPath,
                `ralph-relcwd-${basename(scriptPath)}`,
                'evil',
              ),
            ),
          ).toContain('ralph-loop');
          ...
          expect(
            contextOf(
              runFromSandbox(
                scriptPath,
                `ralph-traversal-${basename(scriptPath)}`,
                join('..', basename(sandboxCwd), 'evil'),
              ),
            ),
          ).toContain('ralph-loop');
          ...
          expect(
            contextOf(
              runFromSandbox(
                scriptPath,
                `ralph-abscwd-${basename(scriptPath)}`,
                join(sandboxCwd, 'evil'),
              ),
            ),
          ).not.toContain('ralph-loop');
        }
...
describe('pre-tool-use packaged artifacts', () => {
  it('does not warn for .json commands just because .js is a substring', () => {
    ...
    expect(
      runPreToolHook(scriptPath, 'cat settings.json > backup.txt'),
    ).toEqual({
      continue: true,
      suppressOutput: true,
    });
    ...
    expect(
      JSON.stringify(runPreToolHook(scriptPath, 'cat app.js > backup.txt')),
    ).toContain('Bash command may modify source files');
  });
});
...
describe('atomic write packaged helpers', () => {
  it.each([
    ...
    [
      ...
      join(packageRoot, 'templates', 'hooks', 'lib', 'atomic-write.mjs'),
    ],
  ])(
    'allows its own recovery claim to converge while preserving foreign claim artifacts through the %s',
    async (_label, helperPath) => {
      const tempDir = mkdtempSync(
        join(tmpdir(), 'atomic-write-recovery-claim-'),
      );
      ...
      try {
        ...
        const { recoverEmergencyStateFile } = await import(
          pathToFileURL(helperPath).href
        );
        const authorizeState = (candidate: Record<string, unknown>) =>
          candidate.project_path === projectPath;

        expect(recoverEmergencyStateFile(statePath, { authorizeState })).toBe(
          true,
        );
...
describe('workflow profile runtime packaged artifacts (#3487)', () => {
  it('ships the same descriptor and stop-transition helper with plugin and standalone hook payloads', () => {
    const templateHelper = readFileSync(
      join(
        packageRoot,
        'templates',
        'hooks',
        'lib',
        'workflow-profile-runtime.mjs',
      ),
      'utf-8',
    );
    const pluginHelper = readFileSync(
      join(packageRoot, 'scripts', 'lib', 'workflow-profile-runtime.mjs'),
      'utf-8',
    );
