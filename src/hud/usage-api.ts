    ...
    if (cache.data) {
      ...
      if (Array.isArray(cache.data.scopedWeeklyBuckets)) {
        for (const bucket of cache.data.scopedWeeklyBuckets) {
          ...
          bucket.resetsAt = isNaN(parsedResetsAt.getTime())
            ? null
            : parsedResetsAt;
        }
      }
  ...
  try {
    ...
    if (existing.claudeAiOauth && typeof existing.claudeAiOauth === 'object') {
      ...
      if (creds.expiresAt != null)
        (existing as Record<string, unknown>).expiresAt = creds.expiresAt;
      if (creds.refreshToken)
        (existing as Record<string, unknown>).refreshToken = creds.refreshToken;
    }
    ...
    if (process.env.OMC_DEBUG) {
      console.error(
        '[usage-api] Failed to write back refreshed credentials to Keychain',
      );
    }
  }
}
...
export function parseUsageResponse(
  ...
  const hasUsableEnterprise =
    isEnterpriseContext && usedCredits != null && minorDivisor != null;
  ...
  if (extra != null) {
    ...
    if (
      ...
      result.enterpriseLimitUsd =
        extra.monthly_limit == null ? null : extra.monthly_limit / minorDivisor;
 ...
 */
function fetchUsageFromKimi(
  apiKey: string,
): Promise<FetchResult<KimiUsageResponse>> {
  return new Promise((resolve) => {
    const baseUrl = process.env.ANTHROPIC_BASE_URL;
        ...
        (res) => {
          ...
          res.on('end', () => {
            if (res.statusCode === 200) {
              ...
              if (process.env.OMC_DEBUG) {
                console.error(
                  `[usage-api] Kimi API returned 429 (rate limited)`,
                );
              }
              resolve({ data: null, rateLimited: true });
            } else {
...
export async function getUsage(): Promise<UsageResult> {
  ...
  const currentSource: UsageSource = isMinimax
    ? 'minimax'
    : isKimi
      ? 'kimi'
      : isZai && authToken
