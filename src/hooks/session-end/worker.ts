...
export function spawnSessionEndWorker(
  payload: SessionEndWorkerPayload,
): boolean {
  try {
    const child = spawn(
      process.execPath,
      [fileURLToPath(import.meta.url), WORKER_ARG, JSON.stringify(payload)],
      {
        detached: true,
        stdio: 'ignore',
...
export async function executeSessionEndAction(
  name: SessionEndActionName,
  payload: SessionEndWorkerPayload,
  deadlineAt: number,
): Promise<void> {
  const legacy = await import('./index.js');
  const routing = readSessionEndJob(payload.directory, payload.sessionId)
    ?.actions.notification.payload;
  ...
  if (name === 'foreground-cleanup')
    return legacy
      .runForegroundSessionEndCleanup(
        payload.directory,
        payload.sessionId,
        false,
      )
  ...
  if (name === 'wiki-capture') {
    const intent = readSessionEndJob(payload.directory, payload.sessionId)
      ?.actions['wiki-capture'].payload;
    const { commitWikiSessionEndCaptureIntent } =
      await import('../wiki/session-hooks.js');
    if (
      !(await commitWikiSessionEndCaptureIntent(
        intent as unknown as Parameters<
          typeof commitWikiSessionEndCaptureIntent
        >[0],
        { deadlineAt: Math.min(deadlineAt, Date.now() + 9_000) },
      ))
    )
  ...
  if (name === 'team-cleanup') {
    const names = readSessionEndJob(payload.directory, payload.sessionId)
      ?.actions['team-cleanup'].payload.initialTeamNames;
    const result = await legacy.cleanupSessionOwnedTeams(
      payload.directory,
      payload.sessionId,
      Array.isArray(names)
    ...
    if (result.failed.length > 0)
      throw new Error(
        `team-cleanup-incomplete:${result.failed.map((item) => item.teamName).join(',')}`,
      );
    return;
  }
  if (name === 'python-cleanup')
    return legacy
      .cleanupSessionPython(payload.directory, payload.sessionId)
      .then(() => undefined);
  if (name === 'reply-cleanup')
    return legacy.cleanupSessionReplies(payload.sessionId);
  if (name === 'callback')
    return legacy.runSessionEndCallbacks(
      payload.directory,
      payload.sessionId,
      readSessionEndJob(payload.directory, payload.sessionId)?.actions.callback
        .idempotencyKey,
      true,
    );
  if (name === 'notification')
    return legacy.runSessionEndNotifications(
      payload.directory,
      payload.sessionId,
      true,
    );
  return legacy.runSessionEndOpenClaw(
    payload.directory,
    payload.sessionId,
    true,
  );
}
async function reapIfProvenStale(
  payload: SessionEndWorkerPayload,
  deadlineAt: number,
): Promise<void> {
...
export async function processSessionEndWorker(
  ...
  if (!identity) return;
  let claimed = claimSessionEndJob(
    payload.directory,
    payload.sessionId,
    nonce,
    identity,
  ...
  const graceExpired = admitted
    ? Date.now() >= Date.parse(admitted.producerGraceExpiresAt)
    : false;
  ...
  let producerReady = Boolean(
    admitted &&
    ['sealed', 'no-op'].includes(admitted.producers.core.state) &&
    ['sealed', 'no-op'].includes(admitted.producers.wiki.state),
  );
    ...
    for (const name of Object.keys(claimed.actions) as SessionEndActionName[]) {
      ...
      if (
        ...
        (name !== 'foreground-cleanup' ||
          !graceExpired ||
          before.producers.core.state !== 'prepared')
      )
        continue;
      if (name === 'wiki-capture' && before.producers.wiki.state === 'absent')
        continue;
      const owned = claimSessionEndAction(
        payload.directory,
        payload.sessionId,
        nonce,
        name,
      ...
      if (!owned || !action || action.status !== 'claimed' || !action.runner)
        continue;
      const renewed = renewSessionEndLease(
        payload.directory,
        payload.sessionId,
        nonce,
        generation,
      ...
      if (
        !markSessionEndActionRunner(
          ...
          nonce,
          name,
          action.runner.runnerNonce,
          'started',
        )
      )
        break;
      const stopWatchdog = armSessionEndActionWatchdog({
        directory: payload.directory,
        jobId: owned.jobId,
        action: name,
        attempt: action.attempts,
        runnerNonce: action.runner.runnerNonce,
        deadlineAt: Math.min(deadlineAt, Date.now() + action.budgetMs),
      });
      const actionDeadline = Math.min(deadlineAt, Date.now() + action.budgetMs);
      let leaseLost = false;
      const heartbeatTimer = setInterval(() => {
        const renewedLease = renewSessionEndLease(
          payload.directory,
          payload.sessionId,
          nonce,
          generation,
        ...
        if (!renewedLease?.owner) leaseLost = true;
        else generation = renewedLease.owner.leaseGeneration;
      }, 250);
      heartbeatTimer.unref();
      const result = await runSessionEndAction(
        {
          directory: payload.directory,
        ...
        () => executeSessionEndAction(name, payload, actionDeadline),
      );
      ...
      if (name === 'foreground-cleanup' && result.completed) {
        recoverPreparedCoreProducer(payload.directory, payload.sessionId);
        const recovered = readSessionEndJob(
          payload.directory,
          payload.sessionId,
        );
        producerReady = Boolean(
          recovered &&
          ['sealed', 'no-op'].includes(recovered.producers.core.state) &&
          ['sealed', 'no-op'].includes(recovered.producers.wiki.state),
        );
      }
      const heartbeat = renewSessionEndLease(
        payload.directory,
        payload.sessionId,
        nonce,
        generation,
...
export function reconcileSessionEndJobs(
  directory: string,
  ...
  for (const ticket of tickets) {
    const spawned = spawnSessionEndWorker({
      directory,
...
if (workerIndex >= 0) {
  try {
    ...
    ) as SessionEndWorkerPayload;
    void processSessionEndWorker(payload);
  } catch {
    /* invalid child input exits naturally */
  }
}
