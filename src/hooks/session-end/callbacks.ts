...
export function interpolatePath(
  ...
  const safeIdempotencyKey = (idempotencyKey ?? '').replace(
    /[^A-Za-z0-9_-]/g,
    '_',
  );
  return normalize(
    pathTemplate
      ...
      .replace(/\{idempotency_key\}/g, safeIdempotencyKey)
      .replace(/\{date\}/g, date)
      .replace(/\{time\}/g, time),
  );
}
...
async function sendTelegram(
  ...
  if (
    !config.botToken ||
    !config.chatId ||
    !/^[0-9]+:[A-Za-z0-9_-]+$/.test(config.botToken)
  )
    return;
...
async function runLegacyCallbacks(
  ...
  const idempotencyKey =
    ...
    `session-end:${metrics.session_id}:legacy-callback`;
  ...
  if (
    ...
    work.push(
      writeToFile(
        callbacks.file,
        formatSessionSummary(metrics, callbacks.file.format || 'markdown'),
        metrics.session_id,
        idempotencyKey,
      ),
    );
  if (!skipPlatforms.has('telegram') && callbacks.telegram?.enabled)
    work.push(
      sendTelegram(
        callbacks.telegram,
        prefixMessageWithTags(
          formatSessionSummary(metrics),
          normalizeTelegramTagList(callbacks.telegram.tagList),
        ),
        signal,
        idempotencyKey,
      ),
  ...
  if (!skipPlatforms.has('discord') && callbacks.discord?.enabled)
    work.push(
      sendDiscord(
        ...
        prefixMessageWithTags(
          formatSessionSummary(metrics),
          normalizeDiscordTagList(callbacks.discord.tagList),
        ),
        signal,
        idempotencyKey,
      ),
...
function stringList(value: unknown): Array<'file' | 'telegram' | 'discord'> {
  return Array.isArray(value)
    ? value.filter(
        (item): item is 'file' | 'telegram' | 'discord' =>
          item === 'file' || item === 'telegram' || item === 'discord',
      )
    : [];
}
...
export async function runSessionEndDeferredAction(
  action: SessionEndDeferredAction,
  context: SessionEndActionContext,
): Promise<SessionEndActionOutcome> {
  const manifestDeadline = Date.parse(context.deadlineAt);
  const deadlineAt = Math.min(
    Number.isFinite(manifestDeadline) ? manifestDeadline : Date.now(),
    Date.now() + Math.max(0, action.budgetMs),
  );
      ...
      switch (action.name) {
        ...
        case 'notification': {
          ...
          const result = await notify('session-end', {
            ...
            durationMs: context.metrics.duration_ms,
            agentsSpawned: context.metrics.agents_spawned,
            agentsCompleted: context.metrics.agents_completed,
            modesUsed: context.metrics.modes_used,
            reason: context.metrics.reason,
            timestamp: context.metrics.ended_at,
            profileName:
              typeof action.payload.profileName === 'string'
