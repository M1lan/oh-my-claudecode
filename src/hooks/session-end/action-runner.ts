...
import type {
  OpenClawRoutingSnapshot,
  SessionEndActionName,
...
function runDirectory(context: ActionRunContext): string {
  return path.join(
    getOmcRoot(context.directory),
    'state',
    'session-end-jobs',
    'runs',
    context.job.jobId,
    context.actionName,
    String(context.action.attempts),
    context.runnerNonce,
  );
}
function openClawRoutingEnvironment(
...
function runnerEnvironment(context: ActionRunContext): NodeJS.ProcessEnv {
  ...
  const keys =
    context.actionName === 'callback' || context.actionName === 'notification'
      ? [...baseKeys, ...notificationKeys]
      : baseKeys;
  ...
  const enabled =
    context.action.payload.openClawEnabled === true
      ? { OMC_OPENCLAW: '1' }
      : {};
  return {
    ...exact,
    ...enabled,
    ...openClawRoutingEnvironment(context.action.payload),
  };
}
...
export async function runSessionEndAction(
  ...
  try {
    ...
    const childInput = {
      ...
      attempt: context.action.attempts,
      ownerNonce: context.ownerNonce,
      runnerNonce: context.runnerNonce,
      runPath,
      deadlineAt: context.deadlineAt,
    };
    const child = spawn(
      process.execPath,
      [fileURLToPath(import.meta.url), RUNNER_ARG, JSON.stringify(childInput)],
      {
        detached: true,
        stdio: 'ignore',
        windowsHide: true,
    ...
    const terminate = async (): Promise<void> => {
      const postKillWait = new Promise<void>((resolve) => {
        const timer = setTimeout(
          resolve,
          Math.max(1, context.deadlineAt + POST_KILL_SETTLE_MS - Date.now()),
        );
        timer.unref();
      });
    ...
    if (settled) {
      ...
      return {
        code: exitCode === null ? 'runner-deadline' : `runner-exit-${exitCode}`,
        completed: false,
      };
    }
    atomicWriteJsonSync(path.join(runPath, 'control.json'), {
      jobId: context.job.jobId,
      action: context.actionName,
      attempt: context.action.attempts,
      runnerNonce: context.runnerNonce,
      ownerNonce: context.ownerNonce,
      runner: { pid: child.pid, processStartIdentity: identity },
      deadlineAt: new Date(context.deadlineAt).toISOString(),
      idempotencyKey: context.action.idempotencyKey,
    });
    atomicWriteJsonSync(path.join(runPath, 'arm.json'), {
      runnerNonce: context.runnerNonce,
      ownerNonce: context.ownerNonce,
    ...
    const completed =
      terminal.code === 0 && !terminal.terminated && !deadlineTermination;
    const code = completed
      ? 'completed'
      : deadlineTermination || terminal.terminated || terminal.code === null
        ? 'runner-deadline'
        : `runner-exit-${terminal.code}`;
    atomicWriteJsonSync(path.join(runPath, 'result.json'), {
      code,
    ...
    const code =
      error instanceof Error ? error.name || 'action-failed' : 'action-failed';
    try {
      atomicWriteJsonSync(path.join(runPath, 'result.json'), {
        code,
        retryable: true,
      ...
      /* manifest retains retry authority */
    }
...
async function runActionRunnerEntrypoint(): Promise<void> {
  ...
  try {
    const input = JSON.parse(process.argv[runnerIndex + 1] ?? '') as {
      directory: string;
      sessionId: string;
      jobId: string;
      actionName: SessionEndActionName;
    ...
    while (Date.now() < input.deadlineAt) {
      ...
      try {
        const arm = JSON.parse(
          fs.readFileSync(path.join(input.runPath, 'arm.json'), 'utf8'),
        ) as { runnerNonce?: string; ownerNonce?: string };
        ...
        armed =
          ...
          job.owner?.nonce === input.ownerNonce &&
          action?.status === 'claimed' &&
          action.attempts === input.attempt &&
          action.claimantNonce === input.ownerNonce &&
          action.runner?.runnerNonce === input.runnerNonce &&
          action.runner.phase === 'armed' &&
          arm.runnerNonce === input.runnerNonce &&
          arm.ownerNonce === input.ownerNonce;
      } catch {
        /* publication is not complete yet */
      }
      if (armed) break;
