...
describe('SessionEnd durable worker', () => {
  ...
  it('reschedules a core-only manifest through producer grace after slow required actions and runs deferred callbacks once', async () => {
    ...
    expect(
      mutateSessionEndJob(directory, sessionId, initial.revision, (job) => {
    ...
    expect(Date.parse(manifest.bestEffortDeadlineAt)).toBeGreaterThan(
      Date.parse(manifest.producerGraceExpiresAt),
    );
    expect(actions.runSessionEndCallbacks).toHaveBeenCalledTimes(1);
    expect(actions.runSessionEndNotifications).toHaveBeenCalledTimes(1);
  });
  ...
  it('reschedules prepared core through grace when wiki already sealed', async () => {
    vi.useFakeTimers();
    actionRunner.runSessionEndAction.mockResolvedValue({
      code: 'completed',
      completed: true,
    });
    const directory = project();
    ...
    expect(prepareCoreManifest(directory, sessionId, {})).not.toBeNull();
    expect(
      sealWikiManifest(directory, sessionId, { captured: true }),
    ).not.toBeNull();
    const initial = readSessionEndJob(directory, sessionId)!;
    expect(
      mutateSessionEndJob(directory, sessionId, initial.revision, (job) => {
  ...
  it('fails closed after grace when a wiki-first manifest never receives core, without a 250ms recovery loop', async () => {
    ...
    expect(manifest).toMatchObject({
      ...
      owner: null,
      producers: {
        core: { state: 'no-op', sealedBy: 'recovery' },
        wiki: { state: 'sealed' },
      },
      actions: {
        'foreground-cleanup': {
          status: 'expired',
          lastOutcomeCode: 'required-core-producer-absent',
        },
        'team-cleanup': {
          status: 'expired',
          lastOutcomeCode: 'required-core-producer-absent',
        ...
        callback: {
          status: 'expired',
          lastOutcomeCode: 'best-effort-core-producer-absent',
        },
        notification: {
          status: 'expired',
          lastOutcomeCode: 'best-effort-core-producer-absent',
  ...
  it('persists only bounded original OpenClaw routing and clears it from worker ambient environments', () => {
    ...
    expect(
      prepareCoreManifest(directory, 'routing-snapshot', {}),
    ).not.toBeNull();

    const routing = readSessionEndJob(directory, 'routing-snapshot')!.actions
      .openclaw.payload.openClawRouting;
    ...
    expect(workerEnvironment()).not.toHaveProperty(
      'UNRELATED_SESSION_END_SECRET',
    );
  });
  ...
  it('terminalizes a core-only manifest after three failed foreground cleanups without timer churn', async () => {
    vi.useFakeTimers();
    actionRunner.runSessionEndAction.mockResolvedValue({
      code: 'foreground-cleanup-failed',
      completed: false,
    });
    const directory = project();
    ...
    expect(manifest).toMatchObject({
      ...
      actions: {
        'foreground-cleanup': {
          status: 'expired',
          attempts: 3,
          lastOutcomeCode: 'required-foreground-cleanup-exhausted',
        },
        'team-cleanup': {
          status: 'expired',
          lastOutcomeCode: 'required-core-producer-unavailable',
        },
        callback: {
          status: 'expired',
          lastOutcomeCode: 'best-effort-core-producer-unavailable',
        },
      },
    });
    expect(
      manifest.actions['team-cleanup'].payload.terminalization,
    ).toMatchObject({
    ...
    expect(
      Object.values(manifest.actions).every(
