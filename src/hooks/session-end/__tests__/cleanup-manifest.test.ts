...
function preparedAndSealed(directory: string, sessionId = 'session-a') {
  expect(
    prepareCoreManifest(directory, sessionId, {
...
describe('durable SessionEnd cleanup manifest', () => {
  it('serializes concurrent core/wiki producers and rejects a second worker claim', async () => {
    ...
    await Promise.all([
      ...
      Promise.resolve().then(() =>
        sealWikiManifest(directory, sessionId, {
          source: 'wiki',
          capture: true,
        }),
      ),
    ...
    expect(before?.actions.callback.payload).toMatchObject({
      source: 'core',
      callback: true,
    });
    expect(before?.actions['wiki-capture'].payload).toMatchObject({
      source: 'wiki',
      capture: true,
    });
    ...
    expect(readSessionEndJob(directory, sessionId)?.owner?.nonce).toBe(
      first?.owner?.nonce ?? second?.owner?.nonce,
    );
  });
  ...
  it('uses expected-revision CAS rather than accepting a stale mutation', () => {
    ...
    const current = mutateSessionEndJob(
      directory,
      sessionId,
      initial!.revision,
      (job) => {
    ...
    const stale = mutateSessionEndJob(
      directory,
      sessionId,
      initial!.revision,
      (job) => {
    ...
    expect(readSessionEndJob(directory, sessionId)?.phase).toBe(
      'recoverable-failure',
    );
  });
  ...
  it('renews leases and reaps only expired, positively dead or PID-reused owners', () => {
    ...
    const claimed = claimSessionEndJob(
      directory,
      sessionId,
      'owner',
      'old-start',
    ...
    expect(claimed?.owner).toMatchObject({
      nonce: 'owner',
      leaseGeneration: 1,
    });
    ...
    const expired = JSON.parse(readFileSync(path, 'utf8')) as {
      owner: { leaseExpiresAt: string };
    };
    ...
    expect(
      reapStaleSessionEndOwner(directory, sessionId, 'owner', 2, 'dead')?.owner,
    ).toBeNull();

    const again = claimSessionEndJob(
      directory,
      sessionId,
      'reused-owner',
      'reused-start',
    ...
    const expiresAgain = JSON.parse(readFileSync(path, 'utf8')) as {
      owner: { leaseExpiresAt: string };
    };
    expiresAgain.owner.leaseExpiresAt = new Date(
      Date.now() - 1_000,
    ).toISOString();
  ...
  it('persists action claim, runner arm, result, and terminality separately from discovery tickets', () => {
    ...
    const owner = claimSessionEndJob(
      directory,
      sessionId,
      'owner',
      'identity',
    ...
    const claimed = claimSessionEndAction(
      directory,
      sessionId,
      'owner',
      'team-cleanup',
    ...
    expect(claimed.actions['team-cleanup']).toMatchObject({
      status: 'claimed',
      attempts: 1,
      runner: { phase: 'reserved' },
    });
    expect(
      markSessionEndActionRunner(
        directory,
    ...
    for (const [name] of Object.entries(current.actions)) {
      ...
      current = mutateSessionEndJob(
        directory,
        sessionId,
        current.revision,
        (job) => {
          job.actions[name as keyof typeof job.actions].status = 'completed';
          job.actions[name as keyof typeof job.actions].runner = {
            attempt: 1,
            runnerNonce: `${name}-runner`,
            phase: 'terminal',
    ...
    writeFileSync(
      join(sessionEndJobsDirectory(directory), 'discovery.json'),
      JSON.stringify({
        version: 2,
        cursor: 0,
        tickets: [
          { sessionId, attempts: 0, retryAt: new Date(0).toISOString() },
        ],
      }),
    );
    expect(takeSessionEndDiscoveryPage(directory, 1)).toEqual([]);
    expect(isManifestTerminal(readSessionEndJob(directory, sessionId)!)).toBe(
      true,
    );
  });
  ...
  it('keeps more than sixteen queued jobs discoverable through bounded rotating pages', () => {
    ...
    for (const sessionId of sessionIds) {
      expect(
        prepareCoreManifest(directory, sessionId, { sequence: sessionId }),
      ).not.toBeNull();
      expect(sealCoreManifest(directory, sessionId)).not.toBeNull();
    }
  ...
  it('leaves every simulated crash boundary recoverable instead of silently completing work', () => {
    ...
    const action = claimSessionEndAction(
      directory,
      sessionId,
      'crashed-owner',
      'python-cleanup',
    ...
    expect(
      readSessionEndJob(directory, sessionId)?.actions['python-cleanup'].runner
        ?.phase,
    ).toBe('reserved');
    expect(
      markSessionEndActionRunner(
    ...
    expect(
      readSessionEndJob(directory, sessionId)?.actions['python-cleanup'],
    ).toMatchObject({
      status: 'retryable',
      lastOutcomeCode: 'simulated-crash',
  ...
  it('bounds required failures and never retries a failed remote delivery attempt', () => {
    ...
    expect(
      claimSessionEndAction(
        ...
        'team-cleanup',
        Date.now() + 5_000,
      )?.actions['team-cleanup'],
    ).toMatchObject({
      status: 'expired',
      attempts: 3,
      lastOutcomeCode: 'required-attempt-limit',
    });
    ...
    for (const name of ['callback', 'notification', 'openclaw'] as const) {
      ...
      expect(
        readSessionEndJob(directory, sessionId)?.actions[name],
      ).toMatchObject({
        status: 'expired',
        attempts: 1,
        lastOutcomeCode: 'response-lost',
      });
      expect(
        claimSessionEndAction(
          directory,
  ...
  it('does not requeue a best-effort action after its owner dies with delivery uncertain', () => {
    ...
    const manifestPath = join(
      sessionEndJobsDirectory(directory),
      `${sessionId}.json`,
    );
    const expired = JSON.parse(readFileSync(manifestPath, 'utf8')) as {
      owner: { leaseExpiresAt: string };
    };
    expired.owner.leaseExpiresAt = new Date(Date.now() - 1_000).toISOString();
    ...
    expect(
      ...
      status: 'expired',
      lastOutcomeCode: 'delivery-uncertain-owner-reaped',
      runner: { phase: 'terminal' },
    });
    const replacement = claimSessionEndJob(
      directory,
      sessionId,
      'replacement',
      'identity',
