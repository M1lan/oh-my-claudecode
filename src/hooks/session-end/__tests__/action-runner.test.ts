...
const processUtils = vi.hoisted(() => ({
  ...
  terminateOwnedProcessTree: vi.fn(
    async (): Promise<'terminated' | 'unknown'> => 'terminated',
  ),
}));
const manifest = vi.hoisted(() => ({
  markSessionEndActionRunner: vi.fn(() => ({})),
}));

vi.mock('child_process', () => childProcess);
vi.mock('../../../platform/process-utils.js', () => processUtils);
vi.mock('../cleanup-manifest.js', async () => {
  const actual = await vi.importActual<typeof import('../cleanup-manifest.js')>(
    '../cleanup-manifest.js',
  );
  return {
    ...actual,
    markSessionEndActionRunner: manifest.markSessionEndActionRunner,
  };
});
...
describe('SessionEnd action runner', () => {
  it('observes a fast child exit via microtask after synchronous identity capture', async () => {
    ...
    await expect(
      runSessionEndAction(context(directory), async () => undefined),
    ).resolves.toEqual({
      code: 'runner-exit-7',
      completed: false,
    });
  ...
  it('waits for a delayed process-tree kill after a deadline even when the child exits immediately', async () => {
    ...
    let finishKill!: () => void;
    processUtils.terminateOwnedProcessTree.mockImplementation(
      () =>
        new Promise<'terminated'>((resolve) => {
          finishKill = () => resolve('terminated');
        }),
    );
    const result = runSessionEndAction(
      { ...context(directory), deadlineAt: now + 10 },
      async () => undefined,
    );
    ...
    expect(processUtils.terminateOwnedProcessTree).toHaveBeenCalledWith(
      expect.objectContaining({
        pid: 12347,
        expectedStartIdentity: 'sync-identity',
        force: true,
      }),
    );
    child.emit('exit', 0);
  ...
  it('passes notification credentials only to notification action children', async () => {
    ...
    await runSessionEndAction(
      context(directory, 'notification'),
      async () => undefined,
    );
    const notificationEnvironment = childProcess.spawn.mock.calls[0][2]
    ...
    await runSessionEndAction(
      context(directory, 'foreground-cleanup'),
      async () => undefined,
    );
    const cleanupEnvironment = childProcess.spawn.mock.calls[1][2]
      .env as NodeJS.ProcessEnv;
  ...
  it('uses original bounded OpenClaw routing instead of a recovering session ambient environment', async () => {
    ...
    const environment = childProcess.spawn.mock.calls[0][2]
      .env as NodeJS.ProcessEnv;
  ...
  it('fails closed without signalling any PID when synchronous identity capture returns null', async () => {
    const directory = mkdtempSync(
      join(tmpdir(), 'omc-action-runner-identity-null-'),
    );
    directories.push(directory);
    const child = Object.assign(new EventEmitter(), {
      pid: 99999,
    ...
    processUtils.getProcessStartIdentitySync.mockReturnValueOnce(
  ...
  it('uses synchronous identity for owned-tree termination on deadline', async () => {
    ...
    processUtils.terminateOwnedProcessTree.mockImplementation(
      () =>
        new Promise<'terminated'>((resolve) =>
          queueMicrotask(() => resolve('terminated')),
        ),
    );
    manifest.markSessionEndActionRunner.mockReturnValue({});
    ...
    expect(processUtils.terminateOwnedProcessTree).toHaveBeenCalledWith(
      expect.objectContaining({
        pid: 88888,
        expectedStartIdentity: 'sync-id-88888',
        force: true,
      }),
    );
    expect(result.completed).toBe(false);
