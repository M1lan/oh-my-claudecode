...
describe('SessionEnd action runner', () => {
  it('observes a fast child exit before publishing an arm or duplicate action', async () => {
    ...
    processUtils.getProcessStartIdentity.mockImplementation(
      () => new Promise((resolve) => setTimeout(() => resolve('identity'), 0)),
    );
  ...
  it('waits for a delayed process-tree kill after a deadline even when the child exits immediately', async () => {
    ...
    let finishKill!: () => void;
    processUtils.killProcessTree.mockImplementation(
      () =>
    ...
    const result = runSessionEndAction(
      { ...context(directory), deadlineAt: Date.now() + 10 },
      async () => undefined,
    );
