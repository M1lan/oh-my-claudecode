...
export type SessionEndActionName =
  | 'foreground-cleanup'
  | 'wiki-capture'
  | 'team-cleanup'
  | 'python-cleanup'
...
export interface SessionEndJobV1 {
  ...
  producers: { core: ProducerSlot; wiki: ProducerSlot };
...
export function sessionEndJobPath(
  directory: string,
  sessionId: string,
): string {
      ...
      try {
        ...
        if (parsed.processStartIdentity && process.platform === 'linux') {
          try {
            ...
            if (
              (stat.substring(stat.lastIndexOf(')') + 2).split(' ')[19] ??
...
function withLock<T>(file: string, body: () => T): T {
  ...
  try {
    ...
    fs.closeSync(fd);
    try {
      const holder = JSON.parse(fs.readFileSync(lock, 'utf8')) as ManifestLock;
      if (holder.nonce === nonce) fs.unlinkSync(lock);
    } catch {
      /* missing or replaced lock is not ours to remove */
    }
  }
}
function readPath(jobPath: string): SessionEndJobV1 | null {
  try {
    return JSON.parse(fs.readFileSync(jobPath, 'utf8')) as SessionEndJobV1;
  } catch {
    return null;
  }
}
function discoveryPath(directory: string): string {
    ...
    if (parsed.version === 2 && Array.isArray(parsed.tickets))
      return {
        ...
        tickets: parsed.tickets
          .filter(
            (ticket): ticket is DiscoveryTicket =>
              !!ticket && typeof ticket.sessionId === 'string',
          )
          .map((ticket) => ({
            sessionId: ticket.sessionId,
            attempts: Number.isInteger(ticket.attempts) ? ticket.attempts : 0,
            retryAt:
              typeof ticket.retryAt === 'string' ? ticket.retryAt : nowIso(),
...
function updateTicket(
  ...
  withLock(file, () => {
    ...
    if (!present)
      index.tickets = index.tickets.filter(
...
function newAction(
  name: SessionEndActionName,
  actionClass: 'required' | 'best-effort',
  payload: Record<string, unknown>,
): SessionEndActionState {
  return {
    class: actionClass,
    phase:
      actionClass === 'required' ? 'deferred-required' : 'deferred-best-effort',
    status: 'pending',
    attempts: 0,
...
export function isManifestTerminal(job: SessionEndJobV1): boolean {
  return (
    ...
    job.owner === null &&
    Object.values(job.actions).every(
      (action) => !action.runner || action.runner.phase === 'terminal',
    )
  );
}
export function readSessionEndJob(
  directory: string,
  sessionId: string,
...
export function mutateSessionEndJob(
  directory: string,
  sessionId: string,
  expectedRevision: number,
  mutate: (job: SessionEndJobV1) => void,
): SessionEndJobV1 | null {
  let terminal = false;
  try {
...
function mutateLatest(
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const result = mutateSessionEndJob(
      directory,
      sessionId,
      current.revision,
      mutate,
...
export function completeForegroundCleanup(
  directory: string,
  sessionId: string,
...
export function sealCoreManifest(
  directory: string,
  sessionId: string,
): SessionEndJobV1 | null {
  return mutateLatest(directory, sessionId, (job) => {
  ...
  try {
    const result = withLock(jobPath, () => {
      ...
      if (!existing) {
        const now = nowIso();
        const actions = Object.fromEntries(
          ACTIONS.map(([name, klass]) => [name, newAction(name, klass, {})]),
        ) as SessionEndJobV1['actions'];
        const wiki = payload
          ? {
        ...
        if (payload) actions['wiki-capture'].payload = payload;
        else {
      ...
      if (payload) next.actions['wiki-capture'].payload = payload;
      else {
...
export function claimSessionEndJob(
  directory: string,
  sessionId: string,
  nonce: string,
  identity: string,
...
export function renewSessionEndLease(
  directory: string,
  sessionId: string,
  nonce: string,
  generation: number,
...
export function reapStaleSessionEndOwner(
  ...
  return mutateSessionEndJob(directory, sessionId, current.revision, (job) => {
    ...
    if (
      Object.values(job.actions).some(
        (action) =>
...
export function releaseSessionEndJob(
  directory: string,
  sessionId: string,
  nonce: string,
  generation: number,
...
export function claimSessionEndAction(
  directory: string,
  sessionId: string,
  ownerNonce: string,
  name: SessionEndActionName,
...
export function markSessionEndActionRunner(
  directory: string,
  sessionId: string,
  ownerNonce: string,
  name: SessionEndActionName,
  runnerNonce: string,
  phase: 'started' | 'armed' | 'result-recorded',
): SessionEndJobV1 | null {
  return updateSessionEndJob(directory, sessionId, ownerNonce, (job) => {
    const action = job.actions[name];
    if (
...
export function finishSessionEndAction(
  directory: string,
  sessionId: string,
  ownerNonce: string,
  name: SessionEndActionName,
...
export function claimSessionEndDiscoveryTickets(
  ...
  limit = 4,
  leaseMs = 15_000,
): Array<{ sessionId: string; nonce: string }> {
  try {
    const file = discoveryPath(directory);
    ...
    const claimed = withLock(file, () => {
      ...
      for (
        let offset = 0;
        offset < index.tickets.length && claimed.length < Math.max(0, limit);
        offset++
      ) {
        const ticket =
...
export function releaseSessionEndDiscoveryTicket(
  directory: string,
  sessionId: string,
  nonce: string,
  spawned: boolean,
  ...
  try {
    ...
    withLock(file, () => {
      const index = readIndex(file);
      const ticket = index.tickets.find((item) => item.sessionId === sessionId);
      if (!ticket || ticket.claimNonce !== nonce) return;
      ticket.claimNonce = undefined;
      ticket.leaseExpiresAt = undefined;
      if (!spawned) {
        ticket.attempts++;
        ticket.retryAt = new Date(
          Date.now() +
            Math.min(30_000, 250 * 2 ** Math.min(ticket.attempts, 7)),
        ).toISOString();
      } else ticket.retryAt = nowIso();
      atomicWriteJsonSync(file, index);
    });
  } catch {
...
export function takeSessionEndDiscoveryPage(
  directory: string,
  limit = 4,
): string[] {
  return claimSessionEndDiscoveryTickets(directory, limit).map(
    (ticket) => ticket.sessionId,
  );
}

/** Recovery may seal a prepared core only after foreground cleanup has a durable completed result. */
export function recoverPreparedCoreProducer(
  directory: string,
  sessionId: string,
): SessionEndJobV1 | null {
  return mutateLatest(directory, sessionId, (job) => {
    if (
      !['prepared', 'sealed'].includes(job.producers.core.state) ||
