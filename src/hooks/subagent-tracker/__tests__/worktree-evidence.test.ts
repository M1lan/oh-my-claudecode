...
describe('collectWorktreeDirtyEvidence', () => {
  ...
  it('is fail-closed and never throws', () => {
    expect(() =>
      collectWorktreeDirtyEvidence(join(makeTempDir(), 'nope')),
    ).not.toThrow();
    const repo = makeTempDir();
    initRepo(repo);
    expect(() =>
...
describe('isAbnormalTermination', () => {
  it('treats explicit failure as abnormal', () => {
    ...
    expect(
      isAbnormalTermination({ success: false, output: 'any output' }),
    ).toBe(true);
  });
  ...
  it('never classifies a successful final report as abnormal (issue #3663 B6)', () => {
    ...
    expect(
      isAbnormalTermination({
        success: true,
        output:
          'The parser now quotes "Agent terminated early due to an API error" correctly.',
      }),
    ).toBe(false);
    ...
    expect(
      isAbnormalTermination({
        output:
          'Final report: the fix covers "Response stalled mid-stream" handling and <status>failed</status> quoting in docs.',
      }),
    ).toBe(false);
    expect(
      isAbnormalTermination({
  ...
  it('treats normal completion and cancel-like stops as non-abnormal', () => {
    ...
    expect(isAbnormalTermination({ success: true, output: 'done' })).toBe(
      false,
    );
    expect(
      isAbnormalTermination({
