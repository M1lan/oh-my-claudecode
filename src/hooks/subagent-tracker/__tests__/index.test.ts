...
describe('subagent-tracker', () => {
  ...
  describe('processSubagentStart', () => {
    ...
    it('persists Agent-tool name/description for unnamed-agent addressability (#3665)', () => {
      ...
      const tracked = state.agents.find(
        (a) => a.agent_id === 'ae1e2be26cb41fc74',
      );
  ...
  describe('processSubagentStop', () => {
    it('updates tracking state without injecting additional context into the stopping subagent', () => {
      ...
      const agent = state.agents.find(
        (item) => item.agent_id === 'worker-stop-output',
      );
      expect(agent?.status).toBe('completed');
      expect(agent?.output_summary).toBe(
        'Detailed final report with implementation evidence.',
      );
    ...
    it('closes the sole running agent when a fork stop arrives with an unmatched agent_id', () => {
      ...
      expect(state.agents.filter((a) => a.status === 'running')).toHaveLength(
        0,
      );
      const reconciled = state.agents.find(
        (a) => a.agent_id === 'registered-agent',
      );
      ...
      expect(
        state.agents.some((a) => a.agent_id === 'native-fork-agent-id'),
      ).toBe(false);
      expect(state.total_completed).toBe(1);
    });
    ...
    it('reconciles an unmatched fork stop by agent_type when one type matches', () => {
      ...
      expect(state.agents.some((a) => a.agent_id === 'native-fork-id')).toBe(
        false,
      );
      expect(state.total_completed).toBe(1);
    });
    ...
    it('reaps stale running agents and records a synthetic stop when reconciliation is ambiguous', () => {
      ...
      expect(state.agents.find((a) => a.agent_id === 'stale-1')?.status).toBe(
        'failed',
      );
      expect(state.agents.find((a) => a.agent_id === 'stale-2')?.status).toBe(
        'failed',
      );
    ...
    it('does not corrupt fresh running agents from a different concurrent stop', () => {
      ...
      expect(state.agents.find((a) => a.agent_id === 'fresh-1')?.status).toBe(
        'running',
      );
      expect(state.agents.find((a) => a.agent_id === 'fresh-2')?.status).toBe(
        'running',
      );
      expect(
  ...
  describe('processSubagentStop — worktree dirty evidence (#3663)', () => {
    ...
    it('records bounded dirty-worktree evidence on abnormal stop', () => {
      ...
      const stop = events.find(
        (e) =>
          e.event === 'agent_stop' && e.agent === 'ag-dirty-1'.substring(0, 7),
      );
    ...
    it('derives lifecycle success from abnormal classification across all surfaces (issue #3663 B2)', () => {
      ...
      const replayStop = readReplayEvents(repoDir, 'sess-b2-marker').find(
        (e) =>
          e.event === 'agent_stop' &&
          e.agent === 'ag-b2-marker'.substring(0, 7),
      );
      expect(replayStop?.success).toBe(false);
      expect(getReplaySummary(repoDir, 'sess-b2-marker').agents_failed).toBe(1);
      expect(getReplaySummary(repoDir, 'sess-b2-marker').dirty_worktrees).toBe(
        1,
      );

      // Mission board reflects the failure too.
      const mission = readMissionBoardState(
        repoDir,
        'sess-b2-marker',
      )?.missions.find((m) => m.id.startsWith('session:sess-b2-marker:'));
      const missionAgent = mission?.agents.find(
        (a) => a.ownership === 'ag-b2-marker',
      );
    ...
    it('treats a successful final report that mentions API-error phrases as completed (issue #3663 B6)', () => {
      ...
      const replayStop = readReplayEvents(repoDir, 'sess-b6-quote').find(
        (e) =>
          e.event === 'agent_stop' && e.agent === 'ag-b6-quote'.substring(0, 7),
      );
    ...
    it('clears stale dirty-worktree evidence on agent-id reuse to running (issue #3663 B4)', () => {
      ...
      flushPendingWrites();
      expect(
        readTrackingState(repoDir, 'sess-b4-reuse').agents.find(
          (a) => a.agent_id === 'ag-reused-1',
      ...
      expect(getReplaySummary(repoDir, 'sess-b4-reuse').dirty_worktrees).toBe(
        1,
      );
      const replayStops = readReplayEvents(repoDir, 'sess-b4-reuse').filter(
        (e) =>
          e.event === 'agent_stop' && e.agent === 'ag-reused-1'.substring(0, 7),
      );
    ...
    it('persists durably under the already-held lock instead of re-entering it (issue #3663 R1)', () => {
      ...
      // unlocked overwrite.
      expect(disk.agents.map((a) => a.agent_id)).toContain(
        'ag-concurrent-peer',
      );
