...
describe('session isolation', () => {
  ...
  beforeEach(() => {
    testDir = join(
      tmpdir(),
      `subagent-isolation-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it('session-scoped files do not interfere with legacy (no-session) path', () => {
    ...
    expect(readLegacy.agents.map((a) => a.agent_id)).not.toContain(
      'agent-session',
    );

    expect(readSession.agents.map((a) => a.agent_id)).toContain(
      'agent-session',
    );
    expect(readSession.agents.map((a) => a.agent_id)).not.toContain(
      'agent-legacy',
    );
  });
});
