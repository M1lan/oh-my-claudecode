...
describe("nonInteractiveEnvHook", () => {
  it("warns for simple banned interactive commands", async () => {
    const result =
      await nonInteractiveEnvHook.beforeCommand?.("less README.md");

    expect(result).toEqual({
      command: "less README.md",
      warning:
        "Warning: 'less' is an interactive command that may hang in non-interactive environments.",
    });
  });
  ...
  it("keeps git warnings when also prepending env vars", async () => {
    const result = await nonInteractiveEnvHook.beforeCommand?.(
      "git add -p src/hooks/non-interactive-env/index.ts",
    );
