...
describe('merge-readiness standalone tool flow', () => {
  ...
  it('start -> set_content -> record_answer reaches pass', async () => {
    ...
    const startRes = await start.handler({
      summary: '/merge-readiness --quick fix a bug',
    ...
    const pendingReport = await report.handler({
      workingDirectory: tempDir,
      session_id: session,
    });
    ...
    const firstAnswer = await recordAnswer.handler({
      questionId: 'q1',
      optionId: 'a',
      workingDirectory: tempDir,
      session_id: session,
    ...
    const pendingState = await stateRead.handler({
    ...
    for (const q of questions.slice(1)) {
      const res = await recordAnswer.handler({
        questionId: q.id,
        optionId: 'a',
        workingDirectory: tempDir,
        session_id: session,
  ...
  it('exposes cancellation and state_clear as durable audit-preserving operations', async () => {
    ...
    await start.handler({
      summary: '/merge-readiness --quick change',
      workingDirectory: tempDir,
      session_id: session,
    });
    const cancelResult = await cancel.handler({
      workingDirectory: tempDir,
      session_id: session,
    });
    ...
    const clearResult = await clear.handler({
      mode: 'merge-readiness',
      workingDirectory: tempDir,
      session_id: session,
    });
    ...
    expect(readMergeReadinessState(tempDir, session)?.result).toBe('cancelled');
    const repeatedClear = await clear.handler({
      mode: 'merge-readiness',
      workingDirectory: tempDir,
      session_id: session,
    });
    expect(textOf(repeatedClear)).toContain(
      'No active merge-readiness gate found',
    );
    expect(readMergeReadinessState(tempDir, session)?.result).toBe('cancelled');
  });
  ...
  it('state_get_status does not leak correctOptionId/rationale while pending', async () => {
    ...
    const status = await getStatus.handler({
      mode: 'merge-readiness',
      workingDirectory: tempDir,
      session_id: session,
    });
  ...
  it("state_clear without session_id only cancels the caller's session", async () => {
    ...
    try {
      ...
      if (prevSid === undefined) delete process.env.CLAUDE_SESSION_ID;
      else process.env.CLAUDE_SESSION_ID = prevSid;
    }
    // sess-A cancelled, sess-B untouched (no cross-session clear).
    expect(readMergeReadinessState(tempDir, 'sess-A')?.result).toBe(
      'cancelled',
    );
    expect(readMergeReadinessState(tempDir, 'sess-B')?.active).toBe(true);
  });
  ...
  it('state_clear does not advance on-disk state on failed write (durable non-advance)', async () => {
    ...
    expect(readMergeReadinessState(tempDir, session)?.active).toBe(true);
    expect(readMergeReadinessState(tempDir, session)?.result).not.toBe(
      'cancelled',
    );
  });
  ...
  it('record_answer surfaces a failed write as an error, not a normal terminal', async () => {
    ...
    let res: any;
    try {
      res = await recordAnswer.handler({
        questionId: 'q1',
        optionId: 'a',
        workingDirectory: tempDir,
        session_id: session,
