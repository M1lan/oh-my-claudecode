...
describe('merge-readiness Windows cross-platform contract', () => {
  ...
  it('constructs the session-scoped state path with native separators and validates the id', () => {
    ...
    expect(
      statePath.endsWith(join(sessionId, 'merge-readiness-state.json')),
    ).toBe(true);
  ...
  it('resolves the worktree root from a subdirectory (not a subdirectory itself)', () => {
    ...
    if (isWin32) {
      ...
      expect(
        norm(root).endsWith(norm(tempDir).split('/').slice(-2).join('/')),
      ).toBe(true);
    }
    if (isWin32) {
      // Drive-letter or UNC prefix must survive the climb on win32.
      expect(/^[A-Za-z]:[\\/]/.test(root) || root.startsWith('\\\\')).toBe(
        true,
      );
    }
  });
  ...
  it('collects git evidence on the native platform (git exec resolves on win32)', () => {
    ...
    const gitGracefulFailures = evidence.missingEvidence.filter((m) =>
      /git \w+ failed\s\(\(\d\)\)/i.test(m),
    );
  ...
  it('normalizes backslash artifact paths to forward slashes on win32 (listArtifactFiles contract)', () => {
    ...
    writeFileSync(
      join(tempDir, '.omc', 'specs', 'design.md'),
      'spec for the change\n',
    );
    // Plant a merge-readiness artifact that must be EXCLUDED from source evidence.
    mkdirSync(join(tempDir, '.omc', 'artifacts', 'merge-readiness'), {
      recursive: true,
    });
    writeFileSync(
      join(tempDir, '.omc', 'artifacts', 'merge-readiness', 'self.md'),
      'self\n',
    );
    ...
    expect(evidence.sourceArtifacts.every((f) => f.indexOf(BS) === -1)).toBe(
      true,
    );
    // Merge-readiness own artifacts are excluded (no self-reference in evidence).
    expect(
      evidence.sourceArtifacts.every((f) => !f.includes('merge-readiness')),
    ).toBe(true);
  ...
  it('persists session-scoped state readable via the same resolver (short-name tmpdir on win32)', () => {
    ...
    if (isWin32 && tmpdir().indexOf('~1') === -1) {
      console.warn(
        '[win-cross-platform] tmpdir is long-form; short-name round-trip not exercised on this host',
      );
    }
    const sessionId = 'win-persist-session';
    const state = createInitialMergeReadinessState(
      tempDir,
      '/merge-readiness --standard win change',
      sessionId,
    );
  ...
  it('slugifies change summaries identically across platforms (no separator leakage)', () => {
    ...
    expect(slugifyMergeReadiness('Fix auth on Windows: D:\\path issue')).toBe(
      'fix-auth-on-windows-d-path-issue',
    );
  ...
  it('invokes git.exe successfully on win32 (no ETIMEDOUT / no exec spawn failure)', () => {
    ...
    expect(evidence.missingEvidence.some((m) => /timed out/i.test(m))).toBe(
      false,
    );
    // No "git <cmd> failed (exit ?)" with an unknown exit (spawn/ENOENT) => git resolved.
    const spawnFailures = evidence.missingEvidence.filter(
      (m) => /failed \(exit \?\)\)/i.test(m) && !/\d\)/.test(m),
    );
