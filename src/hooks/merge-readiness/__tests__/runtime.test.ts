...
vi.mock('../../../lib/mode-state-io.js', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('../../../lib/mode-state-io.js')>();
  return {
    ...actual,
    writeModeState: (
      mode: string,
...
describe('merge-readiness runtime', () => {
  ...
  const originalPrincipal =
    process.env.OMC_MERGE_READINESS_AUTHENTICATED_PRINCIPAL;
  const originalMaintainers = process.env.OMC_MERGE_READINESS_MAINTAINERS;

  beforeEach(() => {
    process.env.OMC_MERGE_READINESS_AUTHENTICATED_PRINCIPAL =
      'github:trusted-maintainer';
  ...
  it('creates the authoritative audit state without writing a report artifact', () => {
    ...
    expect(state.required_dimensions).toEqual([
      'why',
      'change',
      'tradeoff',
      'risk',
    ...
    expect(
      existsSync(join(tempDir, '.omc', 'artifacts', 'merge-readiness')),
    ).toBe(false);
    ...
    expect(persisted?.evidence.changedFiles).toEqual(
      state.evidence.changedFiles,
    );
  });
  ...
  it('uses quick profile thresholds (0.70 / 3 rounds / why-change-risk)', () => {
    const state = createInitialMergeReadinessState(
      tempDir,
      '/merge-readiness --quick docs gate',
      sessionId,
    );
  ...
  it('uses deep profile thresholds (0.90 / 8 rounds / all five dims)', () => {
    const state = createInitialMergeReadinessState(
      tempDir,
      '/merge-readiness --deep risky change',
      sessionId,
    );
  ...
  it('setMergeReadinessContent persists doc + MCQs in state and arms first MCQ', () => {
    createInitialMergeReadinessState(
      tempDir,
      '/merge-readiness --standard explain docs change',
      sessionId,
    );
    ...
    expect(readMergeReadinessState(tempDir, sessionId)?.whatChanged).toBe(
      'What changed text',
    );
    expect(
      existsSync(join(tempDir, '.omc', 'artifacts', 'merge-readiness')),
  ...
  it('rejects content that does not cover every required dimension (anti-gaming)', () => {
    ...
    const state = setMergeReadinessContent(
      tempDir,
      {
        ...
        tradeoffs: 't',
        risksConsidered: 'r',
        teamUnderstanding: 'tu',
        questions: [
          makeQuestion('q1', 'why'),
          makeQuestion('q2', 'why'),
          makeQuestion('q3', 'risk'),
        ],
      },
      sessionId,
    );
    expect(state?.validation_errors?.some((e) => e.includes('change'))).toBe(
      true,
    );
    expect(state?.phase).toBe('content');
  ...
  it('does not start the gate on an unrelated artifact (fail-open fix)', () => {
    ...
    try {
      ...
      const state = createInitialMergeReadinessState(
        dir,
        '/merge-readiness --standard change',
        sessionId,
      );
  ...
  it('preserves a full prior-attempt audit record with questions, answers, and scores', () => {
    ...
    expect(
      prior?.questions.every((q) => typeof q.correctOptionId === 'string'),
    ).toBe(true);
    ...
    expect(Object.keys(prior?.dimension_scores ?? {}).length).toBeGreaterThan(
      0,
    );
    ...
    const state2 = createInitialMergeReadinessState(
      tempDir,
      '/merge-readiness --quick retry2',
      sessionId,
    );
  ...
  it('retains the five narrative sections in a prior-attempt audit record', () => {
    ...
    setMergeReadinessContent(
      tempDir,
      {
        ...
        whatChanged: 'wc-text',
        tradeoffs: 't-text',
        risksConsidered: 'r-text',
        teamUnderstanding: 'tu-text',
        questions: [
          makeQuestion('q1', 'why'),
          makeQuestion('q2', 'change'),
          makeQuestion('q3', 'risk'),
  ...
  it('redacts prior-attempt answer keys on state_read while the current attempt is pending', () => {
    ...
    expect(state.result).toBe('pending');
    expect(
      state.prior_attempts?.[0].questions.every(
        (q) => typeof q.correctOptionId === 'string',
      ),
    ).toBe(true);
    ...
    expect(
      redacted.questions.every((q) => q.correctOptionId === undefined),
    ).toBe(true);
    expect(
      redacted.prior_attempts[0].questions.every(
        (q) => q.correctOptionId === undefined,
      ),
    ).toBe(true);
    expect(
      redacted.prior_attempts[0].answers.every(
        (answer) =>
  ...
  it('binds the resolved base ref into the no-diff recovery tool call', () => {
    ...
    expect(msg).toContain(
      'merge_readiness_start with summary "--from-diff" and baseRef "origin/dev"',
    );
  ...
  it('mutators never surface a phantom pass/override when the write cannot land', () => {
    ...
    expect(() =>
      overrideMergeReadiness(tempDir, 'Maintainer override.', 'bad/session'),
    ).toThrow();
  ...
  it("fails closed without recursing when a mutator's write cannot land (read-only FS / full disk)", () => {
    ...
    try {
      const state = overrideMergeReadiness(
        tempDir,
        'Maintainer override.',
        sessionId,
      );
      ...
      expect(
        state?.validation_errors?.filter((e) => e.includes('persisted')),
      ).toHaveLength(1);
    } finally {
  ...
  it.runIf(process.platform !== 'win32')(
    'fails closed on a real disposable POSIX directory write fault',
    () => {
      createInitialMergeReadinessState(
        tempDir,
        '/merge-readiness --quick change',
      ...
      try {
        const result = setMergeReadinessContent(
          tempDir,
          {
            why: 'w',
            whatChanged: 'wc',
  ...
  it('blocks when minimal evidence is missing (no diff/change signal)', () => {
    ...
    try {
      ...
      createInitialMergeReadinessState(
        emptyDir,
        '/merge-readiness --standard no changes here',
        sessionId,
      );
      setMergeReadinessContent(
        emptyDir,
  ...
  it('parses source flags exactly and rejects conflicting source modes', () => {
    const lookalike = createInitialMergeReadinessState(
      tempDir,
      '/merge-readiness summary--from-diff',
      sessionId,
    );
    ...
    expect(conflicting.validation_errors).toContain(
      '--from-diff and --from-artifacts cannot be used together.',
    );
    expect(
      setMergeReadinessContent(
        tempDir,
  ...
  it('does not accept untracked files as --from-diff evidence', () => {
    const emptyDir = mkdtempSync(
      join(tmpdir(), 'omc-merge-readiness-untracked-'),
    );
    try {
      execFileSync('git', ['init'], {
        cwd: emptyDir,
      ...
      const state = createInitialMergeReadinessState(
        emptyDir,
        '/merge-readiness --from-diff only untracked',
        sessionId,
      );
  ...
  it('cancels a blocked gate while retaining a terminal audit record', () => {
    ...
    try {
      ...
      const state = createInitialMergeReadinessState(
        emptyDir,
        '/merge-readiness --from-diff no changes',
        sessionId,
      );
      expect(state.result).toBe('blocked');
      ...
      expect(readMergeReadinessState(emptyDir, sessionId)?.result).toBe(
        'cancelled',
      );
  ...
  it('keeps invalid content recoverable and does not arm a dead-end quiz', () => {
    ...
    expect(state?.validation_errors?.join(' ')).toContain(
      "Narrative section 'why'",
    );
  });
  ...
  it('releases the soft gate only through a reasoned override', async () => {
    ...
    expect(overrideMergeReadiness(tempDir, '', sessionId)?.result).toBe(
      'pending',
    );
    expect(
      overrideMergeReadiness(
        tempDir,
        'Maintainer accepts the documented gap.',
        sessionId,
      )?.result,
    ).toBe('overridden');
  ...
  it('does not start the gate on an unrelated artifact in --from-artifacts mode', () => {
    ...
    try {
      ...
      const state = createInitialMergeReadinessState(
        dir,
        '/merge-readiness --from-artifacts change',
        sessionId,
      );
      expect(state.result).toBe('blocked');
    } finally {
  ...
  it('counts an active ralph mode-state file as evidence', () => {
    ...
    try {
      ...
      expect(state.evidence.sourceArtifacts).toContain(
  ...
  it('override records the authenticated maintainer principal, not caller-supplied session_id', () => {
    ...
    expect(readMergeReadinessState(tempDir, sessionId)?.override_owner).toBe(
      'github:trusted-maintainer',
    );
  });
  ...
  it('rejects an override when the server has no allowlisted authenticated maintainer principal', () => {
    ...
    expect(
      state?.validation_errors?.some((error) =>
        error.includes('authenticated maintainer principal'),
      ),
    ).toBe(true);
  ...
  describe('15-case failed-write durable-non-advance transition matrix', () => {
    ...
    it.each([
      ...
      },
      {
        ...
        action: (sid: string) =>
          recordMergeReadinessMCQAnswer(tempDir, 'q3', 'a', sid),
      },
      {
        ...
        action: (sid: string) =>
          recordMergeReadinessMCQAnswer(tempDir, 'q3', 'b', sid),
      },
      {
        name: 'maintainer override transition',
        setup: (sid: string) => seedPending(sid),
        action: (sid: string) =>
          overrideMergeReadiness(tempDir, 'Maintainer override.', sid),
      },
      {
      ...
      },
      {
        ...
        action: (sid: string) =>
          overrideMergeReadiness(tempDir, 'Attempt blocked override.', sid),
      },
      {
      ...
      'does not advance durable state on failed write: $name',
      ({ setup, action }) => {
    ...
    it('set-content does not advance on-disk state on failed write', () => {
      ...
      persistFail.failWrites = true;
      try {
        const res = setMergeReadinessContent(
          tempDir,
          {
            why: 'w2',
            whatChanged: 'wc2',
    ...
    it('answer does not advance on-disk state on failed write', () => {
      ...
      persistFail.failWrites = true;
      try {
        const res = recordMergeReadinessMCQAnswer(
          tempDir,
          'q1',
          'a',
          sessionId,
    ...
    it('history append (re-start) does not advance on-disk state on failed write', () => {
      ...
      recordMergeReadinessMCQAnswer(tempDir, 'q2', 'b', sessionId);
      recordMergeReadinessMCQAnswer(tempDir, 'q3', 'b', sessionId);
      expect(readMergeReadinessState(tempDir, sessionId)?.result).toBe(
        'paused',
      );
      const before = readMergeReadinessState(tempDir, sessionId)!;
  ...
  it("counts the current session's session-scoped mode-state file as evidence", () => {
    ...
    try {
      ...
      mkdirSync(join(dir, '.omc', 'state', 'sessions', sessionId), {
        recursive: true,
      });
      writeFileSync(
      ...
      expect(state.evidence.sourceArtifacts).toContain(
        `state/sessions/${sessionId}/ralph-state.json`,
      );
      expect(state.result).toBe('pending');
    } finally {
      rmSync(dir, { recursive: true, force: true });
  ...
  it('uses the authenticated maintainer principal for legacy/no-session override', () => {
    ...
    expect(state?.override_owner).toBe('github:trusted-maintainer');
    expect(readMergeReadinessState(tempDir, undefined)?.override_owner).toBe(
      'github:trusted-maintainer',
    );
  });
  ...
  it('cancel records the operator identity (cancel_owner parity with override)', () => {
    ...
    expect(readMergeReadinessState(tempDir, sessionId)?.cancel_owner).toBe(
      sessionId,
    );
  });
