  ...
  return questions
    .map((question, index) => {
      ...
      const options = question.options.map((option) => {
        const marks: string[] = [];
        if (
          (revealAll || (revealAnswered && answer)) &&
      ...
      const correctness =
        answer && (revealAll || revealAnswered)
...
function renderPriorAttempt(
  ...
  const header = [
    ...
    attempt.override_reason
      ? `- Override reason: ${attempt.override_reason}`
      : '',
  ...
  const dimensions =
    attempt.required_dimensions.length > 0
      ? attempt.required_dimensions
          .map(
            (dimension) =>
              `- ${dimension}: ${Math.round((attempt.dimension_scores[dimension] ?? 0) * 100)}%`,
          )
          .join('\n')
...
export function formatMergeReadinessReport(state: MergeReadinessState): string {
  ...
  const dimensions = revealAssessment
    ? state.required_dimensions
        .map(
          (dimension) =>
            `- ${dimension}: ${Math.round((state.dimension_scores[dimension] ?? 0) * 100)}%`,
        )
        .join('\n')
  ...
  return [
    ...
    '',
    renderList(
      state.evidence.missingEvidence,
    ...
    state.prior_attempts && state.prior_attempts.length > 0
      ? [
          '## Prior Attempts',
          '',
          ...state.prior_attempts.map((a, i) =>
            renderPriorAttempt(a, i, revealAssessment),
          ),
        ].join('\n')
      : '',
    '',
...
export function redactMergeReadinessState(
  ...
  const redacted: Record<string, unknown> = {
    ...
    questions: state.questions.map((question) =>
      shouldRevealQuestion(question) ? question : redactQuestion(question),
    ),
    pending_question: state.pending_question
      ? shouldRevealQuestion(state.pending_question)
        ? state.pending_question
        : redactQuestion(state.pending_question)
      : undefined,
