...
export const WORKFLOW_TRANSCRIPT_RECORD_TOO_LARGE =
  'workflow_transcript_record_too_large';
const namedWorkflowTranscriptFailures = new Map<string, string>();
export function takeNamedWorkflowTranscriptFailure(
...
function clearNamedWorkflowTranscriptFailure(
  sessionId: string | undefined,
): void {
...
function validBoundary(
  ...
  const boundary = value as unknown as NonNullable<
    NonNullable<AutopilotState['pipelineTracking']>['activationBoundary']
  >;
  ...
  try {
    ...
    if (
      ...
      return false;
    return (
      hashTranscriptRange(opened.fd, 0, boundary.byteOffset) ===
...
function scanTranscriptJsonl(
  ...
  const emitRecord = (crlf: boolean): boolean => {
    const length =
      recordBytes -
      (crlf && recordBytes > 0 && record[recordBytes - 1] === 0x0d ? 1 : 0);
    ...
    return (
      ...
      callback(
        ...
        byteOffset,
        lineNumber,
        createHash('sha256').update(record.subarray(0, length)).digest('hex'),
      )
    );
  };
  for (let offset = start; offset < end;) {
    const maxRead =
      recordBytes >= MAX_JSONL_RECORD_BYTES
        ? 1
        : MAX_JSONL_RECORD_BYTES + 1 - recordBytes;
    const count = readSync(
      fd,
      chunk,
      0,
      Math.min(chunk.length, end - offset, maxRead),
      offset,
    );
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      if (byte === 0x0a) {
        ...
        (recordBytes === MAX_JSONL_RECORD_BYTES && byte !== 0x0d) ||
        recordBytes > MAX_JSONL_RECORD_BYTES
      ) {
        namedWorkflowTranscriptFailures.set(
          sessionId,
          WORKFLOW_TRANSCRIPT_RECORD_TOO_LARGE,
        );
        return false;
...
type StableTranscript = {
  fd: number;
  path: string;
  identity: RecordValue;
  hashRange: (start: number, end: number) => string | null;
  scanJsonl: (
    start: number,
    end: number,
    callback?: (
...
function closeStableTranscript(transcript: StableTranscript | null): void {
  if (transcript) closeSync(transcript.fd);
}
function readStableTranscript(
  path: string,
  sessionId: string,
  root: string,
): StableTranscript | null {
    ...
    return {
      ...
      hashRange: (start, end) =>
        start >= 0 && end >= start && end <= size
          ? hashTranscriptRange(fd, start, end)
          : null,
      scanJsonl: (start, end, callback) =>
        start >= 0 && end >= start && end <= size
          ? scanTranscriptJsonl(fd, start, end, sessionId, callback)
          : false,
    };
  } catch {
    return null;
  } finally {
    if (opened.fd !== -1) closeSync(opened.fd);
  }
}
...
export function validateNamedWorkflowStateStructure(
  ...
  for (
    ...
    if (
      ...
      !validBoundaryShape(observation.activationBoundary, sessionId) ||
...
export function refreshNamedWorkflowBoundaryForCommit(
  ...
  try {
    ...
    const observation =
      advance.updated.pipelineTracking?.completionObservations.at(-1);
...
function findCompletionEvidence(
  ...
  const valid = transcript.scanJsonl(
    boundaryOffset,
    endOffset,
    (line, byteOffset, lineNumber, hash) => {
