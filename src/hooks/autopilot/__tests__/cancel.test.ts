...
describe('AutopilotCancel', () => {
  ...
  describe('cancelAutopilot', () => {
    ...
    it('should successfully cancel active autopilot and preserve state', () => {
      ...
      expect(result.message).toContain(
        'Autopilot cancelled at phase: expansion',
      );
    ...
    it('should preserve state at different phases', () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain(
        'Autopilot cancelled at phase: planning',
      );
    ...
    it('cancels a structurally valid exact named run', () => {
      ...
      expect(cancelAutopilot(testDir, sessionId)).toMatchObject({
        success: true,
        preservedState: { active: false, workflowRunId: state.workflowRunId },
      });
      expect(readAutopilotState(testDir, sessionId)).toMatchObject({
        active: false,
    ...
    it.skipIf(process.platform !== 'linux')(
      'does not clean linked state when the primary named mutation lock is held',
      () => {
        ...
        const stat = require('fs').readFileSync(
    ...
    it('retries failed dependent cleanup for an already-paused named run', () => {
      ...
      expect(ralphLoop.clearRalphState).toHaveBeenCalledWith(
        testDir,
        sessionId,
      );
    });
    ...
    it('should not clear other session ralph/ultraqa state when sessionId provided', () => {
      ...
      expect(ultraqaLoop.readUltraQAState).toHaveBeenCalledWith(
        testDir,
        sessionId,
      );
      expect(ralphLoop.clearRalphState).not.toHaveBeenCalled();
  ...
  describe('canResumeAutopilot', () => {
    ...
    it('does not let stale resume cleanup delete a replacement run', () => {
      ...
      expect(readAutopilotState(testDir)).toMatchObject({
        active: true,
        originalIdea: 'replacement run',
      });
    });
  ...
  describe('resumeAutopilot', () => {
    ...
    it('should preserve all state data on resume', () => {
      ...
      expect(result.success).toBe(true);
      expect(result.state?.execution.files_created).toEqual([
        'file1.ts',
        'file2.ts',
      ]);
    ...
    it('rejects a descriptor-less paused workflow marker without mutating bytes', () => {
      ...
      expect(canResumeAutopilot(testDir)).toMatchObject({
        canResume: false,
        integrityFailed: true,
      });
      expect(resumeAutopilot(testDir)).toMatchObject({
        success: false,
        message: 'workflow_descriptor_integrity_failed',
      });
    ...
    it.skipIf(process.platform !== 'linux')(
      'rejects a named traversal boundary without mutating paused bytes',
      () => {
        ...
        Object.assign(state, {
          ...
          pipelineTracking: {
            ...
            trackingRevision: 0,
            activationBoundary: {
              transcriptPath: `${encodedProject}${sep}nested${sep}..${sep}${sessionId}.jsonl`,
              transcriptRoot: root,
              transcriptBasename: `${sessionId}.jsonl`,
              sessionId,
              byteOffset: 0,
        ...
        writeAutopilotState(testDir, state, sessionId);
        const stateFile = resolveSessionStatePath(
          'autopilot',
          sessionId,
          testDir,
        );
        ...
        Object.assign(
          state.pipelineTracking!.activationBoundary!.fileIdentity,
          {
            device: validStat.dev,
            inode: validStat.ino,
          },
        );
        ...
        process.env.OMC_TEST_CONDITIONAL_WRITE_REPLACEMENT_BASE64 = Buffer.from(
          JSON.stringify(replacement),
        ).toString('base64');
        expect(resumeAutopilot(testDir, sessionId)).toMatchObject({
          success: false,
        ...
        expect(finalResume.message).toBe(
          'Resuming autopilot at phase: ralplan',
        );
        expect(finalResume).toMatchObject({
          success: true,
          state: { active: true, workflowRunId: state.workflowRunId },
        });
      },
    );
    it.skipIf(process.platform !== 'linux')(
      'rejects forged completion observations and resumes an authenticated advanced named workflow',
      () => {
        ...
        forged.pipelineTracking!.completionObservations![0].recordContentSha256 =
          '0'.repeat(64);
        ...
        const skippedIdentity = {
          device: skippedStat.dev,
          inode: skippedStat.ino,
          size: skippedStat.size,
          mtimeNs: '0',
          ctimeNs: '0',
          contentSha256: createHash('sha256')
            .update(skippedContent)
            .digest('hex'),
        };
        const skipped = structuredClone(state);
        skipped.pipelineTracking!.completionObservations![0].stableFile =
          skippedIdentity;
        skipped.pipelineTracking!.completionObservations![0].recordContentSha256 =
          createHash('sha256').update(skippedRecord).digest('hex');
        skipped.pipelineTracking!.activationBoundary!.fileIdentity =
          skippedIdentity;
        skipped.pipelineTracking!.activationBoundary!.byteOffset =
          skippedIdentity.size;
        ...
        expect(resumeAutopilot(testDir, sessionId)).toMatchObject({
          success: true,
          state: { active: true, phase: 'execution' },
        });
      },
    );
  });
  ...
  describe('formatCancelMessage', () => {
    ...
    it('should handle zero progress in summary', () => {
      ...
      const result: CancelResult = {
        success: true,
        message:
          'Autopilot cancelled at phase: expansion. Progress preserved for resume.',
    ...
    it('should handle cleanup message in preserved state format', () => {
      ...
      const result: CancelResult = {
        success: true,
        message:
          'Autopilot cancelled at phase: expansion. Cleaned up: ralph, ultrawork. Progress preserved for resume.',
        preservedState: state,
      };
