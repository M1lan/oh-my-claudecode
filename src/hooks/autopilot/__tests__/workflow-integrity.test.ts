...
describe('workflow descriptor integrity enforcement (#3487)', () => {
  ...
  it.skipIf(process.platform !== 'linux')(
    ...
    async () => {
      ...
      expect(persisted.pipelineTracking?.completionObservations).toHaveLength(
        1,
      );
      ...
      truncatedTerminal.pipelineTracking!.activationBoundary = structuredClone(
        terminal.pipelineTracking!.completionObservations![1]
          .activationBoundary,
      );
      expect(
        validateNamedWorkflowState(truncatedTerminal, sessionId),
      ).toBeNull();
