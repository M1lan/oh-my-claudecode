...
describe("Wiki Lint", () => {
  ...
  describe("orphan detection", () => {
    it("should detect orphan pages (no incoming links)", () => {
      ...
      const orphanIssue = report.issues.find(
        (i) => i.type === "orphan" && i.page === "orphan.md",
      );
    ...
    it("should not flag pages with incoming links as orphans", () => {
      ...
      writePage(
        tempDir,
        makePage("source.md", { title: "Source", links: ["target.md"] }),
      );

      const report = lintWiki(tempDir);
      const targetOrphan = report.issues.find(
        (i) => i.type === "orphan" && i.page === "target.md",
      );
  ...
  describe("stale detection", () => {
    it("should detect stale pages", () => {
      ...
      writePage(
        tempDir,
        makePage("stale.md", { title: "Stale", updated: oldDate }),
      );
    ...
    it("should not flag recent pages as stale", () => {
      ...
      const staleIssue = report.issues.find(
        (i) => i.type === "stale" && i.page === "fresh.md",
      );
  ...
  describe("low confidence", () => {
    it("should flag low confidence pages", () => {
      writePage(
        tempDir,
        makePage("low.md", { title: "Low", confidence: "low" }),
      );
  ...
  describe("oversized pages", () => {
    it("should flag pages exceeding max size", () => {
      ...
      writePage(
        tempDir,
        makePage("big.md", { title: "Big", content: bigContent }),
      );
    ...
    it("should not flag normal-sized pages", () => {
      writePage(
        tempDir,
        makePage("small.md", { title: "Small", content: "Short content." }),
      );
  ...
  describe("structural contradictions", () => {
    ...
    it("should detect tags appearing in different categories", () => {
      ...
      const report = lintWiki(tempDir);
      const tagContra = report.issues.find(
        (i) =>
          i.type === "structural-contradiction" &&
