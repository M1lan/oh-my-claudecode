...
export function formatCompactSummary(checkpoint: CompactCheckpoint): string {
  ...
  if (refs?.prd || refs?.boulder) {
    ...
    if (refs.prd) {
      const prd = refs.prd;
      lines.push(
        `- **PRD:** ${prd.title} (${prd.stories_completed}/${prd.stories_total} stories, status: ${prd.status})`,
      );
    ...
    if (refs.boulder) {
      ...
      lines.push(
        `- **Boulder plan:** ${boulder.plan_name} (${boulder.progress.completed}/${boulder.progress.total} steps)`,
      );
  ...
  lines.push(
    'Review active modes and plan references above to ensure continuity after compaction.',
  );
