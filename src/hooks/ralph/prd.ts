...
function normalizeCriterionAmendment(
  candidate: unknown,
): CriterionAmendment | null {
...
function normalizeReconciliation(
  candidate: unknown,
): PrdReconciliationConfig | undefined {
  ...
  if (
    raw.observableChecks &&
    typeof raw.observableChecks === 'object' &&
    !Array.isArray(raw.observableChecks)
  ) {
    ...
    if (Object.keys(observableChecks).length > 0) {
      config.observableChecks =
        observableChecks as PrdReconciliationConfig['observableChecks'];
    }
  }
...
export function readPrdFromPath(prdPath: string): {
  prd?: PRD;
  error?: string;
} {
  try {
...
function applyCriterionAmendment(
  ...
  if (
    typeof original !== 'string' ||
    original.trim() === '' ||
    !story.acceptanceCriteria.includes(original)
  ) {
...
export function amendCriterion(
  ...
  return applyCriterionAmendment(
    directory,
    storyId,
    'replaced',
    input,
...
export function formatCriterionAmendments(story: UserStory): string {
  ...
  const lines: string[] = [
    '**Amended/Superseded Criteria (evidence ledger):**',
  ];
  for (const amendment of amendments) {
    const action = amendment.kind === 'replaced' ? 'replaced by' : 'superseded';
    const target =
      amendment.kind === 'replaced' ? `: ${amendment.replacement}` : '';
    lines.push(
