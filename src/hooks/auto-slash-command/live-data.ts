...
function executeCommand(invocation: ParsedCommandInvocation): {
  stdout: string;
  error: boolean;
} {
  try {
...
export function hasLiveDataScriptArgumentPlaceholder(content: string): boolean {
  ...
  return extractScriptBlocks(lines, codeBlockRanges).some(
    (block) =>
      block.shell.includes('$ARGUMENTS') || block.body.includes('$ARGUMENTS'),
  );
}
...
function getExecutableScriptStartIndexes(content: string): Set<number> {
  ...
  const codeBlockRanges = getCodeBlockRanges(lines);
  return new Set(
    extractScriptBlocks(lines, codeBlockRanges).map((block) => block.startLine),
  );
}
...
export function introducesLiveDataDirective(
  ...
  const templateExecutableLines =
    getExecutableLiveDataLineIndexes(templateContent);
  const resolvedExecutableLines =
    getExecutableLiveDataLineIndexes(resolvedContent);
