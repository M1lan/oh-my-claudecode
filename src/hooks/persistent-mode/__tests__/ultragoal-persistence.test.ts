...
describe('ultragoal persistence and Claude /goal enforcement', () => {
  ...
  it('releases both hooks only for a fresh awaiting-confirmation timestamp', () => {
    ...
    writeUltragoalState(preferred, {
      awaiting_confirmation: true,
      awaiting_confirmation_set_at: fresh,
    });
    ...
    writeUltragoalState(insideTtl, {
      awaiting_confirmation: true,
      awaiting_confirmation_set_at: new Date(
        Date.now() - 110_000,
      ).toISOString(),
    });
    ...
    for (const [name, overrides] of [
      [
        'false',
        { awaiting_confirmation: false, awaiting_confirmation_set_at: fresh },
      ],
      ['missing', {}],
      [
        'stale',
      ...
      [
        ...
        {
          awaiting_confirmation: true,
          awaiting_confirmation_set_at: 'invalid',
          started_at: fresh,
        },
      ],
    ] as const) {
      const cwd = makeTempProject(
        `omc-ultragoal-awaiting-${name.replace(/\s/g, '-')}-`,
      );
    ...
    for (const preferred of [undefined, '   '] as const) {
      ...
      writeUltragoalState(cwd, {
        awaiting_confirmation: true,
        awaiting_confirmation_set_at: preferred,
        started_at: fresh,
      });
  ...
  it('requires /goal after confirmation and allows the matching goal', () => {
    ...
    runHook(keywordScript, {
      cwd,
      session_id: 'session-a',
      prompt: '$ultragoal fix issue #3506',
    });
    expectAwaitingConfirmationRelease(cwd);
    ...
    confirmedState.claude_goal_objective =
      'Complete issue #3506 ultragoal confirmation parity.';
    writeFileSync(
      ultragoalStatePath(cwd),
      `${JSON.stringify(confirmedState, null, 2)}\n`,
    );
    const matchingGoal = runHook(preToolScript, {
      ...
      tool_input: {},
      goal: {
        objective: confirmedState.claude_goal_objective,
        status: 'active',
      },
    });
    expect(matchingGoal.hookSpecificOutput?.permissionDecision).not.toBe(
      'deny',
    );
  });
  ...
  it('releases two keyword-detector activation and confirmation cycles in one session and project', () => {
    ...
    for (const prompt of [
      '$ultragoal fix issue #3506',
      'run ultragoal for issue #3506 again',
    ]) {
      ...
      const confirmedState = readUltragoalState(cwd);
      confirmedState.claude_goal_objective =
        'Complete issue #3506 recurring ultragoal run.';
      writeFileSync(
        ultragoalStatePath(cwd),
        `${JSON.stringify(confirmedState, null, 2)}\n`,
      );
