...
describe('Persistent Mode Session Isolation (Issue #311)', () => {
  ...
  describe('checkPersistentModes session isolation', () => {
    ...
    it.each([
      ...
      'does not let a requested-at-only Ultrawork cancellation suppress a %s autopilot without flock',
      async (_name, replace) => {
        ...
        writeFileSync(
          join(sessionDir, 'autopilot-state.json'),
          JSON.stringify(replace(state as unknown as Record<string, unknown>)),
        );
        writeFileSync(
          join(sessionDir, 'cancel-signal-state.json'),
          JSON.stringify({
    ...
    it.each([
      [
        'stage advance',
        (state: Record<string, unknown>) => ({ ...state, phase: 'execution' }),
      ],
      [
        'replacement run',
        (state: Record<string, unknown>) => ({
      ...
      'does not let a cancel signal for a prior autopilot generation suppress a %s',
      async (_name, replace) => {
    ...
    it.each([
      ...
      'applies requested_at freshness to an exact-digest active legacy autopilot cancellation (%s)',
      async (_name, offsetMs, shouldBlock) => {
    ...
    it.each([
      ...
      'applies requested_at freshness to requested_at-only non-autopilot cancellation (%s)',
      async (_name, offsetMs, shouldBlock) => {
  ...
  describe('persistent-mode.mjs script session isolation', () => {
    ...
    it.each([
      [
        ...
        {
          active: false,
          session_id: 'session-cancel-coexist',
          project_path: '/inactive-project',
        },
      ],
      [
        'cross-project',
      ...
      'allows requested_at-only cancellation for ultrawork with a %s autopilot record',
      (_kind, autopilotState) => {
        ...
        expect(
          runPersistentModeScript({ directory: tempDir, sessionId }),
