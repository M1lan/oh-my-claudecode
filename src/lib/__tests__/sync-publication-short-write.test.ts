    ...
    writeSync: (...args: Parameters<WriteSync>) => {
      ...
      if (fsControl.maxBytes > 0 && typeof buffer === 'string') {
        const bytes = Buffer.from(
          buffer,
          typeof length === 'string' ? (length as BufferEncoding) : 'utf8',
        );
        return writeSync(
          fd,
          bytes,
          0,
          Math.min(bytes.length, fsControl.maxBytes),
      ...
      if (
        ...
        return writeSync(
          ...
          buffer,
          offset,
          Math.min(length, fsControl.maxBytes),
          position,
        );
      }
...
function fixturePath(name: string): string {
  const directory = mkdtempSync(
    join(tmpdir(), 'sync-publication-short-write-'),
  );
...
describe('synchronous publication short writes', () => {
  it('completes TypeScript state and mutation-lock publications before exposing either path', () => {
    ...
    expect(() => readFileSync(`${statePath}.mutation.lock`, 'utf8')).toThrow(
      /ENOENT/,
    );
    expect(fsControl.calls).toBeGreaterThan(
  ...
  it.each([
    ...
    'completes short writes for %s state and lock publications',
    (_name, atomicWrite) => {
      const statePath = fixturePath('state.json');
      ...
      expect(
        atomicWrite.withStateFileLockSync(statePath, () => 'held'),
