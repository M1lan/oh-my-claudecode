...
function makeTempRoot(): string {
  const root = join(
    tmpdir(),
    `omc-cleanup-safety-${process.pid}-${tempDirs.length}-${Date.now()}`,
  );
...
describe('validateWorktreeRemovalTarget', () => {
  it.each(['', '   ', '.', '..', '~'])(
    'refuses suspicious path %j',
    (candidatePath) => {
      const root = makeTempRoot();
      expect(() =>
        validateWorktreeRemovalTarget({ candidatePath, expectedRoots: [root] }),
      ).toThrow(/empty|suspicious/);
    },
  );
  ...
  it('refuses NUL bytes before filesystem checks', () => {
    ...
    expect(() =>
      validateWorktreeRemovalTarget({
        candidatePath: `bad${String.fromCharCode(0)}path`,
        expectedRoots: [root],
      }),
  ...
  it('refuses filesystem root and home directory', () => {
    ...
    expect(() =>
      validateWorktreeRemovalTarget({
        candidatePath: parse(root).root,
        expectedRoots: [root],
      }),
    ).toThrow(/filesystem_root/);
    expect(() =>
      validateWorktreeRemovalTarget({
        candidatePath: homedir(),
        expectedRoots: [root],
      }),
    ).toThrow(/home_directory|outside_expected_roots/);
  });
  ...
  it('rejects paths outside the expected worktree root after realpath normalization', () => {
    ...
    ).toThrow(/outside_expected_roots/);
  });
  ...
  it('allows a registered-looking worktree child with a .git file under an expected root', () => {
    ...
    writeFileSync(
      join(worktree, '.git'),
      'gitdir: /repo/.git/worktrees/repo-3090\n',
    );
