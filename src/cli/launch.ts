...
function extractOAuthCandidate(
  parsed: Record<string, unknown>,
): OAuthCredentialCandidate | null {
  const inspect = (
...
function hasLinkableCredentials(inspection: CredentialFileInspection): boolean {
  ...
  return (
    ...
    source.accessToken.trim().length > 0
  );
}
...
function credentialExpiry(parsed: Record<string, unknown>): number | null {
  const source = isJsonObject(parsed.claudeAiOauth)
...
function accountsProvenSame(
  ...
  const sourceAccount = isJsonObject(sourceClaudeJson?.oauthAccount)
    ? sourceClaudeJson.oauthAccount
    : null;
  const runtimeAccount = isJsonObject(runtimeClaudeJson?.oauthAccount)
    ? runtimeClaudeJson.oauthAccount
    : null;
  if (!sourceAccount || !runtimeAccount) return false;

  const sourceUuid =
    typeof sourceAccount.accountUuid === 'string'
      ? sourceAccount.accountUuid.trim()
      : '';
  const runtimeUuid =
    typeof runtimeAccount.accountUuid === 'string'
      ? runtimeAccount.accountUuid.trim()
      : '';
...
function credentialIdentitiesConflict(
  ...
  const baseUuid =
    typeof baseCandidate.fields.accountUuid === 'string'
      ? baseCandidate.fields.accountUuid.trim()
      : '';
  const runtimeUuid =
    typeof runtimeCandidate.fields.accountUuid === 'string'
...
function compareOnboardingVersion(
  left: unknown,
  right: unknown,
): number | null {
...
function refreshRuntimeClaudeJson(
  ...
  sourceClaudeJson = readJsonObject(
    join(dirname(baseConfigDir), '.claude.json'),
  ),
): void {
  ...
  if (
    sourceClaudeJson.hasCompletedOnboarding === true &&
  ...
  if (typeof sourceVersion === 'string' || typeof sourceVersion === 'number') {
    const runtimeHasVersion = hasOwn(
      runtimeClaudeJson,
      'lastOnboardingVersion',
    );
    const compared = compareOnboardingVersion(
      sourceVersion,
      runtimeClaudeJson.lastOnboardingVersion,
    );
  ...
  if (changed) {
    writeFileSync(
      runtimeClaudeJsonPath,
      JSON.stringify(runtimeClaudeJson, null, 2),
    );
  }
}
...
function reconcileRuntimeCredentials(
  ...
  if (
    !baseInspection.readable ||
    !baseInspection.valid ||
    !baseInspection.parsed
  ) {
  ...
  if (
    hasOwn(baseInspection.parsed, 'claudeAiOauth') ||
    runtimeCandidate.nested
  ) {
    const existingNested = isJsonObject(baseInspection.parsed.claudeAiOauth)
      ? baseInspection.parsed.claudeAiOauth
      : {};
    mergedBaseCredentials.claudeAiOauth = {
      ...existingNested,
      ...runtimeCandidate.fields,
    };
  } else {
  ...
  atomicWriteJsonSync(
    resolveCredentialWritePath(baseCredentialsPath),
    mergedBaseCredentials,
  );
}

function swapRuntimeConfigDir(
  ...
  try {
    ...
    try {
      if (
        movedPrevious &&
        !pathExists(runtimeConfigDir) &&
        pathExists(previousConfigDir)
      ) {
...
export function prepareOmcLaunchConfigDir(
  ...
  const lifecycleLockPath = lockPathFor(
    join(baseConfigDir, '.omc-launch.prepare.lock'),
  );
  ...
  return withFileLockSync(
    ...
    () => {
      ...
      try {
        ...
        const baseCredentialsPath = join(baseConfigDir, '.credentials.json');
        const baseCredentialInspection =
          inspectCredentialFile(baseCredentialsPath);
...
function runClaudeOutsideTmux(
  ...
  try {
    configureTmuxClipboardForSession(sessionName, {
      stripTmux: true,
      stdio: 'ignore',
    });
  } catch {
