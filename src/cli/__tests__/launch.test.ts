...
describe('extractNotifyFlag', () => {
  ...
  it('strips --notify from remainingArgs', () => {
    const result = extractNotifyFlag([
      '--madmax',
      '--notify',
      'false',
      '--print',
...
describe('runClaude — exit code propagation', () => {
  ...
  describe('direct policy', () => {
    ...
    it('bypasses tmux for --print mode', () => {
      ...
      expect(
        vi.mocked(execFileSync).mock.calls.find(([cmd]) => cmd === 'tmux'),
      ).toBeUndefined();
      expect(
        vi
          .mocked(execFileSync)
          .mock.calls.find(([cmd]) => cmd === 'claude')?.[1],
      ).toEqual(['--print']);
    });
...
describe('runClaude outside-tmux — mouse scrolling (issue #890)', () => {
  ...
  it('places mouse mode setup before attach-session', () => {
    ...
    const attachIdx = tmuxCalls.findIndex(
      (args) => args[0] === 'attach-session',
    );
  ...
  it('applies session-scoped OSC 52 clipboard options before attach-session', () => {
    ...
    expect(
      tmuxCalls.find((args) => args.includes('set-clipboard')),
    ).not.toContain('-g');

    const clipboardIdx = tmuxCalls.findIndex((args) =>
      args.includes('set-clipboard'),
    );
    const attachIdx = tmuxCalls.findIndex(
      (args) => args[0] === 'attach-session',
    );
...
describe('prepareOmcLaunchConfigDir / launchCommand OMC companion loading', () => {
  ...
  it('uses a runtime launch profile when a preserved CLAUDE-omc.md companion exists', async () => {
    ...
    expect(readFileSync(join(runtimeDir, 'CLAUDE.md'), 'utf-8')).toContain(
      '# OMC companion',
    );
  ...
  it('repairs retired team MCP entries in the runtime settings copy', () => {
    ...
    const runtimeSettings = JSON.parse(
      readFileSync(join(runtimeDir, 'settings.json'), 'utf-8'),
    ) as {
      theme?: string;
  ...
  it('mirrors keybindings.json, rules/, and themes/ into the runtime config dir', () => {
    ...
    writeFileSync(
      join(configDir, 'themes', 'custom-theme.json'),
      '{"name":"custom"}',
    );
    ...
    expect(
      readFileSync(join(runtimeDir, 'themes', 'custom-theme.json'), 'utf-8'),
    ).toBe('{"name":"custom"}');
  });
  ...
  it('does not copy credential content when credential symlink creation fails', async () => {
    ...
    try {
      const { prepareOmcLaunchConfigDir: prepareWithFailedSymlink } =
      ...
      writeFileSync(
        credentialsPath,
        JSON.stringify({ accessToken: 'test-only-token', expiresAt: 1000 }),
      );
  ...
  it.skipIf(process.platform === 'win32')(
    'falls back to a hard link when credential symlink creation fails and remains reconcilable',
    async () => {
      vi.resetModules();
      const actualFs =
      ...
      try {
        ...
        expect(statSync(runtimeCredentialsPath).ino).toBe(
          statSync(credentialsPath).ino,
        );

        rmSync(runtimeCredentialsPath, { force: true });
        writeFileSync(
          join(runtimeDir, '.claude.json'),
          JSON.stringify({
            oauthAccount: { accountUuid: 'same-account' },
          }),
        );
        ...
        const baseCredentials = JSON.parse(
          readFileSync(credentialsPath, 'utf-8'),
        ) as Record<string, unknown>;
        expect(
          (baseCredentials.claudeAiOauth as Record<string, unknown>)
            .accessToken,
        ).toBe('runtime-token');
        expect(
          lstatSync(
            join(rebuiltRuntimeDir, '.credentials.json'),
          ).isSymbolicLink(),
        ).toBe(false);
        expect(statSync(join(rebuiltRuntimeDir, '.credentials.json')).ino).toBe(
          statSync(credentialsPath).ino,
        );
      } finally {
        vi.doUnmock('fs');
  ...
  it('preserves runtime .claude.json across runtime config dir rebuilds', () => {
    ...
    expect(readFileSync(join(rebuiltRuntimeDir, '.claude.json'), 'utf-8')).toBe(
      '{"session":"keep-me"}',
    );
  });
  ...
  it('seeds missing runtime .claude.json mcpServers from source .claude.json', () => {
    ...
    const runtimeDir = prepareOmcLaunchConfigDir(configDir);
    const runtimeClaudeJson = JSON.parse(
      readFileSync(join(runtimeDir, '.claude.json'), 'utf-8'),
    ) as Record<string, unknown>;
  ...
  it('refreshes runtime mcpServers from source while preserving runtime metadata', () => {
    ...
    const rebuiltRuntimeDir = prepareOmcLaunchConfigDir(configDir);
    const runtimeClaudeJson = JSON.parse(
      readFileSync(join(rebuiltRuntimeDir, '.claude.json'), 'utf-8'),
    ) as Record<string, unknown>;
  ...
  it('inherits onboarding without mcpServers while preserving runtime session and projects', () => {
    ...
    expect(runtimeClaudeJson.projects).toEqual({
      '/repo': { history: ['keep-project'] },
    });
  ...
  it('replaces mismatched oauthAccount and deletes it when source removes the account', () => {
    ...
    writeFileSync(
      sourceClaudeJsonPath,
      JSON.stringify({ oauthAccount: { accountUuid: 'source-account' } }),
    );
    ...
    let runtimeClaudeJson = JSON.parse(
      readFileSync(
        join(prepareOmcLaunchConfigDir(configDir), '.claude.json'),
        'utf-8',
      ),
    ) as Record<string, unknown>;
    expect(runtimeClaudeJson.oauthAccount).toEqual({
      accountUuid: 'source-account',
    });
    expect(runtimeClaudeJson.session).toBe('keep-session');
    writeFileSync(sourceClaudeJsonPath, JSON.stringify({ oauthAccount: null }));
    runtimeClaudeJson = JSON.parse(
      readFileSync(
        join(prepareOmcLaunchConfigDir(configDir), '.claude.json'),
        'utf-8',
      ),
  ...
  it('promotes a fresher nested runtime credential and preserves unrelated base keys', () => {
    ...
    writeFileSync(
      join(tempRoot!, '.claude.json'),
      JSON.stringify({ oauthAccount: { accountUuid: 'same-account' } }),
    );
    const credentialsPath = join(configDir, '.credentials.json');
    writeFileSync(
      credentialsPath,
    ...
    expect(
      (baseCredentials.claudeAiOauth as Record<string, unknown>).accessToken,
    ).toBe('runtime-token');
  ...
  it('does not promote a high-expiry runtime credential without an access token', () => {
    ...
    expect(
      (baseCredentials.claudeAiOauth as Record<string, unknown>).expiresAt,
    ).toBe(100);
  });
  ...
  it('does not resurrect a runtime credential when the base credential file is missing', () => {
    ...
    expect(existsSync(join(rebuiltRuntimeDir, '.credentials.json'))).toBe(
      false,
    );
  });
  ...
  it('blocks credential promotion when source and runtime account identities differ', () => {
    ...
    writeFileSync(
      join(runtimeDir, '.claude.json'),
      JSON.stringify({ oauthAccount: { accountUuid: 'different-account' } }),
    );
    rmSync(join(runtimeDir, '.credentials.json'), { force: true });
    writeFileSync(
      join(runtimeDir, '.credentials.json'),
  ...
  it('blocks credential promotion when only the base credential has a stable identity', () => {
    ...
    expect(
      (baseCredentials.claudeAiOauth as Record<string, unknown>).refreshToken,
    ).toBe('base-refresh');
  });
  ...
  it('blocks credential promotion when either account identity is missing', () => {
    ...
    writeFileSync(
      join(runtimeDir, '.claude.json'),
      JSON.stringify({ session: 'no-account-meta' }),
    );
    rmSync(join(runtimeDir, '.credentials.json'), { force: true });
    writeFileSync(
      join(runtimeDir, '.credentials.json'),
  ...
  it('inherits lastOnboardingVersion using semver-aware comparison', () => {
    ...
    writeFileSync(
      join(tempRoot!, '.claude.json'),
      JSON.stringify({ lastOnboardingVersion: '2.10.0' }),
    );

    const runtimeDir = prepareOmcLaunchConfigDir(configDir);
    writeFileSync(
      join(runtimeDir, '.claude.json'),
      JSON.stringify({ lastOnboardingVersion: '2.9.0' }),
    );
    const rebuilt = prepareOmcLaunchConfigDir(configDir);
    const runtimeClaudeJson = JSON.parse(
      readFileSync(join(rebuilt, '.claude.json'), 'utf-8'),
    ) as Record<string, unknown>;
  ...
  it('updates a symlink target during credential promotion without replacing the symlink', () => {
    ...
    const targetCredentials = JSON.parse(
      readFileSync(targetCredentialsPath, 'utf-8'),
    ) as Record<string, unknown>;
    expect(
      (targetCredentials.claudeAiOauth as Record<string, unknown>).accessToken,
    ).toBe('runtime-token');
    expect(targetCredentials.unrelated).toBe('preserve-me');
  });

  it.skipIf(process.platform === 'win32')(
    'resolves a credential symlink chain beyond the former depth cap without replacing links',
    () => {
      const configDir = join(tempRoot!, '.claude');
      mkdirSync(configDir, { recursive: true });
      writeFileSync(
      ...
      const chainPaths = Array.from({ length: 20 }, (_, index) =>
        join(tempRoot!, `credentials-link-${index}.json`),
      );
      const credentialsPath = join(configDir, '.credentials.json');
      writeFileSync(
      ...
      for (let index = chainPaths.length - 1; index >= 0; index -= 1) {
        symlinkSync(
          chainPaths[index + 1] ?? finalCredentialsPath,
          chainPaths[index],
        );
      }
      ...
      for (let index = 0; index < chainPaths.length; index += 1) {
        ...
        expect(readlinkSync(chainPaths[index])).toBe(
          chainPaths[index + 1] ?? finalCredentialsPath,
        );
      }
      expect(lstatSync(finalCredentialsPath).isSymbolicLink()).toBe(false);
      const finalCredentials = JSON.parse(
        readFileSync(finalCredentialsPath, 'utf-8'),
      ) as Record<string, unknown>;
      expect(
        (finalCredentials.claudeAiOauth as Record<string, unknown>).accessToken,
      ).toBe('runtime-token');
    },
  );
  ...
  it('preserves runtime .claude.json when source .claude.json is absent, invalid, or has no mcpServers', () => {
    ...
    prepareOmcLaunchConfigDir(configDir);
    expect(readFileSync(runtimeClaudeJsonPath, 'utf-8')).toBe(
      '{"session":"keep-absent"}',
    );
...
describe('runClaude — print mode bypasses tmux (issue #1665)', () => {
  ...
  it('runs claude directly when --print is present (inside-tmux policy)', () => {
    ...
    runClaude(
      '/tmp',
      ['--dangerously-skip-permissions', '--print', 'say hello'],
      'sid',
    );
...
describe('buildEnvExportPrefix', () => {
  ...
  it('builds semicolon-separated exports for multiple set vars', () => {
    ...
    const result = buildEnvExportPrefix([
      'TEST_VAR_A',
      'TEST_VAR_B',
      'TEST_VAR_C',
    ]);
...
describe('hasMadmaxFlag', () => {
  ...
  it('returns false when neither flag is present', () => {
    expect(hasMadmaxFlag(['--print', '--dangerously-skip-permissions'])).toBe(
      false,
    );
  });
...
describe('runClaude — --madmax on macOS forces tmux', () => {
  ...
  it('does not require tmux on darwin without --madmax', () => {
    ...
    expect(resolveLaunchPolicy).toHaveBeenCalledWith(process.env, [], {
      requireTmux: false,
    });
  });
