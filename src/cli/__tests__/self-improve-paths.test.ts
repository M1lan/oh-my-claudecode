...
describe('self-improve path scoping helpers', () => {
  ...
  it('defaults new runs to a scoped default topic root', () => {
    ...
    expect(result.root).toBe(
      join(root, '.omc', 'self-improve', 'topics', 'default'),
    );
  });

  it('uses a slugified topic-specific root when topic text is provided', () => {
    const result = readJson('node', [
      RESOLVER,
      '--project-root',
      root,
      '--topic',
    ...
    expect(result.root).toBe(
      join(root, '.omc', 'self-improve', 'topics', 'latency-throughput'),
    );
  });
  ...
  it('validate.sh auto-discovers a single scoped settings file', () => {
    ...
    writeFileSync(
      join(scopedConfigDir, 'settings.json'),
      JSON.stringify({ sealed_files: [] }),
      'utf-8',
    );
    ...
    const expectedSettings = normalize(
      join(scopedConfigDir, 'settings.json'),
    ).replace(/^\/private(?=\/var\/)/, '');
    const normalizedOutput = normalize(output).replace(
      /^Settings: \/private(?=\/var\/)/m,
      'Settings: ',
    );
  ...
  it('validate.sh errors when multiple scoped topics exist without an explicit selector', () => {
    ...
    writeFileSync(
      join(configA, 'settings.json'),
      JSON.stringify({ sealed_files: [] }),
      'utf-8',
    );
    writeFileSync(
      join(configB, 'settings.json'),
      JSON.stringify({ sealed_files: [] }),
      'utf-8',
    );

    expect(() =>
      execFileSync('bash', [VALIDATE], {
        cwd: root,
        encoding: 'utf-8',
  ...
  it('validate.sh resolves a selected topic via project root and slug', () => {
    ...
    const output = execFileSync(
      'bash',
      [VALIDATE, '--project-root', root, '--slug', 'alpha'],
      {
        cwd: root,
        encoding: 'utf-8',
      },
