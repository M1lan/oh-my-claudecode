...
} from '../installer/claude-md-transaction.js';
...
const COMPILED_ENGINE_VERSION =
  typeof __OMC_COORDINATOR_ENGINE_VERSION__ === 'string'
    ? __OMC_COORDINATOR_ENGINE_VERSION__
    : '';
const COMPILED_SOURCE_SHA256 =
  typeof __OMC_COORDINATOR_SOURCE_SHA256__ === 'string'
    ? __OMC_COORDINATOR_SOURCE_SHA256__
    : '';
...
export function runClaudeMdCoordinator(input: unknown): {
  exitCode: number;
  ...
  try {
    ...
    const { mode } = input;
    if (
      input.schemaVersion !== CLAUDE_MD_COORDINATOR_SCHEMA_VERSION ||
      input.engineVersion !== COMPILED_ENGINE_VERSION ||
      (mode !== 'local' &&
        mode !== 'global-overwrite' &&
        mode !== 'global-preserve') ||
      typeof input.configRoot !== 'string' ||
      typeof input.pluginRoot !== 'string' ||
    ...
    if (
      sourceSha256 !== COMPILED_SOURCE_SHA256 ||
      input.sourceSha256 !== COMPILED_SOURCE_SHA256 ||
      input.sourceVersion !== COMPILED_ENGINE_VERSION
    )
      return {
        exitCode: 2,
...
function main(): void {
  ...
  try {
    parsed = JSON.parse(
      new TextDecoder('utf-8', { fatal: true }).decode(readFileSync(0)),
    );
  } catch {
...
if (
  process.argv[1] &&
  /claude-md-coordinator\.(?:[cm]?js|ts)$/.test(process.argv[1])
)
  main();
