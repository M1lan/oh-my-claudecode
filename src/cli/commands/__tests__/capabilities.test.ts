...
describe('capabilities lock/check', () => {
  ...
  it('writes a deterministic lockfile and checks it successfully', async () => {
    await withTempCwd(async (cwd) => {
      ...
      await expect(
        capabilitiesLockCommand({ json: true, lockfile }),
      ).resolves.toBe(0);
      await expect(
        capabilitiesCheckCommand({ json: true, lockfile }),
      ).resolves.toBe(0);
  ...
  it('returns a machine-readable failure for a missing lockfile', async () => {
    await withTempCwd(async (cwd) => {
      const logSpy = vi
        .spyOn(console, 'log')
        .mockImplementation(() => undefined);
      const exitCode = await capabilitiesCheckCommand({
        json: true,
        lockfile: join(cwd, 'missing.json'),
      });
  ...
  it('checks required-arg, hallucinated-tool, and no-tool restraint fixtures', () => {
    ...
    const results = runDeterministicCapabilityFixtures(
      lockfile.fixtures,
      lockfile.surface,
    );

    expect(
      results.find((result) => result.kind === 'required_args'),
    ).toMatchObject({ ok: true, outcome: 'pass' });
    expect(
      results.find((result) => result.kind === 'no_hallucinated_tool'),
    ).toMatchObject({ ok: true, outcome: 'pass' });
    expect(
      results.find((result) => result.kind === 'tool_restraint'),
    ).toMatchObject({ ok: true, outcome: 'pass' });
  });
  ...
  it('extracts skill names from win32-style SKILL.md paths without leaking parent paths', () => {
    expect(
      skillNameFromSkillFilePath('C:\\repo\\skills\\deep-interview\\SKILL.md'),
    ).toBe('deep-interview');
    expect(
      skillNameFromSkillFilePath('C:\\repo\\skills\\nested.skill\\SKILL.md'),
    ).toBe('nested.skill');
  });
