...
describe('omc ultragoal CLI', () => {
  ...
  it('complete-goals positional id starts exactly the named pending goal', async () => {
    await withTempCwd(async (cwd) => {
      ...
      const result = JSON.parse(captured.out.join('')) as {
        goal: { id: string };
      };
      ...
      const plan = JSON.parse(
        ...
        activeGoalId?: string;
        goals: Array<{ id: string; status: string; attempt: number }>;
      };
      expect(plan.activeGoalId).toBe('G003-third');
      expect(plan.goals.find((goal) => goal.id === 'G001-first')?.status).toBe(
        'pending',
      );
      expect(plan.goals.find((goal) => goal.id === 'G003-third')?.attempt).toBe(
        1,
      );
    });
  });
