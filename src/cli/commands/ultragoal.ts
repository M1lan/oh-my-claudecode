...
function parseGoalArg(raw: string): {
  title?: string;
  objective: string;
  tokenBudget?: number;
} {
...
function normalizeClaudeGoalMode(
  ...
  throw new UltragoalError(
    'Invalid --claude-goal-mode; expected aggregate or per-story.',
  );
}
  ...
  try {
    ...
    if (command === 'create' || command === 'create-goals') {
      ...
      const plan = await createUltragoalPlan(cwd, {
        brief,
        goals,
        claudeGoalMode: normalizeClaudeGoalMode(
          readValue(rest, '--claude-goal-mode'),
        ),
      ...
      if (json)
        printJson({
          ...
          plan,
          planId: plan.planId,
          summary: summarizeUltragoalPlan(plan),
        });
      else {
        ...
        if (plan.planId) {
          console.log('');
          console.log(
            `Subsequent commands MUST pass --plan-id ${plan.planId} (or run in a workspace where this is the only plan).`,
          );
        }
      }
      return;
    ...
    if (command === 'list-plans') {
      ...
      if (json) printJson({ ok: true, plans: ids });
        ...
        console.log(
          'ultragoal: no multi-plans (use --plan-id or --auto-plan-id with create-goals to create one).',
        );
    ...
    if (command === 'status') {
      const planId = await resolveActivePlanId(
        cwd,
        readValue(rest, '--plan-id'),
      );
      const plan = await readUltragoalPlan(cwd, planId);
      const snapshot = await readClaudeGoalSnapshotInput(
        readValue(rest, '--claude-goal-json'),
        cwd,
      );
      const activeGoal = plan.goals.find(
        (goal) =>
          goal.id === plan.activeGoalId || goal.status === 'in_progress',
      );
      const expectedObjective =
        plan.claudeGoalMode === 'aggregate'
      ...
      if (json)
        ...
        if (reconciliation && !reconciliation.ok)
          console.log(
            `claude goal warning: ${formatClaudeGoalReconciliation(reconciliation)}`,
          );
    ...
    if (command === 'record-review-blockers') {
      ...
      const claudeGoal = await parseClaudeGoalJson(
        readValue(rest, '--claude-goal-json'),
      );
      const planId = await resolveActivePlanId(
        cwd,
    ...
    if (command === 'checkpoint') {
      ...
      if (json)
        ...
        const goal = plan.goals.find(
          (candidate: UltragoalItem) => candidate.id === goalId,
        );
        console.log(
    ...
    if (
      error instanceof UltragoalError ||
      error instanceof ClaudeGoalSnapshotError
    ) {
