...
export async function doctorTeamRoutingCommand(options: {
  ...
  if (options.json) {
    ...
    const claudeFound = probes.some(
      (probe) => probe.provider === 'claude' && probe.found,
    );
      ...
      if (p.found) {
        ...
        const version = p.version
          ...
          : p.error
            ...
            : '';
        console.log(
          `  ${colors.green('✓')} ${p.provider}${resolvedPath}${version}`,
        );
      } else {
        const fallback =
          p.provider === 'claude'
        ...
        console.log(
          `  ${colors.yellow('⚠')} ${p.provider}: not found on PATH — ${fallback}`,
        );
      }
    }
    if (missing.length === 0) {
