...
const VALID_TEAM_CLI_AGENT_TYPES = new Set([
  'claude',
  'codex',
  'gemini',
  'grok',
...
const TEAM_API_OPERATION_NOTES: Partial<Record<TeamApiOperation, string>> = {
  ...
  'recover-worker':
    'v2 live-session recovery only: it proceeds only after confirmed worker death, refuses unknown liveness, and replays idempotently by request_id. A timeout may return before the durable result is final; use read-recovery-result to look it up. In-progress tasks require a checkpoint; idle dead workers recover without one.',
  'write-task-checkpoint':
    'Authenticated worker checkpoint producer. The resume_payload must be JSON and no larger than 64 KiB; repeat the same sequence and payload to replay safely.',
  'read-recovery-result':
    'Looks up the durable pending, succeeded, failed, or commit_unknown recovery outcome by request_id in the canonical team workspace state.',
};
...
export function resolveAvailableTeamName(
  baseName: string,
  cwd: string,
): string {
  ...
  throw new Error(
    `Unable to allocate a fresh team name for ${sanitizedBase}; remove stale .omc/state/team entries or choose a more specific launch task.`,
  );
}
...
export async function assertTeamSpawnAllowed(
  ...
  const { teamReadConfig, teamReadManifest } =
...
export function buildTeamLaunchTasks(
  ...
  const isPreauthoredScopeList =
    decomposition.strategy === 'numbered' ||
  ...
  for (let i = 0; i < effectiveWorkerCount; i++) {
    ...
    const roleLabel = workerSpec?.role ? ` (${workerSpec.role})` : '';
    const source = canUseDecomposition ? decomposition.subtasks[i] : undefined;
...
function sampleValueForField(field: string): unknown {
  switch (field) {
    ...
    case 'sequence':
      return 1;
    case 'resume_payload':
      return { cursor: 'safe-boundary' };
    case 'request_id':
      return 'recovery-request-123';
    case 'timeout_ms':
      return 30000;
    default:
      return `<${field}>`;
