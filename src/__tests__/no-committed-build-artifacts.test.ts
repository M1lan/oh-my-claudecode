...
const CLASSIFIER = join(
  ...
  'scripts',
  'ci',
  'check-no-committed-build-artifacts.mjs',
);
...
function run(root: string, base: string, head: string) {
  return spawnSync(
    process.execPath,
    [CLASSIFIER, '--base', base, '--head', head],
    {
      cwd: root,
      encoding: 'utf8',
    },
...
describe('no committed build artifacts candidate classifier', () => {
  it('uses only Node built-ins and Git, with no approval capability', () => {
    ...
    expect(CLASSIFIER_SOURCE).not.toMatch(
      /GH_TOKEN|github|npm |plugin:shipping|coordinator/,
    );
  });
  ...
  it('holds candidate generated deltas with a control-safe path diagnostic', () => {
    ...
    expect(result.stderr).toContain(
      'OWNER_CONFIRMATION_REQUIRED: candidate generated delta: "bridge/candidate\\nname.cjs"',
    );
  });
  ...
  it('rejects malformed, unavailable, duplicate, unknown, and mismatched head inputs', () => {
    ...
    const duplicate = spawnSync(
      process.execPath,
      [CLASSIFIER, '--base', head, '--base', head, '--head', head],
      { cwd: root, encoding: 'utf8' },
    );
    expect(duplicate.status).toBe(1);
    const unknown = spawnSync(
      process.execPath,
      [CLASSIFIER, '--base', head, '--head', head, '--unexpected'],
      { cwd: root, encoding: 'utf8' },
    );
    expect(unknown.status).toBe(1);
    const missing = spawnSync(process.execPath, [CLASSIFIER, '--base', head], {
      cwd: root,
      encoding: 'utf8',
    });
    ...
    expect(run(root, head, head).stderr).toContain(
      'checked-out HEAD does not match --head',
    );
  });
  ...
  it('rejects unrelated roots and deterministic criss-cross merge bases', () => {
    ...
    expect(
      git(root, ['merge-base', '--all', a2, b2]).trim().split(/\s+/),
    ).toHaveLength(2);
    git(root, ['checkout', '--quiet', '--detach', b2]);
    expect(run(root, a2, b2).stderr).toContain(
      'ambiguous merge base: expected one, found 2',
    );
  });
});
