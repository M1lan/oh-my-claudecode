...
describe('plugin shipping release guidance', () => {
  ...
  it('keeps candidate artifact containment non-authoritative and credential-free', () => {
    ...
    expect(CONTRIBUTING).toContain(
      'credential-free, candidate-side classifier',
    );
    expect(CONTRIBUTING).toContain(
      'non-authoritative for every contributor and maintainer',
    );
    ...
    expect(CONTRIBUTING).toContain(
      'remove this ordinary candidate check from required checks or supersede it',
    );
    expect(CONTRIBUTING).not.toContain(
      'cryptographically signed by that owner',
    );
  ...
  it('uses the narrow signed maintainer transaction instead of broad staging or protected pushes', () => {
    ...
    expect(RELEASE_SCRIPT).not.toMatch(
      /git add -f(?:\s+--)?\s+(?:dist|bridge)\/?\b/,
    );
    expect(SHIPPING_SCRIPT).toContain(
