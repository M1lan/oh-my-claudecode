...
describe('release generation', () => {
  ...
  it('enforces the release publication boundary around one exact archive', () => {
    ...
    expect(workflow).toContain(
      'OMC_SETUP_PLUGIN_ROOT="$SMOKE_PACKAGE_ROOT" CLAUDE_PLUGIN_ROOT="$SMOKE_PACKAGE_ROOT"',
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain(
      'bash -c \'bash "${OMC_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/scripts/setup-claude-md.sh" local\'',
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain(
      'archiveManifest.files.find(({ path }) => path === "package/bridge/claude-md-coordinator.cjs")',
    );
    expect(workflow).toContain(
      'env -i PATH="$PATH" HOME="$SMOKE_HOME" CLAUDE_CONFIG_DIR="$SMOKE_CLAUDE_CONFIG_DIR" GJC_CONFIG_DIR="$SMOKE_GJC_CONFIG_DIR"',
    );
    expect(workflow).toContain(
      'env -i PATH="$PATH" HOME="$SMOKE_HOME" CLAUDE_CONFIG_DIR="$SMOKE_CLAUDE_CONFIG_DIR" GJC_CONFIG_DIR="$SMOKE_GJC_CONFIG_DIR" OMC_SETUP_PLUGIN_ROOT="$SMOKE_PACKAGE_ROOT" CLAUDE_PLUGIN_ROOT="$SMOKE_PACKAGE_ROOT" "$SMOKE_PREFIX/node_modules/.bin/omc" --version',
    );
  ...
  it('requires stable releases to be promoted before publication', () => {
    ...
    expect(
      workflow.indexOf('name: Re-assert marketplace promotion before publish'),
    ).toBeLessThan(
      workflow.indexOf('name: Publish exact archive and verify registry'),
    );
