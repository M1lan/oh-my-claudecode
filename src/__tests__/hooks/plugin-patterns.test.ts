...
function makeTempDir(): string {
  const dir = join(
    tmpdir(),
    `omc-plugin-patterns-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
describe('validateCommitMessage', () => {
  describe('default types (no config)', () => {
    ...
    it('rejects an unknown type', () => {
      ...
      expect(
        result.errors.some((e) => e.includes('conventional commit format')),
      ).toBe(true);
    });
  ...
  describe('custom types via config.types', () => {
    ...
    it('includes custom types in the error message', () => {
      const result = validateCommitMessage('unknown: change', {
        types: ['ship', 'rollback'],
      });
    ...
    it('accepts a custom type with scope', () => {
      const result = validateCommitMessage('ship(api): deploy api changes', {
        types: ['ship'],
      });
      expect(result.valid).toBe(true);
    });
    ...
    it('accepts a custom type with breaking-change marker', () => {
      const result = validateCommitMessage('ship!: breaking deploy', {
        types: ['ship'],
      });
      expect(result.valid).toBe(true);
    });
  ...
  describe('other config options still work alongside custom types', () => {
    ...
    it('enforces requireScope with custom types', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.includes('Scope is required'))).toBe(
        true,
      );
    });
    ...
    it('enforces requireBody with custom types', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.includes('body is required'))).toBe(
        true,
      );
    });
  });
...
describe('runPreCommitChecks', () => {
  ...
  it('returns canCommit: false when commit message is invalid', () => {
    const result = runPreCommitChecks(
      testDir,
      'bad commit message without type',
    );
  ...
  it('includes Commit Message check only when commitMessage is provided', () => {
    ...
    expect(
      withoutMsg.checks.find((c) => c.name === 'Commit Message'),
    ).toBeUndefined();

    const withMsg = runPreCommitChecks(testDir, 'feat(scope): add feature');
    expect(
      withMsg.checks.find((c) => c.name === 'Commit Message'),
    ).toBeDefined();
  });
});
...
describe('win32 spawn hardening (#2721)', () => {
  ...
  // below spawn pnpm, which resolves to pnpm.cmd on Windows, so
  ...
  it('runTypeCheck spawnSync("pnpm", …) must pass shell:true on win32', () => {
    const src = readFileSync(sourcePath, 'utf-8');
    expect(src).toMatch(
      /spawnSync\(['"]pnpm['"], \[['"]exec['"], ['"]tsc['"], ['"]--noEmit['"]\], \{[^}]*?shell:\s*process\.platform === ['"]win32['"][^}]*?\}\s*\);/,
    );
  });
