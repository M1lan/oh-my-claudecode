...
function runHook(
  ...
  transcriptPath = join(
    ...
    'claude-config',
    'projects',
    'workflow-activation-fixture.jsonl',
  ),
...
function runHookAsync(
  ...
  return new Promise<{ hookSpecificOutput?: { additionalContext?: string } }>(
    (resolve, reject) => {
...
function advancePausedWorkflowState(
  state: any,
  transcriptPath: string,
  signal = 'PIPELINE_RALPLAN_COMPLETE',
) {
  const record = JSON.stringify({
    sessionId: 'workflow-activation-fixture',
    type: 'assistant',
    message: {
      role: 'assistant',
      content: [{ type: 'text', text: `Signal: ${signal}` }],
    },
  });
  ...
  state.pipelineTracking.stages[0] = {
    id: 'ralplan',
    status: 'complete',
    iterations: 0,
    startedAt: now,
  ...
  state.pipelineTracking.stages[1] = {
    id: 'execution',
    status: 'active',
    iterations: 0,
    startedAt: now,
  ...
  state.pipelineTracking.completionObservations = [
    {
      stageId: 'ralplan',
      sessionId: 'workflow-activation-fixture',
      ...
      lineNumber: 0,
      byteOffset: 0,
      recordContentSha256: createHash('sha256').update(record).digest('hex'),
      stableFile,
  ...
  state.pipelineTracking.activationBoundary = {
    transcriptPath,
    transcriptRoot: boundary.transcriptRoot,
    transcriptBasename: boundary.transcriptBasename,
    sessionId: boundary.sessionId,
    byteOffset: stableFile.size,
    fileIdentity: stableFile,
  };
...
function completePausedWorkflowState(state: any, transcriptPath: string) {
  ...
  state.pipelineTracking.completionObservations.push({
    stageId: 'execution',
    sessionId: 'workflow-activation-fixture',
...
function writeEmergencyJournal(
  ...
  writeFileSync(
    `${statePath}.emergency-journal.json`,
    JSON.stringify(
      {
        version: 1,
        transactionId,
        ...
        ...(intent === 'publish'
          ? {
              intendedDigest: createHash('sha256')
                .update(intended!)
                .digest('hex'),
            }
          : {}),
...
function createFixture(
  cwd = mkdtempSync(join(tmpdir(), 'omc-workflow-activation-')),
) {
  ...
  writeFileSync(
    join(cwd, 'claude-config', 'projects', 'workflow-activation-fixture.jsonl'),
    '',
  );
  writeFileSync(
    join(configHome, 'claude-omc', 'config.jsonc'),
    `{
...
describe.skipIf(process.platform !== 'linux')(
  'workflow profile activation hook fixtures (#3487)',
  () => {
    it.each(HOOKS)(
      'activates root project profiles from a nested git CWD through %s',
      (script) => {
    ...
    it.each(HOOKS)(
      'serializes task display and pseudo-call prompt values through %s',
      (script) => {
        ...
        try {
          const output = runHook(
            script,
            `/autopilot --workflow release-flow ${task}`,
            cwd,
            configHome,
          );
    ...
    it.each(HOOKS)(
      'ignores profile config above the git root and falls back to the user profile through %s',
      (script) => {
        ...
        try {
          ...
          writeFileSync(
            join(parent, '.claude', 'omc.jsonc'),
            '{ "autopilot": { "workflows": { "release-flow": { "version": 1, "stages": ["ralplan", "execution", "qa"] } } } }',
          );
          runHook(
            script,
            '/autopilot --workflow release-flow ship the release',
          ...
          expect(JSON.parse(stateBytes(nested)!.toString())).toMatchObject({
            workflow: {
              workflowName: 'release-flow',
              stages: ['ralplan', 'execution', 'ralph'],
            },
          });
        } finally {
          rmSync(parent, { recursive: true, force: true });
    ...
    it.each(HOOKS)(
      'does not load profile config above a literal non-Git project through %s',
      (script) => {
        const parent = mkdtempSync(
          join(tmpdir(), 'omc-workflow-profile-literal-'),
        );
    ...
    it.each(HOOKS)(
      'falls back to user profiles instead of loading config above a literal non-Git project through %s',
      (script) => {
        const parent = mkdtempSync(
          join(tmpdir(), 'omc-workflow-profile-user-fallback-'),
        );
        const cwd = join(parent, 'project');
        const { configHome } = createFixture(cwd);
        try {
    ...
    it.each(HOOKS)(
      'does not load profile config above a non-Git workspace boundary through %s',
      (script) => {
        const parent = mkdtempSync(
          join(tmpdir(), 'omc-workflow-profile-workspace-'),
        );
        ...
        try {
          ...
          runHook(
            ...
            configHome,
            transcriptPath,
            { CLAUDE_CONFIG_DIR: join(workspace, 'claude-config') },
          );
          expect(JSON.parse(stateBytes(nested)!.toString())).toMatchObject({
            workflow: {
              workflowName: 'release-flow',
    ...
    it.each(HOOKS)(
      'rejects named workflows explicitly on unsupported platforms through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        try {
          const output = runHook(
          ...
          expect(output.hookSpecificOutput?.additionalContext).toContain(
            'named autopilot workflow profiles require Linux',
          );
    ...
    it.each(HOOKS)(
      'rejects named workflows before mutation when flock is unavailable through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        try {
          const output = runHook(
    ...
    it.each(HOOKS)(
      'recovers an abandoned activation lock through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          ...
          const output = runHook(
            script,
            '/autopilot --workflow release-flow ship it',
            cwd,
            configHome,
          );
          expect(output.hookSpecificOutput?.additionalContext).toContain(
    ...
    it.each(HOOKS)(
      'rejects a second active workflow activation through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        try {
          runHook(
            script,
            '/autopilot --workflow release-flow first task',
            cwd,
            configHome,
          );
          ...
          const second = runHook(
            script,
            '/autopilot --workflow release-flow second task',
            cwd,
            configHome,
          );
          expect(second.hookSpecificOutput?.additionalContext).toContain(
            'Could not persist workflow state',
          );
    ...
    it.each(HOOKS)(
      'preserves named workflow state before routing /cancel through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        try {
          runHook(
          ...
          writeFileSync(
            dependentPath,
            JSON.stringify({ active: true, linked_ultrawork: true }),
          );
    ...
    it.each(HOOKS)(
      'reactivates the exact persisted named run through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          ...
          writeFileSync(
            join(cwd, '.claude', 'omc.jsonc'),
            '{ invalid later config',
          );
          const output = runHook(
            script,
            '/autopilot --workflow release-flow ignored replacement task',
            cwd,
            configHome,
          );
    ...
    it.each(HOOKS)(
      'starts a fresh run after valid terminal workflow history through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          ...
          expect(fresh).toMatchObject({
            ...
            phase: 'ralplan',
            prompt: 'second task',
            pipelineTracking: { currentStageIndex: 0, trackingRevision: 0 },
          });
    ...
    it.each(HOOKS)(
      'recovers a quarantined publish journal before resuming through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          ...
          const { quarantinePath } = writeEmergencyJournal(
            statePath,
            original,
            'publish',
            'quarantined',
    ...
    it.each(HOOKS)(
      'converges a quarantined clear journal before activating through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          ...
          const original = Buffer.from(
            '{"active":true,"sentinel":"interrupted-cancellation"}\n',
          );
          ...
          expect(JSON.parse(readFileSync(statePath, 'utf8'))).toMatchObject({
            active: true,
            workflow: { workflowName: 'release-flow' },
          });
          expect(existsSync(`${statePath}.emergency-journal.json`)).toBe(false);
          expect(existsSync(quarantinePath)).toBe(false);
        } finally {
    ...
    it.each(HOOKS)(
      'rejects an unrelated replacement beside an interrupted journal through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          mkdirSync(join(statePath, '..'), { recursive: true });
          const original = Buffer.from(
            '{"active":true,"sentinel":"original"}\n',
          );
          const replacement = Buffer.from(
            '{"active":true,"sentinel":"replacement"}\n',
          );
          const { quarantinePath } = writeEmergencyJournal(
            statePath,
            original,
    ...
    it.each(HOOKS)(
      'recovers a dead PID-reused preparing journal through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          mkdirSync(join(statePath, '..'), { recursive: true });
          const original = Buffer.from(
            '{"active":true,"sentinel":"pid-reused-preparing"}\n',
          );
          writeFileSync(statePath, original);
          const { quarantinePath } = writeEmergencyJournal(
            statePath,
    ...
    it.each(HOOKS)(
      'reclaims a stale recovery claim and discards an uninitialized dead preparing journal through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          mkdirSync(join(statePath, '..'), { recursive: true });
          const original = Buffer.from(
            '{"active":false,"sentinel":"partial-preparing"}\n',
          );
          const transactionId = randomUUID();
          const quarantinePath = `${statePath}.emergency-quarantine.${transactionId}`;
          writeFileSync(statePath, original);
          ...
          writeFileSync(
            `${statePath}.emergency-recovery.claim`,
            abandonedLockOwner(),
          );
          ...
          expect(existsSync(`${statePath}.emergency-journal.json`)).toBe(false);
          expect(existsSync(`${statePath}.emergency-recovery.claim`)).toBe(
            false,
          );
          expect(existsSync(quarantinePath)).toBe(false);
          expect(JSON.parse(readFileSync(statePath, 'utf8'))).toMatchObject({
    ...
    it.each(HOOKS)(
      'discards a dead preparing publish journal with an absent payload when its original remains through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          mkdirSync(join(statePath, '..'), { recursive: true });
          const original = Buffer.from(
            '{"active":false,"sentinel":"missing-payload"}\n',
          );
    ...
    it.each(HOOKS)(
      'fails closed when a preparing owner start identity is unknown through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          mkdirSync(join(statePath, '..'), { recursive: true });
          const original = Buffer.from(
            '{"active":true,"sentinel":"unknown-owner"}\n',
          );
          writeFileSync(statePath, original);
          const { quarantinePath } = writeEmergencyJournal(
            statePath,
          ...
          expect(output.hookSpecificOutput?.additionalContext).toContain(
            'workflow_emergency_recovery_failed',
          );
    ...
    it.each(HOOKS)(
      'converges transaction artifacts while preserving a conflicting replacement through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
    ...
    it.each(HOOKS)(
      'does not unlink a replacement made between recovery authentication and capture through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          ...
          const output = runHook(
            ...
            undefined,
            {
              ...
              OMC_TEST_EMERGENCY_CAPTURE_REPLACEMENT_BASE64:
    ...
    it.each(HOOKS)(
      'authenticates paused workflow transcript boundaries through %s',
      (script) => {
        const cases: Array<
          [
            string,
            (
              state: WorkflowStateWithBoundary,
              projects: string,
              transcriptPath: string,
            ) => void,
            boolean,
        ...
        > = [
          ...
          [
            'final symlink',
            (_state, projects, transcriptPath) => {
          ...
          [
            ...
            (state, projects) => {
              ...
              state.pipelineTracking.activationBoundary.transcriptPath = join(
                alias,
                'workflow-activation-fixture.jsonl',
              );
            },
          ...
          try {
            ...
            if (resumes) {
              ...
              expect(
                ...
                name,
              ).toContain('## PIPELINE STAGE: RALPLAN');
              expect(resumed).toMatchObject({
                active: true,
                workflowRunId: paused.workflowRunId,
                pipelineTracking: paused.pipelineTracking,
              });
            } else {
              expect(
                output.hookSpecificOutput?.additionalContext,
                name,
              ).toContain('workflow_descriptor_integrity_failed');
    ...
    it.each(HOOKS)(
      'rejects forged paused completion observations and resumes authenticated advanced state through %s',
      (script) => {
        for (const [name, signal, hash, resumes] of [
          ...
          try {
            ...
            if (hash)
              paused.pipelineTracking.completionObservations[0].recordContentSha256 =
                hash;
            writeFileSync(statePath, JSON.stringify(paused, null, 2));
            const before = readFileSync(statePath);
            const output = runHook(
            ...
            if (resumes) {
              ...
              expect(
                output.hookSpecificOutput?.additionalContext,
                name,
              ).toContain('## PIPELINE STAGE: EXECUTION');
              expect(resumed).toMatchObject({
                active: true,
                phase: 'execution',
                workflowRunId: paused.workflowRunId,
    ...
    it.each(HOOKS)(
      'retires an older-run cancel signal during activation through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        ...
        try {
          ...
          writeFileSync(
            ...
            JSON.stringify({
              active: true,
              mode: 'autopilot',
              target_workflow_run_id: '11111111-1111-4111-8111-111111111111',
            }),
          );
          runHook(
            script,
    ...
    it.each(HOOKS)(
      'rejects activation without a stable canonical transcript through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          ...
          for (const transcriptPath of [
            join(cwd, 'claude-config', 'projects', 'missing.jsonl'),
            join(cwd, 'claude-config', 'projects'),
            canonical,
          ]) {
            const output = runHook(
              script,
    ...
    it.each(HOOKS)(
      'loads a user-only profile from the canonical XDG config path through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        try {
          writeFileSync(
            ...
            JSON.stringify({
              autopilot: {
                workflows: {
                  'user-only': { version: 1, stages: ['ralplan', 'execution'] },
                },
              },
            }),
          );
          const output = runHook(
            script,
            '/autopilot --workflow user-only ship it',
            cwd,
            configHome,
          );
          expect(output.hookSpecificOutput?.additionalContext).toContain(
          ...
          expect(
            JSON.parse(stateBytes(cwd)!.toString()).workflow.workflowName,
          ).toBe('user-only');
        } finally {
          rmSync(cwd, { recursive: true, force: true });
        }
    ...
    it.each(HOOKS)(
      'serializes activation through the shared state mutation lock in %s',
      async (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
    ...
    it.each(HOOKS)(
      'accepts the /omc:autopilot named workflow command through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        try {
          const output = runHook(
          ...
          expect(JSON.parse(stateBytes(cwd)!.toString()).prompt).toBe(
            'ship the release',
          );
        } finally {
          rmSync(cwd, { recursive: true, force: true });
    ...
    it.each(HOOKS)(
      'keeps later --workflow tokens in named workflow task text through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        try {
          const output = runHook(
            script,
            '/autopilot --workflow release-flow explain --workflow literally',
            cwd,
            configHome,
          );
          expect(output.hookSpecificOutput?.additionalContext).toContain(
    ...
    it.each(HOOKS)(
      'resolves project workflow profiles from a literal non-Git working directory through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        ...
        try {
          ...
          writeFileSync(
            join(nested, '.claude', 'omc.jsonc'),
            JSON.stringify({
              autopilot: {
                workflows: {
                  'root-only': { version: 1, stages: ['ralplan', 'execution'] },
                },
              },
            }),
          );
          const output = runHook(
            script,
            '/autopilot --workflow root-only ship it',
            nested,
            configHome,
            transcriptPath,
            { CLAUDE_CONFIG_DIR: join(cwd, 'claude-config') },
    ...
    it.each(HOOKS)(
      'preserves partial own named markers for generic and named activation through %s',
      (script) => {
        const { cwd, configHome } = createFixture();
        const statePath = join(
          cwd,
        ...
        try {
          ...
          for (const marker of [
            ...
            const before = readFileSync(statePath);
            for (const prompt of [
              'autopilot legacy activation',
              '/autopilot --workflow release-flow named activation',
            ]) {
    ...
    it('preserves foreign shared-home recovery claims and publication temps while the template activates project A', () => {
      ...
      try {
        ...
        const foreignState = Buffer.from(
          JSON.stringify(
            { active: true, project_path: projectB.cwd, sentinel: 'project-b' },
            null,
            2,
          ),
        );
        for (const artifact of [
          ...
          const output = runHook(
            script,
            'autopilot activate project A',
            projectA.cwd,
            projectA.configHome,
            undefined,
            { HOME: projectA.cwd, USERPROFILE: projectA.cwd },
          );
          expect(output.hookSpecificOutput?.additionalContext).toContain(
            'autopilot',
          );
    ...
    it.each([
      ...
      [
        '/autopilot --workflow=release-flow ship it',
        'Use --workflow <name> followed by a task.',
      ],
      [
        '/autopilot --workflow release-flow',
        'Provide a task after the workflow name.',
      ],
      [
        '/autopilot --workflow Release_Flow ship it',
        'Workflow name must match',
      ],
      [
        '/autopilot --workflow unknown-flow ship it',
        'workflow profile "unknown-flow" was not found',
      ],
      ...
      writeFileSync(
        join(statePath, 'autopilot-state.json'),
        '{"sentinel":true}\n',
      );
      const before = stateBytes(cwd);
      try {
        for (const script of HOOKS) {
          const output = runHook(script, prompt, cwd, configHome);
          expect(output.hookSpecificOutput?.additionalContext).toContain(
            `[AUTOPILOT WORKFLOW ERROR] ${error}`,
          );
          expect(stateBytes(cwd)).toEqual(before);
        }
      } finally {
    ...
    it.each([
      ...
      'rejects a %s in plugin and template profile validation',
      (_name, stages) => {
        const { cwd, configHome } = createFixture();
        try {
          writeFileSync(
            ...
            JSON.stringify({
              autopilot: {
                workflows: { 'release-flow': { version: 1, stages } },
              },
            }),
          );
          for (const script of HOOKS) {
