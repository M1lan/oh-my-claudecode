...
const forbiddenSignoffPattern =
  /(?:done|complete|nothing further|looks good|no further comments)/i;
...
describe('advisory agent final output contract', () => {
  test.each(advisoryAgents)(
    '%s prompt requires substantive final response',
    (agentName) => {
      ...
      for (const marker of requiredMarkers[agentName]) {
        expect(
          prompt,
          `${agentName} prompt should include ${marker}`,
        ).toContain(marker);
      }

      expect(
        prompt,
        `${agentName} must explain final message is the Task deliverable`,
      ).toMatch(
        /LAST assistant message is the deliverable surfaced to callers/i,
      );
      expect(prompt, `${agentName} must forbid content-free sign-offs`).toMatch(
        forbiddenSignoffPattern,
      );
      expect(
        prompt,
        `${agentName} must require repeating earlier findings in final response`,
      ).toMatch(
  ...
  test.each(advisoryAgents)(
    '%s registry prompt includes final response contract',
    (agentName) => {
