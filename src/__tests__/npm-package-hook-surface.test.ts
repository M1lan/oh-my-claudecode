...
describe('npm package hook surface regression', () => {
  it('builds the coordinator for packaging without mutating ordinary test entrypoints', () => {
    ...
    expect(packageJson.scripts?.build).toMatch(
      /pnpm run compose-docs && pnpm run generate:prompt-projections && pnpm run build:claude-md-coordinator/,
    );
    expect(
      packageJson.scripts?.build?.indexOf('pnpm run compose-docs'),
    ).toBeLessThan(
      packageJson.scripts?.build?.indexOf('pnpm run generate:prompt-projections') ??
        -1,
    );
    expect(
      packageJson.scripts?.build?.indexOf('pnpm run generate:prompt-projections'),
    ).toBeLessThan(
      packageJson.scripts?.build?.indexOf('pnpm run build:claude-md-coordinator') ??
        -1,
    );
