...
describe("omc config-stop-callback tag options", () => {
  it("updates telegram tagList options and preserves existing config fields", () => {
    ...
    const replace = runCli(
      ["config-stop-callback", "telegram", "--tag-list", "@alice,bob"],
      homeDir,
    );
    ...
    expect(config.stopHookCallbacks?.telegram?.tagList).toEqual([
      "@alice",
      "bob",
    ]);

    const add = runCli(
      ["config-stop-callback", "telegram", "--add-tag", "charlie"],
      homeDir,
    );
    ...
    const remove = runCli(
      ["config-stop-callback", "telegram", "--remove-tag", "bob"],
      homeDir,
    );
  ...
  it("applies and clears discord tags and ignores tag options for file callback", () => {
    ...
    const add = runCli(
      ["config-stop-callback", "discord", "--add-tag", "role:123"],
      homeDir,
    );
    expect(add.status).toBe(0);
    ...
    const clear = runCli(
      ["config-stop-callback", "discord", "--clear-tags"],
      homeDir,
    );
    ...
    const file = runCli(
      ["config-stop-callback", "file", "--tag-list", "@ignored"],
      homeDir,
    );
  ...
  it("configures slack stop-callback with webhook and tags", () => {
    ...
    expect(config.stopHookCallbacks?.slack?.webhookUrl).toBe(
      "https://hooks.slack.com/services/T00/B00/xxx",
    );
    expect(config.stopHookCallbacks?.slack?.tagList).toEqual([
      "<!here>",
      "<@U1234567890>",
    ]);
    ...
    // Add a tag
    const add = runCli(
      ["config-stop-callback", "slack", "--add-tag", "<!channel>"],
      homeDir,
    );
    expect(add.status).toBe(0);
    ...
    // Remove a tag
    const remove = runCli(
      ["config-stop-callback", "slack", "--remove-tag", "<!here>"],
      homeDir,
    );
    expect(remove.status).toBe(0);
