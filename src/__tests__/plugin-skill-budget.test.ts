...
function readPluginJson(): { skills?: unknown; commands?: unknown } {
  return JSON.parse(readFileSync(PLUGIN_JSON, 'utf-8')) as {
    skills?: unknown;
    commands?: unknown;
  };
}
...
function bundledSkillDirs(): string[] {
  return readdirSync(SKILLS_DIR, { withFileTypes: true })
    .filter(
      (entry) =>
        entry.isDirectory() &&
        existsSync(join(SKILLS_DIR, entry.name, 'SKILL.md')),
    )
...
function pluginSkillDirs(): string[] {
  ...
  return (skills as string[])
    .map((skillPath) =>
      skillPath.replace(/^\.\/skills\//, '').replace(/\/$/, ''),
    )
    .sort();
}
...
function skillPayloadBytes(root: string): number {
  return readdirSync(join(root, 'skills'), { withFileTypes: true })
    ...
    .reduce(
      (total, entry) =>
        total +
        readFileSync(join(root, 'skills', entry.name, 'SKILL.md')).length,
      0,
    );
}
...
describe('plugin skill context budget gate (issues #2943, #2986)', () => {
  ...
  it('compacts installed plugin SKILL.md files while archiving full on-demand skill bodies', () => {
    ...
    try {
      ...
      for (const skillDir of allSkillDirs) {
        ...
        expect(
          Buffer.byteLength(shim),
          `${skillDir} compact shim size`,
        ).toBeLessThan(COMPACT_PLUGIN_SKILL_PER_FILE_BUDGET_BYTES);
        expect(
          shim,
          `${skillDir} shim should point to archived body`,
        ).toContain(`../../skill-bodies/${skillDir}/SKILL.md`);
        expect(
          shim,
          `${skillDir} shim should expose runtime body override`,
        ).toContain('omc-full-body:');
        expect(
          shim,
          `${skillDir} shim should prefer plugin root env vars`,
        ).toContain(
          `\${CLAUDE_PLUGIN_ROOT:-\${OMC_PLUGIN_ROOT}}/skill-bodies/${skillDir}/SKILL.md`,
        );
        expect(
          shim,
          `${skillDir} shim should define plugin root by containing directories`,
        ).toContain(
          'The plugin root is the directory containing both `skills/` and `skill-bodies/`.',
        );
        expect(
          shim,
          `${skillDir} shim should reject file-relative skill-bodies lookup`,
        ).toContain(
          `Do not resolve \`skill-bodies/${skillDir}/SKILL.md\` under this shim's \`skills/${skillDir}/\` directory`,
        );
        expect(
          shim,
          `${skillDir} shim should not instruct ambiguous SKILL.md-relative resolution`,
        ).not.toContain('Resolve that path relative to this SKILL.md file');
        expect(
          archived,
          `${skillDir} full skill body should be preserved`,
        ).toBe(source);
      }
    } finally {
  ...
  it('uses platform-safe containment for archived full-body skill paths', () => {
    const winRoot =
      'C:\\Users\\me\\.claude\\plugins\\cache\\omc\\oh-my-claudecode\\4.13.7';
    const winArchivedBody = win32.join(
      winRoot,
      'skill-bodies',
      'plan',
      'SKILL.md',
    );
    const winEscapedBody = win32.join(
      winRoot,
      '..',
      'other-plugin',
      'SKILL.md',
  ...
  it('materializes declared plugin command wrappers into cache sync targets', () => {
    ...
    try {
      ...
      writeFileSync(
        join(sourceRoot, 'commands', 'omc-setup.md'),
        'Read skills/omc-setup/SKILL.md and pass $ARGUMENTS.\n',
      );
      writeFileSync(
        join(sourceRoot, 'dist', 'hooks', 'skill-bridge.cjs'),
      ...
      writeFileSync(
        ...
        JSON.stringify({ name: 'oh-my-claude-sisyphus', version: '4.14.1' }),
      );
      ...
      expect(
        readFileSync(join(targetRoot, 'commands', 'omc-setup.md'), 'utf-8'),
      ).toContain('$ARGUMENTS');
    } finally {
      rmSync(tempRoot, { recursive: true, force: true });
    }
