...
function createFixture(options: FixtureOptions = {}): Fixture {
  ...
  const coordinatorDigest =
    options.coordinatorDigest ??
    createHash('sha256').update(canonicalClaudeMd).digest('hex');
  ...
  writeFileSync(
    ...
    "export { fixture } from './runtime.js';\n",
  );
  writeFileSync(
    join(root, 'dist', 'runtime.js'),
    'export const fixture = true;\n',
  );
  writeFileSync(
    join(root, 'bridge', 'cli.cjs'),
    "module.exports = require('./mcp-server.cjs');\n",
  );
  writeFileSync(
    join(root, 'bridge', 'mcp-server.cjs'),
    "module.exports = require('./mcp-helper.cjs');\n",
  );
  if (options.includeMcpHelper !== false) {
    writeFileSync(
      join(root, 'bridge', 'mcp-helper.cjs'),
      'module.exports = true;\n',
    );
  }
  if (options.includeCoordinator !== false) {
    const decoy = options.coordinatorDecoyDigest
      ? `// ${options.coordinatorDecoyDigest}\n`
      : '';
    writeFileSync(
...
describe('plugin shipping surface transaction', () => {
  ...
  it('expands declared generated directories into exact runtime payload files only', async () => {
    ...
    writeFileSync(
      join(fixture.root, 'dist', 'stale.js'),
      'export default true;\n',
    );
    writeFileSync(join(fixture.root, 'dist', 'stale.js.map'), '{}\n');
    writeFileSync(
      join(fixture.root, 'dist', 'fixtures', 'ignored.js'),
      'export default true;\n',
    );
    ...
    expect(git(fixture.root, ['diff', '--cached', '--name-only']).trim()).toBe(
      'dist/stale.js',
    );
  });
  ...
  it('excludes tracked generated test and fixture paths from the runtime baseline', async () => {
    ...
    expect(surface.requiredPaths).not.toContain(
      'dist/__tests__/generated.test.js',
    );
    expect(surface.requiredPaths).not.toContain(
      'bridge/fixtures/non-runtime.cjs',
  ...
  it('discovers helper-computed generated runtime children from local constant strings and arrays', () => {
    ...
    writeFileSync(
      join(fixture.root, 'bridge', 'daemon.js'),
      'module.exports = true;\n',
    );
  ...
  it('fails closed when a helper-computed generated runtime child is missing', () => {
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain(
      'required generated runtime file is missing: bridge/daemon.js',
    );
  });
  ...
  it('resolves static new URL local runtime loads relative to import metadata', () => {
    ...
    writeFileSync(
      join(fixture.root, 'bridge', 'daemon.cjs'),
      'module.exports = true;\n',
    );
  ...
  it('constructs and executes an exact forced staging command for closure paths only', async () => {
    ...
    expect(
      module.buildStageArguments([
        'bridge/cli.cjs',
        'bridge/mcp-server.cjs',
        'bridge/cli.cjs',
      ]),
    ).toEqual(['add', '-f', '--', 'bridge/cli.cjs', 'bridge/mcp-server.cjs']);
  ...
  it('stages a deletion from the prior runtime closure with its desired replacement', () => {
    const fixture = createFixture();
    writeFileSync(
      join(fixture.root, 'bridge', 'mcp-server.cjs'),
      'module.exports = true;\n',
    );
    ...
    ).toEqual(['bridge/mcp-helper.cjs', 'bridge/mcp-server.cjs']);
  });

  it('refuses deletion of an unrelated generated artifact', () => {
    const fixture = createFixture({
      trackedGeneratedTestPaths: ['bridge/unrelated.cjs'],
    });
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain(
      'refusing to stage unrelated generated artifacts: bridge/unrelated.cjs',
    );
  ...
  it('refuses unrelated ignored generated extras instead of broad staging', () => {
    const fixture = createFixture();
    writeFileSync(
      join(fixture.root, 'bridge', 'unrelated.cjs'),
      'module.exports = true;\n',
    );
  ...
  it('refuses ignored untracked dist runtime extras without broad staging', () => {
    const fixture = createFixture({ trackCli: false });
    writeFileSync(
      join(fixture.root, 'dist', 'unrelated.js'),
      'export default true;\n',
    );
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain(
      'refusing to stage unrelated generated artifacts: dist/unrelated.js',
    );
    expect(git(fixture.root, ['diff', '--cached', '--name-only'])).toBe('');
  });
  ...
  it('rejects initial runtime entrypoints that escape through a symlink', () => {
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain(
      'must not traverse a symbolic link: bridge/cli.cjs',
    );
  });
  ...
  it('verifies an ordinary committed runtime surface without staging anything', () => {
    ...
    expect(result.stdout).toContain(
      '0 ignored-and-untracked artifact(s) await staging',
    );
  ...
  it('seeds the closure from recursive public package entrypoints and stages only those untracked artifacts', async () => {
    ...
    writeFileSync(
      join(fixture.root, 'dist', 'public.js'),
      'export default true;\n',
    );
    writeFileSync(
    ...
    writeFileSync(
      join(fixture.root, 'dist', 'auxiliary.d.ts'),
      "export * from './nested.js';\n",
    );
    writeFileSync(
      join(fixture.root, 'dist', 'nested.d.ts'),
    ...
    expect(surface.ignoredUntrackedRequiredPaths).toEqual([
      'dist/auxiliary.d.ts',
      'dist/nested.d.ts',
      'dist/public.d.ts',
      'dist/public.js',
    ]);
    expect(surface.stagePaths).toEqual([
      'dist/auxiliary.d.ts',
      'dist/nested.d.ts',
      'dist/public.d.ts',
      'dist/public.js',
  ...
  it('rejects missing and escaping public package entrypoints', () => {
    ...
    expect(run(fixture.root, 'verify').stderr).toContain(
      'required generated runtime file is missing: dist/missing.js',
    );
    ...
    expect(run(fixture.root, 'verify').stderr).toContain(
      'package.json main must stay within the package root',
    );
  });
  ...
  it('accepts a PR diff that changes only computed runtime closure artifacts', () => {
    ...
    writeFileSync(
      join(fixture.root, 'bridge', 'mcp-helper.cjs'),
    ...
    git(fixture.root, [
      ...
      '--quiet',
      '-m',
      'update generated runtime helper',
    ]);
  ...
  it('accepts a PR deletion from the previous runtime closure', () => {
    ...
    git(fixture.root, [
      ...
      '--quiet',
      '-m',
      'remove obsolete generated runtime helper',
    ]);
  ...
  it('rejects a PR diff with an out-of-closure generated artifact', () => {
    ...
    git(fixture.root, [
      ...
      '--quiet',
      '-m',
      'add unrelated generated artifact',
    ]);
  ...
  it('does not let a base-owned generated directory bless a candidate-only runtime artifact', () => {
    ...
    git(fixture.root, [
      ...
      '--quiet',
      '-m',
      'add candidate-only generated artifact',
    ]);
  ...
  it('rejects changes to a base-tracked generated module that is unreachable from plugin entrypoints', () => {
    const fixture = createFixture({
      trackedGeneratedTestPaths: ['dist/unreachable.js'],
    });
    const base = git(fixture.root, ['rev-parse', 'HEAD']).trim();
    writeFileSync(
      join(fixture.root, 'dist', 'unreachable.js'),
      'export const fixture = "unreachable change";\n',
    );
    ...
    git(fixture.root, [
      ...
      '--quiet',
      '-m',
      'change unreachable generated module',
    ]);
  ...
  it('does not let a PR-controlled package files entry bless an unrelated artifact', () => {
    ...
    git(fixture.root, [
      ...
      '--quiet',
      '-m',
      'attempt to bless unrelated artifact',
    ]);
  ...
  it('does not let a PR-controlled generated directory bless new runtime candidates', () => {
    ...
    git(fixture.root, [
      ...
      '--quiet',
      '-m',
      'attempt generated directory blessing',
    ]);
  ...
  it('does not let a PR-controlled package exports target bless an unrelated artifact', () => {
    ...
    git(fixture.root, [
      ...
      '--quiet',
      '-m',
      'attempt exports target blessing',
    ]);
  ...
  it('rejects ambiguous computed local generated runtime loads', () => {
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain(
      'ambiguous local runtime load in bridge/cli.cjs',
    );
  });
  ...
  it('rejects generated tests or fixtures reached by runtime code', () => {
    ...
    writeFileSync(
      join(fixture.root, 'bridge', 'cli.cjs'),
      "module.exports = require('./fixtures/runtime.cjs');\n",
    );
    writeFileSync(
      join(fixture.root, 'bridge', 'fixtures', 'runtime.cjs'),
      'module.exports = true;\n',
    );
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain(
      'generated test or fixture cannot enter runtime closure: bridge/fixtures/runtime.cjs',
    );
  });
  ...
  it('rejects malformed or missing PR base commits', () => {
    ...
    expect(malformed.stderr).toContain(
      'check-pr base must be a 40-character hexadecimal commit SHA',
    );
    expect(missing.status).not.toBe(0);
    expect(missing.stderr).toContain(
      `check-pr base commit is not available: ${'0'.repeat(40)}`,
    );
  });
  ...
  it('uses the unique merge base when the supplied local base is ahead of HEAD', () => {
    ...
    const aheadBase = git(fixture.root, [
      'commit-tree',
      tree,
      '-p',
      head,
  ...
  it('rejects local check-pr histories without one unique merge base', () => {
    ...
    expect(
      git(fixture.root, ['merge-base', '--all', a2, b2]).trim().split(/\s+/),
    ).toHaveLength(2);
