...
describe("project-session-manager review worktree hardening", () => {
  ...
  it("bootstraps review worktrees with best-effort dependency reuse when package.json matches", () => {
    ...
    execFileSync(
      ...
      [
        "-lc",
        'source "$SCRIPT_PATH"; psm_bootstrap_review_dependencies "$REPO_DIR" "$WORKTREE_DIR"',
      ],
      {
        env: {
          ...process.env,
    ...
    expect(resolve(worktree, readlinkSync(linkedNodeModules))).toBe(
      join(repo, "node_modules"),
    );
    expect(WORKTREE_LIB).toContain(
  ...
  it("guides PR review flows toward focused verification before full-suite fallback", () => {
    expect(PR_REVIEW_TEMPLATE).toContain(
      "pnpm run test:run -- <changed-test-paths>",
    );
    expect(PR_REVIEW_TEMPLATE).toContain("preferred focused verification");
    expect(PR_REVIEW_TEMPLATE).toContain(
      "symlinked node_modules from the source repo",
    );
  });
});
