...
describe('run.cjs — graceful fallback for stale plugin paths', () => {
  ...
  it('keeps UserPromptSubmit manifest timeouts aligned for prompt hooks', () => {
    ...
    expect(hooksDoc).toContain(
      'A command that never reaches `run.cjs` can consume its full 30s outer fuse.',
    );
    expect(referenceDoc).toContain(
      '| **UserPromptSubmit**   | `keyword-detector.mjs`, `skill-injector.mjs`',
    );
    expect(referenceDoc).toContain(
      '30s outer fuse per command; 8s, 12s trusted Worker limits',
    );
    expect(referenceDoc).toContain(
      'A command that never starts the runner can take the entire 30s per-command fuse',
    );
  });
  ...
  it('caps only trusted prompt Worker execution without extending lower manifest limits', () => {
    ...
    const values = JSON.parse(
      execFileSync(NODE, ['-e', policyProbe, RUN_CJS_PATH, trustedPluginRoot], {
        encoding: 'utf-8',
      }),
    );
