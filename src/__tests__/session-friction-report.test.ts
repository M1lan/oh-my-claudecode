...
describe('session friction report', () => {
    ...
    writeJsonl(join(currentProjectDir, 'session-current.jsonl'), [
      {
        ...
        message: {
          role: 'user',
          content: 'secret prompt text that must not appear in reports',
        },
        ...
        message: {
          ...
          content: [
            ...
            },
            {
              type: 'tool_result',
              is_error: true,
              content: 'raw failing tool output that must not appear',
            },
          ],
        },
      },
  ...
  it('reports local metadata without raw prompt or tool-result content', async () => {
    ...
    expect(
      report.sessions[0].signals.some(
        (signal) => signal.code === 'context-high',
      ),
    ).toBe(true);
  ...
  it('includes OMC replay artifacts for all-project reports', async () => {
    ...
    const session = report.sessions.find(
      (candidate) => candidate.sessionId === 'session-current',
    );
