...
describe('classifyAgentSpawn', () => {
  ...
  it('keeps kind but leaves spawnedBy absent for legacy payloads without a session id', () => {
    ...
    expect(classifyAgentSpawn({ hasName: false })).toEqual({
      kind: 'subagent',
    });
  });
});
...
describe('parseIncomingAgentWrapper', () => {
  ...
  it('treats a wrapper without an identity attribute as an unknown sender', () => {
    expect(
      parseIncomingAgentWrapper(
        '<teammate-message color="red">x</teammate-message>',
      ),
    ).toEqual({
      kind: 'teammate',
      senderId: 'unknown',
    ...
    expect(
      parseIncomingAgentWrapper('<agent-message>hi</agent-message>'),
    ).toEqual({
      kind: 'peer-session',
      senderId: 'unknown',
      spawnedBy: undefined,
  ...
  it('returns null for non-wrapper or legacy plain text', () => {
    ...
    expect(
      parseIncomingAgentWrapper('<div>not an agent wrapper</div>'),
    ).toBeNull();
  });
  ...
  it('marks a named Task spawn as a teammate with spawnedBy = its session', async () => {
    const path = createTempTranscript([
      toolUse(
        'toolu_team_001',
        'Task',
        { name: 'worker-1', subagent_type: 'executor' },
        'sess-a',
      ),
    ]);
  ...
  it('keeps proxy_ Task/Agent classification transparent to the spawner', async () => {
    const path = createTempTranscript([
      toolUse(
        'toolu_proxy_001',
        'proxy_Agent',
        { name: 'worker-2', subagent_type: 'executor' },
        'sess-b',
      ),
      toolUse(
        'toolu_proxy_002',
...
describe('parseTranscript — incoming wrapper messages', () => {
  ...
  it('records a task-notification sender while still completing the correlated agent (listing correlation)', async () => {
    ...
    const result = await parseTranscript(path, STALE_OPTS);
    expect(result.agents.find((a) => a.id === 'toolu_bg_001')?.status).toBe(
      'completed',
    );
    expect(result.incomingMessages).toEqual([
      {
  ...
  it('recognizes wrappers inside text blocks of array content', async () => {
    const path = createTempTranscript([
      userText([
        {
          type: 'text',
          text: '<agent-message from="peer-x">hello from peer</agent-message>',
        },
      ]),
    ]);
    const result = await parseTranscript(path, STALE_OPTS);
          ...
          content: [
            {
              ...
              content:
                '<agent-message from="general-purpose">quoted inside agent output</agent-message>',
            },
          ],
        },
      },
  ...
  it("keeps messages from a peer session isolated from the coordinator's agent listing", async () => {
    const path = createTempTranscript([
      ...
      userText(
        '<agent-message from="general-purpose">peer traffic</agent-message>',
        'sess-coord',
      ),
    ]);
    const result = await parseTranscript(path, STALE_OPTS);
    ...
    expect(result.incomingMessages?.[0]).toMatchObject({
      kind: 'peer-session',
      spawnedBy: undefined,
    });
  });
