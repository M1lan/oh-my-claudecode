...
describe('mission board state tracking', () => {
  it('records session-scoped agent starts and completions', () => {
    ...
    expect(mission.timeline.map((entry) => entry.kind)).toEqual([
      'update',
      'completion',
    ]);
  });
  ...
  it('syncs team missions from existing team state files and preserves session missions', () => {
    ...
    writeFileSync(
      join(teamRoot, 'workers', 'worker-1', 'status.json'),
      JSON.stringify(
        {
          state: 'working',
          current_task_id: '1',
    ...
    writeFileSync(
      join(teamRoot, 'workers', 'worker-1', 'heartbeat.json'),
      JSON.stringify(
        {
    ...
    writeFileSync(
      join(teamRoot, 'workers', 'worker-2', 'status.json'),
      JSON.stringify(
        {
          state: 'done',
    ...
    writeFileSync(
      ...
      [
        JSON.stringify({
          ...
          worker: 'worker-2',
          task_id: '2',
          created_at: '2026-03-09T07:03:00.000Z',
        }),
        JSON.stringify({
          type: 'team_leader_nudge',
          worker: 'worker-1',
          reason: 'continue working',
          created_at: '2026-03-09T07:04:00.000Z',
        }),
      ].join('\n'),
    );

    writeFileSync(
      join(teamRoot, 'mailbox', 'worker-1.json'),
      JSON.stringify(
        {
          messages: [
            {
    ...
    const state = refreshMissionBoardState(
      cwd,
      {
        enabled: true,
        maxMissions: 5,
    ...
    const teamMission = state.missions.find(
      (mission) => mission.source === 'team',
    );
    expect(teamMission?.name).toBe('demo');
    expect(teamMission?.status).toBe('running');
    expect(teamMission?.taskCounts.inProgress).toBe(1);
    expect(teamMission?.agents[0]?.currentStep).toContain(
      'implementing renderer',
    );
    expect(teamMission?.agents[1]?.completedSummary).toContain(
      'Added mission board tests',
    );
    expect(
      teamMission?.timeline.some((entry) => entry.kind === 'handoff'),
    ).toBe(true);
    expect(
      teamMission?.timeline.some((entry) => entry.kind === 'completion'),
    ).toBe(true);
    ...
    const persisted = JSON.parse(
      readFileSync(paths.effectiveWrite, 'utf-8'),
    ) as {
      missions: Array<{ source: string }>;
    };
    expect(
      persisted.missions.some((mission) => mission.source === 'session'),
    ).toBe(true);
    expect(
      persisted.missions.some((mission) => mission.source === 'team'),
    ).toBe(true);
  });
  ...
  it('session isolation: two sessions write interleaved but each reads back only its own data', () => {
    ...
    expect(missionsA[0]!.agents[0]?.completedSummary).toContain(
      'Session A completed',
    );
    ...
    expect(missionsB[0]!.agents[0]?.latestUpdate).toContain(
      'Session B encountered',
    );
  ...
  it('legacy data does not bleed into a fresh session read (no-fallback regression)', () => {
    ...
    const allOwnerships = state!.missions.flatMap((m) =>
      m.agents.map((a) => a.ownership),
    );
  ...
  it('deduplicates duplicate team worker rows when refreshing mission board state', () => {
    ...
    const mission = state.missions.find(
      (entry) => entry.source === 'team' && entry.teamName === 'dedupe-demo',
    );
