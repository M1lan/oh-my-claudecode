...
describe('getUsage routing', () => {
      ...
      const result = parseUsageResponse({
        ...
        limits: [
          ...
          },
          {
            ...
            scope: {
              model: { id: null, display_name: 'Opus 4.6' },
              surface: null,
            },
          },
          {
            ...
            is_active: false,
            scope: {
              model: { id: null, display_name: 'Haiku Lite' },
              surface: null,
            },
          },
        ],
    ...
    it('ignores malformed/null limits entries without throwing', () => {
      const result = parseUsageResponse({
        ...
        limits: [
          ...
          {
            kind: 'weekly_scoped',
            scope: { model: { display_name: 'NaNModel' } },
            percent: NaN,
          },
          {
            kind: 'session',
            scope: { model: { display_name: 'NotWeekly' } },
            percent: 50,
          }, // wrong kind
        ],
      } as unknown as Parameters<typeof parseUsageResponse>[0]);
  ...
  it('rehydrates scoped reset dates from serialized cache before rendering', async () => {
    ...
    expect(stripAnsi(renderRateLimits(result.rateLimits, false)!)).toContain(
      'fable:61%',
    );
    expect(
      stripAnsi(renderRateLimitsWithBar(result.rateLimits, 8, false)!),
    ).toContain('fable:[');
    expect(httpsModule.default.request).not.toHaveBeenCalled();
  });
...
describe('parseKimiResponse', () => {
  ...
  const livePayload = {
    ...
    usage: {
      ...
      used: '45',
      remaining: '55',
      resetTime: '2026-07-31T00:54:55.628002Z',
    },
    limits: [
      {
        ...
        detail: {
          ...
          used: '2',
          remaining: '98',
          resetTime: '2026-07-25T21:54:55.628002Z',
        },
      },
    ],
    parallel: { limit: '30' },
  ...
  it('parses the live payload shape (string numbers, nano resetTime)', () => {
    ...
    expect(result!.fiveHourResetsAt).toBeInstanceOf(Date);
    expect(result!.fiveHourResetsAt!.getTime()).toBe(
      new Date('2026-07-25T21:54:55.628Z').getTime(),
    );
    expect(result!.weeklyResetsAt).toBeInstanceOf(Date);
    expect(result!.weeklyResetsAt!.getTime()).toBe(
      new Date('2026-07-31T00:54:55.628Z').getTime(),
    );
  });
  ...
  it('returns null when no usable quota rows exist', () => {
    ...
    expect(
      parseKimiResponse({ usage: { limit: 'abc', used: 'def' } }),
    ).toBeNull();
  });
  ...
  it('accepts reset_at/resetAt aliases and survives non-string reset fields', () => {
      ...
      limits: [
        {
          ...
          detail: {
            limit: '100',
            used: '2',
            resetTime: 123 as unknown as string,
          },
        },
      ],
    });
    ...
    expect(result!.weeklyResetsAt).toBeInstanceOf(Date);
    expect(result!.weeklyResetsAt!.getTime()).toBe(
      new Date('2026-07-31T00:54:55Z').getTime(),
    );
    expect(result!.fiveHourPercent).toBe(2);
  ...
  it('survives a non-string currency without throwing', () => {
    const result = parseKimiResponse({
      ...
      boosterWallet: {
        ...
        monthlyChargeLimit: {
          priceInCents: 2000,
          currency: 840 as unknown as string,
        },
...
describe('getUsage routing - kimi', () => {
  ...
  it('writes cache with source kimi', async () => {
    ...
    const writeCall = vi
      .mocked(fs.writeFileSync)
      .mock.calls.find((c) => String(c[0]).includes('.usage-cache-kimi.json'));
    expect(writeCall).toBeTruthy();
    const written = JSON.parse(String(writeCall![1]));
