...
describe("renderSkills", () => {
  const inactiveUltrawork: UltraworkStateForHud = {
    active: false,
    reinforcementCount: 0,
  };
  const activeUltrawork: UltraworkStateForHud = {
    active: true,
    reinforcementCount: 0,
  };
  const inactiveRalph: RalphStateForHud = {
    active: false,
    iteration: 0,
    maxIterations: 10,
  };
  const activeRalph: RalphStateForHud = {
    active: true,
    iteration: 3,
    maxIterations: 10,
  };
  ...
  describe("last skill rendering", () => {
    it("renders last skill when no modes are active", () => {
      const lastSkill: SkillInvocation = {
        name: "plan",
        timestamp: new Date(),
      };
    ...
    it("truncates long args", () => {
      const lastSkill: SkillInvocation = {
        name: "plan",
        args: "this is a very long argument",
        timestamp: new Date(),
      };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
      expect(result).toContain("skill:plan");
  ...
  describe("namespaced skill names", () => {
    ...
    it("handles multiple colons in skill name", () => {
      const lastSkill: SkillInvocation = {
        name: "namespace:subcategory:action",
        timestamp: new Date(),
      };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
