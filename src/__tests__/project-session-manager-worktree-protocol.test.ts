...
const REAL_GIT = execFileSync('bash', ['-lc', 'command -v git'], {
  encoding: 'utf-8',
}).trim();
...
describe('project-session-manager worktree result protocol', () => {
  ...
  it.each([
    {
      creator: 'psm_create_pr_worktree',
      args: ['demo', '42', 'head'],
      fields: 2,
      branch: 'psm-pr-42-review',
    },
    {
      creator: 'psm_create_issue_worktree',
      args: ['demo', '42', 'demo-fix', 'main'],
      fields: 3,
    },
    {
      creator: 'psm_create_feature_worktree',
      args: ['demo', 'Demo Feature', 'main'],
      fields: 3,
    },
  ])(
    'keeps $creator stdout isolated from git output',
    ({ creator, args, fields, branch }) => {
      ...
      execFileSync(REAL_GIT, ['config', 'user.email', 'test@example.com'], {
        cwd: repo,
      });
      execFileSync(REAL_GIT, ['config', 'user.name', 'Test User'], {
        cwd: repo,
      });
...
`,
      ...
      const stdout = execFileSync(
        ...
        [
          '-c',
          'source "$SCRIPT_PATH"; psm_get_worktree_root() { printf "%s" "$WORKTREE_ROOT"; }; psm_sanitize() { echo "$1" | tr "[:upper:]" "[:lower:]" | sed "s/[^a-z0-9-]/-/g" | sed "s/--*/-/g" | head -c 30; }; "$CREATOR" "$REPO" "$@"',
          'protocol',
          ...args,
        ],
        {
  ...
  it.each([
    ...
    { raw: 'created|/tmp/wt|', schema: 'branched', valid: false },
    {
      raw: 'created|/tmp/wt|feature/x|extra',
      schema: 'branched',
      valid: false,
    },
    ...
    'validates raw protocol records ($schema, $raw)',
    ({ raw, schema, valid }) => {
      const result = commandExit(
        'source "$PSM"; psm_validate_worktree_result "$ARG" "$SCHEMA"',
        {
          ...process.env,
          PSM: PSM_PATH,
          ARG: raw,
  ...
  it.each([
    {
      result: 'exists|/tmp/wt|feature/demo',
      creatorRc: '1',
      status: 0,
      malformed: false,
    },
    ...
    {
      result: 'HEAD is now at abc\ncreated|/tmp/wt|feature/demo',
      creatorRc: '3',
      status: 1,
      malformed: true,
    },
  ])(
    'handles cmd_feature creator results under errexit',
    ({ result, creatorRc, status, malformed }) => {
...
`,
  ...
  it('captures safely and validates before side effects in every consumer', () => {
    ...
    for (const entry of cases) {
      ...
      expect(body.slice(captureOffset, captureLineEnd)).toContain(
        '|| worktree_rc=',
      );
      ...
      expect(body.slice(readOffset, validatorOffset)).toMatch(
        /^IFS='\|' read -r[^\n]*<<< "\$worktree_result"\n\s*if ! \s*$/,
      );
      expect(body.slice(readOffset)).toMatch(
