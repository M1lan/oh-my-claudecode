...
describe('PSM tmux-safe naming contract (issue #3528)', () => {
  describe('canonical contract', () => {
    it('psm_tmux_safe_name translates both ":" and "."', () => {
      const out = runShell(
        `${SOURCE} psm_tmux_safe_name "omc:pr-123"; echo; psm_tmux_safe_name "repo.js:feat-a.b"`,
      );
  ...
  describe('create + registration', () => {
    it('creates the session under the tmux-safe name', () => {
      const out = runShell(
        `${SOURCE} psm_create_tmux_session "psm:omc:pr-123" "/tmp"`,
      );
...
`;
      const out = runShell(
        `CREATED_FILE=$(mktemp); ${listMock} source "${TMUX_SH}"; psm_list_tmux_sessions`,
      );
