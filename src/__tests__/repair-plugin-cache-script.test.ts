...
describe('repair-plugin-cache.mjs', () => {
  it('rewrites stale installed_plugins.json and keeps old cache path as a symlink fallback', () => {
      ...
      JSON.stringify(
        {
          ...
          plugins: {
            'oh-my-claudecode@omc': [
              { installPath: oldRoot, version: '4.11.6', enabled: true },
            ],
          },
        },
        null,
    ...
    const result = spawnSync(process.execPath, [SCRIPT_PATH], {
      env: {
        ...
        OMC_REPAIR_PLUGIN_CACHE_PLATFORM: 'linux',
      },
      encoding: 'utf-8',
    });
  ...
  it.runIf(process.platform !== 'win32')(
    'repairs Unix cache hooks from direct node to the find-node bootstrap',
    () => {
  ...
  it.runIf(process.platform !== 'win32')(
    'repairs every bundled direct-node hook command to find-node on Unix/macOS',
    () => {
      const root = mkdtempSync(
        join(tmpdir(), 'omc-repair-unix-bundled-hooks-'),
      );
      tempRoots.push(root);
  ...
  it('repairs Windows cache hooks from find-node to direct node', () => {
    ...
    const result = spawnSync(process.execPath, [SCRIPT_PATH], {
      env: {
        ...process.env,
        CLAUDE_CONFIG_DIR: configDir,
        OMC_REPAIR_PLUGIN_CACHE_PLATFORM: 'win32',
      },
      encoding: 'utf-8',
    });
  ...
  it('setup instructions repair cache references before prompts and avoid direct cache deletion', () => {
    ...
    expect(
      setupSkill.indexOf(
        'repair-plugin-cache.mjs',
        setupSkill.indexOf('Active Plugin Root Resolution'),
      ),
    ).toBeLessThan(setupSkill.indexOf('## Pre-Setup Check'));
