...
describe('keyword-detector.mjs mode-message dispatch', () => {
  ...
  it('does not activate a magic keyword from a delegated /ask antigravity payload', () => {
    const tempDir = mkdtempSync(
      join(tmpdir(), 'keyword-detector-ask-antigravity-'),
    );
...
+ autopilot build me an app`);
  ...
  it.each([
    ...
    'does not activate ralph for issue #3411 false-positive prompt: %s',
    (prompt) => {
      const cwd = mkdtempSync(
        join(tmpdir(), 'keyword-detector-ralph-3411-negative-'),
      );
  ...
  it.each(['/oh-my-claudecode:ralph issue #3411', 'ralph this'])(
    'still activates ralph for issue #3411 explicit invocation: %s',
    (prompt) => {
      const cwd = mkdtempSync(
        join(tmpdir(), 'keyword-detector-ralph-3411-positive-'),
      );
  ...
  it.each([
    ...
    'does not activate workflow for informational Thai prompt "%s"',
  ...
  it.each([
    ...
    const cwd = mkdtempSync(
      join(tmpdir(), 'keyword-detector-autopilot-thai-creation-'),
    );
  ...
  it('does not activate autopilot for colon-prefixed heading help question', () => {
    const cwd = mkdtempSync(
      join(tmpdir(), 'keyword-detector-autopilot-colon-help-'),
    );
    const sessionId = 'session-autopilot-colon-help';
    const output = runKeywordDetector(
      'autopilot: what is it and how do I use it?',
      cwd,
      sessionId,
    );
    const context = output.hookSpecificOutput?.additionalContext ?? '';
  ...
  it('does not activate autopilot for English help-style use questions in the script copy', () => {
    const cwd = mkdtempSync(
      join(tmpdir(), 'keyword-detector-autopilot-help-question-'),
    );
  ...
  it.each([
    ...
    const cwd = mkdtempSync(
      join(tmpdir(), 'keyword-detector-autopilot-colon-positive-'),
    );
  ...
  it('does not activate ralph for a leading proper-noun mention ("Ralph Step 0a wiring")', () => {
    const cwd = mkdtempSync(
      join(tmpdir(), 'keyword-detector-ralph-proper-noun-'),
    );
    const sessionId = 'session-ralph-proper-noun';
    const output = runKeywordDetector(
      'Ralph Step 0a wiring is advisory, not a hook.',
      cwd,
      sessionId,
    );
    const context = output.hookSpecificOutput?.additionalContext ?? '';
  ...
  it('does not activate ralph for a hyphenated identifier mention ("wire ralph-step-0a.sh")', () => {
    ...
    const output = runKeywordDetector(
      'wire ralph-step-0a.sh into its callers',
      cwd,
      sessionId,
    );
    const context = output.hookSpecificOutput?.additionalContext ?? '';
  ...
  it('does not activate ralph for a hyphenated state filename mention ("ralph-state.json")', () => {
    const cwd = mkdtempSync(
      join(tmpdir(), 'keyword-detector-ralph-state-file-'),
    );
    const sessionId = 'session-ralph-state-file';
    const output = runKeywordDetector(
      'inspect ralph-state.json without starting the hook',
      cwd,
      sessionId,
    );
    const context = output.hookSpecificOutput?.additionalContext ?? '';
  ...
  it('still activates ralph for a leading imperative task ("ralph fix the tests")', () => {
    const cwd = mkdtempSync(
      join(tmpdir(), 'keyword-detector-ralph-leading-imperative-'),
    );
...
describe('keyword-detector.mjs global disable values', () => {
  ...
  it.each(['', '0', 'false', 'TRUE', 'yes'])(
    'does not treat DISABLE_OMC=%s as a global disable',
    (value) => {
      const output = runKeywordDetector(
        'deepsearch this codebase',
        process.cwd(),
