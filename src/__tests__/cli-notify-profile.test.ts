...
describe("omc config-stop-callback --profile", () => {
  it("creates a discord profile and stores it in notificationProfiles", () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({ silentAutoUpdate: false }, null, 2),
    );
    ...
    expect(config.notificationProfiles.work.discord.webhookUrl).toBe(
      "https://discord.com/api/webhooks/test",
    );
  });
  ...
  it("creates a telegram profile", () => {
    ...
    expect(config.notificationProfiles.personal.telegram.botToken).toBe(
      "123:abc",
    );
  ...
  it("creates a discord-bot profile with --channel-id", () => {
    ...
    expect(config.notificationProfiles.ops["discord-bot"].botToken).toBe(
      "bot-token-123",
    );
    expect(config.notificationProfiles.ops["discord-bot"].channelId).toBe(
      "channel-456",
    );
  });
  ...
  it("adds multiple platforms to the same profile", () => {
    ...
    runCli(
      [
        ...
        "--enable",
        "--webhook",
        "https://discord.com/api/webhooks/multi",
      ],
      homeDir,
    );
  ...
  it("does not affect legacy stopHookCallbacks when using --profile", () => {
    ...
    expect(config.stopHookCallbacks.discord.webhookUrl).toBe(
      "https://discord.com/api/webhooks/legacy",
    );
    // New profile created separately
    expect(config.notificationProfiles.new.discord.webhookUrl).toBe(
      "https://discord.com/api/webhooks/new",
    );
  });
  ...
  it("shows profile config with --show", () => {
    ...
    const result = runCli(
      ["config-stop-callback", "discord", "--profile", "work", "--show"],
      homeDir,
    );
...
describe("omc config-notify-profile", () => {
      ...
      JSON.stringify(
        {
          ...
          notificationProfiles: {
            work: {
              ...
              discord: {
                enabled: true,
                webhookUrl: "https://discord.com/api/webhooks/w",
              },
            },
            personal: {
              enabled: true,
  ...
  it("deletes a profile", () => {
    ...
    const result = runCli(
      ["config-notify-profile", "work", "--delete"],
      homeDir,
    );
    expect(result.status).toBe(0);
