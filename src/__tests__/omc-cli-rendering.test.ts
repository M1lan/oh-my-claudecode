...
describe("omc CLI rendering", () => {
  ...
  it("falls back to the plugin bridge when omc is unavailable but CLAUDE_PLUGIN_ROOT is set", () => {
    ...
    expect(
      formatOmcCliInvocation('autoresearch --mission "m"', {
        omcAvailable: false,
        env,
      }),
    ).toBe(
      'node "$CLAUDE_PLUGIN_ROOT"/bridge/cli.cjs autoresearch --mission "m"',
    );
  });
  ...
  it("routes ask invocations through the plugin bridge inside an active Claude session when CLAUDE_PLUGIN_ROOT is set", () => {
    ...
    expect(
      formatOmcCliInvocation('ask codex --prompt "check"', {
        omcAvailable: false,
        env,
      }),
    ).toBe(
      'node "$CLAUDE_PLUGIN_ROOT"/bridge/cli.cjs ask codex --prompt "check"',
    );
    ...
    expect(output).toContain(
      '`node "$CLAUDE_PLUGIN_ROOT"/bridge/cli.cjs ask codex "review"`',
    );
    expect(output).toContain(
      '> node "$CLAUDE_PLUGIN_ROOT"/bridge/cli.cjs ask gemini --prompt "improve docs"',
    );
  });
