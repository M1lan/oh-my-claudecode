...
describe('auto-update reconciliation', () => {
    ...
    mockedStatSync.mockImplementation(
      (path: Parameters<typeof statSync>[0]) => {
        if (!mockedExistsSync(path)) {
          throw new Error(
            `ENOENT: no such file or directory, stat '${String(path)}'`,
          );
        }
  ...
  it('reconciles runtime state without re-injecting settings hooks', () => {
    ...
    expect(mockedMkdirSync).toHaveBeenCalledWith('/tmp/omc-test-hooks', {
      recursive: true,
    });
  ...
  it('syncs active plugin cache roots and logs when copy occurs', () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      '[omc update] Synced plugin cache',
    );
  });
    ...
    mockedExistsSync.mockImplementation(
      (path: Parameters<typeof existsSync>[0]) => {
        ...
        if (
          ...
          normalized === activeRoot.replace(/\\/g, '/')
        ) {
          return true;
        }
  ...
  it('updates installed_plugins.json to the new cache version after plugin cache sync succeeds', () => {
          ...
          return JSON.stringify({
            ...
            plugins: {
              'oh-my-claudecode@omc': [
                { installPath: oldRoot, version: '4.14.0', enabled: true },
              ],
            },
          });
        }
    ...
    mockedExistsSync.mockImplementation(
      (path: Parameters<typeof existsSync>[0]) => {
        const normalized = String(path).replace(/\\/g, '/');
        return (
          normalized.endsWith('/plugins/cache/omc/oh-my-claudecode') ||
          normalized.endsWith('/plugins/installed_plugins.json') ||
          normalized.startsWith(
            join(cacheRoot, '4.14.1').replace(/\\/g, '/'),
          ) ||
    ...
    const written = String(
      mockedWriteFileSync.mock.calls.find(([path]) =>
        String(path).includes('installed_plugins.json.tmp-'),
      )?.[1],
    );
    expect(written).toContain(
      `"installPath": "${newRoot.replace(/\\/g, '\\\\')}"`,
    );
  ...
  it('preserves Windows-style installPath separators when rewriting installed_plugins.json', () => {
    ...
    const oldWindowsRoot =
      'C:\\Users\\bellman\\.claude\\plugins\\cache\\omc\\oh-my-claudecode\\4.14.0';
    const newWindowsRoot =
      'C:\\Users\\bellman\\.claude\\plugins\\cache\\omc\\oh-my-claudecode\\4.14.1';
    ...
    mockedReadFileSync.mockImplementation(
      (path: Parameters<typeof readFileSync>[0]) => {
        ...
        if (
          normalized ===
          'C:/Users/bellman/AppData/Roaming/npm/node_modules/oh-my-claude-sisyphus/package.json'
        ) {
          return JSON.stringify({ version: '4.14.1' });
        }
        if (normalized.endsWith('/plugins/installed_plugins.json')) {
          return JSON.stringify({
            ...
            plugins: {
              'oh-my-claudecode@omc': [
                { installPath: oldWindowsRoot, version: '4.14.0' },
              ],
            },
          });
        }
    ...
    mockedExistsSync.mockImplementation(
      (path: Parameters<typeof existsSync>[0]) => {
        const normalized = String(path).replace(/\\/g, '/');
        return (
          normalized === cacheRoot.replace(/\\/g, '/') ||
          normalized.endsWith('/plugins/installed_plugins.json') ||
          normalized.startsWith(
            join(cacheRoot, '4.14.1').replace(/\\/g, '/'),
          ) ||
          normalized.startsWith(
            'C:/Users/bellman/AppData/Roaming/npm/node_modules/oh-my-claude-sisyphus',
          )
        );
      },
    );
    ...
    expect(
      JSON.parse(written).plugins['oh-my-claudecode@omc'][0],
    ).toMatchObject({
      version: '4.14.1',
  ...
  it('does not rewrite installed_plugins.json when plugin cache sync reports copy errors', () => {
      ...
      (path: Parameters<typeof readFileSync>[0]) => {
        ...
        if (normalized.endsWith('/plugins/installed_plugins.json')) {
          return JSON.stringify({
            ...
            plugins: {
              'oh-my-claudecode@omc': [
                { installPath: join(cacheRoot, '4.14.0'), version: '4.14.0' },
              ],
            },
          });
        }
    ...
    expect(result.errors).toContain(
      `Failed to sync dist to ${join(cacheRoot, '4.14.1')}: copy failed`,
    );
    expect(
      mockedWriteFileSync.mock.calls.some(([path]) =>
        String(path).includes('installed_plugins.json.tmp-'),
      ),
    ).toBe(false);
  ...
  it('does not rewrite installed_plugins.json when the versioned cache is missing runtime-critical files after sync', () => {
    ...
    mockedExistsSync.mockImplementation(
      (path: Parameters<typeof existsSync>[0]) => {
        ...
        if (
          normalized ===
          `${versionedCacheRoot.replace(/\\/g, '/')}/bridge/claude-md-coordinator.cjs`
        ) {
          return false;
        }
        return (
          ...
          normalized.endsWith('/plugins/installed_plugins.json') ||
          normalized.startsWith(
            '/usr/lib/node_modules/oh-my-claude-sisyphus',
          ) ||
    ...
    expect(result.errors).toContain(
      `${versionedCacheRoot}: Missing required plugin payload file: bridge/claude-md-coordinator.cjs`,
    );
    expect(
      mockedWriteFileSync.mock.calls.some(([path]) =>
        String(path).includes('installed_plugins.json.tmp-'),
  ...
  it('syncs the plugin cache directory when cache root exists', () => {
    ...
    expect(mockedMkdirSync).toHaveBeenCalledWith(versionedCacheRoot, {
      recursive: true,
    });
    expect(mockedCpSync).toHaveBeenCalledWith(
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      '[omc update] Plugin cache synced',
    );
  });
  ...
  it('only blocks standalone update inside an active plugin session', () => {
    ...
    process.env.CLAUDE_PLUGIN_ROOT =
      '/tmp/.claude/plugins/cache/omc/oh-my-claudecode/4.1.5';
    expect(shouldBlockStandaloneUpdateInCurrentSession()).toBe(false);
  ...
  it('dedupes plugin roots and ignores missing targets during sync', () => {
    ...
    expect(result.success).toBe(true);
    const targetCalls = mockedCpSync.mock.calls.filter(([, destination]) =>
      String(destination).startsWith(activeRoot),
    );
    expect(targetCalls.length).toBeGreaterThan(0);
    expect(
      mockedCpSync.mock.calls.some(([, destination]) =>
        String(destination).startsWith(staleRoot),
      ),
    ).toBe(false);
  ...
  it('restores global Claude Code when npm removes an existing global install during update', async () => {
    ...
    mockedExecFileSync.mockImplementation(
      (command: string, args?: readonly string[]) => {
        if (
          command === 'pnpm' &&
          args?.join(' ') === 'add -g @anthropic-ai/claude-code@1.2.3'
        ) {
          return '';
        }
        throw new Error(
          `Unexpected execFileSync command: ${command} ${args?.join(' ') ?? ''}`,
        );
      },
    );
    ...
    try {
      ...
      expect(mockedExecFileSync).toHaveBeenCalledWith(
        'pnpm',
        ['add', '-g', '@anthropic-ai/claude-code@1.2.3'],
        expect.any(Object),
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        '[omc update] Restoring global @anthropic-ai/claude-code@1.2.3 after pnpm update...',
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        '[omc update] Restored global @anthropic-ai/claude-code',
      );
    } finally {
  ...
  it('does not install global Claude Code when it was absent before update', async () => {
    ...
    expect(mockedExecFileSync).not.toHaveBeenCalledWith(
      'pnpm',
      ['add', '-g', expect.stringContaining('@anthropic-ai/claude-code@')],
      expect.any(Object),
    );
  });
  ...
  it('detects native Windows Claude Code via claude --version and does not attempt npm restore', async () => {
    ...
    mockedExistsSync.mockImplementation(
      (path: Parameters<typeof existsSync>[0]) => {
        ...
        if (
          normalized ===
          'C:/Users/bellman/AppData/Roaming/npm/node_modules/@anthropic-ai/claude-code/package.json'
        ) {
          return false;
        }
        if (normalized.endsWith('/plugins/marketplaces/omc')) {
    ...
    expect(mockedExecSync).not.toHaveBeenCalledWith(
      'pnpm add -g @anthropic-ai/claude-code@2.1.142',
      expect.any(Object),
    );
    expect(mockedExecFileSync).not.toHaveBeenCalledWith(
      'pnpm',
  ...
  it('uses Windows-safe npm options when restoring global Claude Code', async () => {
    ...
    expect(result.success).toBe(true);
    expect(mockedExecSync).toHaveBeenCalledWith(
      'pnpm add -g @anthropic-ai/claude-code@1.2.3',
      expect.objectContaining({
        windowsHide: true,
      }),
    );
  ...
  it('skips marketplace auto-sync when the marketplace clone has local modifications', async () => {
    ...
    expect(mockedExecFileSync).toHaveBeenCalledWith(
      ...
      [
        '-C',
        expect.stringContaining('/plugins/marketplaces/omc'),
        'status',
        '--porcelain',
