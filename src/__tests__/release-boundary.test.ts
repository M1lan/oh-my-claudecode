...
describe('release-boundary.mjs', () => {
  ...
  it('asserts the final archive surfaces and records tamper-evident byte and content manifests', async () => {
    ...
    expect(evidence.archiveManifest.files).toContainEqual(
    ...
    const missingCoordinatorPath = writeTarball(
      root,
      'missing-coordinator.tgz',
      makeTarball([
        {
          path: 'package/package.json',
          content: `${JSON.stringify(packageManifest(SHA), null, 2)}\n`,
        },
        {
          path: 'package/.claude-plugin/plugin.json',
          content: JSON.stringify({
        ...
        },
        {
          ...
          content: JSON.stringify({
            version: VERSION,
            plugins: [
              { name: 'oh-my-claudecode', version: VERSION, source: './' },
            ],
          }),
        },
        {
    ...
    expect(() =>
      assertArchive(missingCoordinatorPath, { version: VERSION, gitHead: SHA }),
    ).toThrow('bridge/claude-md-coordinator.cjs');
    await expect(
      cliMain([
        'assert-evidence',
        '--tarball',
...
describe('assert-marketplace-consistency', () => {
  function createPromotionRepository(
    ...
    writeFileSync(
      join(root, 'docs', 'CLAUDE.md'),
      `<!-- OMC:VERSION:${opts.claudeMdVersion ?? version} -->\n`,
    );

    execFileSync('git', ['init'], { cwd: root, stdio: 'ignore' });
    execFileSync('git', ['config', 'user.name', 'Promotion Test'], {
      cwd: root,
      stdio: 'ignore',
    });
    execFileSync('git', ['config', 'user.email', 'promotion@example.test'], {
      cwd: root,
      stdio: 'ignore',
    });
    ...
    execFileSync('git', ['add', '.'], { cwd: root, stdio: 'ignore' });
    execFileSync('git', ['commit', '-m', 'promotion fixture'], {
      cwd: root,
      stdio: 'ignore',
    });
    const sha = execFileSync('git', ['rev-parse', 'HEAD'], {
    ...
    if (opts.branch && opts.branch !== 'main') {
      execFileSync('git', ['branch', opts.branch], {
        cwd: root,
        stdio: 'ignore',
      });
    }
  ...
  function commitVersionBump(
    ...
    execFileSync('git', ['add', '.'], { cwd: root, stdio: 'ignore' });
    execFileSync('git', ['commit', '-m', `bump to ${version}`], {
      cwd: root,
      stdio: 'ignore',
    });
  }
  ...
  it('passes without --sha when metadata matches even if commit differs', () => {
    ...
    execFileSync('git', ['add', '.'], { cwd: root, stdio: 'ignore' });
    execFileSync('git', ['commit', '-m', 'add gitignore'], {
      cwd: root,
      stdio: 'ignore',
    });
  ...
  it('fails when docs/CLAUDE.md has duplicate version markers', () => {
    ...
    execFileSync('git', ['add', '.'], { cwd: root, stdio: 'ignore' });
    execFileSync('git', ['commit', '-m', 'duplicate docs marker'], {
      cwd: root,
      stdio: 'ignore',
    });
  ...
  it('passes when a protected-main merge commit contains the released commit', () => {
    ...
    writeFileSync(join(root, '.gitignore'), 'node_modules\n');
    execFileSync('git', ['add', '.'], { cwd: root, stdio: 'ignore' });
    execFileSync('git', ['commit', '-m', 'protected-main merge closure'], {
      cwd: root,
      stdio: 'ignore',
    });
