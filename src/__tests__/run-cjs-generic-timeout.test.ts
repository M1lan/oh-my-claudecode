...
describe('run.cjs generic hook timeout supervisor', () => {
  ...
  it('uses the source-owned supervisor only for Windows generic hooks', () => {
    expect(
      runCjs.resolveGenericChildCommand(HUNG_PARENT, ['argument'], 'win32'),
    ).toEqual([
      RUN_CJS_PATH,
    ...
    expect(
      runCjs.resolveGenericChildCommand(HUNG_PARENT, ['argument'], 'linux'),
    ).toEqual([HUNG_PARENT, 'argument']);
  });
  ...
  it('reaps the supervised hook tree when its IPC parent disappears', async () => {
    const directory = mkdtempSync(
      join(tmpdir(), 'omc-supervisor-parent-death-'),
    );
    const pidfile = join(directory, 'grandchild.pid');
    let grandchildPid: number | undefined;
    const supervisor = spawn(
      process.execPath,
      [RUN_CJS_PATH, '--generic-child-supervisor', HUNG_PARENT],
      {
    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () =>
              reject(new Error('supervisor did not exit after IPC disconnect')),
            5000,
          ),
        ),
      ]);
  ...
  it('preserves normal child completion through the supervisor', async () => {
    const directory = mkdtempSync(
      join(tmpdir(), 'omc-supervisor-normal-exit-'),
    );
    const fixture = join(directory, 'numeric-exit.cjs');
    writeFileSync(fixture, 'process.exit(3);');
    const supervisor = spawn(
      process.execPath,
      [RUN_CJS_PATH, '--generic-child-supervisor', fixture],
      {
        stdio: ['ignore', 'ignore', 'ignore', 'ipc'],
