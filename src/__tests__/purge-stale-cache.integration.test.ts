    ...
    JSON.stringify({
      ...
      plugins: {
        'oh-my-claudecode@omc': [
          { installPath: join(pluginDir, ACTIVE), version: ACTIVE },
        ],
      },
    }),
  );
...
describe('invariant across every cache shape', () => {
  ...
  function shape(
    occupant: (typeof OCCUPANTS)[number],
    ...
    for (const p of [V, aside].filter(Boolean) as string[]) {
      try {
        ...
        /* symlinks and plain files cannot be aged */
      }
    }
    return { V, aside };
  }
  ...
  it('never ends with a broken pinned path and nothing to show for it', () => {
    ...
    for (const occupant of OCCUPANTS) {
      for (const backup of BACKUPS) {
        ...
        let result:
          ReturnType<typeof purgeStalePluginCacheVersions> | undefined;
        ...
        const accounted =
          (result?.errors.length ?? 0) > 0 || (result?.skipped ?? 0) > 0;
        const held =
          threw === null &&
          (!hadPayload ||
            hookResolves(STALE) ||
            (intactBackupSurvives() && accounted));
        if (!held) {
          violations.push(
...
describe.skipIf(process.platform === 'win32')(
  'syscall semantics this implementation relies on',
  () => {
    let root: string;
    beforeEach(() => {
      root = mkdtempSync(join(tmpdir(), 'omc-posix-'));
    });
    afterEach(() => {
    ...
    it('rename over a symlink reports ENOTDIR, dangling or not', () => {
      ...
      expect(
        code(() => renameSync(payloadDir('src3'), join(root, 'linkLive'))),
      ).toBe('ENOTDIR');
      expect(
        code(() => renameSync(payloadDir('src4'), join(root, 'linkDead'))),
      ).toBe('ENOTDIR');
    });
    ...
    it('symlink onto an occupied path reports EEXIST', () => {
      ...
      expect(
        code(() =>
          symlinkSync(join(root, 'taken'), join(root, 'taken'), 'dir'),
        ),
      ).toBe('EEXIST');
    });
