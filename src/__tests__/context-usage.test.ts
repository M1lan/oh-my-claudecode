...
function writeTranscript(payload: unknown): string {
  const filePath = join(
    tempDir,
    `transcript-${Date.now()}-${Math.random()}.jsonl`,
  );
...
describe('resolveHudCacheContextPercent', () => {
  it('uses HUD native used_percentage from the session cache', async () => {
    ...
    expect(
      await resolveHudCacheContextPercent({ session_id: sessionId }, tempDir),
    ).toBe(getContextPercent(payload as StatuslineStdin));
  });
  ...
  it('falls back to the most recently modified session cache', async () => {
    const oldPath = writeHudCache(
      'hud-old-session',
      makeHudPayload({
        context_window: { used_percentage: 12 },
      }),
    );
    const freshPath = writeHudCache(
      'hud-fresh-session',
      makeHudPayload({
  ...
  it('uses the env-identified session cache even when a foreign session cache is newer', async () => {
    const envSessionPath = writeHudCache(
      'hud-env-session',
      makeHudPayload({
        context_window: { used_percentage: 30 },
      }),
    );
    const foreignPath = writeHudCache(
      'hud-foreign-session',
      makeHudPayload({
  ...
  it('falls back to the legacy flat cache before scanning sessions when no identity is known', async () => {
    ...
    const legacyPayload = makeHudPayload({
    ...
    const sessionPath = writeHudCache(
      'hud-unrelated-session',
      makeHudPayload({
  ...
  it('skips an invalid payload session_id and falls back to a valid env identity', async () => {
    ...
    try {
      expect(
        await resolveHudCacheContextPercent({ session_id: '../evil' }, tempDir),
      ).toBe(61);
    } finally {
      if (originalClaude === undefined) delete process.env.CLAUDE_SESSION_ID;
      else process.env.CLAUDE_SESSION_ID = originalClaude;
  ...
  it('falls back to the legacy flat cache when every identity candidate is invalid', async () => {
    ...
    try {
      expect(
        await resolveHudCacheContextPercent(
          { session_id: '../still-evil' },
          tempDir,
        ),
      ).toBe(38);
    } finally {
  ...
  it('returns null for a valid but cache-less primary identity instead of falling through to a secondary candidate', async () => {
    ...
    writeHudCache(
      'hud-unrelated-secondary-with-cache',
      makeHudPayload({
  ...
  it('returns null for a valid env-bound identity with no cache even when the legacy flat cache is populated', async () => {
    ...
    writeFileSync(
      join(legacyDir, HUD_CACHE_FILENAME),
      JSON.stringify(
        makeHudPayload({ context_window: { used_percentage: 95 } }),
      ),
      'utf-8',
    );
  ...
  it('returns null when the cache has no context_window key', async () => {
    ...
    expect(
      await resolveHudCacheContextPercent(
        { session_id: 'missing-context-key' },
        tempDir,
      ),
    ).toBeNull();
  });
...
describe('resolveContextPercent orchestration', () => {
  it('prefers transcript, then hook payload, then HUD cache', async () => {
    ...
    expect(await resolveContextPercent({}, missingTranscript, tempDir)).toBe(
      45,
    );
  });
  ...
  it('returns null when no source has usable context data', async () => {
    expect(
      await resolveContextPercent({}, join(tempDir, 'missing.jsonl'), tempDir),
    ).toBeNull();
  });
});
