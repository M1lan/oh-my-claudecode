...
function makePlugin(root: string, source: string, timeout = 10) {
  ...
  writeFileSync(
    join(root, 'scripts', 'skill-injector.mjs'),
    'process.exit(0);',
  );
  writeFileSync(
...
function runDelayedPromptRunner(root: string, target: string) {
  ...
  const result = spawnSync(
    NODE,
    ['-e', launcher, NODE, RUN_CJS_PATH, root, target, '11000', '30000'],
    {
      encoding: 'utf-8',
      timeout: 35000,
    },
...
describe('Windows-safe prompt hook runner paths', () => {
  it('keeps trusted Worker selection and generic fallbacks correct from a root with spaces', () => {
    const cacheBase = mkdtempSync(
      join(tmpdir(), 'omc prompt root with spaces-'),
    );
    ...
    expect(run(target, root, {}, ['not-worker-eligible'])).toMatchObject({
      status: 0,
      stdout: 'child',
    });
  });
  ...
  it('uses the explicitly selected stale sibling root and terminalizes a timed-out prompt Worker', () => {
    const cacheBase = mkdtempSync(
      join(tmpdir(), 'omc stale root with spaces-'),
    );
    tempDirs.push(cacheBase);
    ...
    expect(
      run(join(staleRoot, 'scripts', 'keyword-detector.mjs'), staleRoot),
    ...
    makePlugin(
      timeoutRoot,
      "setInterval(() => {}, 1000); setTimeout(() => process.stdout.write('late'), 20);",
      1,
    );
    ...
    expect(result.stderr).toContain(
      'Hook keyword-detector.mjs timed out after 1ms; exiting fail-open.',
    );
  });
  ...
  it('models an argv-delayed launch crossing 10s and failing open before the 30s host fuse', () => {
    ...
    expect(model.evidence).toContain(
      'model; not a native Windows reproduction',
    );
    ...
    expect(model.finishedAt - model.runnerStartedAt).toBeGreaterThanOrEqual(
      7500,
    );
