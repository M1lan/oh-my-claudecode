...
const modelContractMocks = vi.hoisted(() => ({
  ...
  resolveValidatedBinaryPath: vi.fn(
    (agentType: string) => `/usr/bin/${agentType}`,
  ),
}));
...
const tmuxSessionMocks = vi.hoisted(() => ({
  ...
  spawnOwnedWorkerInPane: vi.fn(
    async (
      ...
      attempt: {
        attempt_id: `attempt-${ownership.paneId}`,
        currentPath: '/tmp/current',
        decisionPath: '/tmp/decision',
...
vi.mock('../runtime-owner-client.js', () => ({
  resolveRuntimeCliPath: () => '/runtime-cli.js',
}));
...
describe('scaleUp launch config', () => {
  ...
  beforeEach(async () => {
    ...
    vi.clearAllMocks();
    workerLaunchMocks.retireAndCleanupCurrentWorkerLaunchAttempt.mockImplementation(
      async (
        _attempt: unknown,
        _reason: string,
        cleanup: () => Promise<boolean>,
    ...
    monitorMocks.migrateTeamConfigRevision.mockImplementation(async () => {
      ...
      return config
        ? { config, stateRevision: config.state_revision ?? 0 }
        : null;
    });
    monitorMocks.readRevisionedTeamConfig.mockImplementation(async () =>
      monitorMocks.currentConfig
        ? {
            config: monitorMocks.currentConfig,
            stateRevision: monitorMocks.currentConfig.state_revision ?? 0,
          }
        : null,
    );
    monitorMocks.saveTeamConfigAtRevision.mockImplementation(
      async (next: TeamConfig, expectedRevision: number) => {
        if (
          !monitorMocks.currentConfig ||
          (monitorMocks.currentConfig.state_revision ?? 0) !== expectedRevision
        )
          return false;
  ...
  it.each([
    ...
    async (agentType: CliAgentType, workerArgv: readonly string[]) => {
      ...
      expect(reservation!.active_scale_up).toEqual(
        expect.objectContaining({ phase: 'effects' }),
      );
      expect(reservation!.workers[0]).toMatchObject({
        worker_cli: agentType,
      ...
      expect(
        monitorMocks.saveTeamConfigAtRevision.mock.invocationCallOrder.find(
          (_, index) => {
            const candidate = monitorMocks.saveTeamConfigAtRevision.mock.calls[
              index
            ]?.[0] as TeamConfig;
            return candidate.workers.some(
              (worker) =>
                worker.name === 'worker-1' &&
                worker.operational_state === 'starting',
            );
  ...
  it.each([
    ...
    [
      'untrusted',
      "Resolved CLI binary 'codex' to untrusted location: /tmp/shadow/codex",
    ],
    ['missing', "CLI binary 'codex' not found in PATH"],
  ])(
    'fails %s scale-up provider preflight before worker side effects',
    async (_case, reason) => {
      ...
      expect(result).toEqual({
        ok: false,
        error: `Failed strict provider preflight for worker-1 (codex): ${reason}`,
      });
      expect(
  ...
  it('rolls back a pending worktree when scale-up fails before worker config is saved', async () => {
    ...
    tmuxSessionMocks.spawnOwnedWorkerInPane.mockRejectedValueOnce(
      new Error('boom'),
    );
  ...
  it('restores managed overlays for reused worktrees during scale-down without deleting them', async () => {
    ...
    expect(
      workerLaunchMocks.retireAndCleanupCurrentWorkerLaunchAttempt,
    ).toHaveBeenCalledWith(
  ...
  it('preserves pane and state when scale-down launch ownership is not accepted', async () => {
    ...
    tmuxSessionMocks.getWorkerLiveness.mockResolvedValue('dead');
    workerLaunchMocks.retireAndCleanupCurrentWorkerLaunchAttempt.mockResolvedValueOnce(
      false,
    );
    ...
    expect(result).toMatchObject({
      ok: false,
      error: 'provider_cleanup_unverified:worker-1',
    });
    expect(
      workerLaunchMocks.retireAndCleanupCurrentWorkerLaunchAttempt,
    ).toHaveBeenCalled();
    expect(tmuxSessionMocks.killOwnedWorkerPane).not.toHaveBeenCalled();
    expect(
      monitorMocks.currentConfig?.workers.map((worker) => worker.name),
  ...
  it('preserves worktree and config when target pane remains alive after kill request', async () => {
    ...
    expect(result).toMatchObject({
      ok: false,
      error: expect.stringContaining('pane_still_alive'),
    });
