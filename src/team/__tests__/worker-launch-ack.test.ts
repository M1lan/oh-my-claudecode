...
describe('worker launch acknowledgement', () => {
  it('accepts only the exact child-written acknowledgement before running the provider', async () => {
    ...
    const spec = buildWorkerLaunchBootstrapSpec(
      ...
      [
        process.execPath,
        '-e',
        `const fs=require('node:fs');setInterval(()=>{if(fs.existsSync(${JSON.stringify(stopPath)}))process.exit(0)},10)`,
      ],
      cwd,
    );
    ...
    await expect(
      awaitWorkerLaunchAcknowledgement(launchAttempt, {
        timeoutMs: 2_000,
        pollIntervalMs: 5,
      }),
    ).resolves.toEqual({ ok: true });
    await expect(
      awaitWorkerLaunchProviderStarted(launchAttempt, {
        timeoutMs: 10_000,
        pollIntervalMs: 5,
      }),
    ).resolves.toBe(true);
    ...
    await expect(isWorkerLaunchAttemptAccepted(launchAttempt)).resolves.toBe(
      true,
    );

    const decision = JSON.parse(
      await readFile(launchAttempt.decisionPath, 'utf8'),
    );
  ...
  it.each([
    ...
    'rejects %s provider-start handoff evidence',
    async (_case, pid, processStartIdentity) => {
      const launchAttempt = await attempt();
      await writeFile(
        launchAttempt.ackPath,
        JSON.stringify({
  ...
  it('rejects a provider that exits after publishing start evidence but before handoff', async () => {
    const launchAttempt = await attempt();
    const bootstrap = runWorkerLaunchBootstrap(
      buildWorkerLaunchBootstrapSpec(
        launchAttempt,
    ...
    await vi.waitFor(
      async () => {
        await expect(
          isWorkerLaunchProviderStarted(launchAttempt),
        ).resolves.toBe(true);
      },
  ...
  it('kills provider descendants when the provider exits around start publication', async () => {
    ...
    await expect(
      readFile(`${launchAttempt.startedPath}.terminal`, 'utf8').then(
        JSON.parse,
      ),
    ).resolves.toMatchObject({ cleanup_verified: true });
    expect(isProcessAlive(process.pid)).toBe(true);
    await expect(
      readFile(`${launchAttempt.startedPath}.terminal`, 'utf8'),
    ).resolves.toContain('worker_launch_provider_terminal');
  });
  ...
  it('revokes a timed-out attempt and treats a later acknowledgement as losing evidence', async () => {
    ...
    await expect(runWorkerLaunchBootstrap(spec)).resolves.toEqual({
      outcome: 'revoked',
    });
    await expect(readFile(providerMarker, 'utf8')).rejects.toMatchObject({
      code: 'ENOENT',
    });
    await expect(isWorkerLaunchAttemptAccepted(launchAttempt)).resolves.toBe(
      false,
  ...
  it('rejects a mismatched nonce and seals the attempt against later acceptance', async () => {
    ...
    const expected = JSON.parse(
      await readFile(launchAttempt.expectedPath, 'utf8'),
    );
    await writeFile(
      launchAttempt.ackPath,
      JSON.stringify({
  ...
  it('does not clean a superseded or expected-only current launch attempt', async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        worker_name: accepted.worker_name,
        pane_id: accepted.pane_id,
    ...
    await expect(
      awaitWorkerLaunchAcknowledgement(accepted, {
        timeoutMs: 500,
        pollIntervalMs: 5,
      }),
    ).resolves.toEqual({ ok: true });
    await expect(
      retireWorkerLaunchAttempt(accepted, 'superseded'),
    ).resolves.toBe(true);
    const successor = await prepareWorkerLaunchAttempt({
      cwd,
      teamName: accepted.team_name,
      workerName: accepted.worker_name,
      paneId: '%3',
      provider: accepted.provider,
      runtimeCliPath: accepted.runtimeCliPath,
    });
  ...
  it('reuses exact durable cleanup-complete evidence without touching a successor or pane again', async () => {
    ...
    await writeFile(
      `${launchAttempt.startedPath}.terminal`,
      JSON.stringify({
        ...expected,
  ...
  it.runIf(process.platform !== 'win32')(
    'publishes termination-complete on an identity-bound already-dead retry after a prior request',
    async () => {
      const launchAttempt = await attempt();
      const expected = JSON.parse(
        await readFile(launchAttempt.expectedPath, 'utf8'),
      ...
      await writeFile(
        `${launchAttempt.startedPath}.termination-request`,
        JSON.stringify({
          ...expected,
          kind: 'worker_launch_termination_request',
          pid: 999_999_999,
          process_start_identity: '1',
          containment_nonce: launchAttempt.nonce,
          written_at: new Date().toISOString(),
        }),
        'utf8',
      );
      ...
      await expect(
        terminateWorkerLaunchProvider(launchAttempt, 100),
      ).resolves.toBe(true);
      await expect(
        readFile(
          `${launchAttempt.startedPath}.termination-complete`,
          'utf8',
        ).then(JSON.parse),
  ...
  it('does not treat a reused provider PID as cleaned without terminal descendant proof', async () => {
    ...
    await expect(terminateWorkerLaunchProvider(launchAttempt, 0)).resolves.toBe(
      false,
    );
    expect(isProcessAlive(process.pid)).toBe(true);
  });
  ...
  it('terminates a started provider process tree when durable start publication fails', async () => {
    ...
    const pids = JSON.parse(await readFile(pidMarker, 'utf8')) as {
      parent: number;
      child: number;
    };
    await vi.waitFor(
  ...
  it('retires and terminates the exact started provider process tree', async () => {
    ...
    await expect(
      retireWorkerLaunchAttempt(launchAttempt, 'pane_cleanup'),
    ).resolves.toBe(true);
    await expect(terminateWorkerLaunchProvider(launchAttempt)).resolves.toBe(
      true,
    );
  ...
  it('reloads an accepted attempt only for the exact pane and provider identity', async () => {
    ...
    const bootstrap = runWorkerLaunchBootstrap(spec);
    await awaitWorkerLaunchAcknowledgement(launchAttempt, {
      timeoutMs: 2_000,
      pollIntervalMs: 5,
    });
    await bootstrap;
  ...
  it('keeps revocation terminal when a valid acknowledgement is already present', async () => {
    ...
    await vi.waitFor(
      async () => {
        const acknowledgement = JSON.parse(
          await readFile(launchAttempt.ackPath, 'utf8'),
        );
    ...
    await expect(
      revokeWorkerLaunchAttempt(launchAttempt, 'timeout'),
    ).resolves.toBe(true);
    await expect(
      awaitWorkerLaunchAcknowledgement(launchAttempt, {
        timeoutMs: 2_000,
  ...
  it('prevents provider spawn after durable launch retirement wins ordering', async () => {
    ...
    await expect(
      readFile(`${launchAttempt.decisionPath}.retired`, 'utf8'),
    ).resolves.toContain('worker_launch_retired');
  });
  ...
  it('keeps an accepted decision terminal when revocation arrives later', async () => {
    ...
    await expect(
      revokeWorkerLaunchAttempt(launchAttempt, 'late_timeout'),
    ).resolves.toBe(false);
    await expect(bootstrap).resolves.toEqual({
      outcome: 'provider_spawn_failed',
    });
  ...
  it('prevents an older acknowledged attempt from releasing a provider after supersession', async () => {
    cwd = await mkdtemp(
      join(tmpdir(), 'omc-worker-launch-recovery-generation-'),
    );
    ...
    const newerAttempt = await prepareWorkerLaunchAttempt({
      ...
      context: {
        ...
        pane_attempt_id: 'pane-new',
      },
    });

    await expect(
      awaitWorkerLaunchAcknowledgement(olderAttempt, {
        timeoutMs: 2_000,
        pollIntervalMs: 5,
      }),
    ).resolves.toEqual({ ok: false, reason: 'attempt_superseded' });
    ...
    await expect(isWorkerLaunchAttemptAccepted(olderAttempt)).resolves.toBe(
      false,
    );
    await expect(isWorkerLaunchAttemptAccepted(newerAttempt)).resolves.toBe(
      false,
    );
    const current = JSON.parse(
      await readFile(newerAttempt.currentPath, 'utf8'),
    );
  ...
  it('reloads the accepted current recovery launch with its durable context', async () => {
    ...
    const started = JSON.parse(
      await readFile(launchAttempt.startedPath, 'utf8'),
    );
    ...
    await expect(
      retireWorkerLaunchAttempt(launchAttempt, 'test_cleanup'),
    ).resolves.toBe(true);
    await expect(terminateWorkerLaunchProvider(launchAttempt)).resolves.toBe(
      true,
    );
  ...
  it('materializes an attempt-owned Windows transport without exposing provider secrets in pane text', async () => {
    ...
    expect(materialized.wrapperRelativePath).toMatch(
      /^\.omc\\state\\team\\launch-team\\workers\\worker-1\\launch-attempts\\[0-9a-f-]+\\launch\.cmd$/,
    );
    expect(
      Buffer.byteLength(materialized.wrapperRelativePath, 'utf8'),
    ).toBeLessThan(256);
    const wrapper = await readFile(materialized.wrapperPath, 'utf8');
    expect(wrapper).toContain('setlocal DisableDelayedExpansion');
    expect(wrapper).toContain(
      'OMC_WORKER_LAUNCH_SPEC_FILE=%~dp0bootstrap.json',
    );
    expect(wrapper).toContain('--worker-launch');
    for (const value of [
      secret,
      providerEnv.PROVIDER_URL,
      providerEnv.SYNTHETIC_METACHARS,
      providerEnv.SYNTHETIC_UNICODE,
      providerEnv.SYNTHETIC_CRLF,
      longValue,
    ]) {
    ...
    const descriptorRaw = await readFile(
      materialized.bootstrapDescriptorPath,
      'utf8',
    );
    ...
    const consumed = (await readAndConsumeWorkerLaunchDescriptor(
      materialized.bootstrapDescriptorPath,
    )) as Record<string, unknown>;
    expect(consumed).toMatchObject({
      attempt_id: launchAttempt.attempt_id,
      provider_env: { PROVIDER_TOKEN: secret },
    });
    await expect(
      readFile(materialized.bootstrapDescriptorPath, 'utf8'),
    ).rejects.toMatchObject({ code: 'ENOENT' });
    await expect(
      cleanupWorkerLaunchTransport(launchAttempt, 'test_cleanup'),
    ).resolves.toBe(true);
    await expect(
      readFile(materialized.wrapperPath, 'utf8'),
    ).rejects.toMatchObject({ code: 'ENOENT' });
    await expect(
      cleanupWorkerLaunchTransport(launchAttempt, 'test_cleanup_retry'),
    ).resolves.toBe(true);
    await expect(
      readFile(launchAttempt.transportCleanupCompletePath, 'utf8').then(
        JSON.parse,
      ),
    ).resolves.toMatchObject({
      attempt_id: launchAttempt.attempt_id,
      kind: 'worker_launch_transport_cleanup_complete',
    });
  });

  it('uses a safe relative wrapper command when worker cwd is nested below the leader state root', async () => {
    const launchAttempt = await attempt();
    const workerCwd = join(
      cwd,
      '.omc',
      'team',
      'launch-team',
    ...
    expect(materialized.wrapperRelativePath).toMatch(
      /^(?:\.\.\\)+state\\team\\launch-team\\workers\\worker-1\\launch-attempts\\[0-9a-f-]+\\launch\.cmd$/,
    );
    expect(materialized.wrapperRelativePath).not.toMatch(/[\s"%!^&|()]/);
    await expect(
      cleanupWorkerLaunchTransport(launchAttempt, 'nested_worktree_cleanup'),
    ).resolves.toBe(true);
  });
  ...
  it('removes only partial current-attempt transport files when exclusive materialization fails', async () => {
    ...
    await expect(
      readFile(launchAttempt.transportOwnerPath, 'utf8'),
    ).rejects.toMatchObject({ code: 'ENOENT' });
    await expect(
      readFile(launchAttempt.bootstrapDescriptorPath, 'utf8'),
    ).rejects.toMatchObject({ code: 'ENOENT' });
    await expect(readFile(launchAttempt.wrapperPath, 'utf8')).resolves.toBe(
      'foreign-wrapper',
    );
  });
  ...
  it('refuses transport cleanup when the durable owner belongs to another attempt identity', async () => {
    const launchAttempt = await attempt();
    await materializeWorkerLaunchTransport({
      attempt: launchAttempt,
      providerArgv: ['codex'],
      cwd,
    });
    ...
    await writeFile(
      launchAttempt.transportOwnerPath,
      JSON.stringify({
        ...expected,
        nonce: '00000000-0000-4000-8000-000000000000',
    ...
    await expect(
      readAndConsumeWorkerLaunchDescriptor(
        launchAttempt.bootstrapDescriptorPath,
      ),
    ).rejects.toThrow('worker_launch_descriptor_owner_invalid');
    await expect(
      readFile(launchAttempt.bootstrapDescriptorPath, 'utf8'),
    ).resolves.toContain(launchAttempt.attempt_id);
    await expect(
      cleanupWorkerLaunchTransport(launchAttempt, 'foreign_owner'),
    ).resolves.toBe(false);
    await expect(
      readFile(launchAttempt.wrapperPath, 'utf8'),
    ).resolves.toContain('--worker-launch');
    await expect(
      readFile(launchAttempt.bootstrapDescriptorPath, 'utf8'),
    ).resolves.toContain(launchAttempt.attempt_id);
  });
  ...
  it('validates provider environment keys and propagates only explicit provider values', async () => {
    const launchAttempt = await attempt();
    expect(() =>
      buildWorkerLaunchBootstrapSpec(launchAttempt, ['codex'], cwd, {
  ...
  it('routes native Windows batch shims through a percent-safe temporary wrapper without changing POSIX argv', async () => {
    ...
    const windowsInvocation = buildProviderSpawnInvocation(
      providerArgv,
      'win32',
    ...
    const materialized =
      await materializeProviderSpawnInvocation(windowsInvocation);
    const wrapperPath = materialized.args[4]!.slice(1, -1);
    await expect(readFile(wrapperPath, 'utf8')).resolves.toBe(
      windowsInvocation.batchScript,
    );
    await materialized.cleanup();
    await expect(readFile(wrapperPath, 'utf8')).rejects.toMatchObject({
      code: 'ENOENT',
    });
    const supervised = await materializeProviderSpawnInvocation(
      windowsInvocation,
      { superviseWindowsTree: true },
    );
    expect(supervised.completionPath).toBeTruthy();
    await expect(
      readFile(supervised.args[4]!.slice(1, -1), 'utf8'),
    ).resolves.toContain(':omc_hold');
    await expect(
      readFile(supervised.args[4]!.slice(1, -1), 'utf8'),
    ).resolves.toContain('provider-exit.txt');
    ...
    expect(
      ...
      command: 'cmd.exe',
      args: ['/d', '/v:off', '/s', '/c'],
      batchScript: expect.stringContaining('codex.exe'),
    });
  });
  ...
  it('cleans up a temporary provider wrapper when wrapper write fails', async () => {
    ...
    const writeFileMock = vi.fn(
      async (
        ...
        ...args: Parameters<typeof actualFs.writeFile> extends [
          unknown,
          ...infer Rest,
        ]
          ? Rest
    ...
    try {
      ...
      await expect(
        workerLaunch.materializeProviderSpawnInvocation({
          command: 'cmd.exe',
          args: ['/d', '/v:off', '/s', '/c'],
      ...
      expect(rmMock).toHaveBeenCalledWith(
        expect.stringContaining('omc-provider-'),
  ...
  it('materializes POSIX supervision without changing direct provider argv', async () => {
    ...
    const invocation = await materializeProviderSpawnInvocation(
      buildProviderSpawnInvocation(
        ['/usr/bin/codex', '--prompt', 'literal & value'],
        'linux',
      ),
  ...
  it.runIf(process.platform === 'win32')(
    'distinguishes provider starts created within the same wall-clock second',
    async () => {
      const first = spawn(process.execPath, ['-e', 'setTimeout(()=>{},5000)']);
      const second = spawn(process.execPath, ['-e', 'setTimeout(()=>{},5000)']);
      await Promise.all([
        new Promise<void>((resolve, reject) => {
          first.once('spawn', resolve);
          first.once('error', reject);
        }),
        new Promise<void>((resolve, reject) => {
          second.once('spawn', resolve);
          second.once('error', reject);
        }),
      ]);
      try {
        const [firstIdentity, secondIdentity] = await Promise.all([
          getProcessStartIdentity(first.pid!),
          getProcessStartIdentity(second.pid!),
        ]);
  ...
  it.runIf(process.platform === 'win32').each(['cmd', 'bat'])(
    'round-trips native .%s arguments through a real batch shim',
    async (extension) => {
  ...
  it.runIf(process.platform === 'win32')(
    'keeps a supervisor root alive until an early-exit provider tree is terminated',
    async () => {
      ...
      const invocation = await materializeProviderSpawnInvocation(
        buildProviderSpawnInvocation([providerPath], 'win32', {
          ComSpec: process.env.ComSpec ?? process.env.COMSPEC ?? 'cmd.exe',
        }),
        { superviseWindowsTree: true },
      );
      const supervisor = spawn(invocation.command, invocation.args, {
        stdio: 'ignore',
        windowsHide: true,
      });
      await expect
      ...
      await expect
        .poll(() => isProcessAlive(childPid), { timeout: 2_000, interval: 20 })
        .toBe(false);
      await invocation.cleanup();
    },
  );
  ...
  it('excludes ambient secret environment values and rejects Windows aliases', async () => {
    ...
    try {
      expect(() =>
        buildWorkerLaunchBootstrapSpec(launchAttempt, ['codex'], cwd, {
          providerEnv: { OMC_WORKER_LAUNCH_SPEC_FILE: 'x' },
        }),
      ).toThrow('worker_launch_provider_env_reserved');
      expect(() =>
        buildWorkerLaunchBootstrapSpec(launchAttempt, ['codex'], cwd, {
          providerEnv: { PATH: 'one', Path: 'two' },
        }),
      ).toThrow('worker_launch_provider_env_key_alias_conflict');
      expect(() =>
        buildWorkerLaunchBootstrapSpec(launchAttempt, ['codex'], cwd, {
          providerEnv: { SystemRoot: 'D:\\attacker' },
        }),
      ).toThrow('worker_launch_provider_env_reserved');
      expect(() =>
        buildWorkerLaunchBootstrapSpec(launchAttempt, ['codex'], cwd, {
  ...
  it('quotes exact Windows CreateProcess arguments', () => {
    ...
    expect(quoteWindowsCreateProcessArgument('C:\\path with space\\')).toBe(
      '"C:\\path with space\\\\"',
    );
    expect(quoteWindowsCreateProcessArgument('say "hello"')).toBe(
      '"say \\"hello\\""',
    );
    expect(() => quoteWindowsCreateProcessArgument('bad\r\narg')).toThrow(
      'worker_launch_provider_argv_invalid',
    );
  });
  ...
  it('rejects substituted authority fields and descriptor symlinks', async () => {
    ...
    const descriptor = JSON.parse(
      await readFile(materialized.bootstrapDescriptorPath, 'utf8'),
    ) as Record<string, unknown>;
    ...
    ).rejects.toThrow('worker_launch_descriptor_invalid');
    await rm(materialized.bootstrapDescriptorPath, { force: true });
    await symlink(
      materialized.wrapperPath,
      materialized.bootstrapDescriptorPath,
    );
    await expect(
      readAndConsumeWorkerLaunchDescriptor(
