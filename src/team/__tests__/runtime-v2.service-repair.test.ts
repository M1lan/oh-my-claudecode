...
const cadenceMocks = vi.hoisted(() => ({
  installCommitCadence: vi.fn(
    async (_context?: {
      workerName?: string;
    }): Promise<{ method: 'hook' | 'fallback-poll' }> => ({ method: 'hook' }),
  ),
...
type LifecycleRead = {
  config: { lifecycle_state?: TeamLifecycleState };
  ...
  const workers: WorkerInfo[] = [
    {
      ...
      worker_cli: 'codex',
      assigned_tasks: [],
      worktree_path: '/repo/.omc/team/demo/worktrees/worker-1',
      launch_descriptor: launch('codex'),
    },
    {
      name: 'worker-2',
      index: 2,
      ...
      worker_cli: 'gemini',
      assigned_tasks: [],
      worktree_path: '/repo/.omc/team/demo/worktrees/worker-2',
      launch_descriptor: launch('gemini'),
    },
  ];
  return {
    name: 'demo',
...
describe('runtime-v2 committed service reconciliation', () => {
  ...
  it('reconstructs every committed worker using persisted providers and service identity', async () => {
    await expect(
      reconcileCommittedTeamServices(config(), '/repo'),
    ).resolves.toBe('synced');
    expect(mergeMocks.startMergeOrchestrator).toHaveBeenCalledWith(
      expect.objectContaining({
        serviceGeneration: 3,
        serviceAttemptId: '3:owner',
        leaderBranch: 'main',
        repoRoot: '/repo',
      }),
    );
    expect(
      mergeMocks.registerWorker.mock.calls.map(([worker]) => worker),
  ...
  it('does not duplicate exact-generation worker services on repeated reconciliation', async () => {
    ...
    await expect(
      reconcileCommittedTeamServices(enabled, '/repo'),
    ).resolves.toBe('synced');
    await expect(
      reconcileCommittedTeamServices(enabled, '/repo'),
    ).resolves.toBe('synced');
  ...
  it('installs newly authoritative same-generation workers after reconciling the prior exact set', async () => {
    const oneWorker = config({
      name: 'demo-same-generation-expand',
      worker_count: 1,
    ...
    await expect(
      reconcileCommittedTeamServices(oneWorker, '/repo'),
    ).resolves.toBe('synced');
    await expect(
      reconcileCommittedTeamServices(expanded, '/repo'),
    ).resolves.toBe('synced');
    await expect(
      reconcileCommittedTeamServices(expanded, '/repo'),
    ).resolves.toBe('synced');
    ...
    expect(
      cadenceMocks.installCommitCadence.mock.calls.map(
  ...
  it('retains an incomplete expansion for retry after cadence installation fails', async () => {
    const oneWorker = config({
      name: 'demo-same-generation-install-retry',
      worker_count: 1,
      workers: [config().workers[0]!],
    });
    ...
    cadenceMocks.installCommitCadence.mockRejectedValueOnce(
      new Error('cadence settings unavailable'),
    );
    await expect(
      reconcileCommittedTeamServices(expanded, '/repo'),
    ).resolves.toBe('repair_required');
  ...
  it('removes stale same-generation services after scale-down and retries retained teardown', async () => {
    ...
    cadenceMocks.installCommitCadence.mockImplementation(
      async (context?: { workerName?: string }) =>
        context?.workerName === 'worker-2'
          ? { method: 'fallback-poll' as const }
    ...
    const scaledDown = config({
      name: 'demo-scale-down-convergence',
      worker_count: 1,
      workers: [enabled.workers[0]!],
    });
    cadenceMocks.uninstallCommitCadence.mockRejectedValueOnce(
      new Error('cadence hook busy'),
    );
    await expect(
      reconcileCommittedTeamServices(scaledDown, '/repo'),
    ).resolves.toBe('repair_required');
  ...
  it('does not repair services while a durable non-committed scale-up fence is active', async () => {
    ...
    scaling.active_scale_up = {
      operation_id: 'scale-up-1',
      phase: 'effects',
      pid: 1234,
      process_started_at: 'linux:123',
      state_revision: 2,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
    await expect(
      reconcileCommittedTeamServices(scaling, '/repo'),
    ).resolves.toBe('repair_required');
    expect(mergeMocks.startMergeOrchestrator).not.toHaveBeenCalled();
    expect(cadenceMocks.installCommitCadence).not.toHaveBeenCalled();
  });
  ...
  it('repairs services while only a committed scale-up fence remains', async () => {
    ...
    scaling.active_scale_up = {
      ...
      pid: 1234,
      process_started_at: 'linux:123',
      state_revision: scaling.state_revision ?? 1,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
    await expect(
  ...
  it('reports repair_required when any committed worker metadata is incomplete', async () => {
    ...
    await expect(reconcileCommittedTeamServices(broken, '/repo')).resolves.toBe(
      'repair_required',
    );
    expect(mergeMocks.startMergeOrchestrator).not.toHaveBeenCalled();
  });
  it('drains stale enabled local services before reporting disabled state synced', async () => {
    ...
    const disabled = config({
      name: 'demo-disable-transition',
      service_descriptor: {
        schema_version: 1,
        service_generation: 4,
        service_attempt_id: '4:owner',
        auto_merge_enabled: false,
        workspace_root: '/repo',
        cadence_policy: 'disabled',
      },
    ...
    await expect(
      reconcileCommittedTeamServices(disabled, '/repo'),
    ).resolves.toBe('synced');
  ...
  it('aborts service side effects when authoritative lifecycle is no longer active', async () => {
    ...
    monitorMocks.readRevisionedTeamConfig
      ...
      .mockResolvedValue({
        config: {
          lifecycle_state: 'shutting_down' satisfies TeamLifecycleState,
        },
        stateRevision: 2,
      });

    const enabled = config({
      name: 'demo-lifecycle-race',
  ...
  it('aborts cadence install when lifecycle flips after orchestrator already exists', async () => {
    ...
    await expect(
      reconcileCommittedTeamServices(
        config({ name: 'demo-lifecycle-mid' }),
        '/repo',
      ),
    ).resolves.toBe('synced');
    expect(mergeMocks.startMergeOrchestrator).toHaveBeenCalledTimes(1);
    ...
    const expanded = config({
      ...
      workers: [
        ...config().workers,
        {
          ...
          worker_cli: 'codex',
          assigned_tasks: [],
          worktree_path: '/repo/.omc/team/demo/worktrees/worker-3',
          launch_descriptor: launch('codex'),
        },
      ],
      service_descriptor: {
    ...
    const cadenceCallsBefore =
      cadenceMocks.installCommitCadence.mock.calls.length;
    await expect(
      reconcileCommittedTeamServices(expanded, '/repo'),
    ).resolves.toBe('repair_required');
    // No additional cadence installs after lifecycle flipped
    expect(cadenceMocks.installCommitCadence.mock.calls.length).toBe(
      cadenceCallsBefore,
    );
  });
});
