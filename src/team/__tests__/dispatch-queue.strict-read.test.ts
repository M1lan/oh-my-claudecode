...
describe('readDispatchRequestStrict', () => {
  ...
  async function writeStore(value: unknown): Promise<string> {
    ...
    await mkdir(join(cwd, '.omc', 'state', 'team', teamName, 'dispatch'), {
      recursive: true,
    });
  ...
  it('distinguishes a missing store, malformed JSON, and non-array JSON', async () => {
    await expect(
      readDispatchRequestStrict(teamName, 'request-1', cwd),
    ).resolves.toEqual({ kind: 'store_missing' });
  ...
  it('does not use compatibility team rewriting or status defaulting', async () => {
    ...
    await expect(
      readDispatchRequest(teamName, 'request-1', cwd),
  ...
  it('rejects duplicate target IDs, unrelated duplicate IDs, and missing targets', async () => {
    await writeStore([
      request(),
      request({ updated_at: '2026-07-13T00:01:00.000Z' }),
    ]);
    await expect(
      readDispatchRequestStrict(teamName, 'request-1', cwd),
    ).resolves.toEqual({
    ...
    await expect(
      readDispatchRequestStrict(teamName, 'missing-request', cwd),
    ).resolves.toEqual({ kind: 'request_missing' });
  });
  ...
  it('patches only one current pending request and preserves delivery markers', async () => {
    ...
    expect(stored[0]).toMatchObject({
      status: 'pending',
      last_reason: 'mailbox_target_foreign',
    });
    ...
    await writeStore([
      request({ status: 'notified', notified_at: '2026-07-13T00:02:00.000Z' }),
    ]);
    await expect(
      patchPendingDispatchReason(teamName, 'request-1', 'ignored', cwd),
    ).resolves.toMatchObject({
      kind: 'not_pending',
  ...
  it('does not mutate ambiguous or invalid stores just to record a reason', async () => {
    ...
    await expect(
      patchPendingDispatchReason(teamName, 'request-1', 'must-not-write', cwd),
    ).resolves.toMatchObject({
      kind: 'unsafe',
      read: { kind: 'malformed_row', field: 'attempt_count' },
    });
    ...
    await expect(
      ...
      kind: 'unsafe',
      read: { kind: 'duplicate_request_id' },
    });
