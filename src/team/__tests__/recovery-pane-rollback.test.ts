...
const paneMocks = vi.hoisted(() => ({
  ...
  splitTeamWorkerPaneWithEvidence: vi.fn(async () => ({
    ...
    rawOutput: '%2\n',
    stderr: '',
    paneId: '%2' as string | null,
  })),
  spawnWorkerInPane: vi.fn(
    async (_sessionName: string, _paneId: string, _config: unknown) => {
      throw new Error('spawn failed --api-key SUPERSECRET after pane creation');
    },
  ),
  spawnOwnedWorkerInPane: vi.fn(),
  killTeamPane: vi.fn(async (_paneId: string) => {
...
vi.mock('../model-contract.js', async (importOriginal) => ({
  ...
  resolveValidatedBinaryPath: vi.fn((agentType?: string) =>
    agentType === 'gemini' ? process.execPath : '/bin/echo',
  ),
  buildWorkerArgv: vi.fn(() => ['/bin/echo']),
  getWorkerEnv: vi.fn(() => ({})),
}));

paneMocks.spawnOwnedWorkerInPane.mockImplementation(
  async (
    sessionName: string,
...
paneMocks.killOwnedWorkerPane.mockImplementation(
  async (ownership: { paneId: string }) => {
...
async function expectRecoveryLockReleased(
  teamName: string,
  workerName: string,
  suffix: string,
): Promise<void> {
  ...
  const followup = await Promise.race([
    ...
    new Promise<never>((_, reject) => {
      timeout = setTimeout(
        () =>
          reject(
            new Error('recovery lock remained held after terminal failure'),
          ),
        2_000,
      );
    }),
  ...
  const config = JSON.parse(
    readFileSync(absPath(cwd, TeamPaths.config(teamName)), 'utf8'),
  );
  expect(config.active_recovery?.recovery_id).not.toBe(followupRecoveryId);
}

async function attachPersistedPriorLaunch(
  teamName: string,
  configPath: string,
): Promise<void> {
  ...
  await expect(
    awaitWorkerLaunchAcknowledgement(attempt, {
      timeoutMs: 1_000,
      pollIntervalMs: 5,
    }),
  ).resolves.toEqual({ ok: true });
  writeFileSync(
    `${attempt.startedPath}.terminal`,
    JSON.stringify({
      ...expected,
...
describe('recovery pane rollback evidence', () => {
  it('retains the attempt and publishes durable evidence when pane cleanup cannot be verified', async () => {
    ...
    const evidenceRoot = absPath(
      cwd,
      `.omc/state/team/${teamName}/recovery/rollback-failures/${recoveryId}`,
    );
    ...
    expect(evidence).toMatchObject({
      ...
      pane_id: '%2',
      reason: 'spawn failed --api-key=<redacted> after pane creation',
      liveness: 'alive',
    });
  ...
  it('cleans a pre-upgrade dead pane (descriptor+pane, no launch_attempt_id) and continues recovery', async () => {
    ...
    const persisted = JSON.parse(readFileSync(configPath, 'utf8'));
    expect(persisted.active_recovery).toMatchObject({
      recovery_id: recoveryId,
      worker_name: 'worker-1',
    });
  });
  ...
  it('kills a live pre-upgrade owned pane before replacement when launch_attempt_id is absent', async () => {
    ...
    expect(paneMocks.adoptWorkerPaneOwnership).toHaveBeenCalledWith(
    ...
    expect(paneMocks.killOwnedWorkerPane).toHaveBeenCalledWith(
      expect.objectContaining({ paneId: '%1' }),
    );
    expect(paneMocks.splitTeamWorkerPaneWithEvidence).toHaveBeenCalled();
  });
  ...
  it('fail-closes legacy recovery when pane liveness is unknown', async () => {
    ...
    await expect(
      ...
      outcome: 'failed',
      error: 'worker_cleanup_incomplete',
      recoveryId,
    });
    expect(paneMocks.splitTeamWorkerPaneWithEvidence).not.toHaveBeenCalled();
  ...
  it('terminates the dead-pane provider before allocating a replacement pane', async () => {
    ...
    const oldAttempt = await prepareWorkerLaunchAttempt({
      cwd,
      teamName,
      workerName: 'worker-1',
      paneId: '%1',
    ...
    await expect(
      awaitWorkerLaunchAcknowledgement(oldAttempt, {
        timeoutMs: 2_000,
        pollIntervalMs: 5,
      }),
    ).resolves.toEqual({ ok: true });
    await expect(
      awaitWorkerLaunchProviderStarted(oldAttempt, {
        timeoutMs: 10_000,
        pollIntervalMs: 5,
      }),
    ).resolves.toBe(true);
    const oldPid = JSON.parse(readFileSync(oldAttempt.startedPath, 'utf8'))
      .pid as number;
    const configPath = absPath(cwd, TeamPaths.config(teamName));
    mkdirSync(join(configPath, '..'), { recursive: true });
    writeFileSync(
        ...
        workers: [
          {
            ...
            index: 1,
            ...launchMetadata,
            launch_attempt_id: oldAttempt.attempt_id,
            pane_id: '%1',
            replacement_generation: 1,
            working_dir: cwd,
          },
    ...
    paneMocks.splitTeamWorkerPaneWithEvidence.mockImplementationOnce(
      async () => {
  ...
  it('completes an idle Gemini recovery without requiring fabricated progress evidence', async () => {
        ...
        workers: [
          {
            ...
            launch_descriptor: {
              ...
              args: [
                '-e',
                `const fs=require('node:fs');fs.writeFileSync(${JSON.stringify(providerObservedPath)},fs.readFileSync(${JSON.stringify(inboxPath)},'utf8'));setTimeout(()=>process.exit(0),5000)`,
              ],
            },
            pane_id: '%1',
            replacement_generation: 1,
    ...
    paneMocks.spawnOwnedWorkerInPane.mockImplementationOnce(
      async (
        _sessionName: string,
        ownership: { paneId: string },
        config: {
        ...
        bootstrapResult = runWorkerActivationGate({
          ...gateSpec,
    ...
    await expect(bootstrapResult!).resolves.toEqual({
      outcome: 'ran',
      exitCode: 0,
      signal: null,
    });
    ...
    expect(readFileSync(providerObservedPath, 'utf8')).toContain(
      'Recovery completed for this idle worker.',
    );
    expect(readFileSync(providerObservedPath, 'utf8')).not.toContain(
      'STALE PRE-RECOVERY INBOX',
    );
    ...
    const followup = await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        lockTimeout = setTimeout(
          () => reject(new Error('recovery lock remained held')),
          2_000,
        );
      }),
    ]);
  ...
  it('publishes durable orphan evidence when split succeeds without a parseable pane id', async () => {
    ...
    expect(evidence).toMatchObject({
      ...
      recovery_id: recoveryId,
      pane_id: null,
      reason: 'pane_identity_pane_id_missing',
      liveness: 'unknown',
      unaddressable: true,
      split: {
        commandSucceeded: true,
    ...
    await expectRecoveryLockReleased(
      teamName,
      'worker-1',
      'unaddressable-split',
    );
  ...
  it('persists failed split stdout and stderr as durable recovery orphan evidence', async () => {
    ...
    expect(evidence).toMatchObject({
      reason: 'pane_identity_split_failed',
      unaddressable: true,
      split: {
        commandSucceeded: false,
        provider: 'tmux',
  ...
  it('records a valid recovery split that loses provider-target membership before activation', async () => {
    ...
    await expect(
      ...
      outcome: 'failed',
      error: 'worker_activation_failed',
      recoveryId,
    });
    expect(paneMocks.spawnOwnedWorkerInPane).not.toHaveBeenCalled();
    ...
    expect(evidence).toMatchObject({
      reason: 'pane_membership_unverified',
      unaddressable: true,
  ...
  it('rejects a persisted provider descriptor whose validated path has changed', async () => {
        ...
        workers: [
          {
            ...
            ...launchMetadata,
            launch_descriptor: {
              ...launchMetadata.launch_descriptor,
              binary: '/tmp/path-shadow/claude',
            },
            pane_id: '%1',
            replacement_generation: 1,
            working_dir: cwd,
    ...
    await expect(
      ...
      outcome: 'failed',
      error: 'launch_descriptor_unresolvable',
      recoveryId,
    });
    expect(paneMocks.splitTeamWorkerPaneWithEvidence).not.toHaveBeenCalled();
    expect(paneMocks.spawnOwnedWorkerInPane).not.toHaveBeenCalled();
  ...
  it('never launches or kills when recovery split aliases the leader pane', async () => {
    ...
    expect(evidence).toMatchObject({
      reason: 'pane_identity_leader_alias',
      pane_id: null,
      unaddressable: true,
    });
  });
