...
const pending = (phase: 'reserved' | 'active') => ({
  ...
  phase,
  continuation: 'reserved' as const,
  adoption: 'pending' as const,
  services: 'pending' as const,
  manifest: 'repair_required' as const,
  updated_at: new Date().toISOString(),
});
const successResult = (requestId: string, recoveryId: string) => ({
  ...
  manifestSync: 'synced' as const,
  servicesSync: 'synced' as const,
  warnings: [],
  requestId,
  recoveryId,
  teamName: requestId === 'request-a' ? 'team-a' : 'deleted-team',
  workerName: 'worker-a',
  updatedAt: new Date().toISOString(),
});
const failureResult = (requestId: string, recoveryId: string) => ({
  outcome: 'failed' as const,
  committed: false as const,
  error: 'worker_not_found' as const,
  requestId,
  recoveryId,
...
const corruptNewestPhase = (from: string, to: string) => {
  ...
  const directory = join(
    absPath(cwd, TeamPaths.recoveryRequestPending('request-a')),
    '..',
    'phases',
    'request-a',
  );
...
describe('global recovery request store', () => {
  it('joins a repeated request with the same canonical payload and rejects a reused ID before a new recovery is reserved', () => {
    const first = reserveRecoveryRequest(
      cwd,
      'request-a',
      payload,
      'recovery-a',
    ...
    expect(first).toMatchObject({
      kind: 'created',
      reservation: { recovery_id: 'recovery-a' },
    });
    expect(
      reserveRecoveryRequest(cwd, 'request-a', payload, 'recovery-b'),
    ).toMatchObject({
      kind: 'joined',
      reservation: { recovery_id: 'recovery-a' },
    });
    expect(
      reserveRecoveryRequest(
        cwd,
        'request-a',
        { ...payload, teamName: 'team-b' },
        'recovery-b',
      ),
    ).toMatchObject({ kind: 'conflict', reservation: { team_name: 'team-a' } });
  });

  it('publishes a deterministic alias to an active compatible recovery and refuses a hash/team mismatch', () => {
    const active = reserveRecoveryRequest(
      cwd,
      'request-a',
      payload,
      'recovery-a',
  ...
  it('resolves a disconnected alias to its canonical phase before a final is published', () => {
    ...
    expect(readRecoveryOutcome(cwd, 'request-b')).toEqual(
      readRecoveryOutcome(cwd, 'request-a'),
    );
  });
  ...
  it('resolves a disconnected alias to its canonical final after team state is deleted', () => {
    ...
    writeRecoveryFinal(cwd, {
      ...
      worker_name: 'worker-a',
      outcome: 'succeeded',
      result: successResult('request-a', 'recovery-a'),
      continuation: 'none',
  ...
  it('fails closed for self-inconsistent immutable reservation tuples without publishing a final', () => {
    ...
    for (const [name, from, to] of mutations) {
      ...
      expect(readRecoveryFinalState(cwd, requestId), name).toEqual({
        kind: 'missing',
      });
      rmSync(path);
    }
  ...
  it('rejects traversal request ids before any recovery path is derived', () => {
    ...
    expect(() =>
      reserveRecoveryRequest(cwd, 'request-safe', payload, '../recovery'),
    ).toThrow('invalid_recovery_request_id');
    expect(() => readRecoveryFinalState(cwd, '../owned')).toThrow(
  ...
  it('uses final outcome over phases and otherwise returns the newest durable phase', () => {
    ...
    writeRecoveryPhase(cwd, pending('reserved'));
    writeRecoveryPhase(cwd, pending('active'));
    expect(readRecoveryOutcome(cwd, 'request-a')).toMatchObject({
      kind: 'phase',
      phase: 'active',
    });
    writeRecoveryFinal(cwd, {
  ...
  it('rejects phase publication unless its tuple exactly matches the canonical reservation', () => {
    ...
    const conflicts = [
      {
        name: 'recovery',
        phase: { ...pending('active'), recovery_id: 'recovery-b' },
      },
      { name: 'team', phase: { ...pending('active'), team_name: 'team-b' } },
      {
        name: 'worker',
        phase: { ...pending('active'), worker_name: 'worker-b' },
      },
      {
        name: 'request path',
        phase: { ...pending('active'), request_id: 'request-b' },
      },
    ];
    for (const { name, phase } of conflicts) {
      expect(() => writeRecoveryPhase(cwd, phase), name).toThrow(
  ...
  it('repairs a missing workspace-scoped final index with exact immutable bytes', () => {
    ...
    writeRecoveryFinal(cwd, {
      schema_version: 1,
      kind: 'final',
      request_id: 'repair-index',
      recovery_id: 'recovery-index',
      team_name: 'deleted-team',
      worker_name: 'worker-a',
      outcome: 'failed',
      result: failureResult('repair-index', 'recovery-index'),
    ...
    const requestPath = absPath(
      cwd,
      TeamPaths.recoveryRequestResult('repair-index'),
    );
    const indexPath = absPath(
      cwd,
    ...
    expect(readRecoveryFinalState(cwd, 'repair-index')).toMatchObject({
      kind: 'valid',
    });
    ...
    writeFileSync(
      indexPath,
      JSON.stringify({ schema_version: 1, kind: 'final', request_id: 'other' }),
    );
    expect(readRecoveryFinalState(cwd, 'repair-index')).toMatchObject({
      kind: 'valid',
    });
  ...
  it('retains failed and succeeded final lookup independently of deleted team state', () => {
    ...
    writeRecoveryFinal(cwd, {
      ...
      worker_name: 'worker-a',
      outcome: 'failed',
      result: failureResult('failed', 'r1'),
      error: { code: 'worker_not_found', commit_uncertain: false },
      continuation: 'none',
      adoption: 'not_started',
      services: 'terminal_degraded',
    ...
    writeRecoveryFinal(cwd, {
      ...
      worker_name: 'worker-a',
      outcome: 'succeeded',
      result: successResult('succeeded', 'r2'),
      continuation: 'none',
      adoption: 'not_started',
      services: 'synced',
      manifest: 'synced',
    ...
    expect(readRecoveryOutcome(cwd, 'failed')).toMatchObject({
      outcome: 'failed',
      error: { code: 'worker_not_found' },
    });
    expect(readRecoveryResult(cwd, 'succeeded')).toMatchObject({
  ...
  it('rejects a durable final whose outer and embedded identity tuples disagree', () => {
    ...
    writeRawFinal('mismatch', {
      schema_version: 1,
      kind: 'final',
      request_id: 'mismatch',
      recovery_id: 'outer-recovery',
      team_name: 'deleted-team',
      worker_name: 'worker-a',
      outcome: 'succeeded',
      result: successResult('mismatch', 'embedded-recovery'),
      continuation: 'none',
      adoption: 'not_started',
      services: 'synced',
      manifest: 'synced',
  ...
  it('rejects malformed successful pane identities before immutable final publication', () => {
    for (const [requestId, result] of [
      [
        'blank-new-pane',
        { ...successResult('blank-new-pane', 'pane-r1'), newPaneId: '   ' },
      ],
      [
        'blank-old-pane',
        {
          ...successResult('blank-old-pane', 'pane-r2'),
      ...
      expect(readRecoveryFinalState(cwd, requestId)).toEqual({
        kind: 'missing',
      });
  ...
  it('rejects tuple-matching finals whose embedded and outer producer invariants conflict', () => {
    ...
    > = [
      ...
      [
        ...
        (_record, result) => {
          ...
          result.continuationSequenceByTask = { '2': 1 };
        },
      ],
      [
        'outer-adoption-mismatch',
        (record, result) => {
          result.requeuedTaskIds = ['1'];
    ...
    for (const [name, mutate] of cases) {
      ...
      const result: Record<string, unknown> = {
        ...successResult(requestId, recoveryId),
        outcome: 'recovered',
      };
