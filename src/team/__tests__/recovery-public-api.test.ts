...
describe('public dead-worker recovery facade', () => {
  it('classifies authoritative config independently of any manifest before dispatching recovery effects', async () => {
    ...
    try {
      await expect(
        recoverDeadWorkerV2('missing-team', cwd, {
          workerName: 'worker-1',
      ...
      await expect(
        ...
        ok: true,
        data: { result: { outcome: 'failed', error: 'runtime_v2_required' } },
      });

      const malformedConfigPath = join(
        cwd,
        '.omc',
        'state',
        'team',
      ...
      await expect(
        recoverDeadWorkerV2('malformed-team', cwd, {
          workerName: 'worker-1',
      ...
      const malformedRevisionPath = join(
        cwd,
        '.omc',
        'state',
        'team',
        'malformed-revision-team',
        'config.json',
      );
      mkdirSync(join(malformedRevisionPath, '..'), { recursive: true });
      writeFileSync(
        malformedRevisionPath,
        JSON.stringify({
          name: 'malformed-revision-team',
          state_revision: 'one',
        }),
      );
      await expect(
        recoverDeadWorkerV2('malformed-revision-team', cwd, {
          workerName: 'worker-1',
          requestId: 'malformed-revision-request',
          timeoutMs: 180_000,
        }),
      ).resolves.toMatchObject({
        outcome: 'failed',
      ...
      await expect(
        recoverDeadWorkerV2('manifest-only-team', cwd, {
          workerName: 'worker-1',
          requestId: 'manifest-only-request',
          timeoutMs: 180_000,
        }),
      ).resolves.toMatchObject({
        outcome: 'failed',
  ...
  it('preserves the exact package argument boundary and typed result', async () => {
    ...
    await expect(
      recoverDeadWorkerV2('recovery-team', '/workspace', {
        workerName: 'worker-1',
        requestId: 'request-a',
        timeoutMs: 180_000,
      }),
  ...
  it('returns the exact typed invalid_input result and matching API envelopes for an invalid timeout', async () => {
    ...
    ).resolves.toEqual(invalidEnvelope);
    await expect(
      executeSecondaryTeamApiOperation(
        'recover-worker',
        {
          teamName: 'recovery-team',
          workerName: 'worker-1',
  ...
  it('maps canonical snake_case CLI fields to the package facade and returns the canonical envelope', async () => {
    setRuntimeOwnerRecoveryClient({
      requestRuntimeOwnerRecovery: vi.fn(async () => recovered),
    });
  ...
  it('preserves the legacy unsupported-operation envelope outside the recovery operation', async () => {
    await expect(
      executeSecondaryTeamApiOperation('not-real', {}, '/workspace'),
    ).resolves.toEqual({
      ok: false,
      operation: 'not-real',
      error: {
        code: 'UNSUPPORTED_OPERATION',
        message: 'Unsupported omc team api operation: not-real',
      },
    });
  });
    ...
    try {
      ...
      await expect(
        executeTeamApiOperation(
          ...
          { team_name: 'recovery-team', request_id: 'request-a' },
          cwd,
        ),
      ).resolves.toMatchObject({
        ok: true,
        operation: 'read-recovery-result',
        data: { outcome: { kind: 'final', result: recovered } },
      });
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  ...
  it('requires team_name and rejects unsupported read-recovery-result fields', async () => {
    await expect(
      ...
      error: {
        code: 'invalid_input',
        message: 'team_name and request_id are required',
      },
    });
    await expect(
      executeTeamApiOperation(
      ...
      error: {
        code: 'invalid_input',
        message: 'read-recovery-result received unsupported fields: worker',
      },
    });
  });
  ...
  it('exports an async request-first terminal-result reader that preserves canonical durable pane identities', async () => {
    ...
    try {
      ...
      const rootReader: (
        ...
        readRootRecoverDeadWorkerV2Result;
      ...
      expect(readRecoverDeadWorkerV2Outcome(cwd, 'request-a')).toMatchObject({
        kind: 'final',
        result: recovered,
      });
    } finally {
  ...
  it('returns the actual live pane for an already-running durable success', async () => {
    ...
    try {
      ...
      writeRecoveryFinal(cwd, {
        ...
        services: 'synced',
        manifest: 'synced',
        completed_at: alreadyRunning.updatedAt,
        expires_at: '2099-01-01T00:00:00.000Z',
      });

      await expect(
        readRecoverDeadWorkerV2Result('request-a', cwd),
      ).resolves.toEqual(alreadyRunning);
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('does not authorize config cleanup or final publication for malformed owner success', async () => {
    const cwd = mkdtempSync(
      join(tmpdir(), 'recovery-public-malformed-owner-success-'),
    );
