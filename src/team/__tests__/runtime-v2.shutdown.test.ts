...
async function prepareAcceptedLaunch(
  cwd: string,
  teamName: string,
  workerName: string,
  paneId: string,
...
describe('shutdownTeamV2 detached worktree cleanup', () => {
  ...
  beforeEach(() => {
    ...
    tmuxMocks.resolveSplitPaneWorkerPaneIds.mockImplementation(
      async (_session: string | undefined, paneIds: string[]) => paneIds,
    );
    ...
    tmuxMocks.adoptWorkerPaneOwnership.mockImplementation(
      async (input: {
        provider: string;
        providerTarget: string;
        paneId: string;
  ...
  it('keeps worktrees and team state when a worker pane remains alive after shutdown kill', async () => {
    ...
    const launchAttempt = await prepareAcceptedLaunch(
      repoDir,
      teamName,
      'worker-live',
      '%42',
    ...
    const { shutdownTeamV2 } = await import('../runtime-v2.js');
    await expect(
      shutdownTeamV2(teamName, repoDir, { timeoutMs: 0 }),
    ).resolves.toEqual({
      outcome: 'preserved',
    ...
    expect(
      existsSync(`${launchAttempt.decisionPath}.retired.cleanup-complete`),
    ).toBe(false);
  });
  ...
  it('keeps worktrees and team state when tmux cleanup fails before liveness is proven', async () => {
    ...
    await expect(
      ...
      detail: 'tmux cleanup unverified',
    });

    expect(tmuxMocks.killTeamSession).toHaveBeenCalledWith(
      `${teamName}:0`,
      [],
      '%1',
  ...
  it.each([false, true])(
    'blocks %s force shutdown before effects while recovery is active',
    async (force) => {
      const teamName = force
        ? 'shutdown-active-force'
        : 'shutdown-active-normal';
      const teamRoot = join(repoDir, '.omc', 'state', 'team', teamName);
      mkdirSync(teamRoot, { recursive: true });
      ...
      const { shutdownTeamV2 } = await import('../runtime-v2.js');
      await expect(
        shutdownTeamV2(teamName, repoDir, { timeoutMs: 0, force }),
  ...
  it('does not commit shutdown lifecycle or kill panes when manifest projection fails', async () => {
    ...
    const { shutdownTeamV2 } = await import('../runtime-v2.js');
    await expect(
      shutdownTeamV2(teamName, repoDir, { timeoutMs: 0, force: true }),
  ...
  it('blocks shutdown before effects while a scale-down reservation is active', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        state_revision: 4,
        active_scale_down: {
          operation_id: 'scale-down-active',
          phase: 'draining',
          pid: process.pid,
          process_started_at: 'test-process-start',
          workers: [{ name: 'worker-2', pane_id: '%78' }],
          state_revision: 4,
          created_at: now,
          updated_at: now,
        },
  ...
  it('restores active lifecycle when a worker rejects normal shutdown before pane cleanup', async () => {
    ...
    writeFileSync(
      join(workerRoot, 'shutdown-ack.json'),
      JSON.stringify({
        status: 'reject',
    ...
    const { shutdownTeamV2 } = await import('../runtime-v2.js');
    await expect(
      shutdownTeamV2(teamName, repoDir, { timeoutMs: 25 }),
