...
const modelContractMocks = vi.hoisted(() => ({
  buildWorkerArgv: vi.fn((_agentType: string, _config: unknown) => [
    '/usr/bin/claude',
  ]),
  ...
  getPromptModeArgs: vi.fn((_agentType: string, instruction: string) => [
    instruction,
  ]),
...
vi.mock('../worker-launch-ack.js', async (importOriginal) => {
  ...
  return {
    ...
    retireAndCleanupCurrentWorkerLaunchAttempt:
      launchMocks.retireAndCleanupCurrentWorkerLaunchAttempt,
...
describe('runtime v2 startup inbox dispatch', () => {
  ...
  beforeEach(() => {
    ...
    mocks.resolveSplitPaneWorkerPaneIds.mockImplementation(
      async (_session: string | undefined, paneIds: string[]) => paneIds,
    );
    mocks.getWorkerLiveness.mockImplementation(async () =>
      mocks.killOwnedWorkerPane.mock.calls.length > 0 ? 'dead' : 'alive',
    );
    ...
    launchMocks.withWorkerLaunchAttemptFence.mockImplementation(
      async (_attempt: unknown, fn: () => Promise<unknown>) => ({
        ok: true as const,
        value: await fn(),
      }),
    ...
    launchMocks.retireAndCleanupCurrentWorkerLaunchAttempt.mockImplementation(
      async (
        _attempt: unknown,
        _reason: string,
        cleanup: () => Promise<boolean>,
    ...
    mocks.deliverStartupInbox.mockImplementation(
      async (
        context: {
          attempt: {
            attempt_id: string;
        ...
        const sent = await mocks.sendToWorker(
          '',
    ...
    modelContractMocks.buildWorkerArgv.mockImplementation(
      (agentType?: string) => [`/usr/bin/${agentType ?? 'claude'}`],
    );
    ...
    modelContractMocks.getPromptModeArgs.mockImplementation(
      (_agentType: string, instruction: string) => [instruction],
    );
    ...
    cadenceMocks.startFallbackPoller.mockImplementation(() => ({
      stop: vi.fn(),
    }));
    ...
    (mocks.execFile as unknown as Record<PropertyKey, unknown>)[
      promisify.custom
  ...
  it('writes durable inbox dispatch evidence when startup worker notification succeeds', async () => {
      ...
      tasks: [
        {
          ...
          description: 'Verify startup dispatch evidence',
        },
      ],
      cwd,
    });
    ...
    expect(mocks.createTeamSession).toHaveBeenCalledWith(
      'dispatch-team',
      0,
      cwd,
      { newWindow: false },
    ...
    expect(requests[0]?.inbox_correlation_key).toBe(
      'startup:worker-1:1:attempt-worker-1',
    );
    expect(requests[0]?.trigger_message).toContain(
      '.omc/state/team/dispatch-team/workers/worker-1/inbox.md',
    );
    ...
    expect(mocks.applyMainVerticalLayout).toHaveBeenCalledWith(
      'dispatch-session',
    );
    const config = JSON.parse(
      await readFile(
        join(cwd, '.omc', 'state', 'team', 'dispatch-team', 'config.json'),
        'utf-8',
      ),
    );
    const manifest = JSON.parse(
      await readFile(
        join(cwd, '.omc', 'state', 'team', 'dispatch-team', 'manifest.json'),
        'utf-8',
      ),
    );
    expect(config.workers[0].launch_descriptor).toMatchObject({
      provider: 'claude',
      binary: '/usr/bin/claude',
      args: [],
    });
    expect(manifest.workers[0].launch_descriptor).toEqual(
      config.workers[0].launch_descriptor,
    );
    expect(config.service_descriptor).toMatchObject({
      schema_version: 1,
  ...
  it('persists startup task delegation plans and gives executable result evidence instructions', async () => {
    ...
    const task = JSON.parse(await readFile(taskPath, 'utf-8')) as {
  ...
  it('preserves startup failure evidence when a worker launch throws after scaffolding', async () => {
    ...
    mocks.spawnWorkerInPane.mockRejectedValueOnce(
      new Error('claude launch exploded'),
    );
    const { startTeamV2 } = await import('../runtime-v2.js');
    ...
    await expect(
      startTeamV2({
        ...
        tasks: [
          {
            subject: 'Dispatch test',
            description: 'Verify startup failure evidence',
          },
        ],
        cwd,
      }),
  ...
  it('does not persist sensitive cmux worker command payloads in startup failure evidence', async () => {
    cwd = await mkdtemp(
      join(tmpdir(), 'omc-runtime-v2-redacted-startup-failure-'),
    );
    ...
    modelContractMocks.buildWorkerArgv.mockReturnValue([
      '/usr/bin/claude',
      '--api-key',
      secret,
    ]);
    ...
    await expect(
      startTeamV2({
        ...
        tasks: [
          {
            subject: 'Dispatch test',
            description: 'Verify redacted startup failure evidence',
          },
        ],
        cwd,
      }),
  ...
  it('does not persist sensitive primary cmux failure payloads in startup failure evidence', async () => {
    cwd = await mkdtemp(
      join(tmpdir(), 'omc-runtime-v2-redacted-primary-failure-'),
    );
    const secret = 'SECRET_TOKEN_SHOULD_NOT_LEAK';
    modelContractMocks.getWorkerEnv.mockImplementation(() => ({
      OMC_TEAM_WORKER: 'dispatch-team/worker-1',
    ...
    await expect(
      startTeamV2({
        ...
        tasks: [
          {
            subject: 'Dispatch test',
            description: 'Verify redacted primary failure evidence',
          },
        ],
        cwd,
      }),
  ...
  it('keeps dirty worktree preservation metadata when startup rollback records failure evidence', async () => {
    cwd = await mkdtemp(
      join(tmpdir(), 'omc-runtime-v2-dirty-startup-failure-'),
    );
    ...
    await writeFile(
      join(cwd, 'README.md'),
      'dirty startup failure test\n',
      'utf-8',
    );
    await writeFile(join(cwd, 'AGENTS.md'), 'root agents\n', 'utf-8');
    execFileSync('git', ['add', 'README.md', 'AGENTS.md'], {
      cwd,
      stdio: 'pipe',
    });
    execFileSync('git', ['commit', '-m', 'initial'], { cwd, stdio: 'pipe' });
    mocks.spawnWorkerInPane.mockImplementationOnce(
      async (_session, _pane, paneConfig: { cwd?: string }) => {
        await writeFile(
          join(paneConfig.cwd ?? cwd, 'dirty-startup.txt'),
          'preserve me\n',
          'utf-8',
        );
    ...
    await expect(
      startTeamV2({
        ...
        tasks: [
          {
            subject: 'Dispatch test',
            description: 'Verify dirty worktree preservation evidence',
          },
        ],
        cwd,
      }),
    ...
    expect(marker.error).toContain(
      'claude launch exploded after dirty worktree',
    );
    ...
    await expect(readFile(backupPath, 'utf-8')).resolves.toContain(
      'root agents',
    );
    await expect(
      readFile(join(worktreePath, 'dirty-startup.txt'), 'utf-8'),
  ...
  it('persists runtime-v2 worktree contract fields for split-pane teams', async () => {
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Worktree contract',
          description: 'Verify runtime-v2 worktree metadata',
        },
      ],
      cwd,
    });
    ...
    expect(requests[0]?.trigger_message).not.toContain(
      '$OMC_TEAM_STATE_ROOT/team/dispatch-team',
    );
    expect(runtime.config.team_state_root).toBeDefined();
    const teamStateRoot = runtime.config.team_state_root!;
    expect(
      requests[0]?.trigger_message.replace(
        '$OMC_TEAM_STATE_ROOT',
        teamStateRoot,
      ),
    ).toContain(
    ...
    expect(overlay).toContain(
      '$OMC_TEAM_STATE_ROOT/workers/worker-1/status.json',
    );
  ...
  it('fails loudly when explicit auto-merge worker registration fails', async () => {
    ...
    await writeFile(
      join(cwd, 'README.md'),
      'auto merge fail loud test\n',
      'utf-8',
    );
    execFileSync('git', ['add', 'README.md'], { cwd, stdio: 'pipe' });
    execFileSync('git', ['commit', '-m', 'initial'], { cwd, stdio: 'pipe' });
    execFileSync('git', ['checkout', '-b', 'feature-auto-merge'], {
      cwd,
      stdio: 'pipe',
    });
    mergeMocks.registerWorker.mockRejectedValueOnce(
    ...
    await expect(
      startTeamV2({
        ...
        tasks: [
          {
            subject: 'Auto merge fail',
            description: 'Registration failure must abort startup',
          },
        ],
        cwd,
        autoMerge: true,
    ...
    expect(cadenceMocks.installCommitCadence).toHaveBeenCalledWith(
      expect.objectContaining({
        teamName: 'dispatch-team',
        workerName: 'worker-1',
        agentType: 'claude',
    ...
    expect(cadenceMocks.uninstallCommitCadence).toHaveBeenCalledWith(
      expect.objectContaining({
        workerName: 'worker-1',
      }),
    );
  ...
  it('wires auto-merge worker cadence and drains before unregistering on shutdown', async () => {
    ...
    cadenceMocks.installCommitCadence.mockResolvedValue({
      method: 'fallback-poll',
    });
    ...
    await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Auto merge cadence',
          description: 'Install fallback cadence and drain at shutdown',
        },
      ],
      cwd,
      autoMerge: true,
    ...
    if (mergeMocks.drainAndStop.mock.calls.length > 0) {
      expect(mergeMocks.drainAndStop.mock.invocationCallOrder[0]).toBeLessThan(
        mergeMocks.unregisterWorker.mock.invocationCallOrder[0] ?? Infinity,
      );
    }
  });
  ...
  it('drains auto-merge before preserving state for live worker panes on shutdown', async () => {
    ...
    await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Auto merge cadence',
          description: 'Drain before live-pane preserve',
        },
      ],
      cwd,
      autoMerge: true,
  ...
  it('kills the started team session and rolls back worktrees when manifest persistence fails', async () => {
    cwd = await mkdtemp(
      join(tmpdir(), 'omc-runtime-v2-post-session-rollback-'),
    );
    execFileSync('git', ['init'], { cwd, stdio: 'pipe' });
    execFileSync('git', ['config', 'user.email', 'test@example.com'], {
      cwd,
    ...
    await expect(
      startTeamV2({
        ...
        tasks: [
          {
            subject: 'Worktree rollback',
            description: 'Fail after tmux session starts',
          },
        ],
        cwd,
        newWindow: true,
    ...
    await expect(
      readFile(
        join(cwd, '.omc', 'state', 'team', 'dispatch-team', 'worktrees.json'),
        'utf-8',
      ),
    ).rejects.toMatchObject({ code: 'ENOENT' });
    await expect(
  ...
  it('rolls back clean native worktrees when startup fails before config is persisted', async () => {
    ...
    await writeFile(
      join(cwd, 'README.md'),
      'worktree rollback test\n',
      'utf-8',
    );
    execFileSync('git', ['add', 'README.md'], { cwd, stdio: 'pipe' });
    execFileSync('git', ['commit', '-m', 'initial'], { cwd, stdio: 'pipe' });
    mocks.createTeamSession.mockRejectedValueOnce(
      new Error('tmux_start_failed'),
    );
    ...
    await expect(
      startTeamV2({
        ...
        tasks: [
          {
            subject: 'Worktree rollback',
            description: 'Fail before config persists',
          },
        ],
        cwd,
      }),
  ...
  it('uses owner-aware startup allocation when task owners are provided', async () => {
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Owner-routed task',
          description: 'Should start on worker-2',
          owner: 'worker-2',
        },
    ...
    expect(runtime.config.workers.map((worker) => worker.name)).toEqual([
      'worker-1',
      'worker-2',
    ]);
    ...
    expect(requests.map((request) => request.to_worker)).toEqual([
      'worker-2',
      'worker-1',
    ]);
    ...
    expect(spawnedWorkers).toEqual([
      'dispatch-team/worker-2',
      'dispatch-team/worker-1',
    ]);
  });
  ...
  it('preserves explicit worker roles in runtime config during startup fanout', async () => {
    ...
    const persisted = JSON.parse(await readFile(configPath, 'utf-8'));
    expect(
      persisted.workers.map((worker: { role: string }) => worker.role),
    ).toEqual(['architect', 'writer']);
  });
  ...
  it('routes inferred review work through alias-keyed resolved snapshot entries', async () => {
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Review component naming',
          description: 'code review pass for PR',
        },
      ],
      cwd,
    });
    ...
    expect(
      runtime.config.resolved_routing?.['code-reviewer']?.primary.provider,
    ).toBe('gemini');
  ...
  it('passes through dedicated-window startup requests', async () => {
    ...
    await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Dispatch test',
          description: 'Verify new-window startup wiring',
        },
      ],
      cwd,
      newWindow: true,
  ...
  it('fails closed when split aliases the leader pane before any worker launch or inbox delivery', async () => {
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Dispatch test',
          description: 'Never alias the leader pane',
        },
      ],
      cwd,
    });
    ...
    await expect(
      startTeamV2({
        ...
        tasks: [
          {
            subject: 'Dispatch test',
            description: 'Reject foreign split pane',
          },
        ],
        cwd,
      }),
  ...
  it('aborts startup without persisting a live worker when launch acknowledgement fails', async () => {
    ...
    mocks.spawnWorkerInPane.mockRejectedValueOnce(
      new Error('worker_start_ack_ack_timeout:worker-1:%2:attempt'),
    );
    const { startTeamV2 } = await import('../runtime-v2.js');
    ...
    await expect(
      startTeamV2({
        ...
        tasks: [
          {
            subject: 'Dispatch test',
            description: 'Verify start command delivery failure aborts startup',
          },
        ],
        cwd,
      }),
  ...
  it('retires the exact provider when layout fails after launch handoff', async () => {
    ...
    mocks.applyMainVerticalLayout.mockRejectedValueOnce(
    ...
    await expect(
      startTeamV2({
        ...
        agentTypes: ['claude'],
        tasks: [
          { subject: 'Dispatch test', description: 'Layout failure cleanup' },
        ],
        cwd,
      }),
  ...
  it('tears down the owned worker launch when startup readiness fails', async () => {
    ...
    mocks.deliverStartupInbox.mockResolvedValueOnce({
      ok: false,
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Dispatch test',
          description: 'Verify worker pane is preserved for leader cleanup',
        },
      ],
      cwd,
    });
    ...
    expect(runtime.config.workers[0]?.assigned_tasks).toEqual([]);
    expect(
      mocks.execFile.mock.calls.some((call) => call[1]?.[0] === 'kill-pane'),
    ).toBe(false);
    expect(mocks.killOwnedWorkerPane).toHaveBeenCalledWith(
      expect.objectContaining({ paneId: '%2' }),
    );
  });
  ...
  it('tears down the owned worker launch when startup notification fails', async () => {
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Dispatch test',
          description: 'Verify notify failure leaves pane for leader action',
        },
      ],
      cwd,
    });
    ...
    expect(requests[0]?.last_reason).toBe(
      'worker_notify_failed:startup_send_failed',
    );
  ...
  it('fails closed when exact provider process cleanup cannot be verified', async () => {
    cwd = await mkdtemp(
      join(tmpdir(), 'omc-runtime-v2-provider-cleanup-unverified-'),
    );
    mocks.sendToWorker.mockResolvedValue(false);
    launchMocks.retireAndCleanupCurrentWorkerLaunchAttempt.mockResolvedValueOnce(
      false,
    );
    mocks.killOwnedWorkerPane.mockClear();
    const { startTeamV2 } = await import('../runtime-v2.js');
    ...
    await expect(
      startTeamV2({
        ...
        tasks: [
          {
            subject: 'Dispatch test',
            description: 'Reject unverified provider cleanup',
          },
        ],
        cwd,
      }),
  ...
  it('requires Claude startup evidence without resending the startup inbox', async () => {
    cwd = await mkdtemp(
      join(tmpdir(), 'omc-runtime-v2-claude-evidence-missing-'),
    );
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Dispatch test',
          description: 'Verify Claude startup evidence gate',
        },
      ],
      cwd,
    });
  ...
  it('rejects a stale worker status that predates the current startup trigger', async () => {
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Dispatch test',
          description: 'Reject stale status evidence',
        },
      ],
      cwd,
    });
  ...
  it('rejects a stale task claim that predates the current startup trigger', async () => {
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Dispatch test',
          description: 'Reject stale claim evidence',
        },
      ],
      cwd,
    });
  ...
  it('does not treat ACK-only mailbox replies as Claude startup evidence or resend the startup inbox', async () => {
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Dispatch test',
          description: 'Verify Claude mailbox ack evidence',
        },
      ],
      cwd,
    });
  ...
  it.each(['claim', 'status'] as const)(
    'rejects fresh wrong-attempt %s evidence in isolation',
    async (evidenceKind) => {
      cwd = await mkdtemp(
        join(tmpdir(), `omc-runtime-v2-wrong-attempt-${evidenceKind}-`),
      );
      mocks.autoStartupEvidence = false;
      ...
      const runtime = await startTeamV2({
        ...
        tasks: [
          {
            subject: 'Dispatch test',
            description: 'Reject wrong-attempt evidence',
          },
        ],
        cwd,
      });
  ...
  it('accepts Claude startup once the worker claims the task', async () => {
    cwd = await mkdtemp(
      join(tmpdir(), 'omc-runtime-v2-claude-evidence-claim-'),
    );
    mocks.autoStartupEvidence = false;
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Dispatch test',
          description: 'Verify Claude claim evidence',
        },
      ],
      cwd,
    });
  ...
  it('accepts Claude startup once worker status shows task progress', async () => {
    cwd = await mkdtemp(
      join(tmpdir(), 'omc-runtime-v2-claude-evidence-status-'),
    );
    mocks.autoStartupEvidence = false;
    ...
    const runtime = await startTeamV2({
      ...
      tasks: [
        {
          subject: 'Dispatch test',
          description: 'Verify Claude status evidence',
        },
      ],
      cwd,
    });
  ...
  it('direct grok launch resolves model from grok env vars and never calls resolveClaudeWorkerModel', async () => {
    ...
    try {
      ...
      await startTeamV2({
        ...
        tasks: [
          {
            subject: 'Grok dispatch',
            description: 'Verify direct grok model resolution',
          },
        ],
        cwd,
      });
      ...
      expect(
        modelContractMocks.resolveClaudeWorkerModel,
    ...
    try {
      ...
      await startTeamV2({
        ...
        tasks: [
          {
            subject: 'Grok dispatch',
            description: 'Verify grok env model passthrough',
          },
        ],
        cwd,
      });
  ...
  it('keeps gemini prompt-mode launch args to a short inbox pointer and waits for claim evidence', async () => {
    ...
    modelContractMocks.isPromptModeAgent.mockImplementation(
      (agentType?: string) => agentType === 'gemini',
    );
    mocks.spawnWorkerInPane.mockImplementation(
      async (
        _sessionName: string,
        _paneId: string,
      ...
      tasks: [
        {
          subject: 'Dispatch test',
          description:
            'Reviewer seed says the worker may be blocked; verify prompt echo stays quiet.',
        },
      ],
      cwd,
    });
    ...
    const promptModeInstruction =
      modelContractMocks.getPromptModeArgs.mock.calls[0]?.[1];
    expect(promptModeInstruction).toContain(
      'Open .omc/state/team/dispatch-team/workers/worker-1/inbox.md',
    );
    ...
    const launchArgs =
      mocks.spawnWorkerInPane.mock.calls[0]?.[2]?.launchArgs ?? [];
    expect(launchArgs.some((arg: string) => arg.includes('claim-task'))).toBe(
      false,
    );
    expect(
      launchArgs.some((arg: string) => arg.includes('transition-task-status')),
    ).toBe(false);
    expect(launchArgs.some((arg: string) => arg.includes('blocked'))).toBe(
      false,
    );
    expect(
      launchArgs.some((arg: string) => arg.includes('Reviewer seed')),
    ).toBe(false);
    const inboxPath = join(
      cwd,
      '.omc',
