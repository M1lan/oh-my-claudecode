...
describe('team api dispatch-aware messaging', () => {
  ...
  it('returns the top-level operation failure for an unknown broadcast team', async () => {
    ...
    expect(result).toEqual({
      ...
      error: {
        ...
        message: 'Team unknown-team not found',
      },
    });
  });
  ...
  it('notifies an exactly owned worker pane and commits both replay markers', async () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    const outcome = result.data.notification_outcome as {
      reason?: string;
      request_id?: string;
      message_id?: string;
    };
    ...
    expect(
      mailbox.messages.find(
        (message) => message.message_id === outcome.message_id,
      )?.notified_at,
  ...
  it('notifies an exactly owned leader pane and commits both replay markers', async () => {
    ...
    await writeFile(
      configPath,
      JSON.stringify({ ...config, leader_pane_id: '%0' }, null, 2),
    );
