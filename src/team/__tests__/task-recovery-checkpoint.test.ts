...
const access = (current: TeamTaskV2 | null) => ({
  ...
  withTaskLock: async <T>(
    _team: string,
    _task: string,
    _cwd: string,
    fn: () => Promise<T>,
  ) => ({ ok: true as const, value: await fn() }),
});
...
describe('task recovery checkpoints', () => {
  it('authenticates publication against the exact live claim and stores it under the claim-scoped path', async () => {
    const denied = await publishTaskRecoveryCheckpoint(
      input(),
      cwd,
      access({ ...task(), claim: { ...task().claim!, token: 'other' } }),
    );
    expect(denied).toEqual({ ok: false, error: 'claim_conflict' });

    const published = await publishTaskRecoveryCheckpoint(
      input(),
      cwd,
      access(task()),
    );
    expect(published).toMatchObject({
      ok: true,
      replayed: false,
      checkpoint: { claim_token: claimToken, sequence: 1, task_version: 3 },
    });
    if (published.ok)
  ...
  it('enforces the 64 KiB payload boundary and immutable same-sequence replay/conflict', async () => {
    expect(
      await publishTaskRecoveryCheckpoint(
        input(1, 'x'.repeat(MAX_TASK_RECOVERY_CHECKPOINT_BYTES + 1)),
        cwd,
        access(task()),
      ),
    ).toEqual({ ok: false, error: 'invalid_checkpoint' });
    const first = await publishTaskRecoveryCheckpoint(
      input(1, { a: 1 }),
      cwd,
      access(task()),
    );
    expect(first).toMatchObject({ ok: true, replayed: false });
    const retryWithoutTimestamp = {
      ...input(1, { a: 1 }),
      updatedAt: undefined,
    };
    await new Promise((resolve) => setTimeout(resolve, 2));
    const replayed = await publishTaskRecoveryCheckpoint(
      retryWithoutTimestamp,
      cwd,
      access(task()),
    );
    ...
    ).toEqual({ ok: false, error: 'publication_conflict' });
  });
  ...
  it('replays the public checkpoint operation after time advances', async () => {
    ...
    try {
      const first = await executeTeamApiOperation(
      ...
      await new Promise((resolve) => setTimeout(resolve, 2));
      const second = await executeTeamApiOperation(
        'write-task-checkpoint',
        args,
        cwd,
      );
      ...
      if (first.ok && second.ok) {
        ...
        const secondData = second.data as {
          checkpoint: { updated_at: string };
        };
        expect(secondData.checkpoint.updated_at).toBe(
          firstData.checkpoint.updated_at,
        );
      }
    } finally {
  ...
  it('rejects a sole checkpoint whose embedded sequence disagrees with its immutable filename', async () => {
    ...
    await expect(readTaskRecoveryCheckpoint(first.path)).resolves.toEqual({
      ok: false,
      error: 'malformed',
    });
    await expect(
      selectTaskRecoveryCheckpoint(teamName, task(), cwd),
    ).resolves.toEqual({ ok: false, error: 'malformed' });
    await expect(
      publishTaskRecoveryCheckpoint(
        input(1, { cursor: 1 }),
        cwd,
  ...
  it('selects only a unique current highest checkpoint and ignores a stale latest projection after a projection-write crash', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ sequence: 1, path: 'stale' }),
    );
    await expect(
      selectTaskRecoveryCheckpoint(teamName, task(), cwd),
    ).resolves.toMatchObject({ ok: true, checkpoint: { sequence: 2 } });
  });
  ...
  it('distinguishes missing, malformed, and stale checkpoint sets', async () => {
    ...
    await expect(
      selectTaskRecoveryCheckpoint(teamName, { ...task(), version: 4 }, cwd),
