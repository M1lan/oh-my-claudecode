...
describe('runtime owner team mutation contention', () => {
  ...
  it('keeps recovery transient while a durable scale-down reservation is active', async () => {
    ...
    await expect(
      executeRecoverDeadWorkerV2Owner({
        ...
        cwd,
        workerName: 'worker-2',
        requestId: 'scale-down-waiting-request',
      }),
    ).resolves.toMatchObject({
      outcome: 'failed',
      error: 'team_mutation_busy',
      recoveryId: 'scale-down-waiting-recovery',
    });
  ...
  it('terminally rejects a persisted attempt secret with a mismatched durable identity tuple', async () => {
    ...
    const attemptPath = absPath(
      cwd,
      TeamPaths.recoveryAttempt(teamName, 'attempt-recovery'),
    );
    ...
    await expect(
      ...
      recoveryId: 'attempt-recovery',
    });
    expect(readRecoveryOutcome(cwd, 'attempt-request')).toMatchObject({
      kind: 'final',
      outcome: 'failed',
  ...
  it('retains a committed pane on unknown liveness without spawning a duplicate replacement', async () => {
        ...
        workers: [
          {
            ...
            recovery_id: 'committed-recovery',
            replacement_generation: 2,
          },
        ],
        agent_type: 'claude',
        created_at: new Date().toISOString(),
        tmux_session: 'committed-team:0',
        lifecycle_state: 'active',
        state_revision: 3,
        active_recovery: {
    ...
    const attemptPath = absPath(
      cwd,
      TeamPaths.recoveryAttempt(teamName, 'committed-recovery'),
    );
    mkdirSync(join(attemptPath, '..'), { recursive: true });
    writeFileSync(
      attemptPath,
    ...
    await expect(
      ...
      config: {
        active_recovery: { recovery_id: 'committed-recovery' },
        workers: [
          {
            pane_id: '%9',
            pane_attempt_id: 'attempt-a',
  ...
  it.each(['alive', 'unknown', 'missing'] as const)(
    'rechecks %s original-pane liveness after election before replay effects',
    async (liveness) => {
      ...
      const attemptPath = absPath(
        cwd,
        TeamPaths.recoveryAttempt(teamName, recoveryId),
      );
      mkdirSync(join(attemptPath, '..'), { recursive: true });
      writeFileSync(
        attemptPath,
      ...
      if (liveness === 'alive') {
        ...
        expect(persisted?.config.active_recovery).toMatchObject({
          recovery_id: recoveryId,
        });
      }
      if (liveness === 'missing') {
  ...
  it.each([
    ...
    'rejects %s before recovery pane effects',
    async (expectedError, launchDescriptor) => {
  ...
  it('allows recovery past a committed scale-up fence without team_mutation_busy', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        active_scale_up: {
          operation_id: 'scale-up-committed-1',
          phase: 'committed',
          pid: 999999,
          process_started_at: 'linux:1',
          state_revision: 3,
    ...
    const result = await executeRecoverDeadWorkerV2Owner({
      ...
      cwd,
      workerName: 'worker-1',
      requestId: 'committed-scale-up-request',
    });
  ...
  it('does not treat non-committed phase labels as committed even if other fields look durable', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        active_scale_up: {
          ...
          phase: 'effects',
          pid: 1,
          process_started_at: 'linux:foreign',
          state_revision: 3,
          created_at: now,
          updated_at: now,
        },
    ...
    await expect(
      executeRecoverDeadWorkerV2Owner({
        ...
        cwd,
        workerName: 'worker-1',
        requestId: 'stale-label-request',
      }),
    ).resolves.toMatchObject({
      outcome: 'failed',
      error: 'team_mutation_busy',
      recoveryId: 'stale-label-recovery',
    });
  });
});
