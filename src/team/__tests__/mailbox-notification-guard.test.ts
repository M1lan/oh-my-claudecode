...
function message(
  overrides: Partial<TeamMailboxMessage> = {},
): TeamMailboxMessage {
  return {
    message_id: input.messageId,
...
function strictDispatch(
  value: TeamDispatchRequest = request(),
): StrictDispatchReadResult {
  return { kind: 'valid', request: value };
}

function strictMailbox(
  value: TeamMailboxMessage = message(),
): StrictCanonicalMailboxMessageReadResult {
...
function validState(
  overrides: Partial<MailboxNotificationGuardState> = {},
): MailboxNotificationGuardState {
  return {
    config: config(),
...
describe('teamReadCanonicalMailboxMessageStrict', () => {
  ...
  async function writeMailbox(
    ...
    await mkdir(join(cwd, '.omc', 'state', 'team', teamName, 'mailbox'), {
      recursive: true,
    });
    await writeFile(path, JSON.stringify(value, null, 2), 'utf8');
  }

  async function writeRawMailbox(
    raw: string,
    workerName = input.recipient,
  ): Promise<void> {
    const path = join(
  ...
  it('never falls back to legacy JSONL while compatibility reads still do', async () => {
    ...
    await expect(
      teamListMailbox(teamName, input.recipient, cwd),
  ...
  it('distinguishes malformed stores, owner mismatch, malformed messages, missing, and duplicates', async () => {
    ...
    await writeMailbox({
      worker: input.recipient,
      messages: [{ ...message(), body: 42 } as unknown as TeamMailboxMessage],
    });
    await expect(
      teamReadCanonicalMailboxMessageStrict(
        teamName,
    ...
    await writeMailbox({
      worker: input.recipient,
      messages: [message({ message_id: 'other-message' })],
    });
    await expect(
      teamReadCanonicalMailboxMessageStrict(
        teamName,
  ...
  it('rejects recipient mismatch and replay, then returns a fresh exact canonical message', async () => {
    await writeMailbox({
      worker: input.recipient,
      messages: [message({ to_worker: 'worker-2' })],
    });
    await expect(
      teamReadCanonicalMailboxMessageStrict(
        teamName,
    ...
    await writeMailbox({
      worker: input.recipient,
      messages: [message({ notified_at: '2026-07-13T00:01:00.000Z' })],
    });
    await expect(
      teamReadCanonicalMailboxMessageStrict(
        teamName,
...
describe('mailbox notification guard', () => {
  it('allows the deterministic canonical duplicate-worker target only when strict metadata agrees', () => {
    ...
    const result = evaluateMailboxNotificationGuard(
      input,
      validState({ config: duplicateConfig }),
    );

    expect(result).toMatchObject({
      kind: 'allow',
      target: { paneId: '%9', recipientRole: 'worker' },
    });

    const mismatched = evaluateMailboxNotificationGuard(
      input,
      validState({
  ...
  it('maps every strict evidence and provider failure to a stable pre-effect reason', () => {
    expect(
      evaluateMailboxNotificationGuard(input, validState({ config: null })),
    ).toMatchObject({
      kind: 'suppress',
      reason: 'mailbox_team_unavailable',
    });
    expect(
      evaluateMailboxNotificationGuard(
        input,
        validState({ config: config({ name: 'other-team' }) }),
      ),
    ).toMatchObject({
      kind: 'suppress',
      reason: 'mailbox_team_identity_mismatch',
    });
    expect(
      evaluateMailboxNotificationGuard(
        input,
        validState({ config: config({ workers: [] }) }),
      ),
    ).toMatchObject({
      kind: 'suppress',
    ...
    expect(
      ...
      kind: 'suppress',
      reason: 'mailbox_dispatch_store_invalid',
    });
    expect(
      evaluateMailboxNotificationGuard(
        input,
    ...
    expect(
      ...
      kind: 'suppress',
      reason: 'mailbox_request_not_pending',
    });
    expect(
      evaluateMailboxNotificationGuard(
        input,
        validState({
          dispatch: strictDispatch(
            request({ trigger_message: 'Different trigger.' }),
          ),
        }),
      ),
    ).toMatchObject({
    ...
    expect(
      evaluateMailboxNotificationGuard(
        ...
        validState({
          mailbox: {
            kind: 'replay_suppressed',
            message: message({ notified_at: timestamp }),
            marker: 'notified_at',
          },
        }),
      ),
    ...
    expect(
      ...
      kind: 'suppress',
      reason: 'mailbox_target_foreign',
    });
    expect(
      evaluateMailboxNotificationGuard(
        input,
        validState({ ownership: { kind: 'unavailable' } }),
      ),
    ).toMatchObject({
      kind: 'suppress',
  ...
  it('compares named security fields while ignoring diagnostic-only dispatch changes', () => {
    ...
    const diagnosticsOnly = evaluateMailboxNotificationGuard(
      input,
      validState({
        dispatch: strictDispatch(
          request({
    ...
    expect(
      mailboxNotificationSecurityTupleEquals(
        first.securityTuple,
        diagnosticsOnly.securityTuple,
      ),
    ).toBe(true);
    expect(
      mailboxNotificationSecurityTupleEquals(first.securityTuple, {
        ...first.securityTuple,
        requestTriggerMessage: 'Different trigger.',
      }),
    ).toBe(false);

    const changedSecurity = evaluateMailboxNotificationGuard(
      input,
      validState({
  ...
  it('performs strict current reads and an injected ownership check with zero marker or pane effects', async () => {
    ...
    expect(readStrictDispatchRequest).toHaveBeenCalledWith(
      teamName,
      input.requestId,
      '/unused',
    );
    expect(readStrictMailboxMessage).toHaveBeenCalledWith(
      teamName,
      input.recipient,
      input.messageId,
      '/unused',
    );
    expect(verifyProviderOwnership).toHaveBeenCalledWith(
      expect.objectContaining({ paneId: '%9' }),
    );
