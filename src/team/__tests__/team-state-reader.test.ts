...
function manifest(revision?: number) {
  return {
    name: teamName,
    tmux_session: 'manifest-session',
    leader: {
      worker_id: 'leader',
      role: 'leader',
      session_id: 'leader-session',
    },
    workers: [{ name: 'manifest-worker' }],
    ...(revision === undefined ? {} : { state_revision: revision }),
  };
}
...
describe('team state reader authority table', () => {
  it('returns absent when both records are absent, and preserves legacy merge behavior only for two legacy records', () => {
    expect(readTeamState(cwd, teamName)).toMatchObject({
  ...
  it('classifies malformed config and manifest-only legacy records without trusting malformed state', () => {
    ...
    expect(readTeamState(cwd, teamName)).toMatchObject({
      ...
      state: null,
      config: { source: 'malformed' },
    });
    rmSync(configPath());
    expect(readTeamState(cwd, teamName)).toMatchObject({
      classification: 'manifest_only_legacy',
      state: { tmux_session: 'manifest-session' },
    });
  });
});
