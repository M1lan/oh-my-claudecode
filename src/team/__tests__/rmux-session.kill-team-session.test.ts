    ...
    if (args[0] === 'list-windows') {
      if (mocked.listWindowsThrows)
        return {
          stdout: '',
          stderr: '',
          error: new Error('tmux control mode failed'),
        };
...
describe('killTeamSession safeguards', () => {
  ...
  it('kills a different detached session', async () => {
    ...
    await expect(killTeamSession('worker-detached-session')).resolves.toBe(
      true,
    );
  ...
  it('kills only worker panes in split-pane mode', async () => {
    await expect(
      killTeamSession('leader-session:0', ['%10', '%11'], '%10'),
    ).resolves.toBe(true);
  ...
  it('kills an owned team window when session owns that window', async () => {
    await expect(
      killTeamSession('leader-session:3', ['%10', '%11'], '%10', {
  ...
  it('uses only recorded worker panes during split-pane shutdown', async () => {
    ...
    expect(mocked.execCalls.some((args) => args[0] === 'list-panes')).toBe(
      false,
    );
  });
  ...
  it('preserves a recorded worker pane when target membership cannot be proven', async () => {
    ...
    await expect(
      killTeamSession('leader-session:0', ['%11'], '%10'),
    ).resolves.toBe(false);

    expect(
      mocked.execCalls.some(
        (args) => args[0] === 'kill-pane' && args.includes('%11'),
      ),
    ).toBe(false);
  });
  ...
  it('treats already-absent dedicated window as cleanup success after kill fails', async () => {
    ...
    await expect(
      killTeamSession('leader-session:3', [], '%0', {
        sessionMode: 'dedicated-window',
      }),
    ).resolves.toBe(true);
    expect(mocked.execCalls.some((args) => args[0] === 'list-windows')).toBe(
      true,
    );
  });
  ...
  it('does not match window index by substring collision (3 vs 30, 13)', async () => {
    ...
    mocked.listedWindows = '0:bash\n13:other\n';
    await expect(
      killTeamSession('leader-session:13', [], '%0', {
        sessionMode: 'dedicated-window',
      }),
    ).resolves.toBe(false);
  });
  ...
  it('rejects malformed/ambiguous list-windows output', async () => {
    ...
    await expect(
      killTeamSession('leader-session', [], '%0', {
        sessionMode: 'dedicated-window',
      }),
    ).resolves.toBe(false);
  });
