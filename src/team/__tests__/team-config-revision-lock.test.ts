...
import {
  ...
  assertActiveFenceOwnershipTransition,
  withScalingLock,
} from '../monitor.js';
import { absPath, TeamPaths } from '../state-paths.js';
import type { TeamConfig } from '../types.js';
import {
  withProcessIdentityFileLock,
  withProcessIdentityFileLockSync,
} from '../process-identity-lock.js';
...
describe('team config revision transaction', () => {
  it('rejects recovery cleanup and publishes no final after a normal writer wins the revision', async () => {
    ...
    const cleanup = {
      ...
      last_recovery: {
        ...initialConfig().active_recovery!,
        phase: 'adopted' as const,
        state_revision: 2,
      },
    };
    await expect(
      saveTeamConfigAtRevision(cleanup, 1, cwd, finalPublished),
    ).resolves.toBe(false);
    ...
    await expect(
      ...
      config: {
        next_task_id: 2,
        active_recovery: { recovery_id: 'recovery-a' },
      },
    });
  });

  it.each(['shutting_down', 'stopped'] as const)(
    'rejects task admission when lifecycle is %s',
    async (lifecycle_state) => {
      ...
      ).rejects.toThrow('team_mutation_busy');
      expect(existsSync(absPath(cwd, TeamPaths.taskFile(teamName, '1')))).toBe(
        false,
      );
      await expect(
        readRevisionedTeamConfig(teamName, cwd),
  ...
  it('compensates a created task when the counter revision CAS loses', async () => {
    ...
    try {
      ...
      ).rejects.toThrow('stale_state_revision');
    } finally {
      vi.doUnmock('../monitor.js');
      vi.resetModules();
    }

    expect(saveAtRevision).toHaveBeenCalledWith(
  ...
  it('does not recreate a team when the authoritative config disappears before CAS', async () => {
    ...
    await expect(
      saveTeamConfigAtRevision(stale, 1, cwd, afterCommit),
    ).resolves.toBe(false);
  ...
  it('does not commit authoritative config when manifest projection cannot be written', async () => {
    ...
    await expect(saveTeamConfigAtRevision(next, 1, cwd)).rejects.toThrow(
  ...
  it('holds the config lock through terminal publication and rejects a stale competing writer', async () => {
    ...
    const recoveryCommit = saveTeamConfigAtRevision(
      cleanup,
      1,
      cwd,
      async () => {
    ...
    const normalWrite = saveTeamConfig(
      staleNormal,
      cwd,
      staleNormal.state_revision,
    ).finally(() => {
      normalSettled = true;
    });
    ...
    expect(persisted).toMatchObject({
      stateRevision: 2,
      config: { last_recovery: { recovery_id: 'recovery-a' } },
    });
  ...
  it('reclaims a config lock only after its persisted process identity is dead', async () => {
    ...
    await expect(
      saveTeamConfig(config, cwd, config.state_revision),
  ...
  it('rejects a stale pre-incremented writer that would restore cleared recovery state', async () => {
    ...
    await expect(
      saveTeamConfigAtRevision(cleanup, 1, cwd, undefined, {
        release: { active_recovery: true },
      }),
    ).resolves.toBe(true);
    const stalePreincremented = initialConfig();
    stalePreincremented.state_revision = 2;
    stalePreincremented.active_recovery = {
      ...stalePreincremented.active_recovery!,
      state_revision: 2,
    };
    stalePreincremented.next_task_id = 11;

    await expect(saveTeamConfig(stalePreincremented, cwd, 1)).rejects.toThrow(
  ...
  it('migration preserves an immediately preceding normal legacy write', async () => {
    ...
    await expect(
      migrateTeamConfigRevision(teamName, cwd),
    ).resolves.toMatchObject({
      stateRevision: 0,
      config: { next_task_id: 13 },
    });
    await expect(
      readRevisionedTeamConfig(teamName, cwd),
    ).resolves.toMatchObject({
  ...
  describe('legacy config/manifest max_workers merge (#3744)', () => {
    ...
    it('honors an explicit configured cap below the legacy default of 20 in both read paths', async () => {
      ...
      await expect(readTeamConfig(teamName, cwd)).resolves.toMatchObject({
        max_workers: 3,
      });
      await expect(teamReadConfig(teamName, cwd)).resolves.toMatchObject({
        max_workers: 3,
      });
    });
    ...
    it('returns a revisioned sub-ceiling cap verbatim without consulting the manifest merge', async () => {
      ...
      expect(
        (await teamReadConfig(teamName, cwd))?.workers[0]?.pane_id,
      ).not.toBe('%manifest');
      await expect(
        readRevisionedTeamConfig(teamName, cwd),
      ).resolves.toMatchObject({ config: { max_workers: 3, next_task_id: 1 } });
    });
    ...
    it('migrates a manifest-only team at the legacy default cap', async () => {
      ...
      await expect(
        migrateTeamConfigRevision(teamName, cwd),
      ).resolves.toMatchObject({ config: { max_workers: 20 } });
      await expect(readTeamConfig(teamName, cwd)).resolves.toMatchObject({
        max_workers: 20,
      });
    });
    ...
    it.each([0, -1, 1.5])(
      'rejects a legacy config carrying invalid cap %s through every reader and migration',
      async (maxWorkers) => {
        writeLegacyConfig(maxWorkers);
        writeMergeManifest();

        await expect(readTeamConfig(teamName, cwd)).rejects.toThrow(
          'invalid_persisted_state',
        );
        await expect(teamReadConfig(teamName, cwd)).rejects.toThrow(
          'invalid_persisted_state',
        );
        await expect(readRevisionedTeamConfig(teamName, cwd)).rejects.toThrow(
          'invalid_persisted_state',
        );
        await expect(migrateTeamConfigRevision(teamName, cwd)).rejects.toThrow(
          'invalid_persisted_state',
        );
      },
    );
    ...
    it.each([0, -1, 1.5])(
      'rejects a revisioned config carrying invalid cap %s through every reader and migration',
      async (maxWorkers) => {
    ...
    it.each([0, -1, 1.5])(
      'refuses to persist invalid cap %s through both writers',
      async (maxWorkers) => {
        await expect(
          saveTeamConfig({ ...initialConfig(), max_workers: maxWorkers }, cwd),
        ).rejects.toThrow('invalid_persisted_state');
        await expect(
          saveTeamConfigAtRevision(
            { ...initialConfig(), max_workers: maxWorkers, state_revision: 2 },
            1,
            cwd,
          ),
        ).rejects.toThrow('invalid_persisted_state');
    ...
    it.each([1, 20])(
      'keeps configured boundary cap %s valid through both readers',
      async (maxWorkers) => {
        writeLegacyConfig(maxWorkers);
        writeMergeManifest();
  ...
  it('never lets a divergent manifest override revisioned config authority', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        state_revision: 99,
        name: teamName,
        workers: [{ name: 'worker-1', index: 1, pane_id: '%manifest' }],
        worker_count: 1,
        next_task_id: 99,
      }),
    );
    ...
    expect((await readTeamConfig(teamName, cwd))?.workers[0]?.pane_id).not.toBe(
      '%manifest',
    );
    expect((await teamReadConfig(teamName, cwd))?.workers[0]?.pane_id).not.toBe(
      '%manifest',
    );
  ...
  it('refuses malformed authoritative config instead of bootstrapping from a divergent manifest', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        state_revision: 99,
        name: teamName,
        workers: [{ name: 'worker-1', index: 1, pane_id: '%stale' }],
        worker_count: 1,
        next_task_id: 99,
      }),
    );
    ...
    await expect(saveTeamConfig(initialConfig(), cwd)).rejects.toThrow(
      'invalid_persisted_state',
    );
    await expect(
      teamCreateTask(
  ...
  it('fails closed on a malformed manifest when authoritative config is absent', async () => {
    ...
    await expect(readTeamManifest(teamName, cwd)).rejects.toThrow(
      'invalid_persisted_state',
    );
    await expect(teamReadManifest(teamName, cwd)).rejects.toThrow(
      'invalid_persisted_state',
    );
    await expect(readTeamConfig(teamName, cwd)).rejects.toThrow(
      'invalid_persisted_state',
  ...
  it.each([
    ...
    [
      'whitespace canonical-equivalent worker names',
      {
        ...initialConfig(),
        worker_count: 2,
        workers: [
    ...
    [
      'malformed worker',
      { ...initialConfig(), workers: [{ name: 'worker-1', index: 'one' }] },
    ],
    [
      'malformed policy',
      { ...initialConfig(), policy: { display_mode: 'auto' } },
    ],
    [
      'malformed governance',
      { ...initialConfig(), governance: { delegation_only: true } },
    ],
    ...
    [
      ...
      {
        ...initialConfig(),
        resolved_routing: { executor: { primary: {}, fallback: {} } },
      },
    ],
    [
      'mismatched active fence revision',
      {
        ...initialConfig(),
        active_recovery: {
          ...initialConfig().active_recovery!,
    ...
    [
      'mismatched active scale-up fence revision',
      {
        ...initialConfig(),
        active_scale_up: {
          operation_id: 'up',
    ...
    [
      'mismatched active scale-down fence revision',
      {
        ...initialConfig(),
        active_scale_down: {
          operation_id: 'down',
    ...
    [
      'mismatched shutdown fence revision',
      {
        ...initialConfig(),
        shutdown_attempt: {
          nonce: 'shutdown',
    ...
    [
      'mismatched all-dead fence revision',
      {
        ...initialConfig(),
        all_dead_recovery: {
    ...
    [
      'malformed owner',
      { ...initialConfig(), runtime_owner_epoch: { epoch: 1, nonce: 'owner' } },
    ],
    [
      'malformed service',
      {
        ...initialConfig(),
        service_descriptor: { schema_version: 1, service_generation: 1 },
      },
    ],
    ['malformed lifecycle', { ...initialConfig(), lifecycle_state: 'broken' }],
  ])(
    'fails closed on %s authoritative config without consulting or replacing its manifest',
    async (_name, config) => {
      const configPath = absPath(cwd, TeamPaths.config(teamName));
      ...
      writeFileSync(
        absPath(cwd, TeamPaths.manifest(teamName)),
        JSON.stringify({ schema_version: 2, name: teamName, workers: [] }),
      );
  ...
  it('accepts and round-trips a committed scale-up fence through real save/load', async () => {
    ...
    expect(loaded?.config.active_scale_up?.state_revision).toBe(
      loaded?.config.state_revision,
    );
    ...
    const advanced = {
      ...
      active_scale_up: {
        ...loaded!.config.active_scale_up!,
        state_revision: loaded!.stateRevision,
      },
    };
    await expect(
      saveTeamConfigAtRevision(advanced, loaded!.stateRevision, cwd),
    ).resolves.toBe(true);
    const after = await readRevisionedTeamConfig(teamName, cwd);
    expect(after?.config.active_scale_up?.phase).toBe('committed');
    expect(after?.config.active_scale_up?.state_revision).toBe(
      after?.config.state_revision,
    );
    expect(after?.config.active_recovery?.state_revision).toBe(
      after?.config.state_revision,
    );
  });
  ...
  it('rejects a path-mismatched persisted config before migration writes a projection', async () => {
    ...
    expect(
      existsSync(absPath(cwd, TeamPaths.configMutationLock(teamName))),
    ).toBe(false);
  });
  ...
  it('accepts complete revisioned lifecycle fences and migrates only a valid unrevisioned legacy config', async () => {
    ...
    const fenced = {
      ...
      service_descriptor: {
        ...
        service_attempt_id: 'service',
        auto_merge_enabled: false,
        workspace_root: cwd,
        cadence_policy: 'disabled' as const,
      },
    ...
    await expect(
      ...
      stateRevision: 1,
      config: {
        active_scale_up: { operation_id: 'up' },
        active_scale_down: { operation_id: 'down' },
      },
    });
    ...
    await expect(
      migrateTeamConfigRevision(teamName, cwd),
    ).resolves.toMatchObject({ stateRevision: 0 });
  ...
  it('never steals a live task holder lock because its timestamp is old', async () => {
    ...
    await expect(
      withTaskClaimLock(teamName, '1', cwd, effect),
  ...
  it('does not reclaim a lock owned by the current live process identity', async () => {
    ...
    await expect(
      withProcessIdentityFileLock(lockPath, effect, 30),
    ).rejects.toThrow('process_identity_lock_timeout');
    expect(effect).not.toHaveBeenCalled();
    expect(readFileSync(lockPath, 'utf8')).toBe(bytes);
  });
  ...
  it('never reclaims parseable lock records with blank process identity', async () => {
    ...
    await expect(
      withProcessIdentityFileLock(lockPath, asyncEffect, 30),
    ).rejects.toThrow('process_identity_lock_timeout');
    expect(() => withProcessIdentityFileLockSync(lockPath, syncEffect)).toThrow(
      'process_identity_lock_busy',
    );
  ...
  it.each([
    process.platform === 'linux'
      ? 'linux:not-a-start-tick'
      : process.platform === 'win32'
        ? 'win32:not-a-start-tick'
        : 'darwin:not-seconds:not-micros',
    process.platform === 'linux' ? 'win32:123' : 'linux:123',
  ])(
    'does not reclaim unverifiable process identity %s',
    async (processStartedAt) => {
      const lockPath = `${absPath(cwd, TeamPaths.configMutationLock(teamName))}.${processStartedAt.split(':')[0]}`;
      const bytes = JSON.stringify({
        schema_version: 1,
        pid: process.pid,
        process_started_at: processStartedAt,
        nonce: 'unverifiable-identity',
        created_at: new Date().toISOString(),
      });
      writeFileSync(lockPath, bytes);
      const asyncEffect = vi.fn();
      ...
      expect(() =>
        withProcessIdentityFileLockSync(lockPath, syncEffect),
      ).toThrow('process_identity_lock_busy');
      expect(asyncEffect).not.toHaveBeenCalled();
      expect(syncEffect).not.toHaveBeenCalled();
      expect(readFileSync(lockPath, 'utf8')).toBe(bytes);
    },
  ...
  it('reclaims a crashed scaling lock only after positive process death', async () => {
    ...
    await expect(withScalingLock(teamName, cwd, effect, 100)).resolves.toBe(
      'resumed',
    );
  ...
  it('rejects foreign active_scale_up ownership substitution at matching revision', async () => {
    ...
    writeConfig({
      ...
      active_recovery: undefined,
      active_scale_up: {
        operation_id: 'owner-A',
        phase: 'effects' as const,
        pid: 111,
        process_started_at: 'linux:owner-A',
        state_revision: 1,
        created_at: now,
        updated_at: now,
      },
    ...
    const foreign = {
      ...
      active_recovery: undefined,
      active_scale_up: {
        operation_id: 'owner-B',
        phase: 'effects' as const,
        pid: 222,
        process_started_at: 'linux:owner-B',
        state_revision: 2,
        created_at: now,
        updated_at: now,
      },
    };
    await expect(saveTeamConfigAtRevision(foreign, 1, cwd)).rejects.toThrow(
      'invalid_persisted_state',
    );
    const after = await readRevisionedTeamConfig(teamName, cwd);
  ...
  it('accepts same-owner scale-up phase transition with revision rebase', async () => {
    ...
    await expect(saveTeamConfigAtRevision(advanced, 1, cwd)).resolves.toBe(
      true,
    );
    const after = await readRevisionedTeamConfig(teamName, cwd);
  ...
  it('rejects clearing active_scale_up without release authorization', async () => {
    ...
    await expect(saveTeamConfigAtRevision(cleared, 1, cwd)).rejects.toThrow(
      'invalid_persisted_state',
    );
    await expect(
      saveTeamConfigAtRevision(cleared, 1, cwd, undefined, {
  ...
  it('rejects foreign active_scale_down ownership substitution and worker retarget', async () => {
    ...
    writeConfig({
      ...
      active_scale_down: {
        ...
        pid: 111,
        process_started_at: 'linux:A',
        workers: [{ name: 'worker-2' }],
        state_revision: 1,
        failure_reason: 'pane_cleanup_failed',
        created_at: now,
        updated_at: now,
      },
    });
    ...
    expect(after?.config.active_scale_down?.workers).toEqual([
      { name: 'worker-2' },
    ]);
  });
  ...
  it('rejects stale expected revision even for same-owner transition', async () => {
    ...
    await expect(saveTeamConfigAtRevision(advanced, 0, cwd)).resolves.toBe(
      false,
    );
  });
  ...
  it('rejects empty/non-string launch_attempt_id on worker rows', async () => {
    ...
    await expect(
      saveTeamConfigAtRevision(badEmpty as any, 1, cwd),
    ).rejects.toThrow('invalid_persisted_state');
    const badNull = {
      ...base,
      state_revision: 2,
    ...
    await expect(
      saveTeamConfigAtRevision(badNull as any, 1, cwd),
    ).rejects.toThrow('invalid_persisted_state');
  });
  ...
  it('authorizes same-owner failed→draining scale-down resume with identical workers', () => {
    ...
    expect(() =>
      assertActiveFenceOwnershipTransition(current, proposed),
    ).not.toThrow();
  });
  ...
  it('persists same-owner failed→draining scale-down resume through real CAS', async () => {
    ...
    writeConfig({
      ...
      worker_count: 2,
      active_scale_down: {
        operation_id: 'sd-cas-resume',
        phase: 'failed',
        pid: 42,
        process_started_at: 'linux:owner',
