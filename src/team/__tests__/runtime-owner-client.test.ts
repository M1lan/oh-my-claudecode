...
function seedBootstrapRecoveryRequest(
  ...
  const intentPath = absPath(
    cwd,
    TeamPaths.recoveryIntent(teamName, recoveryId),
  );
...
describe('runtime owner durable request admission', () => {
  ...
  it('aliases concurrent identical requests to one recovery identity and one owner dispatch', async () => {
    ...
    try {
      ...
      const later = await requestRuntimeOwnerRecovery({
        teamName: 'recovery-team',
        cwd,
        workerName: 'worker-1',
        requestId: 'request-c',
  ...
  it('rejects self-inconsistent immutable reservations before intent, final, or owner effects', async () => {
    const cwd = mkdtempSync(
      join(tmpdir(), 'runtime-owner-inconsistent-reservation-'),
    );
    try {
      seedV2Team(cwd);
      ...
      for (const [name, from, to] of mutations) {
        ...
        const client = createRecoveryOwnerClient(dispatch, {
          minTimeoutMs: 100,
          maxTimeoutMs: 100,
          pollIntervalMs: 10,
        });
        await expect(
          client.recoverDeadWorker({
            teamName: 'recovery-team',
            cwd,
            workerName: 'worker-1',
            requestId,
          ...
          name,
        ).rejects.toThrow('malformed_recovery_request_reservation');
        expect(dispatch, name).not.toHaveBeenCalled();
        expect(
          existsSync(
            absPath(
              cwd,
              TeamPaths.recoveryIntent(
                'recovery-team',
                'recovery-inconsistent',
              ),
            ),
        ...
        expect(
          existsSync(absPath(cwd, TeamPaths.recoveryRequestResult(requestId))),
          name,
        ).toBe(false);
  ...
  it('reconstructs a missing canonical intent after a crash following reservation publication', async () => {
    ...
    try {
      ...
      setRuntimeOwnerDispatch(dispatch);
      const pending = requestRuntimeOwnerRecovery({
        teamName: 'recovery-team',
        cwd,
        workerName: 'worker-1',
      ...
      const intentPath = absPath(
        cwd,
        TeamPaths.recoveryIntent('recovery-team', 'recovery-crash'),
      );
      for (
        let attempt = 0;
        attempt < 50 && !existsSync(intentPath);
        attempt++
      ) {
    ...
    try {
      ...
      const bootstrapOwner = vi.fn(
        async (
          ...
          const owner = publishOwnerEpoch(cwd, input.teamName, 1, {
            nonce: 'first-owner',
          });
      ...
      const client = createRecoveryOwnerClient(vi.fn(), {
        persistentOwnerBootstrap: true,
        bootstrapOwner,
        minTimeoutMs: 100,
        maxTimeoutMs: 100,
        pollIntervalMs: 10,
  ...
  it('does not bootstrap an unknown owner identity', async () => {
    ...
    try {
      ...
      writeFileSync(
        absPath(cwd, TeamPaths.ownerEpoch('recovery-team', 1)),
        '{',
      );
      ...
      await expect(
        client.recoverDeadWorker({
          ...
          cwd,
          workerName: 'worker-1',
          requestId: 'unknown-owner-request',
          timeoutMs: 100,
        }),
      ).resolves.toMatchObject({ error: 'recovery_request_timeout' });
      expect(
        readRecoveryOutcome(cwd, 'unknown-owner-request'),
      ).not.toMatchObject({ kind: 'final' });
  ...
  it.skipIf(process.platform !== 'linux')(
    'bootstraps exactly one successor after a positively dead owner epoch and leaves execution to that owner',
    async () => {
      ...
      try {
        ...
        const bootstrapOwner = vi.fn(
          async (
            ...
            const owner = publishOwnerEpoch(cwd, 'recovery-team', 2, {
              nonce: 'successor-owner',
            });
            writeFileSync(
              absPath(cwd, TeamPaths.config('recovery-team')),
              JSON.stringify(
                validV2Config('recovery-team', {
                  state_revision: 2,
                  runtime_owner_epoch: owner,
                }),
              ),
  ...
  it('replays a canonical final after team deletion without publishing an empty-recovery result', async () => {
    ...
    try {
      ...
      const admissionDispatch = vi.fn(
        async (input: RecoverDeadWorkerOwnerInput) =>
          publishSuccess(input.cwd, input.requestId),
      );
      ...
      const expected = await admittingClient.recoverDeadWorker({
        teamName: 'recovery-team',
        cwd,
        workerName: 'worker-1',
        requestId,
      ...
      expect(readRecoveryOutcome(cwd, requestId)).toMatchObject({
        kind: 'final',
        recovery_id: expected.recoveryId,
      });
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
    ...
    try {
      ...
      await expect(
        client.recoverDeadWorker({
          ...
          cwd,
          workerName: 'worker-1',
          requestId: 'deleted-conflict-request',
          timeoutMs: 100,
        }),
      ...
      expect(
        readRecoveryRequestReservation(cwd, 'deleted-conflict-request'),
      ).toMatchObject({
  ...
  it.skipIf(process.platform !== 'linux').each([
    ...
    'accepts revisioned config authority with a %s manifest',
    async (_manifestState, manifest) => {
      const cwd = mkdtempSync(
        join(tmpdir(), 'runtime-owner-config-authority-'),
      );
      try {
        ...
        await expect(
          client.recoverDeadWorker({
            ...
            cwd,
            workerName: 'worker-1',
            requestId: `manifest-${_manifestState}`,
            timeoutMs: 100,
          }),
        ).resolves.toMatchObject({ outcome: 'recovered' });
        expect(bootstrapOwner).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ...
    [
      'negative revision',
      validV2Config('recovery-team', { state_revision: -1 }),
    ],
    [
      'malformed worker',
      validV2Config('recovery-team', {
        workers: [{ name: 'worker-1', index: 'bad' }],
        worker_count: 1,
      }),
    ],
    [
      'malformed owner',
      validV2Config('recovery-team', {
        runtime_owner_epoch: { epoch: 1, nonce: 'owner' },
      }),
    ],
    [
      'malformed service',
    ...
    [
      'malformed lifecycle',
      validV2Config('recovery-team', { lifecycle_state: 'unknown' }),
    ],
  ])(
    'classifies %s authoritative config as invalid before owner effects',
    async (_name, config) => {
      const cwd = mkdtempSync(
        join(tmpdir(), 'runtime-owner-malformed-config-'),
      );
      try {
        ...
        await expect(
          client.recoverDeadWorker({
            ...
            cwd,
            workerName: 'worker-1',
            requestId: `malformed-${_name.replace(/\s+/g, '-')}`,
            timeoutMs: 100,
          }),
        ).resolves.toMatchObject({ error: 'invalid_persisted_state' });
        expect(bootstrapOwner).not.toHaveBeenCalled();
      } finally {
        rmSync(cwd, { recursive: true, force: true });
      }
  ...
  it('classifies only a complete unrevisioned config as legacy and a missing config as absent', async () => {
    const legacyCwd = mkdtempSync(
      join(tmpdir(), 'runtime-owner-legacy-config-'),
    );
    const absentCwd = mkdtempSync(
      join(tmpdir(), 'runtime-owner-absent-config-'),
    );
    try {
      ...
      await expect(
        client.recoverDeadWorker({
          ...
          cwd: legacyCwd,
          workerName: 'worker-1',
          requestId: 'legacy-config',
          timeoutMs: 100,
        }),
      ).resolves.toMatchObject({ error: 'runtime_v2_required' });
      await expect(
        client.recoverDeadWorker({
          teamName: 'recovery-team',
          cwd: absentCwd,
          workerName: 'worker-1',
          requestId: 'absent-config',
          timeoutMs: 100,
        }),
      ).resolves.toMatchObject({ error: 'team_not_found' });
    } finally {
  ...
  it('does not take over a verified live owner and keeps the request transient', async () => {
    ...
    try {
      ...
      const bootstrapOwner = vi.fn(
        async (
          _input: RecoverDeadWorkerOwnerInput,
          _priorEpoch: number | null,
        ) => true,
      );
      const dispatch = vi.fn();
      const client = createRecoveryOwnerClient(dispatch, {
  ...
  it('returns the exact typed timeout while leaving the durable intent pending', async () => {
    ...
    try {
      const neverSettles = new Promise<RecoverDeadWorkerV2Result>(
        () => undefined,
      );
      const client = createRecoveryOwnerClient(
        vi.fn(() => neverSettles),
        { minTimeoutMs: 100, maxTimeoutMs: 100, pollIntervalMs: 10 },
      );
      const result = await client.recoverDeadWorker({
        teamName: 'recovery-team',
        cwd,
        workerName: 'worker-1',
      ...
      expect(
        existsSync(
          absPath(
            cwd,
            TeamPaths.recoveryIntent('recovery-team', reservation!.recovery_id),
          ),
        ),
      ).toBe(true);
  ...
  it('rejects a truncated existing canonical intent without dispatching owner effects', async () => {
    ...
    try {
      ...
      const intentPath = absPath(
        cwd,
        TeamPaths.recoveryIntent('recovery-team', 'recovery-truncated'),
      );
      mkdirSync(join(intentPath, '..'), { recursive: true });
  ...
  it('rejects a tuple-matching incomplete final instead of replaying or dispatching', async () => {
    ...
    try {
      ...
      writeFileSync(
        absPath(cwd, TeamPaths.recoveryRequestResult('request-incomplete')),
        JSON.stringify({
          schema_version: 1,
          kind: 'final',
          request_id: 'request-incomplete',
          recovery_id: 'recovery-incomplete',
          team_name: 'recovery-team',
          worker_name: 'worker-1',
          outcome: 'failed',
          result: {
            ...
            recoveryId: 'recovery-incomplete',
            teamName: 'recovery-team',
            workerName: 'worker-1',
            updatedAt: new Date().toISOString(),
          },
...
describe('recovery admission lock crash takeover', () => {
  it.skipIf(process.platform !== 'linux')(
    'reclaims a complete lock record only after its PID identity is confirmed dead',
    async () => {
      const cwd = mkdtempSync(join(tmpdir(), 'runtime-owner-stale-lock-'));
      try {
        const lockPath = absPath(
          cwd,
          TeamPaths.recoveryAdmissionLock('payload-hash'),
        );
        ...
        await expect(
          withRecoveryAdmissionLock(cwd, 'payload-hash', effect),
  ...
  it('accepts only the exact child-owned successor epoch and fence', () => {
    const owner = {
      ...
      epoch: 1,
      pid: 123,
      process_started_at: 'linux:456',
      nonce: 'owner',
      payload_hash: 'hash',
      created_at: new Date().toISOString(),
    };
    expect(
      isExpectedRecoveryOwnerSuccessor(owner, 1, 123, 'linux:456', true),
    ).toBe(true);
    expect(
      isExpectedRecoveryOwnerSuccessor(
        { ...owner, epoch: 2 },
    ...
    expect(
      isExpectedRecoveryOwnerSuccessor(owner, 1, 123, 'linux:999', true),
    ).toBe(false);
    expect(
      isExpectedRecoveryOwnerSuccessor(owner, 1, 123, 'linux:456', false),
    ).toBe(false);
    expect(
      isExpectedRecoveryOwnerSuccessor(
        owner,
...
describe('runtime owner bootstrap spawn lifecycle regressions', () => {
  ...
  it('keeps reservation and intent pending when spawn throws synchronously', async () => {
    ...
    try {
      ...
      expect(readRecoveryRequestReservation(cwd, requestId)).toMatchObject({
        recovery_id: recoveryId,
        kind: 'reservation',
      });
      expect(readRecoveryOutcome(cwd, requestId)).not.toMatchObject({
        kind: 'final',
      });
      expect(
        existsSync(
          absPath(cwd, TeamPaths.recoveryIntent('recovery-team', recoveryId)),
        ),
      ).toBe(true);
    } finally {
      rmSync(cwd, { recursive: true, force: true });
  ...
  it.each(['error', 'exit', 'signal'])(
    'does not report readiness after child %s before publication',
    async (kind) => {
...
describe('recovery owner bootstrap candidates', () => {
  it.skipIf(process.platform !== 'linux')(
    'retries the same canonical request with a second child after the first candidate dies pre-epoch',
    async () => {
      const cwd = mkdtempSync(
        join(tmpdir(), 'runtime-owner-bootstrap-candidates-'),
      );
      try {
        const teamName = 'recovery-team';
        ...
        await recoveryOwnerBootstrapTestHooks.publishCandidate(
          baseInput,
          recoveryId,
          1,
          'dead-child',
        ...
        expect(
          recoveryOwnerBootstrapTestHooks.hasLiveOrUnknownCandidate(
            baseInput,
            recoveryId,
            1,
            null,
        ...
        const input: RecoverDeadWorkerOwnerInput = {
          ...baseInput,
          bootstrap: {
            expectedEpoch: 1,
            predecessorEpoch: 0,
        ...
        await expect(
          prepareRecoveryOwnerBootstrap(input),
        ).resolves.toBeUndefined();
        expect(readRecoveryOutcome(cwd, requestId)).toBeNull();
        const bound = JSON.parse(readFileSync(configPath, 'utf8')) as {
          active_recovery?: { request_id: string };
          runtime_owner_epoch?: { nonce: string };
        };
        expect(bound.runtime_owner_epoch?.nonce).toBe('successor-child');
        expect(bound.active_recovery?.request_id).toBe(requestId);

        await expect(
          executeRecoverDeadWorkerV2Owner(input),
        ).resolves.toMatchObject({
          outcome: 'failed',
        ...
        expect(readRecoveryOutcome(cwd, requestId)).toMatchObject({
          kind: 'final',
          result: { error: 'worker_not_found' },
        });
        const finalized = JSON.parse(readFileSync(configPath, 'utf8')) as {
          active_recovery?: unknown;
          last_recovery?: { request_id: string };
        };
  ...
  it('fails closed when a same-epoch bootstrap candidate sibling is malformed', () => {
    const cwd = mkdtempSync(
      join(tmpdir(), 'runtime-owner-bootstrap-malformed-'),
    );
    try {
      const input = {
        teamName: 'recovery-team',
        cwd,
        workerName: 'worker-1',
        requestId: 'candidate-malformed-request',
      };
      const path = absPath(
        cwd,
        TeamPaths.recoveryOwnerBootstrapCandidate(
          input.teamName,
          1,
          'malformed-child',
        ),
