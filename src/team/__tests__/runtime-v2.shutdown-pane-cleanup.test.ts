...
describe('shutdownTeamV2 split-pane pane cleanup', () => {
  ...
  it('preserves the owned pane and state when provider launch identity is missing', async () => {
    ...
    await expect(
      shutdownTeamV2(teamName, cwd, { timeoutMs: 0 }),
    ...
    expect(
      tmuxCalls.some(
        (args) => args[0] === 'kill-window' || args[0] === 'kill-session',
      ),
    ).toBe(false);
    await expect(
      readFile(join(cwd, teamRoot, 'config.json'), 'utf-8'),
    ).resolves.toContain('pane-cleanup-team');
  });
  it('retires and terminates the exact provider while accepting a proven-dead pane', async () => {
    ...
    await expect(
      awaitWorkerLaunchProviderStarted(attempt, {
        timeoutMs: 10_000,
        pollIntervalMs: 5,
      }),
    ).resolves.toBe(true);
    const providerPid = JSON.parse(await readFile(attempt.startedPath, 'utf8'))
      .pid as number;
    await writeJson(cwd, `${teamRoot}/config.json`, {
      name: teamName,
      task: 'demo',
    ...
    expect(
      tmuxCalls.some((args) => args[0] === 'kill-pane' && args[2] === '%2'),
    ).toBe(false);
    await expect(
      readFile(join(cwd, teamRoot, 'config.json'), 'utf-8'),
