...
const baseConfig = (overrides: Record<string, unknown> = {}) =>
  ({
    ...
    runtime_owner_epoch: { epoch: 1, nonce: 'one' },
    ...overrides,
  }) as TeamConfig;
...
describe('runtime owner epochs', () => {
  ...
  it('makes simultaneous successors observe the winning record without reclaiming or leaving temporary aliases', () => {
    const first = publishOwnerEpoch(cwd, teamName, 1, {
      pid: process.pid,
      processStartedAt: start!,
      nonce: 'first',
    });
    const second = publishOwnerEpoch(cwd, teamName, 1, {
      pid: process.pid,
      processStartedAt: start!,
      nonce: 'second',
    });
    expect(second).toEqual(first);
    expect(readdirSync(absPath(cwd, TeamPaths.ownerEpochs(teamName)))).toEqual([
      '1.json',
    ]);
  });
  ...
  it('rejects a successor-election loser that observes another process identity as winner', () => {
    ...
    expect(() =>
      requireOwnerProcessIdentity(loserObserved, process.pid, start),
    ).toThrow('runtime_owner_fence_lost');
  });
  ...
  it('derives subsecond process-start identities from native macOS sysctl and exact Windows ticks', () => {
    ...
    const exec = vi.fn((file: string) =>
      file === 'powershell.exe' ? '638878752000000000\n' : kinfo,
    ) as unknown as typeof import('node:child_process').execFileSync;
    expect(processStartIdentityForPlatform(42, 'darwin', exec)).toBe(
      'darwin:1783701296:123456',
    );
    expect(processStartIdentityForPlatform(42, 'win32', exec)).toBe(
      'win32:638878752000000000',
    );
  ...
  it('refuses a successor while a process remains live even when its heartbeat is stale, but allows confirmed-dead takeover', () => {
    publishOwnerEpoch(cwd, teamName, 1, {
      ...
      processStartedAt: start!,
      nonce: 'live',
      heartbeat: { observed_at: '2000-01-01T00:00:00.000Z' },
    });
    expect(() =>
      acquireSuccessorOwnerEpoch(cwd, teamName, {
        pid: process.pid,
        processStartedAt: start!,
        nonce: 'blocked',
      }),
    ).toThrow('runtime_owner_not_confirmed_dead');
    ...
    ).toMatchObject({ epoch: 2, nonce: 'successor' });
  });
  ...
  it('fences stale predecessors and recognizes only the exact fresh, rebind, active, and maintenance predicates', () => {
    ...
    publishOwnerEpoch(cwd, teamName, 2, {
      pid: process.pid,
      processStartedAt: start!,
      nonce: 'two',
    });
    expect(checkOwnerFence(cwd, teamName, { epoch: 1, nonce: 'one' })).toEqual({
      ok: false,
      reason: 'superseded',
    });
    expect(() =>
      requireOwnerFence(cwd, teamName, { epoch: 1, nonce: 'one' }),
    ).toThrow('runtime_owner_fence_lost');
    expect(
      isFreshRecoveryElection(baseConfig(), { epoch: 1, nonce: 'one' }, 7),
    ).toBe(true);
    const prior = {
      epoch: 1,
      nonce: 'one',
    ...
    expect(
      isSameAttemptSuccessorRebind(
        baseConfig({ active_recovery: attempt }),
        prior,
        { epoch: 2, nonce: 'two' },
        'request',
        'recovery',
    ...
    expect(
      isActiveRecoveryEffect(
        baseConfig({
          runtime_owner_epoch: { epoch: 2, nonce: 'two' },
          active_recovery: { ...attempt, owner_epoch: 2, owner_nonce: 'two' },
        }),
        { epoch: 2, nonce: 'two' },
        'request',
        'recovery',
    ...
    expect(
      isFencedServiceMaintenance(
        baseConfig({
          runtime_owner_epoch: { epoch: 2, nonce: 'two' },
          service_recovery: { epoch: 2, nonce: 'two' },
        }),
        { epoch: 2, nonce: 'two' },
      ),
    ).toBe(true);
  ...
  it('treats blank or malformed live-PID identities as unverifiable, never dead', () => {
    ...
    expect(
      isProcessIdentityDead({ pid: process.pid, process_started_at: '' }),
    ).toBe(false);
    expect(
      isProcessIdentityDead({
        pid: process.pid,
    ...
    expect(
      isProcessIdentityDead({
        pid: 2_147_483_647,
        process_started_at: malformedSamePlatform,
      }),
    ).toBe(false);
    expect(
      isProcessIdentityDead({
        pid: 2_147_483_647,
        process_started_at: crossPlatform,
      }),
    ).toBe(false);
    expect(() =>
      publishOwnerEpoch(cwd, 'blank-owner-team', 1, {
        pid: process.pid,
        processStartedAt: '',
        nonce: 'blank-owner',
      }),
  ...
  it('never falls back to an older owner when the highest epoch is malformed', () => {
    ...
    expect(() => readLatestOwnerEpoch(cwd, teamName)).toThrow(
      'invalid_owner_epoch_record',
    );
    expect(checkOwnerFence(cwd, teamName, { epoch: 1, nonce: 'one' })).toEqual({
      ok: false,
      reason: 'malformed',
  ...
  it('rejects a valid signed owner epoch copied under a newer filename', () => {
    ...
    expect(checkOwnerFence(cwd, teamName, { epoch: 2, nonce: 'two' })).toEqual({
      ok: false,
      reason: 'malformed',
    });
  });
