...
describe('buildWorkerStartCommand', () => {
    ...
    const cmd = buildWorkerStartCommand({
      ...
      launchAttempt: {
        ...
        transportCleanupCompletePath:
          'C:\\state\\transport-cleanup-complete.json',
  ...
  it('escapes psmux cmd.exe env vars and quoted launch args without PowerShell syntax', () => {
    ...
    expect(cmd).toContain(
      'set "OMC_TEAM_STATE_ROOT=C:\\Users\\Test User\\AppData\\Local\\omc state"',
    );
    expect(cmd).toContain(
      'set "CLAUDE_CODE_USE_BEDROCK=value with spaces & [brackets] ""quotes"""',
    );
    expect(cmd).toContain(
  ...
  it('base64-encodes recovery gate launch identities for native Windows cmd', () => {
    ...
    expect(
      JSON.parse(
        Buffer.from(cmd.slice(encodedStart, encodedEnd), 'base64').toString(
          'utf8',
        ),
      ),
