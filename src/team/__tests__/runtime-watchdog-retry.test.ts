...
async function waitFor(
  ...
  while (Date.now() < deadline) {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, WATCHDOG_WAIT_INTERVAL_MS),
    );
  }
...
async function stopWatchdogAndSettle(stop: () => void): Promise<void> {
  stop();
  await new Promise((resolve) =>
    setTimeout(resolve, WATCHDOG_WAIT_INTERVAL_MS * 3),
  );
}

describe(
  "watchdogCliWorkers dead-pane retry behavior",
  { timeout: 15000 },
  () => {
    let cwd: string;
    ...
    beforeEach(async () => {
      ...
      vi.doMock("child_process", async (importOriginal) => {
        ...
        (mockExecFile as unknown as { [utilPromisify.custom]: unknown })[
    ...
    it("requeues task when dead pane still has retries remaining", async () => {
      ...
      try {
        await waitFor(() => {
          const retryCount =
            readTaskFailure(teamName, "1", { cwd })?.retryCount ?? 0;
      ...
      try {
        await waitFor(() => {
          ...
          const task1 = JSON.parse(
            readFileSync(join(root, "tasks", "1.json"), "utf-8"),
          ) as {
            status: string;
            owner: string | null;
          };
          const task3 = JSON.parse(
            readFileSync(join(root, "tasks", "3.json"), "utf-8"),
          ) as {
            status: string;
            owner: string | null;
          };
    ...
    it("serializes concurrent dead-pane retries across watchdog instances", async () => {
      ...
      try {
        await waitFor(
          () => (readTaskFailure(teamName, "1", { cwd })?.retryCount ?? 0) >= 1,
        );
      } finally {
        await Promise.all([
    ...
    it("does not requeue or increment retries when dead-pane detection races with completion", async () => {
      ...
      expect(
        warnSpy.mock.calls.some(([msg]: [unknown]) =>
          String(msg).includes("dead pane — requeuing task"),
        ),
      ).toBe(false);
    });
