...
describe('cli-detection', () => {
  ...
  it('resolves and enriches a POSIX CLI with the first absolute result', () => {
    ...
    mockSpawnSync
      .mockReturnValueOnce(
        spawnResult({
          status: 0,
          stdout: '/opt/tools/codex\n/usr/bin/codex\n',
        }),
      )
      .mockReturnValueOnce(
        spawnResult({ status: 0, stdout: 'codex 1.0.0\nsecond line\n' }),
      );
  ...
  it('uses the modeled Win32 finder and path flavor independent of the host', () => {
    ...
    mockSpawnSync
      ...
      .mockReturnValueOnce(
        spawnResult({ status: 0, stdout: 'codex 2.0.0\r\n' }),
      );
  ...
  it('uses POSIX path semantics when the host is modeled as POSIX', () => {
    ...
    expect(probeCli('codex')).toMatchObject({
      found: true,
      path: '/opt/codex',
      version: 'codex 3.0.0',
    });
  ...
  it.each([
    [
      ...
      spawnResult({
        status: null,
        signal: 'SIGTERM',
        error: { code: 'ETIMEDOUT' } as NodeJS.ErrnoException,
      }),
    ],
    [
      'finder start error',
    ...
    [
      'finder only relative output',
      spawnResult({ status: 0, stdout: 'relative-tool\r\nother-tool\n' }),
    ],
  ])(
    'returns a resolver error for %s without a version spawn',
    (_name, finderResult) => {
      restorePlatform = setProcessPlatform('linux');
  ...
  it.each([
    ...
    'codex&name',
    'café',
  ])('rejects unsafe bare binary name %j before spawning', (binary) => {
  ...
  it('maps direct status-zero output to the public and legacy results', () => {
    ...
    mockSpawnSync
      .mockReturnValueOnce(
        spawnResult({ status: 0, stdout: '/usr/bin/codex\n' }),
      )
      .mockReturnValueOnce(
  ...
  it('retains found but maps a failed direct version probe to legacy unavailable', () => {
    ...
    mockSpawnSync
      ...
      .mockReturnValueOnce(
        spawnResult({ status: 1, stdout: '', stderr: 'not supported' }),
      );
  ...
  it.each(['ENOENT', 'UNKNOWN', 'EINVAL'])(
    'uses the closed batch fallback for direct %s',
    (code) => {
      restorePlatform = setProcessPlatform('win32');
      ...
      mockSpawnSync
        .mockReturnValueOnce(
          spawnResult({
            status: 0,
            stdout: 'C:\\Program Files\\Provider\\provider.cmd\r\n',
          }),
        )
        .mockReturnValueOnce(
          spawnResult({
            status: null,
            signal: null,
            error: { code } as NodeJS.ErrnoException,
          }),
        )
        .mockReturnValueOnce(
          spawnResult({ status: 0, stdout: 'provider 6.0.0\r\n' }),
        );
      ...
      expect(mockSpawnSync).toHaveBeenNthCalledWith(
        ...
        [
          ...
          '/s',
          '/c',
          '""C:\\Program Files\\Provider\\provider.cmd" --version"',
        ],
        {
          timeout: 3000,
          encoding: 'utf8',
  ...
  it('maps a blank batch wrapper result like any other zero-exit version result', () => {
    ...
    mockSpawnSync
      .mockReturnValueOnce(
        spawnResult({ status: 0, stdout: 'C:\\Tools\\provider.cmd\r\n' }),
      )
      .mockReturnValueOnce(
        spawnResult({
          status: null,
          signal: null,
          error: { code: 'EINVAL' } as NodeJS.ErrnoException,
        }),
      )
      .mockReturnValueOnce(spawnResult({ status: 0, stdout: '\r\n' }));
  ...
  it('rejects a relative Windows resolver candidate before any version or COMSPEC call', () => {
    ...
    mockSpawnSync.mockReturnValueOnce(
      spawnResult({ status: 0, stdout: 'relative\\provider.cmd\r\n' }),
    );
  ...
  it.each([
    ...
    [
      'NUL path',
      `C:\\Tools\\provider${String.fromCharCode(0)}inject.cmd`,
      spawnResult({
        status: null,
        signal: null,
        error: { code: 'EINVAL' } as NodeJS.ErrnoException,
    ...
    [
      'completed zero-exit path',
      'C:\\Tools\\provider.cmd',
      spawnResult({ status: 0, signal: null, stdout: '' }),
    ],
    [
      'completed nonzero path',
      'C:\\Tools\\provider.cmd',
      spawnResult({
        status: 1,
        signal: null,
    ...
    [
      'missing direct error',
      'C:\\Tools\\provider.cmd',
      spawnResult({ status: null, signal: null }),
    ],
    [
      'unadmitted direct error',
      'C:\\Tools\\provider.cmd',
      spawnResult({
        status: null,
        signal: null,
    ...
    mockSpawnSync
      .mockReturnValueOnce(
        spawnResult({ status: 0, stdout: `${resolvedPath}\r\n` }),
      )
    ...
    expect(mockSpawnSync).toHaveBeenCalledTimes(2);
    expect(
      mockSpawnSync.mock.calls.every(
        (call) => (call[2] as { shell?: boolean } | undefined)?.shell !== true,
      ),
    ).toBe(true);
  });
  ...
  it('rejects an unsafe batch path with the fixed degradation error', () => {
    ...
    mockSpawnSync
      .mockReturnValueOnce(
        spawnResult({
          status: 0,
          stdout: 'C:\\Tools\\provider&inject.cmd\r\n',
        }),
      )
      .mockReturnValueOnce(
        spawnResult({
  ...
  it('preserves the legacy detectAllClis keys', () => {
    ...
    expect(Object.keys(detectAllClis())).toEqual([
      'claude',
      'codex',
      'gemini',
      'cursor',
