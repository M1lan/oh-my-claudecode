...
describe('runtime-cli legacy watchdog shutdown', () => {
  ...
  it('holds signal-triggered shutdown until startup ownership settles', async () => {
    ...
    let released = false;
    const waiting = barrier.waitForStartup().then(() => {
      released = true;
    });
    await Promise.resolve();
  ...
  it('does not publish a terminal result when shutdown cleanup fails', async () => {
    ...
    await expect(
      finalizeRuntimeShutdown(
        ...
        async () => {
          phases.push('shutdown');
          throw new Error('team_shutdown_provider_cleanup_unverified:worker-1');
        },
        async () => {
          phases.push('publish');
        },
...
describe('runtime-cli worker launch bootstrap', () => {
  it('rejects malformed launch JSON without echoing its secret payload', async () => {
    process.env.OMC_WORKER_LAUNCH_SPEC =
      '{"provider_argv":["codex","--token","SUPERSECRET"],';
    try {
  ...
  it('prioritizes explicit worker launch and recovery gate modes over inherited owner state', () => {
    const inheritedOwner = {
      OMC_RECOVERY_OWNER_INPUT: '{"requestId":"stale"}',
    };
    ...
    expect(
      selectRuntimeCliMode(
        ['node', 'runtime-cli.cjs', '--recovery-gate'],
        inheritedOwner,
      ),
    ).toBe('recovery-gate');
    expect(
      selectRuntimeCliMode(['node', 'runtime-cli.cjs'], inheritedOwner),
    ).toBe('recovery-owner');
  });
});
