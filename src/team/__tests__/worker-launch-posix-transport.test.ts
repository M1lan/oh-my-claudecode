...
import { buildWorkerStartCommand } from '../rmux-session.js';
import {
...
describe('POSIX supervised worker-launch transport (issue #3655)', () => {
  it('supervised POSIX writer materializes a descriptor the runtime CLI reader accepts and executes', async () => {
    ...
    const descriptorPath = extractEnvAssignment(
      startCmd,
      'OMC_WORKER_LAUNCH_SPEC_FILE',
    );
    ...
    applyEnvAssignments(startCmd, [
      'OMC_WORKER_LAUNCH_SPEC_FILE',
      'OMC_TEAM_WORKER',
      'OMC_WORKER_LAUNCH_ATTEMPT_ID',
    ]);
    exitSpy = vi
      .spyOn(process, 'exit')
    ...
    const marker = JSON.parse(await readFile(providerMarker, 'utf8')) as {
      attempt: string;
    ...
    await expect(
      cleanupWorkerLaunchTransport(attempt, 'test_cleanup'),
    ).resolves.toBe(true);
    await expect(
  ...
  it('accepts the materialized descriptor and consumes it exactly once', async () => {
    ...
    await expect(
      cleanupWorkerLaunchTransport(attempt, 'test_cleanup_after_consume'),
    ).resolves.toBe(true);
  });
  ...
  it('keeps descriptor/source conflict and invalid JSON fail-closed at the runtime CLI', async () => {
    ...
    await expect(runWorkerLaunchFromEnvironment()).rejects.toThrow(
      'worker_launch_invalid_spec_json',
    );
    delete process.env.OMC_WORKER_LAUNCH_SPEC;

    process.env.OMC_WORKER_LAUNCH_SPEC = '{}';
    process.env.OMC_WORKER_LAUNCH_SPEC_FILE = join(
      cwd ?? '',
      'conflicting-worker-launch.json',
    );
    await expect(runWorkerLaunchFromEnvironment()).rejects.toThrow(
      'worker_launch_spec_source_conflict',
    );
    delete process.env.OMC_WORKER_LAUNCH_SPEC;
    ...
    process.env.OMC_WORKER_LAUNCH_SPEC = '{"provider_argv":["codex"]}';
    await expect(runWorkerLaunchFromEnvironment()).rejects.toThrow(
      'worker_launch_descriptor_required',
    );
  });
