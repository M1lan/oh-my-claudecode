...
const processMocks = vi.hoisted(() => ({
  isProcessIdentityLive: vi.fn(
    async () => 'live' as 'live' | 'dead' | 'mismatch' | 'unknown',
  ),
}));
  ...
  return {
    ...
    rmuxExecAsync: vi.fn(async (args: string[]) => {
      ...
      if (args[0] === 'capture-pane') {
        const next =
          tmuxState.captures.length > 1
            ? tmuxState.captures.shift()
            : tmuxState.captures[0];
...
async function acceptedContext(
  provider: StartupPaneContext['provider'],
  ...
  await awaitWorkerLaunchAcknowledgement(attempt, {
    timeoutMs: 100,
    pollIntervalMs: 5,
  });
...
describe('worker pane startup safety', () => {
  it.each([
    ...
    'rejects %s before pane mutation',
    (reason, paneId, leaderPaneId, reservedPaneIds) => {
  ...
  it.each([
    [
      'wide visible command',
      'node runtime-cli.cjs --worker-launch',
      undefined,
      'codex',
    ],
    [
      'narrow wrapped command',
      'node runtime-\ncli.cjs --worker-\nlaunch',
      undefined,
      'claude',
    ],
    [
      'stale scrollback command',
      'old launch command\n› ready',
      undefined,
      'codex',
    ],
    [
      'alternate-screen repaint',
      '\u001b[?1049h\u001b[2Jprovider ui',
      undefined,
      'claude',
    ],
    ...
    [
      'ignored capture-pane -J',
      new Error('unknown option -- J'),
      undefined,
      'claude',
    ],
    ['in-session psmux', 'provider ui', '/tmp/psmux/default,11877,0', 'codex'],
  ] as const)(
    'accepts a stable Windows cmd wrapper independently of %s capture',
    async (_fixture, captured, tmuxEnv, provider) => {
      ...
      expect(launchSend?.at(-1)).toMatch(
        /^\.omc\\state\\team\\startup-team\\workers\\worker-1\\launch-attempts\\[0-9a-f-]+\\launch\.cmd$/,
      );
      ...
      const enterIndex = tmuxState.args.findIndex(
      ...
      expect(tmuxState.args[enterIndex]).toEqual([
        'send-keys',
        '-t',
        '%2',
        'Enter',
  ...
  it('clears an exact Codex directory selector before typing the inbox trigger', async () => {
    ...
    await expect(
      deliverStartupInbox(context, 'Read inbox.md, execute now.'),
    ).resolves.toEqual({
      ok: true,
    ...
    expect(
      tmuxState.args.findIndex((args) => args.at(-1) === '1'),
    ).toBeLessThan(
      tmuxState.args.findIndex(
        (args) => args.at(-1) === 'Read inbox.md, execute now.',
      ),
    );
  });
  ...
  it('does not retry Enter for unrelated pane text', async () => {
    ...
    await expect(
      retryStartupInboxSubmit(context, 'Read inbox.md, execute now.'),
    ).resolves.toBe(false);
    expect(
      tmuxState.args.some(
        (args) => args[0] === 'send-keys' && args.at(-1) === 'Enter',
  ...
  it('emits bounded redacted capture diagnostics and never treats failure as readiness', async () => {
    ...
    expect(diagnostic).toContain(
      'pane capture failed operation=startup-readiness pane=%2',
    );
  ...
  it('retires an accepted launch but preserves the pane when provider cleanup is unverified', async () => {
    ...
    await expect(
      spawnOwnedWorkerInPane('startup:0', ownership(), {
        teamName: 'startup-team',
        workerName: 'worker-1',
        envVars: {},
        launchArgs: ['--version'],
    ...
    const attemptsRoot = join(
      cwd,
      '.omc/state/team/startup-team/workers/worker-1/launch-attempts',
    );
    const files = await readdir(attemptsRoot, { recursive: true });
    expect(
      files.some((file) => String(file).endsWith('decision.json.retired')),
    ).toBe(true);
    expect(tmuxState.paneStatus).toBe('0 cmd\n');
    delete process.env.OMC_TEAM_START_ACK_TIMEOUT_MS;
  });
