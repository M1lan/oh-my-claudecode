...
const checkpoint = {
  ...
  task_version: 3,
  claim_token: token,
  resume_payload_hash: 'checkpoint-hash',
  resume_payload: { cursor: 4 },
  updated_at: '2026-01-01T00:00:00.000Z',
};
const liveTask = (): TeamTaskV2 =>
  ({
    id: '1',
    subject: 'Recover',
    description: 'task',
...
const recoveryInput = {
  ...
  adoptionTokenHash: 'adoption-hash',
};

function deps(
  tasks: Record<string, TeamTaskV2>,
  sidecars: Record<string, any> = {},
  writes: string[] = [],
): RecoveryTaskTransitionDeps {
  return {
    teamName,
    cwd: '/unused',
    readTeamConfig: async () => ({
      workers: [
        { name: 'dead-worker' },
        { name: 'replacement' },
        { name: 'generic' },
    ...
    withTaskClaimLock: async (_team, _id, _cwd, fn) => ({
      ok: true as const,
      value: await fn(),
    }),
    normalizeTask: (value) => value as TeamTaskV2,
    isTerminalTaskStatus: () => false,
    taskFilePath: (_team, id) => id,
    writeAtomic: async (path, data) => {
      writes.push(path);
      tasks[path] = JSON.parse(data) as TeamTaskV2;
    },
    readRecoverySidecar: async (_team, recoveryId, id) =>
      sidecars[`${recoveryId}:${id}`] ?? null,
    writeRecoverySidecar: async (_team, recoveryId, id, sidecar) => {
      writes.push(`sidecar:${recoveryId}:${id}`);
      sidecars[`${recoveryId}:${id}`] = sidecar;
    },
    selectRecoveryCheckpoint: async () => ({
      ok: true as const,
      checkpoint,
      path: '/checkpoint',
    }),
    ...
    verifyAdoptionToken: (candidate, hash) =>
      candidate === 'adoption-token' && hash === 'adoption-hash',
  };
}
...
describe('recovery reservation claim protocol', () => {
  it('writes a sidecar before the pending+reserved task projection and replays each crash boundary safely', async () => {
    ...
    expect(first).toMatchObject({
      ...
      task: {
        ...
        recovery_reservation: {
          ...
          adoption_token_hash: 'adoption-hash',
        },
      },
    });
    expect(writes).toEqual(['sidecar:recovery:1', '1']);
    expect(await requeueRecoveredTask(recoveryInput, d)).toMatchObject({
      ok: true,
      replayed: true,
    });
    ...
    const repaired = await requeueRecoveredTask(
      recoveryInput,
      deps(precommitTasks, precommitSidecars),
    );
    expect(repaired).toMatchObject({
      ...
      task: {
        status: 'pending',
        recovery_reservation: { replacement_worker: 'replacement' },
      },
    });
    const inconsistent = { '1': { ...liveTask(), version: 99 } };
    expect(
      await requeueRecoveredTask(
        recoveryInput,
        deps(inconsistent, precommitSidecars),
      ),
    ).toEqual({ ok: false, error: 'task_requeue_failed' });
  });
  ...
  it('uses an immutable sidecar for each recovery attempt of the same task', async () => {
    ...
    const adopted = await adoptRecoveryReservations(
      ['1'],
      'replacement',
      {
        recoveryId: 'recovery',
    ...
    expect(
      await releaseTaskClaim('1', adoptedClaimToken, 'replacement', d),
    ).toMatchObject({ ok: true });
    expect(await claimTask('1', 'dead-worker', 6, d)).toMatchObject({
      ok: true,
    });
    ...
    expect(sidecars['recovery:1']).toMatchObject({
      recovery_id: 'recovery',
      old_task_version: 3,
    });
    expect(sidecars['recovery-2:1']).toMatchObject({
      recovery_id: 'recovery-2',
      old_task_version: 7,
    });
  ...
  it('rejects generic claims while the pending reservation remains and preserves retry generation/token tuple', async () => {
    ...
    expect(await claimTask('1', 'generic', 4, deps(tasks))).toEqual({
      ok: false,
      error: 'claim_conflict',
    });
    expect(tasks['1'].recovery_reservation).toMatchObject({
      replacement_generation: 2,
      adoption_token_hash: 'adoption-hash',
    });
  });
  ...
  it('adopts runtime-owned reservations in task-id order, rejects wrong token/generation, and replays an already adopted prefix', async () => {
    ...
    expect(
      (
        await adoptRecoveryReservations(
          ['2', '1'],
          'replacement',
          {
            recoveryId: 'recovery',
    ...
    expect(tasks['1']).toMatchObject({
      status: 'in_progress',
      owner: 'replacement',
      recovery_adoption: { replacement_generation: 2 },
    });
    const replay = await adoptRecoveryReservations(
      ['1', '2'],
      'replacement',
      {
        recoveryId: 'recovery',
    ...
    expect(replay.map((result) => result.ok && result.replayed)).toEqual([
      true,
      true,
    ]);
  });
