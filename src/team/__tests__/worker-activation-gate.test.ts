...
let cwd: string;
beforeEach(() => {
  cwd = mkdtempSync(join(tmpdir(), 'recovery-gate-'));
});
afterEach(() => {
...
describe('worker recovery activation gate', () => {
  it('fails closed before direct provider spawn on Windows', async () => {
    const launchAttempt = await prepareWorkerLaunchAttempt({
      ...
      context: {
        kind: 'recovery',
        recovery_id: 'recovery-windows',
        replacement_generation: 1,
        pane_attempt_id: 'pane-windows',
      },
    });
    ...
    try {
      await expect(
        runWorkerActivationGate({
          recoveryId: 'recovery-windows',
          workerName: 'worker-1',
          replacementGeneration: 1,
          paneAttemptId: 'pane-windows',
          readyPath: join(cwd, 'ready'),
          activatePath: join(cwd, 'activate'),
          runPath: join(cwd, 'run'),
  ...
  it('spawns the provider only after matching activate and run records and publishes launched evidence', async () => {
    ...
    const launchAttempt = await acceptedAttempt(
      'worker-1',
      '%2',
      'recovery-a',
      2,
    ...
    expect(
      JSON.parse(readFileSync(`${runPath}.launched`, 'utf8')),
    ).toMatchObject({
      launch_attempt_id: launchAttempt.attempt_id,
      launch_nonce: launchAttempt.nonce,
      pane_attempt_id: 'attempt-a',
  ...
  it.each([0, 7])(
    'does not publish launched evidence for a provider that exits immediately with code %s',
    async (exitCode) => {
      ...
      const record = {
        recovery_id: 'recovery-early-exit',
        worker_name: 'worker-1',
        replacement_generation: 9,
        pane_attempt_id: 'attempt-early-exit',
        launch_attempt_id: launchAttempt.attempt_id,
        launch_nonce: launchAttempt.nonce,
        written_at: new Date().toISOString(),
      };
      ...
      await expect(
        runWorkerActivationGate({
          recoveryId: 'recovery-early-exit',
          workerName: 'worker-1',
          replacementGeneration: 9,
          paneAttemptId: 'attempt-early-exit',
          readyPath,
          activatePath,
          runPath,
          providerArgv: [process.execPath, '-e', `process.exit(${exitCode})`],
          launchAttempt,
          cwd,
          timeoutMs: 1_000,
          pollIntervalMs: 5,
      ...
      expect(
        JSON.parse(readFileSync(`${runPath}.terminal`, 'utf8')),
      ).toMatchObject({ outcome: 'exit', exit_code: exitCode });
    },
  );
  it('kills recovery provider descendants when the root exits immediately', async () => {
    ...
    const launchAttempt = await acceptedAttempt(
      'worker-1',
      '%9',
      'recovery-early-child',
      9,
      'attempt-early-child',
    );
    const record = {
      recovery_id: 'recovery-early-child',
      worker_name: 'worker-1',
      replacement_generation: 9,
      pane_attempt_id: 'attempt-early-child',
      launch_attempt_id: launchAttempt.attempt_id,
      launch_nonce: launchAttempt.nonce,
      written_at: new Date().toISOString(),
    };
    ...
    await expect(
      runWorkerActivationGate({
        recoveryId: 'recovery-early-child',
        workerName: 'worker-1',
        replacementGeneration: 9,
        paneAttemptId: 'attempt-early-child',
        readyPath,
        activatePath,
        runPath,
        providerArgv: [process.execPath, '-e', script],
        launchAttempt,
        cwd,
        timeoutMs: 1_000,
        pollIntervalMs: 5,
  ...
  it('does not publish launched evidence when the provider executable cannot spawn', async () => {
    ...
    await expect(
      runWorkerActivationGate({
        ...
        paneAttemptId: 'attempt-b',
        readyPath,
        activatePath,
        runPath,
        providerArgv: [join(cwd, 'missing-provider')],
        cwd,
        launchAttempt,
        timeoutMs: 1_000,
        pollIntervalMs: 5,
  ...
  it('kills the provider when durable launched evidence cannot be published', async () => {
    ...
    const launchAttempt = await acceptedAttempt(
      ...
      10,
      'attempt-marker-failure',
    );
    const record = {
      recovery_id: 'recovery-marker-failure',
      worker_name: 'worker-1',
      replacement_generation: 10,
      pane_attempt_id: 'attempt-marker-failure',
      launch_attempt_id: launchAttempt.attempt_id,
      launch_nonce: launchAttempt.nonce,
      written_at: new Date().toISOString(),
    };
    ...
    await expect(
      runWorkerActivationGate({
        recoveryId: 'recovery-marker-failure',
        workerName: 'worker-1',
        replacementGeneration: 10,
        paneAttemptId: 'attempt-marker-failure',
        readyPath,
        activatePath,
        runPath,
        providerArgv: [
  ...
  it.each([
    ...
    'rejects recovery markers when only %s differs',
    async (field, replacement) => {
      ...
      const record = {
        recovery_id: 'recovery-exact',
        worker_name: 'worker-1',
        replacement_generation: 8,
        pane_attempt_id: 'attempt-exact',
        launch_attempt_id: launchAttempt.attempt_id,
        launch_nonce: launchAttempt.nonce,
        written_at: new Date().toISOString(),
        [field]: replacement,
  ...
  it('rejects a stale recovery gate before the provider can run', async () => {
    ...
    const record = {
      ...
      pane_attempt_id: 'attempt-stale',
      launch_attempt_id: 'stale-launch',
  ...
  it('blocks a gate superseded after activation but before provider spawn', async () => {
    ...
    const record = {
      ...
      worker_name: 'worker-1',
      replacement_generation: 6,
      pane_attempt_id: 'attempt-old',
      launch_attempt_id: launchAttempt.attempt_id,
      launch_nonce: launchAttempt.nonce,
      written_at: new Date().toISOString(),
    };
    ...
    const gate = runWorkerActivationGate({
      ...
      workerName: 'worker-1',
      replacementGeneration: 6,
      paneAttemptId: 'attempt-old',
      readyPath,
      activatePath,
      runPath,
      providerArgv: [
    ...
    for (
      let index = 0;
      index < 200 && !existsSync(`${readyPath}.adoption-ready`);
      index++
    ) {
