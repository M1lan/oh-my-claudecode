...
import {
  ...
  resolveCommittedRecoveryManifestSync,
  resolveCommittedRecoveryPaneAttempt,
...
describe('runtime owner recovery finalization', () => {
  it('retains resumable active recovery and does not publish a final for transient failure', async () => {
    ...
    expect(
      await finalizeRecoveryOwnerResult(ownerInput, 'recovery-a', result, {
        readRevisionedConfig: async () => ({
          config: activeConfig(),
          stateRevision: 5,
        }),
        saveConfigAtRevision: async (config) => {
          saved.push(config);
          return true;
        },
        publishFinal,
    ...
    expect(saved[0]?.active_recovery).toMatchObject({
      recovery_id: 'recovery-a',
      phase: 'reserved',
      state_revision: 6,
    });
  ...
  it('does not terminalize a failure after a durable task reservation was written', async () => {
    ...
    const result = {
      outcome: 'failed' as const,
      committed: false as const,
      error: 'task_requeue_failed' as const,
    ...
    await expect(
      finalizeRecoveryOwnerResult(ownerInput, 'recovery-a', result, {
        readRevisionedConfig: async () => ({
          config: activeConfig(),
          stateRevision: 5,
        }),
    ...
    expect(
      await finalizeRecoveryOwnerResult(ownerInput, 'recovery-a', result, {
        readRevisionedConfig: async () => ({
          ...
          stateRevision: persistedConfig.state_revision ?? 5,
        }),
        saveConfigAtRevision: async (
          config,
          expectedRevision,
          _cwd,
          afterCommit,
          ...
          expect(config.last_recovery).toMatchObject({
            recovery_id: 'recovery-a',
            phase: 'adopted',
            state_revision: 6,
          });
        ...
        publishFinal: (_input, _recoveryId, published) => {
          order.push('final');
          return published;
        },
      }),
  ...
  it.each([
    ...
    'clears the bound attempt into exact last_recovery before publishing terminal %s',
    async (error) => {
      let persistedConfig = activeConfig();
      const order: string[] = [];
      const result = {
  ...
  it('does not clear an active recovery whose owner fence differs from the authoritative owner', async () => {
    ...
    const result = {
      outcome: 'failed' as const,
      committed: false as const,
      error: 'team_session_dead' as const,
      requestId: 'request-a',
      recoveryId: 'recovery-a',
      teamName: 'recovery-team',
      workerName: 'worker-1',
    ...
    expect(config.active_recovery).toMatchObject({
      recovery_id: 'recovery-a',
      owner_nonce: 'stale-owner',
    });
  ...
  it('does not publish final success when terminal cleanup loses the revision race', async () => {
    ...
    const finalized = await finalizeRecoveryOwnerResult(
      ownerInput,
      'recovery-a',
      result,
      {
  ...
  it.each([
    [
      'request id',
      (last: NonNullable<TeamConfig['last_recovery']>) => {
        last.request_id = 'other-request';
      },
    ],
    [
      'recovery id',
      (last: NonNullable<TeamConfig['last_recovery']>) => {
        last.recovery_id = 'other-recovery';
      },
    ],
    [
      'worker name',
      (last: NonNullable<TeamConfig['last_recovery']>) => {
        last.worker_name = 'worker-2';
      },
    ],
    [
      'owner epoch',
    ...
    [
      'embedded revision',
      (last: NonNullable<TeamConfig['last_recovery']>) => {
        last.state_revision = 5;
      },
    ],
  ])(
    'does not publish a prior cleanup with mismatched %s evidence',
    async (_name, mutate) => {
      const config = activeConfig();
  ...
  it('resumes the exact committed pane and pane-attempt identity', () => {
    ...
    expect(
      resolveCommittedRecoveryPaneAttempt(
        ...
        'recovery-a',
        2,
        { ...worker, recovery_id: 'older-recovery' },
      ),
    ).toBeNull();
  });
