...
function dependencies(
  order: string[],
  overrides: Partial<RecoverySagaDependencies> = {},
): RecoverySagaDependencies {
  return {
    cwd,
    getLiveness: async () => {
      order.push('liveness');
    ...
    persistActive: async () => {
      order.push('persist');
      return { stateRevision: 8, manifestSync: 'synced' };
    },
    activatePane: async () => {
      order.push('activate');
      return { ok: true };
...
describe('recovery saga ordering and rollback contract', () => {
  ...
  it('kills only the uncommitted pane attempt when config persistence fails', async () => {
    const order: string[] = [];
    const result = await runRecoverySaga(
      input,
      dependencies(order, {
        persistActive: async () => {
          order.push('persist');
  ...
  it('does not activate or run the provider when the committed manifest projection is unverified', async () => {
    ...
    const result = await runRecoverySaga(
      ...
      dependencies(order, {
        persistActive: async () => {
          order.push('persist');
          return { stateRevision: 8, manifestSync: 'repair_required' };
        },
      }),
    );
  ...
  it('resumes a committed pane without repeating config persistence or rollback killing', async () => {
    ...
    const result = await runRecoverySaga(
      ...
      dependencies(order, {
        ...
        persistActive: async () => {
          order.push('persist');
          throw new Error('must not persist committed pane');
        },
      }),
    );
  ...
  it('replays the same recovery identity after a later task fails requeue', async () => {
    ...
    const task2 = {
      ...task,
      id: '2',
      subject: 'Continue second task',
    } as TeamTask;
    let failSecond = true;
    const reserved = new Set<string>();
    const deps = dependencies(order, {
      listOwnedInProgressTasks: async () => [task, task2],
      validateCheckpoint: async (_teamName, candidate) => ({
        ok: true,
        sequence: Number(candidate.id) + 3,
      }),
    ...
    await expect(runRecoverySaga(input, deps)).resolves.toMatchObject({
      outcome: 'failed',
      error: 'task_requeue_failed',
      reservationsWritten: true,
    });
    expect(reserved).toEqual(new Set(['1']));
    expect(readRecoveryOutcome(cwd, input.requestId)).toMatchObject({
      kind: 'phase',
