...
describe('model-contract', () => {
  describe('backward-compat API shims', () => {
    ...
    it('resolveCliBinaryPath rejects unsafe names and paths', () => {
      const mockSpawnSync = vi.mocked(spawnSync);
      expect(() => resolveCliBinaryPath('../evil')).toThrow(
        'Invalid CLI binary name',
      );
    ...
    it('exposes compatibility test internals for path policy', () => {
      expect(
        _testInternals.UNTRUSTED_PATH_PATTERNS.some((p) => p.test('/tmp/evil')),
      ).toBe(true);
      expect(
        _testInternals.UNTRUSTED_PATH_PATTERNS.some((p) =>
          p.test('/usr/local/bin/claude'),
        ),
      ).toBe(false);
  ...
  describe('getContract', () => {
    ...
    describe('antigravity Windows headless guard (omc team)', () => {
      it('reports antigravity headless unsupported on win32, supported elsewhere', () => {
        expect(isHeadlessSupportedOnPlatform('antigravity', 'win32')).toBe(
          false,
        );
        expect(isHeadlessSupportedOnPlatform('antigravity', 'darwin')).toBe(
          true,
        );
        expect(isHeadlessSupportedOnPlatform('antigravity', 'linux')).toBe(
          true,
        );
      ...
      it('getPromptModeArgs builds antigravity args normally on non-Windows', () => {
        ...
        try {
          expect(getPromptModeArgs('antigravity', '/path/to/inbox.md')).toEqual(
            ['-p', '/path/to/inbox.md'],
          );
        } finally {
          restore();
      ...
      it('validateCliAvailable refuses antigravity on Windows with a clear message', () => {
        const restore = setProcessPlatform('win32');
        try {
          expect(() => validateCliAvailable('antigravity')).toThrow(
    ...
    it('blocks codex when external LLM is disabled', async () => {
      ...
      try {
        const { clearSecurityConfigCache } =
          await import('../../lib/security-config.js');
        clearSecurityConfigCache();
        expect(() => getContract('codex')).toThrow(
    ...
    it('blocks gemini when external LLM is disabled', async () => {
      ...
      try {
        const { clearSecurityConfigCache } =
          await import('../../lib/security-config.js');
        clearSecurityConfigCache();
        expect(() => getContract('gemini')).toThrow(
          'blocked by security policy',
        );
      } finally {
        if (origSecurity === undefined) {
  ...
  describe('buildLaunchArgs', () => {
    ...
    it('detects Claude bare mode only for non-empty ANTHROPIC_API_KEY', () => {
      expect(shouldUseClaudeBareMode({ ANTHROPIC_API_KEY: 'sk-test' })).toBe(
        true,
      );
    ...
    it('antigravity leads with --dangerously-skip-permissions (no --print; -p is appended later by getPromptModeArgs)', () => {
      const noModel = buildLaunchArgs('antigravity', {
        teamName: 't',
        workerName: 'w',
        cwd: '/tmp',
      });
      ...
      const withModel = buildLaunchArgs('antigravity', {
        teamName: 't',
        workerName: 'w',
        cwd: '/tmp',
      ...
      expect(withModel.indexOf('--dangerously-skip-permissions')).toBeLessThan(
        withModel.indexOf('--model'),
      );
    });
    it('antigravity appends extraFlags after the model flag', () => {
      const args = buildLaunchArgs('antigravity', {
        teamName: 't',
        workerName: 'w',
        cwd: '/tmp',
        model: 'm',
    ...
    it('grok includes --always-approve with no model and appends --model <m> when given', () => {
      const noModel = buildLaunchArgs('grok', {
        teamName: 't',
        workerName: 'w',
        cwd: '/tmp',
      });
      ...
      const withModel = buildLaunchArgs('grok', {
        teamName: 't',
        workerName: 'w',
        cwd: '/tmp',
        model: 'grok-4-fast',
    ...
    it('passes Bedrock ARN model ID through without normalization (issue #1695)', () => {
      const arn =
        'arn:aws:bedrock:us-east-2:123456789012:inference-profile/global.anthropic.claude-sonnet-4-6-v1:0';
      const args = buildLaunchArgs('claude', {
        teamName: 't',
        workerName: 'w',
        cwd: '/tmp',
  ...
  describe('getWorkerEnv', () => {
    ...
    it('propagates allowlisted model selection env vars into worker startup env', () => {
      ...
      expect(env.CLAUDE_CODE_BEDROCK_OPUS_MODEL).toBe(
        'us.anthropic.claude-opus-4-6-v1:0',
      );
      expect(env.CLAUDE_CODE_BEDROCK_SONNET_MODEL).toBe(
        'us.anthropic.claude-sonnet-4-6-v1:0',
      );
      expect(env.CLAUDE_CODE_BEDROCK_HAIKU_MODEL).toBe(
    ...
    it('rejects invalid team names', () => {
      expect(() => getWorkerEnv('Bad-Team', 'worker-1', 'codex')).toThrow(
        'Invalid team name',
      );
    });
  });
  ...
  describe('buildWorkerArgv', () => {
    it('builds codex interactive worker argv without the exec subcommand', () => {
      ...
      const argv = buildWorkerArgv('codex', {
        teamName: 'my-team',
        workerName: 'worker-1',
        cwd: '/tmp',
      });
      ...
      expect(mockSpawnSync).toHaveBeenCalledWith('which', ['codex'], {
        timeout: 5000,
        encoding: 'utf8',
      });
      mockSpawnSync.mockRestore();
    ...
    it('builds claude interactive worker argv without the exec subcommand', () => {
      ...
      expect(argv).not.toContain('exec');
      expect(mockSpawnSync).toHaveBeenCalledWith('which', ['claude'], {
        timeout: 5000,
        encoding: 'utf8',
      });
      mockSpawnSync.mockRestore();
  ...
  describe('isCliAvailable', () => {
    ...
    it('uses shell:true for unresolved binaries on win32', () => {
      ...
      expect(mockSpawnSync).toHaveBeenNthCalledWith(1, 'where', ['gemini'], {
        timeout: 5000,
        encoding: 'utf8',
      });
      expect(mockSpawnSync).toHaveBeenNthCalledWith(
  ...
  describe('resolveClaudeWorkerModel (issue #1695)', () => {
    ...
    it('returns ANTHROPIC_MODEL on Bedrock when set', () => {
      ...
      vi.stubEnv('CLAUDE_MODEL', '');
      expect(resolveClaudeWorkerModel()).toBe(
        'us.anthropic.claude-sonnet-4-5-20250929-v1:0',
      );
      vi.unstubAllEnvs();
    });
    ...
    it('returns ANTHROPIC_MODEL on Vertex when set', () => {
      ...
      vi.stubEnv('ANTHROPIC_MODEL', 'vertex_ai/claude-sonnet-4-6@20250514');
      expect(resolveClaudeWorkerModel()).toBe(
        'vertex_ai/claude-sonnet-4-6@20250514',
      );
      vi.unstubAllEnvs();
    });
