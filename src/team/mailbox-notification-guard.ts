...
function resolveCanonicalTarget(
  ...
  return {
    target: {
      ...
      ...(typeof worker.index === 'number' && Number.isFinite(worker.index)
...
function ownershipFailureReason(
  ownership: MailboxTargetOwnership,
): MailboxNotificationGuardReason | null {
...
export function evaluateMailboxNotificationGuard(
  ...
  if (
    request.worker_index !== undefined &&
    request.worker_index !== target.workerIndex
  ) {
    return {
      kind: 'suppress',
      reason: 'mailbox_target_metadata_mismatch',
  ...
  if (
    ...
    message.to_worker !== input.recipient
  ) {
    return {
      kind: 'suppress',
  ...
  if (ownership.kind !== 'owned') {
    return {
      kind: 'suppress',
      reason:
        ownershipFailureReason(ownership) ?? 'mailbox_membership_unresolvable',
      safePendingRequest,
      target,
    };
  }
  ...
  return {
    ...
    securityTuple: {
      ...
      ...(target.workerIndex !== undefined
        ? { canonicalWorkerIndex: target.workerIndex }
        : {}),
      ...
      ...(request.pane_id !== undefined
        ? { requestPaneId: request.pane_id }
        : {}),
      ...(request.worker_index !== undefined
        ? { requestWorkerIndex: request.worker_index }
        : {}),
...
export async function readCurrentMailboxNotificationGuard(
  ...
  const readStrictDispatch =
    dependencies.readStrictDispatchRequest ?? readDispatchRequestStrict;
  const readStrictMailbox =
    dependencies.readStrictMailboxMessage ??
    teamReadCanonicalMailboxMessageStrict;
  ...
  const [config, dispatch, mailbox] = await Promise.all([
    ...
    readStrictDispatch(input.teamName, input.requestId, cwd).catch(
      () => ({ kind: 'malformed_store', cause: 'json' }) as const,
    ),
    readStrictMailbox(
      input.teamName,
      input.recipient,
  ...
  if (
    initial.kind !== 'suppress' ||
    initial.reason !== 'mailbox_membership_unresolvable' ||
    !initial.target
  ) {
    return initial;
  }
  ...
  const ownership = dependencies.verifyProviderOwnership
    ? await dependencies
        .verifyProviderOwnership(initial.target)
