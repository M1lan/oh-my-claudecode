...
} from '../cli/rmux-clipboard.js';
import type {
  MailboxNotificationTarget,
  MailboxTargetOwnership,
...
export function detectTeamMultiplexerContext(
  ...
  if (env.CMUX_SURFACE_ID) {
    ...
    // detached session (resolveMultiplexerInvocation() still prefers rmux/tmux
...
function redactCmuxFailureMessage(
  error: unknown,
  argLists: string[][],
): string {
...
async function cmuxSplitSurface(
  targetSurfaceId: string,
  direction: 'right' | 'down',
  _cwd: string,
): Promise<{ stdout: string; stderr: string; paneId: string | null }> {
  ...
  try {
    ...
    /* successful split with unparseable identity */
  }
...
async function cmuxSendSurfaceKey(
  surfaceId: string,
  key: string,
): Promise<void> {
...
function parseCmuxResourceIds(
  output: string,
  collectionName: 'panes' | 'surfaces',
): string[] | null {
  let parsed: unknown;
  try {
  ...
  const entries = Array.isArray(parsed)
    ...
    : parsed &&
        ...
        Array.isArray((parsed as Record<string, unknown>)[collectionName])
...
type MailboxOwnershipCommand = (
  args: string[],
) => Promise<{ stdout: string; stderr: string }>;

export interface MailboxTargetOwnershipDependencies {
  rmuxExec: MailboxOwnershipCommand;
...
export async function verifyTeamTargetOwnership(
  ...
  const expectedProvider = target.providerTarget.startsWith('cmux:')
    ? 'cmux'
    : 'tmux';
...
export async function invokeDirectMailboxEffect(
  ...
  try {
    const notified =
      target.recipientRole === 'leader'
        ? await dependencies.sendLeader(
            target.providerTarget,
            target.paneId,
            message,
          )
    ...
    return notified
      ? {
          ...
          reason:
            target.recipientRole === 'leader'
              ? 'leader_pane_notified'
              : 'worker_pane_notified',
        }
      : {
          kind: 'attempted_unconfirmed',
          transport: 'tmux_send_keys',
...
export function redactBoundedDiagnostic(
  error: unknown,
  maxLength = 240,
): string {
  ...
  const redacted = raw
    ...
    .replace(
      /("[A-Za-z_][A-Za-z0-9_]*(?:TOKEN|SECRET|PASSWORD|API_KEY|CREDENTIALS?)"\s*:\s*)"[^"]*"/gi,
      '$1"<redacted>"',
    )
    .replace(
      /(--?[A-Za-z0-9_-]*(?:api[-_]?key|token|secret|password|credential|auth)[A-Za-z0-9_-]*)(?:=|\s+)(?:'[^']*'|"[^"]*"|\S+)/gi,
    ...
    .replace(
      /\b[A-Za-z_][A-Za-z0-9_]*(?:TOKEN|SECRET|PASSWORD|API_KEY|CREDENTIALS?)=[^\s;]+/gi,
      '<redacted>',
    )
    .replace(/\s+/g, ' ')
    .trim();
...
async function verifyWorkerStartCommandSubmitted(
  ...
  while (Date.now() < deadline) {
    const captured = await capturePaneAsync(paneId, { joinWrappedLines: true });
    const normalizedCaptured = normalizeTmuxCapture(captured);
    const commandStillBuffered =
      normalizedCaptured.includes(expected) ||
      (compactExpected.length > 0 &&
        normalizeTmuxCaptureForDelivery(captured).includes(compactExpected));
...
export function buildWorkerStartCommand(config: WorkerPaneConfig): string {
  ...
  if (process.platform === 'win32' && !isUnixLikeOnWindows()) {
    ...
    if (windowsEnvVars.OMC_WORKER_LAUNCH_SPEC) {
      windowsEnvVars.OMC_WORKER_LAUNCH_SPEC_B64 = Buffer.from(
        windowsEnvVars.OMC_WORKER_LAUNCH_SPEC,
        'utf8',
      ).toString('base64');
      delete windowsEnvVars.OMC_WORKER_LAUNCH_SPEC;
    }
    if (windowsEnvVars.OMC_RECOVERY_GATE_SPEC) {
      windowsEnvVars.OMC_RECOVERY_GATE_SPEC_B64 = Buffer.from(
        windowsEnvVars.OMC_RECOVERY_GATE_SPEC,
        'utf8',
      ).toString('base64');
    ...
    const envPrefix = Object.entries(windowsEnvVars)
      ...
      .join(' && ');
    const launch = launchWords
      .map((part) => `"${escapeForCmdSet(part)}"`)
      .join(' ');
...
export function validateTmux(hasTmuxContext = false): void {
  ...
  try {
    rmuxShell('-V', { stripTmux: true, timeout: 5000, stdio: 'pipe' });
  } catch {
    throw new Error(
...
export function createSession(
  ...
  rmuxExec(args, { stripTmux: true, stdio: 'pipe', timeout: 5000 });
  try {
    ...
    /* non-fatal — older tmux builds may not support these options */
  }
...
export type WorkerPaneOwnershipResult =
  ...
  | {
      ...
      reason:
        ...
        | 'leader_alias'
        | 'split_target_alias'
        | 'reserved_worker_alias'
        | 'pane_foreign'
        | 'pane_membership_unavailable';
    };
...
function paneIdentityIsProviderNative(
  provider: WorkerPaneSplitEvidence['provider'],
  paneId: string,
): boolean {
  ...
  return (
    paneId.length <= 256 &&
    !paneId.startsWith('%') &&
    /^[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(paneId)
  );
}
...
export function proveWorkerPaneOwnership(
  ...
  if (
    constraints.requireNewFromSplitTarget !== false &&
    evidence.paneId === evidence.splitTarget
  ) {
...
export async function splitTeamWorkerPaneWithEvidence(
  ...
  direction: 'right' | 'down',
  cwd: string,
  provider: WorkerPaneSplitEvidence['provider'] = isCmuxContext()
    ? 'cmux'
    : 'tmux',
...
export async function splitTeamWorkerPane(
  ...
  return (await splitTeamWorkerPaneWithEvidence(splitTarget, direction, cwd))
    .paneId;
}
...
export async function createTeamSession(
  ...
  await Promise.all(
    workerPaneIds.map((workerPaneId) =>
      waitForShellReady(workerPaneId, { timeoutMs: 5_000 }),
    ),
  );
...
export async function spawnWorkerInPane(
  ...
  const requireAcknowledgement = async (): Promise<void> => {
    ...
    if (!accepted.ok) {
      throw new Error(
        `worker_start_ack_${accepted.reason}:${config.workerName}:${paneId}:${config.launchAttempt.attempt_id.slice(0, 12)}`,
      );
    }
    if (!(await awaitWorkerLaunchProviderStarted(config.launchAttempt))) {
      throw new Error(
        `worker_start_provider_failed:${config.workerName}:${paneId}:${config.launchAttempt.attempt_id.slice(0, 12)}`,
      );
    }
  };
  ...
  try {
    ...
    if (!shellReady) {
      throw new Error(
        `worker_start_shell_not_ready:${config.workerName}:${paneId}:${fingerprint}`,
      );
    }
    ...
    if (!config.launchAttempt) {
      ...
      if (!delivered) {
        throw new Error(
          `worker_start_delivery_unverified:${config.workerName}:${paneId}:${fingerprint}`,
        );
      }
    }
    ...
    if (nativeAttemptTransport) {
      ...
      logWorkerSpawnDiagnostic(
        `worker start post-enter observation session=${sessionName} pane=${paneId} ` +
          `worker=${config.workerName} cmdSha=${fingerprint} paneStatus=${JSON.stringify(
            status
              ? `${status.dead ? '1' : '0'} ${redactBoundedDiagnostic(status.command, 96)}`
              : 'unavailable',
          )} ` +
    ...
    if (config.launchAttempt) {
      ...
      const submitted = await verifyWorkerStartCommandSubmitted(
        paneId,
        startCmd,
      );
      if (!submitted) {
        throw new Error(
          `worker_start_submit_unverified:${config.workerName}:${paneId}:${fingerprint}`,
        );
      }
    }
  } catch (error) {
    if (config.launchAttempt) {
      await revokeWorkerLaunchAttempt(
        config.launchAttempt,
        'launch_failed',
      ).catch(() => undefined);
    }
    if (
      ...
      const cleaned = await cleanupWorkerLaunchTransport(
        config.launchAttempt,
        'launch_failed',
      ).catch(() => false);
      if (!cleaned) {
...
export async function spawnOwnedWorkerInPane(
  ...
  try {
    ...
    if (launchEnv.OMC_RECOVERY_GATE_SPEC) {
      const gate = JSON.parse(launchEnv.OMC_RECOVERY_GATE_SPEC) as Record<
        string,
        unknown
      >;
    ...
    if (!cleaned)
      throw new Error(
        `worker_launch_cleanup_unverified:${config.workerName}:${ownership.paneId}`,
      );
    throw error;
  }
}
...
async function capturePaneObservation(
  ...
  try {
    ...
    const operation = (opts.operation ?? 'capture')
...
export async function killOwnedWorkerPane(
  ownership: WorkerPaneOwnership,
): Promise<void> {
  const membership = await verifyTeamTargetOwnership({
  ...
  await rmuxExecAsync(['kill-pane', '-t', ownership.paneId]);
}

type PaneTrustPromptKind = 'directory' | 'codex_hooks';

function detectPaneTrustPromptKind(
  captured: string,
): PaneTrustPromptKind | null {
  ...
  const hasDirectoryQuestion = tail.some((l) =>
    /Do you trust the contents of this directory\?/i.test(l),
  );
  const hasDirectoryChoices = tail.some((l) =>
  ...
  const hasHookTrustChoice = tail.some((l) =>
    /Continue without trusting/i.test(l),
  );
  const hasHookConfirm = tail.some((l) =>
    /Press enter to confirm or esc to go back/i.test(l),
  );
...
export async function waitForPaneReady(
  ...
  console.warn(
    `[rmux-session] waitForPaneReady: pane ${paneId} timed out after ${timeoutMs}ms ` +
...
async function sendLiteralPaneText(
  ...
  await rmuxExecAsync(['send-keys', '-t', paneId, '-l', '--', text]);
}

async function startupContextIsActive(
  context: StartupPaneContext,
  attemptAlreadyFenced = false,
): Promise<boolean> {
  return (
    context.ownership.paneId === context.attempt.pane_id &&
    context.provider === context.attempt.provider &&
    (await isWorkerLaunchAttemptAccepted(context.attempt)) &&
    (attemptAlreadyFenced ||
...
export async function waitForStartupPaneReady(
  ...
  while (Date.now() < deadline) {
    ...
    if (selector) {
      const providerSupportsSelector =
        selector === 'codex_hooks'
...
function isPaneId(value: string | undefined): value is string {
  return (
    typeof value === 'string' &&
    (/^%\d+$/.test(value.trim()) || isCmuxSurfaceTarget(value))
  );
}
...
export async function killTeamSession(
  ...
  if (sessionMode === 'split-pane') {
    ...
    const provider = sessionName.startsWith('cmux:')
      ? ('cmux' as const)
      : ('tmux' as const);
    let cleaned = true;
    for (const id of workerPaneIds) {
      ...
      try {
        ...
        else await rmuxExecAsync(['kill-pane', '-t', id]);
      } catch {
        cleaned = false;
      }
    }
