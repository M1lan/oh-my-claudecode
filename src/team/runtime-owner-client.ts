...
export type RecoveryOwnerDispatch = (
  input: RecoverDeadWorkerOwnerInput,
) => Promise<RecoverDeadWorkerV2Result>;
...
function resolveCanonicalReservation(
  ...
  while (current.alias_of_request_id && !seen.has(current.request_id)) {
    ...
    if (
      !parent ||
      parent.operation !== reservation.operation ||
      parent.payload_hash !== reservation.payload_hash ||
      parent.workspace_hash !== reservation.workspace_hash ||
      parent.team_name !== reservation.team_name ||
...
function findActiveIdenticalReservation(
  cwd: string,
  ...
  try {
    for (const name of readdirSync(
      ...
      if (
        !reservation ||
        reservation.operation !== payload.operation ||
        reservation.payload_hash !== targetHash ||
        reservation.workspace_hash !== payload.workspaceHash ||
        reservation.team_name !== payload.teamName ||
    ...
    /* request root has not been created */
  }
  return null;
}

export function withRecoveryAdmissionLock<T>(
  cwd: string,
  payloadHash: string,
  fn: () => Promise<T> | T,
): Promise<T> {
...
function canonicalBootstrapCandidate(
  value: Omit<RecoveryOwnerBootstrapCandidate, 'payload_hash'>,
): string {
...
function validateBootstrapCandidate(
  ...
  if (
    ...
    candidate.predecessor_process_started_at !==
      (predecessor?.process_started_at ?? null) ||
    ...
    createHash('sha256')
      .update(
        canonicalBootstrapCandidate(
          unsigned as Omit<RecoveryOwnerBootstrapCandidate, 'payload_hash'>,
        ),
      )
...
async function publishRecoveryOwnerBootstrapCandidate(
  ...
  const candidate: RecoveryOwnerBootstrapCandidate = {
    ...
    payload_hash: createHash('sha256')
      .update(canonicalBootstrapCandidate(unsigned))
      .digest('hex'),
  };
  const path = recoveryOwnerBootstrapCandidatePath(
    input.cwd,
    input.teamName,
    expectedEpoch,
    nonce,
...
function hasLiveOrUnknownBootstrapCandidate(
  ...
  for (const entry of entries) {
    ...
    try {
      value = JSON.parse(
        readFileSync(join(candidateDirectory, entry), 'utf8'),
      ) as Partial<RecoveryOwnerBootstrapCandidate>;
    } catch {
      return true;
    }
    if (
...
export function parseRecoveryIntent(raw: string): RecoveryIntentRecord {
  ...
  if (
    ...
    intent.kind !== 'recover-worker' ||
    intent.operation !== 'recover-worker' ||
    typeof intent.workspace_hash !== 'string' ||
    !/^[a-f0-9]{64}$/.test(intent.workspace_hash) ||
    ...
    typeof intent.recovery_id !== 'string' ||
    intent.recovery_id.length === 0 ||
    typeof intent.team_name !== 'string' ||
    intent.team_name.length === 0 ||
    typeof intent.worker_name !== 'string' ||
    intent.worker_name.length === 0 ||
    intent.payload_hash !==
      canonicalRecoveryPayloadHash({
...
function validateRecoveryIntent(
  ...
  if (
    ...
    intent.recovery_id !== recoveryId ||
    intent.operation !== 'recover-worker' ||
    intent.workspace_hash !== expectedWorkspaceHash ||
    intent.payload_hash !== expectedPayloadHash ||
    intent.team_name !== input.teamName ||
...
async function publishIntent(
  ...
  try {
    validateRecoveryIntent(
      parseRecoveryIntent(await readFile(path, 'utf8')),
      input,
      recoveryId,
    );
    return;
  ...
  const intentPayload: RecoveryRequestPayload = {
    operation: 'recover-worker',
    workspaceHash: workspaceHashValue,
    teamName: input.teamName,
    workerName: input.workerName,
  };
  const intent: RecoveryIntentRecord = {
    schema_version: 1,
    kind: 'recover-worker',
    request_id: input.requestId,
    recovery_id: recoveryId,
...
async function teamRecoveryState(
  ...
  try {
    ...
    return validateLegacyTeamConfig(config, teamName)
      ? 'runtime_v2_required'
      : 'invalid_persisted_state';
  } catch {
...
function persistTerminalResult(
  input: RecoverDeadWorkerOwnerInput,
  recoveryId: string,
  result: RecoverDeadWorkerV2Failure,
): RecoverDeadWorkerV2Failure {
...
function ownerAvailability(cwd: string, teamName: string): OwnerAvailability {
  ...
  try {
    ...
    return currentProcessStartIdentity(owner.pid) === owner.process_started_at
      ? 'live'
      : 'unknown';
  } catch {
    return 'unknown';
...
export function isExpectedRecoveryOwnerSuccessor(
  ...
  return Boolean(
    ...
    owner.epoch === expectedEpoch &&
    owner.pid === childPid &&
    owner.process_started_at === childProcessStartedAt &&
    (!expectedNonce || owner.nonce === expectedNonce) &&
    fenceOk,
  );
}

async function bootstrapPersistentOwner(
  input: RecoverDeadWorkerOwnerInput,
  ...
  if (
    priorEpoch !== (predecessor?.epoch ?? null) ||
    (predecessor && !isProcessIdentityDead(predecessor)) ||
    !reservation ||
    reservation.kind !== 'reservation' ||
    reservation.recovery_id.length === 0 ||
    reservation.team_name !== input.teamName ||
  ...
  if (
    hasLiveOrUnknownBootstrapCandidate(
      input,
  ...
  try {
    child = recoveryOwnerSpawn(process.execPath, [resolveRuntimeCliPath()], {
      ...
      env: {
        ...
        OMC_RECOVERY_OWNER_PREDECESSOR_STARTED_AT:
  ...
  while (Date.now() < deadline) {
    ...
    const fenceOk = owner
      ? checkOwnerFence(input.cwd, input.teamName, {
          epoch: owner.epoch,
          nonce: owner.nonce,
        }).ok
      : false;
    if (
      isExpectedRecoveryOwnerSuccessor(
        owner,
        expectedEpoch,
        child.pid,
        childProcessStartedAt,
      ...
      configBound?.epoch === expectedEpoch &&
      configBound.nonce === bootstrapNonce &&
      configBound.pid === child.pid &&
      configBound.process_started_at === childProcessStartedAt &&
      active?.request_id === input.requestId &&
      active?.recovery_id === reservation.recovery_id &&
      active?.worker_name === input.workerName &&
      active?.owner_epoch === expectedEpoch &&
      active?.owner_nonce === bootstrapNonce
    ) {
    ...
    if (
      owner &&
      (owner.epoch > expectedEpoch ||
        (owner.epoch === expectedEpoch && owner.pid !== child.pid))
    )
      return false;
  ...
  return {
    async recoverDeadWorker(
      ...
      const admitted = await withProcessIdentityFileLock(
        ...
        async () => {
          const admission = await withRecoveryAdmissionLock(
            normalized.cwd,
          ...
          if (admission.kind === 'conflict')
            return { kind: 'conflict' as const, admission };
          const canonical = resolveCanonicalReservation(
            normalized.cwd,
      ...
      if (admitted.kind === 'conflict') {
        return {
          ...
          updatedAt: new Date().toISOString(),
          message:
            'Request ID is already reserved for a different recovery payload.',
        };
      }
      ...
      if (timing.persistentOwnerBootstrap) {
        const ownerReady = await withProcessIdentityFileLock(
          ...
          async () => {
            ...
            try {
              priorEpoch =
                readLatestOwnerEpoch(normalized.cwd, normalized.teamName)
                  ?.epoch ?? null;
            } catch {
              return null;
            }
      ...
      while (Date.now() < deadline) {
        ...
        if (finalState.kind === 'invalid')
          throw new Error('invalid_persisted_state');
        if (
          isMatchingRecoveryFinal(outcome, {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, timing.pollIntervalMs ?? 250),
        );
      }
...
export function setRuntimeOwnerDispatch(
  dispatch: RecoveryOwnerDispatch | undefined,
): void {
 ...
 */
export async function requestRuntimeOwnerRecovery(
  input: RecoverDeadWorkerOwnerInput,
): Promise<RecoverDeadWorkerV2Result> {
  const dispatch =
    installedRecoveryOwnerDispatch ??
    (async (ownerInput: RecoverDeadWorkerOwnerInput) => {
      const reservation = readRecoveryRequestReservation(
        ownerInput.cwd,
