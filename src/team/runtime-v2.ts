...
export function resolveTaskAssignment(
  ...
  const chosen = resolvedBinaryPaths[primaryProvider]
    ? pair.primary
    : pair.fallback;
  return {
...
function resolvePreflightBinaryPath(agentType: CliAgentType): {
  path: string;
  degraded: boolean;
  reason?: string;
} {
...
export async function executeRecoverDeadWorkerV2Owner(
  ...
  try {
    ...
    if (
      ...
      beforeOwner?.config.active_scale_up
    ) {
      ...
      spawnGatedPane: async (sagaInput) => {
        ...
        if (committedPane) {
          ...
          if (committedPaneLiveness === 'alive') {
            ...
            if (!pending) {
              try {
                pending = buildRecoveryPaneContext(
                  input,
                  sagaInput,
                  config,
                  currentWorker,
        ...
        if (!split.paneId) {
          await recordUnaddressableRecoveryPaneFailure(
            ...
            sagaInput.recoveryId,
            paneAttemptId,
            split.commandSucceeded
              ? 'unaddressable_spawned_pane'
              : 'split_command_uncertain',
            split,
          );
...
export async function shutdownTeamV2(
  ...
  const revalidateShutdownFence = async (): Promise<TeamConfig> =>
    withProcessIdentityFileLock(lifecycleLock, async () => {
      ...
      if (
        !current ||
        !['shutting_down', 'stopped'].includes(
  ...
  const commitStoppedFence = async (): Promise<void> =>
    withProcessIdentityFileLock(lifecycleLock, async () => {
      ...
      if (
        !(await saveTeamConfigAtRevision(stopped, current.stateRevision, cwd))
      )
        throw new Error('stale_state_revision');
    });
  ...
  try {
    ...
    if (
      config.tmux_session &&
      (ownsWindow || !config.tmux_session.includes(':'))
    ) {
