...
function publishImmutable<T>(target: string, value: T): T {
  ...
  try {
    ...
    try {
      ...
      /* unique losing temp cleanup is best-effort */
    }
...
export function canonicalRecoveryPayloadHash(
...
export function reserveRecoveryRequest(
  cwd: string,
  requestId: string,
  payload: RecoveryRequestPayload,
  ...
  try {
    ...
    return existing.operation === payload.operation &&
      ...
      ? { kind: 'joined', reservation: existing }
      : { kind: 'conflict', reservation: existing };
  }
}

/** Publish a new immutable request ID that points at an existing recovery. */
export function aliasActiveRecoveryRequest(
  cwd: string,
  requestId: string,
  payload: RecoveryRequestPayload,
  active: RecoveryRequestReservation,
): RequestReservationResult {
  assertSafeRecoveryRequestId(requestId);
  ...
  const payloadHash = canonicalRecoveryPayloadHash(payload);
  if (
    active.operation !== payload.operation ||
    active.payload_hash !== payloadHash ||
    active.team_name !== payload.teamName ||
    active.worker_name !== payload.workerName ||
    active.workspace_hash !== payload.workspaceHash
  )
...
export function readRecoveryRequestReservation(
  cwd: string,
  requestId: string,
): RecoveryRequestReservation | null {
  const reservation = parseCanonical<RecoveryRequestReservation>(
    reservationPath(cwd, requestId),
  );
  if (
    !reservation ||
    reservation.schema_version !== 1 ||
    (reservation.kind !== 'reservation' && reservation.kind !== 'alias') ||
    reservation.request_id !== requestId ||
    reservation.operation !== 'recover-worker' ||
    typeof reservation.payload_hash !== 'string' ||
    !/^[a-f0-9]{64}$/.test(reservation.payload_hash) ||
    typeof reservation.workspace_hash !== 'string' ||
    !/^[a-f0-9]{64}$/.test(reservation.workspace_hash) ||
    typeof reservation.team_name !== 'string' ||
    reservation.team_name.length === 0 ||
    typeof reservation.worker_name !== 'string' ||
    reservation.worker_name.length === 0 ||
    ...
    typeof reservation.recovery_id !== 'string' ||
    !isSafeRecoveryRequestId(reservation.recovery_id) ||
    typeof reservation.created_at !== 'string' ||
    !Number.isFinite(Date.parse(reservation.created_at)) ||
    typeof reservation.expires_at !== 'string' ||
    !Number.isFinite(Date.parse(reservation.expires_at)) ||
    (reservation.kind === 'alias' &&
      (typeof reservation.alias_of_request_id !== 'string' ||
        !isSafeRecoveryRequestId(reservation.alias_of_request_id))) ||
    (reservation.kind === 'reservation' &&
      reservation.alias_of_request_id !== undefined)
  )
    return null;
  return reservation;
}
...
function hasMatchingReservationTuple(
  left: RecoveryRequestReservation,
  right: RecoveryRequestReservation,
): boolean {
  return (
...
function resolveCanonicalRecoveryRequestId(
  cwd: string,
  requestId: string,
): string | null {
  assertSafeRecoveryRequestId(requestId);
...
function hasMatchingRecoveryPhaseTuple(
  phase: RecoveryOutcomePending,
  reservation: RecoveryRequestReservation,
): boolean {
  return (
    phase.request_id === reservation.request_id &&
...
function isValidRecoveryPhase(
  phase: RecoveryOutcomePending | null,
  reservation: RecoveryRequestReservation,
): phase is RecoveryOutcomePending {
  return (
    ...
    ['not_started', 'pending', 'synced', 'repair_required'].includes(
      phase.services,
    ) &&
    ...
    (phase.state_revision === undefined ||
      (Number.isSafeInteger(phase.state_revision) && phase.state_revision >= 0))
  );
}

export function writeRecoveryPhase(
  cwd: string,
  phase: RecoveryOutcomePending,
): RecoveryOutcomePending {
  ...
  return publishImmutable(
    join(phaseDirectory(cwd, phase.request_id), sequence),
    {
      ...phase,
      schema_version: 1,
      kind: 'phase' as const,
...
export function writeRecoveryFinal(
  cwd: string,
  outcome: RecoveryOutcomeFinal,
): RecoveryOutcomeFinal {
  const reservation = readRecoveryRequestReservation(cwd, outcome.request_id);
  if (
    !reservation ||
    reservation.kind !== 'reservation' ||
    reservation.recovery_id !== outcome.recovery_id ||
    reservation.team_name !== outcome.team_name ||
    reservation.worker_name !== outcome.worker_name
  ) {
    throw new Error('invalid_persisted_state');
  }
  const final = {
...
function latestPhase(
  ...
  try {
    ...
    const phase = parseCanonical<RecoveryOutcomePending>(
      join(directory, candidates[0]!),
    );
...
function isValidRecoveryResult(
  value: unknown,
): value is RecoverDeadWorkerV2Result {
  if (!value || typeof value !== 'object') return false;
  const result = value as Record<string, unknown>;
  if (
    typeof result.requestId !== 'string' ||
    typeof result.recoveryId !== 'string' ||
    typeof result.teamName !== 'string' ||
    typeof result.workerName !== 'string' ||
    typeof result.updatedAt !== 'string' ||
    !Number.isFinite(Date.parse(result.updatedAt as string)) ||
  ...
  if (result.outcome === 'recovered' || result.outcome === 'already_running') {
    if (
      ...
      (typeof result.oldPaneId !== 'string' && result.oldPaneId !== null) ||
      typeof result.newPaneId !== 'string' ||
      !result.newPaneId.trim() ||
      (result.outcome === 'recovered' &&
        (typeof result.oldPaneId !== 'string' || !result.oldPaneId.trim())) ||
      ...
      typeof result.stateRevision !== 'number' ||
      !Number.isSafeInteger(result.stateRevision) ||
      (result.activation !== 'active' &&
        result.activation !== 'services_pending') ||
      (result.manifestSync !== 'synced' &&
        result.manifestSync !== 'repair_required') ||
      (result.servicesSync !== 'synced' &&
        result.servicesSync !== 'repair_required') ||
      !isStringArray(result.warnings) ||
      !result.warnings.every((warning) => RECOVERY_WARNINGS.has(warning))
    )
      return false;
    return (
      result.outcome !== 'already_running' ||
      result.requeuedTaskIds.length === 0
    );
  }
  return (
    (result.outcome === 'failed' || result.outcome === 'commit_unknown') &&
    result.committed === false &&
    typeof result.error === 'string' &&
    RECOVERY_ERRORS.has(result.error as RecoverDeadWorkerV2Error) &&
...
export type RecoveryFinalState =
  ...
  | {
      kind: 'valid';
      final: RecoveryOutcomeFinal & { result: RecoverDeadWorkerV2Result };
    };

export function readRecoveryFinalState(
  cwd: string,
  requestId: string,
): RecoveryFinalState {
  ...
  if (
    ...
    final.kind !== 'final' ||
    !isMatchingRecoveryFinal(final, { requestId })
  ) {
    return { kind: 'invalid' };
  }
  const reservation = readRecoveryRequestReservation(cwd, requestId);
  if (
    !reservation ||
    reservation.kind !== 'reservation' ||
    reservation.recovery_id !== final.recovery_id ||
    reservation.team_name !== final.team_name ||
    reservation.worker_name !== final.worker_name
  )
    return { kind: 'invalid' };
  const byTeam = absPath(
    cwd,
  ...
  try {
    ...
    const indexed = existsSync(byTeam)
      ? parseCanonical<RecoveryOutcomeFinal>(byTeam)
      : null;
    if (!indexed || canonicalize(indexed) !== expectedBytes) {
      ...
      withProcessIdentityFileLockSync(lockPath, () => {
        const current = existsSync(byTeam)
          ? parseCanonical<RecoveryOutcomeFinal>(byTeam)
          : null;
...
export function isMatchingRecoveryFinal(
  ...
  if (
    !outcome ||
    outcome.kind !== 'final' ||
    !isValidRecoveryResult(outcome.result) ||
    outcome.request_id !== outcome.result.requestId ||
    outcome.recovery_id !== outcome.result.recoveryId ||
    outcome.team_name !== outcome.result.teamName ||
    outcome.worker_name !== outcome.result.workerName ||
    (expected.requestId !== undefined &&
      outcome.request_id !== expected.requestId) ||
    (expected.recoveryId !== undefined &&
      outcome.recovery_id !== expected.recoveryId) ||
    (expected.teamName !== undefined &&
      outcome.team_name !== expected.teamName) ||
    (expected.workerName !== undefined &&
      outcome.worker_name !== expected.workerName) ||
    typeof outcome.completed_at !== 'string' ||
    !Number.isFinite(Date.parse(outcome.completed_at)) ||
    typeof outcome.expires_at !== 'string' ||
    !Number.isFinite(Date.parse(outcome.expires_at)) ||
    !['none', 'selected', 'reserved', 'adopted'].includes(
      outcome.continuation,
    ) ||
    !['not_started', 'pending', 'adopted'].includes(outcome.adoption) ||
    !['synced', 'repair_required', 'terminal_degraded'].includes(
      outcome.services,
    ) ||
  ...
  if (
    ...
    (succeeded
      ? 'succeeded'
      : outcome.result.outcome === 'commit_unknown'
        ? 'commit_unknown'
        : 'failed')
  )
    return false;
  if (succeeded) {
    ...
    return (
      ...
      outcome.services === success.servicesSync &&
      outcome.manifest === success.manifestSync &&
      success.manifestSync === 'synced' &&
      success.activation ===
        (servicesPending ? 'services_pending' : 'active') &&
      (servicesPending
        ? success.warnings.length === 1 &&
          success.warnings[0] === 'services_pending'
  ...
  return (
    ...
    outcome.adoption === 'not_started' &&
    outcome.error?.code === failure.error &&
    outcome.error?.message === failure.message &&
    outcome.error?.commit_uncertain ===
      (failure.outcome === 'commit_unknown') &&
    outcome.services === 'terminal_degraded' &&
    outcome.manifest === 'repair_required'
  );
}
...
export function readRecoveryResult(
  cwd: string,
  requestId: string,
): RecoverDeadWorkerV2Result | null {
