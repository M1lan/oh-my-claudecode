...
async function publishImmutableCheckpoint(
  path: string,
  content: string,
  ...
  try {
    ...
    return (await readFile(path, 'utf8').catch(() => '')) === content
      ? 'replayed'
      : 'conflict';
  } finally {
...
function parseCheckpoint(value: unknown): TaskRecoveryCheckpoint | null {
  ...
  if (
    ...
    typeof checkpoint.team_name !== 'string' ||
    typeof checkpoint.task_id !== 'string' ||
    typeof checkpoint.worker_name !== 'string' ||
    ...
    taskVersion <= 0 ||
    typeof checkpoint.claim_token !== 'string' ||
    typeof checkpoint.resume_payload_hash !== 'string' ||
    typeof checkpoint.updated_at !== 'string'
  )
    return null;
  try {
    if (
      hashTaskRecoveryCheckpointPayload(checkpoint.resume_payload) !==
      checkpoint.resume_payload_hash
    )
      return null;
  } catch {
    return null;
...
async function readCheckpoint(
  path: string,
): Promise<TaskRecoveryCheckpoint | null> {
...
async function readCheckpointLatest(
  path: string,
...
export async function publishTaskRecoveryCheckpoint(
  ...
  const lock = await access.withTaskLock(
    input.teamName,
    input.taskId,
    cwd,
    async () => {
      ...
      if (
        ...
        task.claim.token !== input.claimToken
      ) {
...
export async function selectTaskRecoveryCheckpoint(
  ...
  if (parsed.some(({ checkpoint }) => !checkpoint))
    return { ok: false, error: 'malformed' };
  const valid = parsed as Array<{
    path: string;
    checkpoint: TaskRecoveryCheckpoint;
  }>;
  const matching = valid.filter(
    ({ checkpoint }) =>
      checkpoint.team_name === teamName &&
      checkpoint.task_id === task.id &&
      checkpoint.worker_name === task.owner &&
      checkpoint.task_version === task.version &&
      checkpoint.claim_token === task.claim?.token,
  );
  ...
  const otherHighest = valid.filter(
    ({ checkpoint }) =>
      checkpoint.sequence === highest &&
      checkpoint.resume_payload_hash !==
        selected[0].checkpoint.resume_payload_hash,
  );
...
export async function readTaskRecoveryCheckpoint(
  path: string,
): Promise<TaskRecoveryCheckpointValidation> {
  const checkpoint = await readCheckpoint(path);
  return checkpoint
    ? { ok: true, checkpoint, path }
    : { ok: false, error: existsSync(path) ? 'malformed' : 'missing' };
}
