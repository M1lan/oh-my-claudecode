...
const BROAD_OBJECTS_RE =
  /\b(?:runtime|system|codebase|architecture|workflow|pipeline|tests?|failures?|flaky|behavior|semantics|flow|feature|bug)\b/i;
...
export function isBroadTeamTaskText(text: string): boolean {
  ...
  const hasNarrowCodeTarget =
    FILE_REF_RE.test(normalized) || SYMBOL_REF_RE.test(normalized);
...
export function inferDelegationPlanForTeamTask(
  text: string,
): TeamTaskDelegationPlan | undefined {
  return isBroadTeamTaskText(text)
    ? { ...BROAD_TASK_DELEGATION_PLAN }
    : undefined;
}
