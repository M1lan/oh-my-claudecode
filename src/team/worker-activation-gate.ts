  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (
        ...
        value.launch_attempt_id === expected.launch_attempt_id &&
        value.launch_nonce === expected.launch_nonce
      )
        return true;
    } catch {
...
export async function runWorkerActivationGate(
  ...
  const fenced = await withWorkerLaunchAttemptFence(
    gate.launchAttempt,
    async () => {
      const {
      ...
      const invocation = await materializeProviderSpawnInvocation(
        buildProviderSpawnInvocation(gate.providerArgv),
        {
      ...
      let terminationResult: Promise<
        Awaited<ReturnType<typeof terminateOwnedProcessTree>>
      > | null = null;
      let finishCompletion!: (
        result: RecoveryActivationGateResult,
        terminal: Record<string, unknown>,
      ) => Promise<void>;
      const completion = new Promise<RecoveryActivationGateResult>(
        (resolve) => {
          const finish = async (
            result: RecoveryActivationGateResult,
            terminal: Record<string, unknown>,
            ...
            try {
              await writeAtomic(`${gate.runPath}.terminal`, {
                ...expected,
              ...
              /* owner may have already cleaned terminal attempt state */
            }
          ...
          child.once('error', () => {
            void finish(
              ...
              { outcome: 'error', cleanup_verified: false },
            );
          });
        },
      );
      ...
      try {
        ...
        providerStartIdentity = providerPid
          ? getProcessStartIdentitySync(providerPid)
          : null;
        if (
          !providerPid ||
          !providerStartIdentity ||
          settled ||
          !isProcessAlive(providerPid)
        ) {
        ...
        if (
          ...
          (await readFile(invocation.completionPath, 'utf8')
            .then(() => true)
            .catch(() => false))
        ) {
        ...
        await writeAtomic(`${gate.runPath}.launched`, {
          ...
          written_at: new Date().toISOString(),
          ...(invocation.completionPath
            ? { supervisor_completion_path: invocation.completionPath }
            : {}),
        });
