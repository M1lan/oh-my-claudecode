...
export async function withProcessIdentityFileLock<T>(
  ...
  const owner: ProcessIdentityLockRecord = {
    schema_version: 1,
    pid: process.pid,
  ...
  try {
      ...
      if (reclaimer) {
        if (isProcessIdentityDead(reclaimer)) {
          try {
            ...
            /* another contender reclaimed it */
          }
          continue;
        }
    ...
    if (readLock(reclaimPath)?.nonce === owner.nonce) {
      try {
        ...
        /* reclaim marker already released */
      }
    }
  }
}
...
export function withProcessIdentityFileLockSync<T>(
  lockPath: string,
  fn: () => T,
): T {
  const reclaimPath = `${lockPath}.reclaim`;
