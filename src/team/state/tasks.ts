...
function reservationFromSidecar(
  sidecar: TaskRecoveryRequeueSidecar,
): TeamTaskRecoveryReservation {
  return {
...
export async function requeueRecoveredTask(
  input: RequeueRecoveredTaskInput,
  deps: RecoveryTaskTransitionDeps,
  ...
  const lock = await deps.withTaskClaimLock(
    ...
    async () => {
      ...
      if (sidecar === 'malformed')
        return { ok: false as const, error: 'task_requeue_failed' as const };
      if (sidecar) {
        const reservation = reservationFromSidecar(sidecar);
        const sameAttempt =
          sidecar.recovery_id === input.recoveryId &&
          sidecar.request_id === input.requestId &&
          sidecar.task_id === input.taskId &&
          sidecar.replacement_worker === input.replacementWorker &&
          sidecar.replacement_generation === input.replacementGeneration &&
          sidecar.adoption_token_hash === input.adoptionTokenHash;
        if (!sameAttempt)
          return { ok: false as const, error: 'task_requeue_failed' as const };
        if (
          task.status === 'pending' &&
          ...
          !task.owner &&
          !task.claim &&
          JSON.stringify(task.recovery_reservation) ===
            JSON.stringify(reservation)
        )
          return { ok: true as const, task, reservation, replayed: true };
        if (
          task.status !== 'in_progress' ||
          task.version !== sidecar.old_task_version ||
          task.owner !== sidecar.old_owner ||
          task.claim?.owner !== sidecar.old_owner ||
          task.claim?.token !== sidecar.old_claim_token ||
          task.claim?.leased_until !== sidecar.old_claim_leased_until
        )
          return { ok: false as const, error: 'task_requeue_failed' as const };
        ...
        if (
          !checkpoint.ok ||
          checkpoint.checkpoint.resume_payload_hash !==
            sidecar.checkpoint_hash ||
          checkpoint.checkpoint.sequence !== sidecar.continuation_sequence
        )
          return { ok: false as const, error: 'task_requeue_failed' as const };
        const updated: TeamTaskV2 = {
          ...task,
      ...
      if (
        ...
        !task.owner ||
        !task.claim ||
        task.claim.owner !== task.owner ||
        task.recovery_reservation
      )
        return { ok: false as const, error: 'task_requeue_failed' as const };
      ...
      if (!selected.ok)
        return { ok: false as const, error: checkpointError(selected.error) };
...
export async function adoptRecoveryReservations(
  taskIds: string[],
  workerName: string,
  ...
  for (const taskId of [...taskIds].sort()) {
      ...
      async () => {
        ...
        if (!reservation) {
          if (
            ...
            task.claim &&
            task.recovery_adoption?.recovery_id === proof.recoveryId &&
            task.recovery_adoption.request_id === proof.requestId &&
            task.recovery_adoption.replacement_generation ===
              proof.replacementGeneration
          ) {
            const checkpoint = await deps.readRecoveryCheckpoint(
        ...
        if (
          ...
          task.owner ||
          task.claim ||
          reservation.recovery_id !== proof.recoveryId ||
          reservation.request_id !== proof.requestId ||
          reservation.replacement_worker !== workerName ||
          reservation.replacement_generation !== proof.replacementGeneration ||
          !deps.verifyAdoptionToken(
            proof.adoptionToken,
        ...
        if (
          !checkpoint.ok ||
          checkpoint.checkpoint.resume_payload_hash !==
            reservation.checkpoint_hash ||
          checkpoint.checkpoint.sequence !== reservation.continuation_sequence
        )
          return {
            ok: false as const,
        ...
        const updated: TeamTaskV2 = {
          ...
          version: task.version + 1,
          recovery_reservation: undefined,
          recovery_adoption: {
    ...
    const result: TaskRecoveryAdoptionResult = lock.ok
      ? lock.value
