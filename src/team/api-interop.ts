...
const TEAM_UPDATE_TASK_MUTABLE_FIELDS = new Set([
  'subject',
  'description',
  'blocked_by',
...
const TEAM_UPDATE_TASK_REQUEST_FIELDS = new Set([
  'team_name',
  'task_id',
  'workingDirectory',
  ...TEAM_UPDATE_TASK_MUTABLE_FIELDS,
]);
const RECOVER_WORKER_REQUEST_FIELDS = new Set([
  'team_name',
  'worker',
  'request_id',
  'timeout_ms',
...
const READ_RECOVERY_RESULT_REQUEST_FIELDS = new Set([
  'team_name',
  'request_id',
]);
...
function parseValidatedTaskIdArray(
  value: unknown,
  fieldName: string,
): string[] {
...
function parseTaskDelegationPlan(value: unknown): TeamTaskDelegationPlan {
  ...
  if ('max_parallel_subtasks' in raw) {
    if (
      !isFiniteInteger(raw.max_parallel_subtasks) ||
      raw.max_parallel_subtasks < 1
    ) {
      throw new Error(
  ...
  if ('spawn_before_serial_search_threshold' in raw) {
    if (
      !isFiniteInteger(raw.spawn_before_serial_search_threshold) ||
      raw.spawn_before_serial_search_threshold < 1
    ) {
      throw new Error(
        'delegation.spawn_before_serial_search_threshold must be a positive integer when provided',
      );
    }
    plan.spawn_before_serial_search_threshold =
      raw.spawn_before_serial_search_threshold;
  }
  ...
  if ('subtask_candidates' in raw) {
    if (
      !Array.isArray(raw.subtask_candidates) ||
      !raw.subtask_candidates.every((item) => typeof item === 'string')
    ) {
      throw new Error(
...
function parseTeamWorkerContextFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): { teamName: string; workerName: string } | null {
  return (
    parseTeamWorkerEnv(env.OMC_TEAM_WORKER) ??
    parseTeamWorkerEnv(env.OMX_TEAM_WORKER)
  );
}

function readTeamStateRootFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): string | null {
  const candidate =
    typeof env.OMC_TEAM_STATE_ROOT === 'string' &&
...
export function resolveTeamApiCliCommand(
  env: NodeJS.ProcessEnv = process.env,
): 'omc team api' | 'omx team api' {
  const hasOmcContext =
    (typeof env.OMC_TEAM_WORKER === 'string' &&
      env.OMC_TEAM_WORKER.trim() !== '') ||
    (typeof env.OMC_TEAM_STATE_ROOT === 'string' &&
      env.OMC_TEAM_STATE_ROOT.trim() !== '');
  if (hasOmcContext) return 'omc team api';

  const hasOmxContext =
    (typeof env.OMX_TEAM_WORKER === 'string' &&
      env.OMX_TEAM_WORKER.trim() !== '') ||
    (typeof env.OMX_TEAM_STATE_ROOT === 'string' &&
      env.OMX_TEAM_STATE_ROOT.trim() !== '');
...
function readTeamStateRootFromFile(path: string): string | null {
  ...
  try {
    ...
    return typeof parsed.team_state_root === 'string' &&
...
function normalizeTeamName(toolOrOperationName: string): string {
  ...
  const withoutPrefix = normalized.startsWith('team_')
    ? normalized.slice('team_'.length)
    : normalized;
...
export function resolveTeamApiOperation(name: string): TeamApiOperation | null {
  ...
  return TEAM_API_OPERATIONS.includes(normalized as TeamApiOperation)
    ? (normalized as TeamApiOperation)
    : null;
}
...
const LEADER_PANE_MISSING_MAILBOX_PERSISTED_REASON =
...
async function notifyMailboxTarget(
  ...
  if (toWorker === 'leader-fixed') {
    ...
    const injected = await injectToLeaderPane(
      sessionName,
      leaderPaneId,
      triggerMessage,
    );
  ...
  const workerPaneId = config.workers
    .find((worker) => worker.name === toWorker)
    ?.pane_id?.trim();
  if (!workerPaneId) return queuedForHookDispatch();

  const notified = await sendToWorker(
    sessionName,
    workerPaneId,
    triggerMessage,
  );
...
export async function executeTeamApiOperation(
  ...
  try {
    ...
    const cwd = teamNameForCwd
      ? resolveTeamWorkingDirectory(teamNameForCwd, fallbackCwd)
      : fallbackCwd;
    ...
    switch (operation) {
      case 'recover-worker': {
        ...
        const normalizedRequestId =
          typeof requestId === 'string' ? requestId.trim() : undefined;
        if (
          !teamName ||
          !workerName ||
          (requestId !== undefined &&
            (normalizedRequestId === undefined ||
              !isSafeRecoveryRequestId(normalizedRequestId))) ||
          (timeoutMs !== undefined &&
            (!isFiniteInteger(timeoutMs) ||
              timeoutMs < 180_000 ||
              timeoutMs > 300_000))
        ) {
          return {
            ...
            error: {
              code: 'invalid_input',
              message:
                'team_name and worker are required; request_id must be a path-safe 1-128 character opaque identifier and timeout_ms must be an integer from 180000 through 300000 when provided',
            },
          };
        }
      ...
      case 'write-task-checkpoint': {
        ...
        if (
          ...
          !workerName ||
          !claimToken ||
          !Object.hasOwn(args, 'resume_payload') ||
          !isFiniteInteger(taskVersion) ||
          taskVersion <= 0 ||
          !isFiniteInteger(sequence) ||
          sequence <= 0
        ) {
          return {
            ok: false,
            ...
            error: {
              code: 'invalid_input',
              message:
                'team_name, task_id, worker, claim_token, positive task_version, positive sequence, and resume_payload are required',
            },
          };
        }
        const workerContext = parseTeamWorkerContextFromEnv();
        if (!workerContext) {
          return {
            ...
            error: {
              ...
              message:
                'write-task-checkpoint requires OMC_TEAM_WORKER or OMX_TEAM_WORKER authentication',
            },
          };
        }
        if (
          workerContext.teamName !== teamName ||
          workerContext.workerName !== workerName
        ) {
          return {
            ok: false,
            operation,
            error: {
              ...
              message:
                'authenticated worker does not match team_name and worker',
            },
          };
        }
      ...
      case 'send-message': {
        ...
        let message: Awaited<ReturnType<typeof sendDirectMessage>> | null =
          null;
        ...
        await queueDirectMailboxMessage({
          ...
          cwd,
          notify: ({ workerName }, triggerMessage) =>
            notifyMailboxTarget(teamName, workerName, triggerMessage, cwd),
          deps: {
        ...
        await queueBroadcastMailboxMessage({
          ...
          deps: {
            ...
            broadcastMessage: async (
              ...
              messages = await broadcastMessage(
                resolvedTeamName,
                resolvedFromWorker,
                resolvedBody,
                resolvedCwd,
      ...
      case 'mailbox-list': {
        ...
        const messages = includeDelivered
          ? all
          : all.filter((m) => !m.delivered_at);
        return {
          ok: true,
          operation,
      ...
      case 'mailbox-mark-delivered': {
        ...
        const updated = await markMessageDelivered(
          teamName,
          worker,
          messageId,
          cwd,
      ...
      case 'mailbox-mark-notified': {
        ...
        const notified = await markMessageNotified(
          teamName,
          worker,
          messageId,
          cwd,
      ...
      case 'claim-task': {
        ...
        if (
          ...
          (!isFiniteInteger(rawExpectedVersion) || rawExpectedVersion < 1)
        ) {
          return {
            ok: false,
            operation,
        ...
        const result = await teamClaimTask(
          ...
          taskId,
          worker,
          (rawExpectedVersion as number | undefined) ?? null,
          cwd,
        );
        return {
          ok: true,
      ...
      case 'transition-task-status': {
        ...
        if (
          ...
          typeof transitionResult !== 'string'
        ) {
          return {
            ok: false,
            operation,
          ...
          cwd,
          {
            result:
              typeof transitionResult === 'string'
                ...
                : undefined,
            error:
              typeof transitionError === 'string' ? transitionError : undefined,
          },
        );
        return {
          ok: true,
      ...
      case 'orphan-cleanup': {
        ...
        if (
          safety.hasEvidence &&
          args.acknowledge_lost_worktree_recovery !== true
        ) {
          return {
            ok: false,
            operation,
            error: {
              code: 'invalid_input',
              message:
                'orphan_cleanup_blocked:worktree_recovery_evidence_present; pass acknowledge_lost_worktree_recovery=true only after manually preserving or intentionally discarding worker worktrees and root AGENTS backups',
            },
          };
        }
        await teamCleanup(teamName, cwd);
      ...
      case 'read-shutdown-ack': {
        ...
        const ack = await teamReadShutdownAck(
          ...
          worker,
          cwd,
          args.min_updated_at as string | undefined,
        );
      ...
      case 'write-task-approval': {
        ...
        if (
          !TEAM_TASK_APPROVAL_STATUSES.includes(
            status as TeamTaskApprovalStatus,
          )
        ) {
          return {
            ok: false,
