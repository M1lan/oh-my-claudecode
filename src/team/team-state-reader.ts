...
function rawJson<T>(path: string): RawTeamState<T> {
  ...
  try {
    ...
    return {
      source:
        typeof (value as { state_revision?: unknown }).state_revision ===
        'number'
          ? 'revisioned'
          : 'legacy',
      value: value as T,
    ...
    return {
      source: code ? 'io_error' : 'malformed',
      value: null,
...
export function readTeamState(
  ...
  const manifest = rawJson<TeamManifestV2>(
  ...
  if (config.source === 'io_error') {
    return {
      ...
      manifest,
      state: null,
      manifestSync: 'repair_required',
    };
  }
  if (config.source === 'revisioned' && config.value) {
    const matching =
      manifest.source === 'revisioned' &&
      manifest.value &&
      (manifest.value as unknown as { state_revision?: number })
        .state_revision === config.value.state_revision;
    return {
  ...
  if (
    ...
    return {
      classification: 'legacy_merged',
      config,
      manifest,
      state: {
        ...manifest.value,
        ...config.value,
        tmux_session: config.value.tmux_session,
      } as TeamConfig,
      manifestSync: 'synced',
    };
  }
  if (config.source === 'legacy' && config.value)
    return {
      ...
      manifest,
      state: config.value,
      manifestSync: manifest.source === 'absent' ? 'repair_required' : 'synced',
    };
  if (
    ...
    return {
      classification: 'manifest_only_legacy',
      config,
      manifest,
...
export function deriveManifestProjection(
  config: TeamConfig,
  existing?: TeamManifestV2 | null,
): TeamManifestV2 {
  ...
  return {
    ...
    leader: {
      ...
      session_id: config.tmux_session,
    },
    ...
    permissions_snapshot: source?.permissions_snapshot ?? {
