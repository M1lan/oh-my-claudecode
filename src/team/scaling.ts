...
export async function scaleUpOwned(
  ...
  return await withScalingLock(
    ...
    async (): Promise<ScaleUpResult | ScaleError> => {
      ...
      const rollbackScaleUp = async (
        ...
        const cleanupPane = async (
          ...
          for (let attempt = 0; attempt < 2; attempt++) {
            ...
            const liveness = await getWorkerLiveness(candidate).catch(
              () => 'unknown' as const,
            );
      ...
      const committedConfig = withScaleUpFenceRevision(
        config,
        configRevision + 1,
      );
      try {
      ...
      if (!(await releaseScaleUpReservation())) {
        return {
          ok: false,
          error: 'scale_up_fence_release_failed_after_commit',
        };
      }
    ...
    async (): Promise<ScaleDownResult | ScaleError> => {
      ...
      try {
        config = await withProcessIdentityFileLock(lifecycleLock, async () => {
          ...
          if (
            existingScaleDown &&
            (existingScaleDown.phase !== 'draining' ||
