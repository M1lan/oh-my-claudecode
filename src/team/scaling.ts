...
import { rmuxSpawn } from '../cli/rmux-utils.js';
import {
  buildWorkerArgv,
...
const CLI_AGENT_TYPES = new Set<CliAgentType>([
  'claude',
  'codex',
  'gemini',
  'grok',
...
function configuredTmuxTarget(tmuxSession: unknown): {
  expectedTarget: string;
  format: string;
} {
  const expectedTarget =
    typeof tmuxSession === 'string' ? tmuxSession.trim() : '';
  return {
    expectedTarget,
    format: expectedTarget.includes(':')
      ? '#{session_name}:#{window_index}'
      : '#{session_name}',
  };
}

function validateSplitTargetPaneInConfiguredSession(
  splitTarget: string,
  tmuxSession: unknown,
): string | null {
...
export async function scaleUpOwned(
  ...
  return await withScalingLock(
    ...
    async (): Promise<ScaleUpResult | ScaleError> => {
      ...
      const withScaleUpFenceRevision = (
        next: TeamConfig,
      ...
      const saveScaleUpConfig = async (
        ...
        try {
          return await saveTeamConfigAtRevision(
            next,
            expectedRevision,
            leaderCwd,
            undefined,
      ...
      try {
        config = await withProcessIdentityFileLock(lifecycleLock, async () => {
          ...
          if (
            !(await saveScaleUpConfig(
              next,
              current.stateRevision,
              existing
                ? { reclaim: { active_scale_up: true as const } }
                : undefined,
            ))
          )
            throw new Error('team_mutation_busy');
        ...
        return {
          ok: false,
          error: error instanceof Error ? error.message : 'team_mutation_busy',
        };
      }
      const releaseScaleUpReservation = async (
        ...
        withProcessIdentityFileLock(lifecycleLock, async () => {
          ...
          if (
            ...
            reservation.process_started_at !== processStartedAt
          )
            return false;
          ...
          if (
            ...
            current.config.lifecycle_state !== 'active'
          ) {
            return false;
          }
          const nextRevision = current.stateRevision + 1;
      ...
      if (!(await reserveScaleUpEffects())) {
        ...
        return {
          ...
          error: released
            ? 'team_mutation_busy'
            : 'scale_up_fence_release_failed',
        };
      }

      const teamStateRoot =
        config.team_state_root ?? `${leaderCwd}/.omc/state/team/${sanitized}`;
      ...
      const cleanupScaledWorkerWorktree = (
        ...
        if (created) {
          ...
          const restored = restoreWorktreeRootAgents(
            sanitized,
            workerName,
            leaderCwd,
          );
          if (restored.reason === 'agents_dirty') {
            throw new Error(
              `agents_dirty: preserving modified worktree root AGENTS.md for ${workerName}`,
            );
          }
        }
      };
      ...
      const rollbackScaleUp = async (
        ...
        const cleanupPane = async (
          ...
          const paneAlreadyDead =
            (await getWorkerLiveness(candidate).catch(
              () => 'unknown' as const,
            )) === 'dead';
          const markPreserve = () => {
          ...
          if (!ownership && !paneAlreadyDead) {
            cleanupFailures.push(
              `${label}:pane_ownership_unverified:${candidate}`,
            );
            markPreserve();
            return false;
          }
          ...
          cleanupFailures.push(
            `${label}:${launch ? 'provider' : 'pane'}:${candidate}`,
          );
          markPreserve();
          return false;
        };
        const cleanupIdentity = async (workerName: string): Promise<void> => {
          ...
          const workerDir = absPath(
            leaderCwd,
            TeamPaths.workerDir(sanitized, workerName),
          );
          for (
            let attempt = 0;
            attempt < 2 && existsSync(workerDir);
            attempt++
          ) {
        ...
        for (const worker of addedWorkers) {
          const idx = config.workers.findIndex(
            (candidate) => candidate.name === worker.name,
          );
          ...
          if (worker.worktree_path) {
            ...
            if (!cleaned || existsSync(worker.worktree_path))
              cleanupFailures.push(
                `${worker.name}:worktree:${worker.worktree_path}`,
              );
          }
        ...
        for (const pending of pendingWorktrees) {
          if (!cleanedWorktrees.has(pending.workerName)) {
            ...
            if (!cleaned || existsSync(pending.path))
              cleanupFailures.push(
                `${pending.workerName}:pending-worktree:${pending.path}`,
              );
          }
        ...
        if (reservedWorkerNames.size > 0) {
          ...
          const reservedRows =
            persisted?.config.workers.filter((worker) =>
              reservedWorkerNames.has(worker.name),
            ) ?? [];
          if (
            persisted &&
            (persisted.config.lifecycle_state ?? 'active') === 'active'
          ) {
            const addedByName = new Map(
              addedWorkers.map((worker) => [worker.name, worker]),
            );
            const safeToRetire = new Set<string>();
            for (const row of reservedRows) {
              const expectedLaunch = reservedLaunchDescriptors.get(row.name);
              const launchMatches =
                JSON.stringify(row.launch_descriptor) ===
                JSON.stringify(expectedLaunch);
              const activated = addedByName.get(row.name);
              if (
                launchMatches &&
                (row.operational_state === 'starting' ||
                  (row.operational_state === 'active' &&
                    activated?.pane_id === row.pane_id))
              ) {
                safeToRetire.add(row.name);
              } else {
                cleanupFailures.push(
                  `scale_up_reservation_fence_lost:${row.name}`,
                );
              }
            }
            if (safeToRetire.size > 0) {
              const retired = withScaleUpFenceRevision(
                {
                  ...persisted.config,
                  workers: persisted.config.workers.filter(
                    (worker) => !safeToRetire.has(worker.name),
                  ),
                },
              ...
              if (
                !(await saveScaleUpConfig(retired, persisted.stateRevision))
              ) {
        ...
        if (cleanupFailures.length > 0) {
          ...
          const evidencePath = absPath(
            leaderCwd,
            TeamPaths.scalingRollbackFailure(sanitized, Date.now()),
          );
          await writeAtomic(
            evidencePath,
            JSON.stringify(
          ...
          return {
            ok: false,
            error: `${error}; rollback incomplete (${cleanupFailures.join(', ')}) evidence=${evidencePath}`,
          };
        }
        if (!(await releaseScaleUpReservation()))
          return {
            ok: false,
            error: `${error}; scale_up_fence_release_failed`,
          };
      ...
      for (let i = 0; i < count; i++) {
        ...
        let skipped = 0;
        while (
          config.workers.some((w) => w.name === `worker-${nextIndex}`) &&
          skipped < maxSkip
        ) {
          nextIndex++;
          skipped++;
        ...
        if (config.workers.some((worker) => worker.name === workerName)) {
          ...
          const advancedConfig = withScaleUpFenceRevision(
            { ...config, next_worker_index: nextIndex },
            configRevision + 1,
          );
        ...
        const splitTargetError = validateSplitTargetPaneInConfiguredSession(
          splitTarget,
          config.tmux_session,
        );
        if (splitTargetError) {
        ...
        try {
          ...
          const ownedRoles = Array.from(
            new Set(workerTasks.map((t) => t.role).filter(Boolean) as string[]),
          );
          const inferredRole: string | undefined =
            ownedRoles.length === 1
              ? ownedRoles[0]
              : workerTasks[0]
                ? routeTaskToRole(
                    workerTasks[0].subject,
                    workerTasks[0].description,
                    'executor',
                  ).role
                : undefined;
          ...
          const canonical: CanonicalTeamRole | null = inferredRole
            ? (() => {
                ...
                return canonicalRoleSet.has(normalized)
          ...
          const routedPair =
            hasExplicitOwnedRole && canonical
          ...
          try {
            ...
            return await rollbackScaleUp(
              `Invalid worker launch descriptor for ${workerName}: ${error instanceof Error ? error.message : String(error)}`,
            );
          }
          const workerTaskRoles = tasks
            .filter((t) => t.owner === workerName)
            .map((t) => t.role)
            .filter(Boolean) as string[];
          const uniqueTaskRoles = new Set(workerTaskRoles);
          const workerRole =
            workerTaskRoles.length > 0 && uniqueTaskRoles.size === 1
              ? workerTaskRoles[0]!
              : agentType;
          ...
          const reservationConfig = withScaleUpFenceRevision(
            {
              ...config,
              workers: [...config.workers, reservedWorker],
              worker_count: config.workers.length + 1,
              next_worker_index: nextIndex,
            },
            configRevision + 1,
          );
          if (!(await saveScaleUpConfig(reservationConfig, configRevision))) {
            return await rollbackScaleUp(
              'Scale-up reservation lost its revision: stale_state_revision',
            );
          }
          ...
          const extraEnv: Record<string, string> = {
            ...
            ...(worktree
              ? {
                  OMC_TEAM_WORKTREE_PATH: worktree.path,
          ...
          if (worktree) {
            try {
              ...
              return await rollbackScaleUp(
                `Failed to install worker overlay for ${workerName}: ${reason}`,
              );
            }
          }
          ...
          if (!ownershipResult.ok) {
            return await rollbackScaleUp(
              `Failed to prove pane ownership for ${workerName}: ${ownershipResult.reason}`,
              undefined,
              `unaddressable_spawned_pane:${paneId}`,
            );
          }
          ...
          try {
            ...
            return await rollbackScaleUp(
              `Failed durable worker launch for ${workerName}: ${error instanceof Error ? error.message : String(error)}`,
              paneId,
            );
          }
          ...
          const pendingIndex = pendingWorktrees.findIndex(
            (pending) => pending.workerName === workerName,
          );
          if (pendingIndex >= 0) pendingWorktrees.splice(pendingIndex, 1);
          const reservedIndex = config.workers.findIndex(
          ...
          config = {
            ...config,
            workers: config.workers.map((candidate, index) =>
              index === reservedIndex ? workerInfo : candidate,
            ),
          ...
          return await rollbackScaleUp(
            `Scale-up post-effect failed for ${workerName}: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
      ...
      const committedConfig = {
        ...
        active_scale_up: {
          ...scaleUpAttempt(config)!,
          phase: 'committed' as const,
          state_revision: configRevision + 1,
          updated_at: new Date().toISOString(),
        },
      };
      try {
        if (!(await saveScaleUpConfig(committedConfig, configRevision))) {
          return await rollbackScaleUp(
            'Scale-up config commit lost its revision: stale_state_revision',
          );
        }
        ...
        return await rollbackScaleUp(
          `Scale-up config commit lost its revision: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
      ...
      if (
        !postCommit ||
        (postCommit.config.lifecycle_state &&
          postCommit.config.lifecycle_state !== 'active')
      ) {
        servicesSync = 'repair_required';
      } else {
        try {
          const { reconcileCommittedTeamServices } =
            await import('./runtime-v2.js');
          servicesSync = await reconcileCommittedTeamServices(
            postCommit.config,
            leaderCwd,
          );
        } catch {
  ...
  return await withScalingLock(
    ...
    async (): Promise<ScaleDownResult | ScaleError> => {
      ...
      if (options.workerNames && options.workerNames.length > 0) {
        ...
        for (const name of options.workerNames) {
          ...
          if (!w) {
            return {
              ok: false,
              error: `Worker ${name} not found in team ${sanitized}`,
            };
          }
        ...
        for (const w of config.workers) {
          const status = await teamReadWorkerStatus(
            sanitized,
            w.name,
            leaderCwd,
          );
      ...
      const workerIdentity = (
        worker: WorkerInfo,
      ): TeamScaleDownAttempt['workers'][number] => ({
        name: worker.name,
      ...
      const identitiesMatch = (
        workers: WorkerInfo[],
        expected: TeamScaleDownAttempt['workers'],
      ): boolean =>
        JSON.stringify(workers.map(workerIdentity)) ===
      ...
      try {
        config = await withProcessIdentityFileLock(lifecycleLock, async () => {
          ...
          if (resumeFailed) {
            ...
            const selected = resumeWorkers.map((w) =>
              current.config.workers.find((worker) => worker.name === w.name),
            );
            ...
            if (
              !(await saveTeamConfigAtRevision(
                ...
                undefined,
                {
                  ...(sameOpOwner
                    ? {}
                    : { reclaim: { active_scale_down: true as const } }),
                  ...(next.active_scale_up === undefined &&
          ...
          const selected = selectedNames.map((name) =>
            current.config.workers.find((worker) => worker.name === name),
          );
          if (
            selected.some((worker): worker is undefined => !worker) ||
            !identitiesMatch(
              selected as WorkerInfo[],
              targetWorkers.map(workerIdentity),
            )
          )
            throw new Error('team_mutation_busy');
          const next = {
            ...
            active_scale_down: {
              ...
              pid: process.pid,
              process_started_at: processStartedAt,
              workers: (selected as WorkerInfo[]).map(workerIdentity),
              state_revision: nextRevision,
              created_at: now,
              updated_at: now,
            },
      ...
      const markScaleDownFailed = async (reason: string): Promise<void> => {
        ...
        try {
          await withProcessIdentityFileLock(lifecycleLock, async () => {
            ...
            if (
              !current ||
              current.config.active_scale_down?.operation_id !== operationId
            )
              return;
            const nextRevision = current.stateRevision + 1;
            if (
          ...
          JSON.stringify(
            {
              ...
              reason,
              ...(configMarkError
                ? { config_mark_error: configMarkError }
                : {}),
              recorded_at: new Date().toISOString(),
            },
            null,
      ...
      const reserveEffects = async (): Promise<boolean> =>
        withProcessIdentityFileLock(lifecycleLock, async () => {
          const current = await readRevisionedTeamConfig(sanitized, leaderCwd);
          ...
          config = next;
          targetWorkers = selectedNames
            .map((name) => next.workers.find((worker) => worker.name === name)!)
            .filter(Boolean);
          return true;
        });
      ...
      for (const w of targetWorkers) {
        ...
        const statusPath = absPath(
          leaderCwd,
          TeamPaths.workerStatus(sanitized, w.name),
        );
      ...
      for (const worker of targetWorkers) {
        ...
        const provider =
          worker.launch_descriptor?.provider ?? worker.worker_cli;
        if (!provider || !worker.launch_attempt_id) {
          ...
          try {
            let lastLegacyLiveness: Awaited<
              ReturnType<typeof getWorkerLiveness>
            > = await getWorkerLiveness(paneId);
            for (
              let attempt = 0;
        ...
        const cleaned = await retireAndCleanupCurrentWorkerLaunchAttempt(
          ...
          async () => {
            try {
              let lastLiveness: Awaited<ReturnType<typeof getWorkerLiveness>> =
                await getWorkerLiveness(paneId);
      ...
      const liveness = await Promise.all(
        targetWorkers.map(async (w) =>
          w.pane_id
            ? ([w.name, await getWorkerLiveness(w.pane_id)] as const)
            : ([w.name, 'unknown'] as const),
        ),
      );
      const aliveNames = liveness
      ...
      const committed = await withProcessIdentityFileLock(
        lifecycleLock,
        async () => {
          const current = await readRevisionedTeamConfig(sanitized, leaderCwd);
          if (
            ...
            return false;
          const workers = current.config.workers.filter(
            (worker) => !removedSet.has(worker.name),
          );
          const nextRevision = current.stateRevision + 1;
          const next = {
            ...current.config,
      ...
      if (!committed) {
        await markScaleDownFailed(
          'scale_down_config_commit_failed_after_effects',
        );
        return {
          ok: false,
          error: 'scale_down_config_commit_failed_after_effects',
        };
      }
