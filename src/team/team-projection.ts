...
function parseManifest(path: string): TeamManifestV2 | null {
  try {
    return JSON.parse(readFileSync(path, 'utf8')) as TeamManifestV2;
  } catch {
    return null;
  }
}
...
function sameRecovery(
  ...
  return (
    ...
    config.active_recovery?.recovery_id === recoveryId ||
    config.last_recovery?.recovery_id === recoveryId
  );
}
 ...
 */
export function repairTeamProjection(
  cwd: string,
  teamName: string,
  options: ProjectionRepairOptions,
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    if (!config || config.state_revision === undefined)
      return {
        classification: 'repair_required',
        revision: null,
        reason: existsSync(configPath) ? 'invalid_config' : 'io_error',
      };
    if (!sameRecovery(config, options.recoveryId))
      return {
        classification: 'repair_required',
        revision: config.state_revision,
