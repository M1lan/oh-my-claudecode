...
function hasAssignedTasks(worker: WorkerInfo): boolean {
  return (
    Array.isArray(worker.assigned_tasks) && worker.assigned_tasks.length > 0
  );
}
...
function chooseWinningWorker(
  existing: WorkerInfo,
  incoming: WorkerInfo,
): { winner: WorkerInfo; loser: WorkerInfo } {
...
export function canonicalizeWorkers(
  ...
  for (const worker of workers) {
    ...
    byName.set(name, {
      ...
      index:
        backfillNumber(winner.index, loser.index, (value) => value > 0) ?? 0,
...
export function canonicalizeTeamConfigWorkers(config: TeamConfig): TeamConfig {
  ...
  return {
    ...
    workers,
    worker_count:
      workers.length > 0 ? workers.length : (config.worker_count ?? 0),
  };
}
