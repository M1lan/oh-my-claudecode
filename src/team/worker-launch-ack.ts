...
const WORKER_LAUNCH_TRANSPORT_OWNER_KIND =
  'worker_launch_transport_owner' as const;
const WORKER_LAUNCH_TRANSPORT_CLEANUP_KIND =
  'worker_launch_transport_cleanup_complete' as const;
...
export function buildWindowsSupervisorSource(): string {
  return [
    ...
    '  $ticks = ([DateTime]((Get-Process -Id $pi.dwProcessId).StartTime)).ToUniversalTime().Ticks; $ready = @{ protocol="' +
      WINDOWS_SUPERVISOR_PROTOCOL +
      '"; kind="ready"; attempt_id=$payload.identity.attempt_id; authority_digest=$payload.authority_digest; containment_nonce=$payload.containment_nonce; pid=$pi.dwProcessId; process_start_identity=("ticks:" + $ticks) } | ConvertTo-Json -Compress; [Console]::Out.WriteLine($ready); [Console]::Out.Flush()',
    '  $readTask = [Console]::In.ReadLineAsync(); while ($true) { if ([O]::WaitForSingleObject($pi.hProcess, 50) -eq 0) { $exitCode = [uint32]0; [O]::GetExitCodeProcess($pi.hProcess, [ref]$exitCode) | Out-Null; [O]::TerminateJobObject($job, $exitCode) | Out-Null; $terminal = @{ protocol="' +
      WINDOWS_SUPERVISOR_PROTOCOL +
      '"; kind="terminal"; attempt_id=$payload.identity.attempt_id; authority_digest=$payload.authority_digest; containment_nonce=$payload.containment_nonce; pid=$pi.dwProcessId; outcome="exit"; cleanup_verified=$true; exit_code=$exitCode } | ConvertTo-Json -Compress; [Console]::Out.WriteLine($terminal); [Console]::Out.Flush(); break }; if (-not $readTask.IsCompleted) { continue }; $line = $readTask.Result; if ($null -eq $line) { throw "worker_launch_authority_lost" }; $readTask = [Console]::In.ReadLineAsync(); if ([string]::IsNullOrWhiteSpace($line) -or $line.Length -gt 4096) { continue }; try { $msg = $line | ConvertFrom-Json } catch { continue }; if ($msg.protocol -ne "' +
      WINDOWS_SUPERVISOR_PROTOCOL +
      '" -or $msg.attempt_id -ne $payload.identity.attempt_id -or $msg.authority_digest -ne $payload.authority_digest -or $msg.containment_nonce -ne $payload.containment_nonce -or $msg.kind -ne "terminate") { continue }; [O]::TerminateJobObject($job, 1) | Out-Null; [O]::WaitForSingleObject($pi.hProcess, 5000) | Out-Null; $terminal = @{ protocol="' +
      WINDOWS_SUPERVISOR_PROTOCOL +
      '"; kind="terminal"; attempt_id=$payload.identity.attempt_id; authority_digest=$payload.authority_digest; containment_nonce=$payload.containment_nonce; pid=$pi.dwProcessId; outcome="terminated"; cleanup_verified=$true } | ConvertTo-Json -Compress; [Console]::Out.WriteLine($terminal); [Console]::Out.Flush(); break }',
...
const WINDOWS_RESERVED_ENV_KEYS = new Set(
  [...WORKER_LAUNCH_INTERNAL_ENV_KEYS, 'SystemRoot'].map((key) =>
    key.toUpperCase(),
  ),
);
const SAFE_BASELINE_ENV_KEYS = [
  'PATH',
  'SystemRoot',
  'SYSTEMROOT',
  'TEMP',
...
function canonicalAuthorityDigest(input: {
  ...
  const env = Object.fromEntries(
    Object.entries(input.providerEnv).sort(([a], [b]) => a.localeCompare(b)),
  );
  const payload = JSON.stringify({
    protocol: WORKER_LAUNCH_AUTHORITY_PROTOCOL,
    nonce: input.containmentNonce ?? '',
    supervisor_source_sha256: input.supervisorSourceSha256 ?? '',
    identity: identityOf(input.identity),
    provider_argv: [...input.providerArgv],
    provider_env: env,
...
function isValidProviderEnvironment(
  value: unknown,
  platform: NodeJS.Platform = process.platform,
): value is Record<string, string> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  try {
    normalizeProviderEnvironment(value as Record<string, string>, platform);
    return true;
  } catch {
    return false;
  }
...
function identityMatches(
  value: unknown,
  expected: WorkerLaunchIdentity,
): boolean {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const record = value as Partial<WorkerLaunchIdentity>;
  return (
    record.schema_version === WORKER_LAUNCH_SCHEMA_VERSION &&
...
async function isCurrentLaunchIdentity(
  currentPath: string,
  identity: WorkerLaunchIdentity,
): Promise<boolean> {
...
async function writeExclusiveTextAtomic(
  path: string,
  value: string,
): Promise<void> {
  await mkdir(dirname(path), { recursive: true });
...
export async function prepareWorkerLaunchAttempt(input: {
  ...
  const attempt: WorkerLaunchAttempt = {
    ...
    currentPath: absPath(
      input.cwd,
      TeamPaths.workerLaunchCurrent(input.teamName, input.workerName),
    ),
    expectedPath: absPath(
      input.cwd,
      TeamPaths.workerLaunchExpected(
        input.teamName,
        input.workerName,
        attemptId,
      ),
    ...
    ackPath: absPath(
      input.cwd,
      TeamPaths.workerLaunchAck(input.teamName, input.workerName, attemptId),
    ),
    decisionPath: absPath(
      input.cwd,
      TeamPaths.workerLaunchDecision(
        input.teamName,
        input.workerName,
        attemptId,
      ),
    ...
    startedPath: absPath(
      input.cwd,
      TeamPaths.workerLaunchStarted(
        input.teamName,
        input.workerName,
        attemptId,
      ),
    ),
    transportOwnerPath: absPath(
      input.cwd,
      TeamPaths.workerLaunchTransportOwner(
        input.teamName,
        input.workerName,
        attemptId,
      ),
    ),
    bootstrapDescriptorPath: absPath(
      input.cwd,
      TeamPaths.workerLaunchBootstrapDescriptor(
        input.teamName,
        input.workerName,
        attemptId,
      ),
    ...
    wrapperPath: absPath(
      input.cwd,
      TeamPaths.workerLaunchWrapper(
        input.teamName,
        input.workerName,
        attemptId,
      ),
    ),
    transportCleanupCompletePath: absPath(
      input.cwd,
      TeamPaths.workerLaunchTransportCleanupComplete(
        input.teamName,
        input.workerName,
        attemptId,
      ),
  ...
  if (
    existsSync(attempt.expectedPath) ||
    existsSync(attempt.ackPath) ||
    existsSync(attempt.decisionPath) ||
    existsSync(attempt.startedPath) ||
    existsSync(attempt.transportOwnerPath) ||
    existsSync(attempt.bootstrapDescriptorPath) ||
    existsSync(attempt.wrapperPath) ||
    existsSync(attempt.transportCleanupCompletePath)
  ) {
...
export async function loadWorkerLaunchAttempt(input: {
  ...
  if (
    identity.attempt_id !== input.attemptId ||
    identity.team_name !== input.teamName ||
    identity.worker_name !== input.workerName ||
    identity.pane_id !== input.paneId ||
  ...
  try {
    return await withFileLock(lockPathFor(currentPath), async () => {
      ...
      if (current.kind !== 'value' || !isValidIdentity(current.value))
        return null;
      const record = current.value as WorkerLaunchIdentity & {
        runtime_cli_path?: unknown;
        context?: unknown;
      };
      if (
        record.team_name !== input.teamName ||
        record.worker_name !== input.workerName ||
        record.provider !== input.provider ||
        !isExactText(record.runtime_cli_path) ||
...
export function buildWorkerLaunchBootstrapSpec(
  ...
  const supervisorSourceSha256 = createHash('sha256')
    .update(buildWindowsSupervisorSource(), 'utf8')
    .digest('hex');
  return {
    ...identityOf(attempt),
    ...
    cwd: absoluteCwd,
    decision_timeout_ms: resolvePositiveInteger(
      process.env.OMC_TEAM_START_ACK_DECISION_TIMEOUT_MS,
      DEFAULT_DECISION_TIMEOUT_MS,
    ),
    release_after_spawn: options.releaseAfterSpawn === true,
    containment_nonce: containmentNonce,
    authority_digest: canonicalAuthorityDigest({
      identity: attempt,
      providerArgv,
      providerEnv,
      cwd: absoluteCwd,
...
function attemptTransportPathsAreDeterministic(
  attempt: WorkerLaunchAttempt,
): boolean {
  const expectedRoot = dirname(attempt.expectedPath);
  return (
    isDeterministicTransportPath(
      attempt.expectedPath,
      attempt.transportOwnerPath,
      'transport-owner.json',
    ) &&
    isDeterministicTransportPath(
      attempt.expectedPath,
      attempt.bootstrapDescriptorPath,
    ...
    isDeterministicTransportPath(
      attempt.expectedPath,
      attempt.transportCleanupCompletePath,
      'transport-cleanup-complete.json',
    ) &&
    resolve(attempt.expectedPath) ===
      resolve(join(expectedRoot, 'expected.json'))
  );
}

function transportOwnerMatches(
  value: unknown,
  attempt: WorkerLaunchIdentity,
  authorityDigest?: string,
): value is WorkerLaunchTransportOwner {
  if (
    !identityMatches(value, attempt) ||
    typeof value !== 'object' ||
    value === null ||
  ...
  return (
    ...
    (value as { authority_digest?: unknown }).authority_digest ===
      authorityDigest
  );
}
...
function cleanupProofMatches(
  value: unknown,
  attempt: WorkerLaunchIdentity,
): value is WorkerLaunchTransportCleanupComplete {
  return (
    identityMatches(value, attempt) &&
    typeof value === 'object' &&
    value !== null &&
    (value as { kind?: unknown }).kind ===
      WORKER_LAUNCH_TRANSPORT_CLEANUP_KIND &&
...
function windowsWrapperRelativePath(cwd: string, wrapperPath: string): string {
  const relativePath = relative(resolve(cwd), resolve(wrapperPath)).replace(
    /\//g,
    '\\',
  );
  if (
...
export async function materializeWorkerLaunchTransport(input: {
  ...
  try {
    await withFileLock(lockPathFor(attempt.currentPath), async () => {
      ...
      if (
        existingOwner.kind === 'malformed' ||
        (existingOwner.kind === 'value' &&
          !transportOwnerMatches(
            existingOwner.value,
            attempt,
    ...
    if (wrapperCreated) {
      await unlink(attempt.wrapperPath).catch(() => {
        cleanupVerified = false;
      });
      cleanupVerified &&= !existsSync(attempt.wrapperPath);
    }
    if (descriptorCreated) {
      await unlink(attempt.bootstrapDescriptorPath).catch(() => {
        cleanupVerified = false;
      });
      cleanupVerified &&= !existsSync(attempt.bootstrapDescriptorPath);
    }
    if (ownerCreated) {
      await unlink(attempt.transportOwnerPath).catch(() => {
        cleanupVerified = false;
      });
    ...
    if (!cleanupVerified) {
      const cleanupError = new Error(
        'worker_launch_transport_partial_cleanup_unverified',
      );
...
async function cleanupWorkerLaunchTransportUnlocked(
  ...
  if (proof.kind === 'value' && !cleanupProofMatches(proof.value, attempt))
    return false;
  const hasTransportFiles =
    existsSync(attempt.bootstrapDescriptorPath) ||
    existsSync(attempt.wrapperPath);
  if (owner.kind === 'absent') {
    return (
      !hasTransportFiles &&
      (proof.kind === 'absent' || cleanupProofMatches(proof.value, attempt))
    );
  }
  ...
  return (
    ...
    cleanupProofMatches(completed.value, attempt) &&
    !existsSync(attempt.bootstrapDescriptorPath) &&
    !existsSync(attempt.wrapperPath)
  );
}

export async function cleanupWorkerLaunchTransport(
  attempt: WorkerLaunchAttempt,
...
export async function readAndConsumeWorkerLaunchDescriptor(
  ...
  try {
    ...
    if (
      ...
      error.message.startsWith('worker_launch_descriptor_')
    )
      throw error;
    throw new Error(
      (error as NodeJS.ErrnoException).code === 'ENOENT'
        ? 'worker_launch_descriptor_missing'
        : 'worker_launch_descriptor_invalid',
    );
  } finally {
  ...
  if (
    owner.kind !== 'value' ||
    !transportOwnerMatches(owner.value, spec, spec.authority_digest) ||
...
async function publishDecision(
  ...
  try {
    ...
    return (
      ...
      (existing.value as Partial<WorkerLaunchDecision>).kind ===
        'worker_launch_decision' &&
      (existing.value as Partial<WorkerLaunchDecision>).decision === decision
    );
  }
}

export async function revokeWorkerLaunchAttempt(
  attempt: WorkerLaunchAttempt,
  reason: string,
): Promise<boolean> {
...
export async function awaitWorkerLaunchAcknowledgement(
  ...
  const timeoutMs =
    ...
    resolvePositiveInteger(
      process.env.OMC_TEAM_START_ACK_TIMEOUT_MS,
      DEFAULT_ACK_TIMEOUT_MS,
    );
  const pollIntervalMs = options.pollIntervalMs ?? DEFAULT_POLL_INTERVAL_MS;
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const result = await acceptObservedAcknowledgement(
      attempt,
      await readJson(attempt.ackPath),
    );
    if (result) {
      return result.ok
        ? result
        : rejectWorkerLaunchAttempt(attempt, result.reason);
    }
    await sleep(pollIntervalMs);
  }
  const finalResult = await acceptObservedAcknowledgement(
    attempt,
    await readJson(attempt.ackPath),
  );
  if (finalResult) {
    return finalResult.ok
      ? finalResult
      : rejectWorkerLaunchAttempt(attempt, finalResult.reason);
  }
  return rejectWorkerLaunchAttempt(attempt, 'ack_timeout');
}

export async function isWorkerLaunchAttemptAccepted(
  attempt: WorkerLaunchAttempt,
): Promise<boolean> {
  ...
  return (
    ...
    (decision.value as Partial<WorkerLaunchDecision>).kind ===
      'worker_launch_decision' &&
    (decision.value as Partial<WorkerLaunchDecision>).decision === 'accepted'
  );
}

export async function isWorkerLaunchAttemptCurrent(
  attempt: WorkerLaunchAttempt,
): Promise<boolean> {
  try {
    return await withFileLock(lockPathFor(attempt.currentPath), () =>
      isCurrentLaunchIdentity(attempt.currentPath, attempt),
    );
  } catch {
    return false;
  }
    ...
    return await withFileLock(
      ...
      async () => {
        ...
        if (existing.kind === 'value') {
          if (
            !identityMatches(
              existing.value as Partial<WorkerLaunchIdentity>,
              attempt,
            )
          )
            return false;
...
export async function retireAndCleanupCurrentWorkerLaunchAttempt(
  ...
  const cleanupIsComplete = async (): Promise<boolean> => {
    ...
    return (
      completed.kind === 'value' &&
      identityMatches(
        completed.value as Partial<WorkerLaunchIdentity>,
        attempt,
      ) &&
      (completed.value as { kind?: unknown }).kind ===
        'worker_launch_cleanup_complete'
    );
  };
...
async function readWorkerLaunchCleanupProof(
  ...
  const startedPath =
    'startedPath' in attempt ? attempt.startedPath : attempt.started_path;
  const terminal = await readJson(`${startedPath}.terminal`);
  if (terminal.kind === 'value') {
    const value = terminal.value as Partial<WorkerLaunchIdentity> &
  ...
  if (completed.kind === 'value') {
    const value = completed.value as Partial<WorkerLaunchIdentity> &
      Record<string, unknown>;
    const matchesStarted =
      !started ||
      (value.pid === started.pid &&
...
export async function terminateWorkerLaunchProvider(
  ...
  const terminalCleanupVerified = await readWorkerLaunchCleanupProof(
    attempt,
    started.kind === 'value'
      ? (started.value as Partial<WorkerLaunchProviderStarted>)
      : undefined,
  );
  ...
  if (
    ...
    (!Number.isSafeInteger(record.process_group_id) ||
      Number(record.process_group_id) <= 0)
  )
    return false;
  ...
  if (existingRequest.kind === 'absent') {
    ...
    const value =
      existingRequest.kind === 'value'
        ? (existingRequest.value as Partial<WorkerLaunchIdentity> &
            Record<string, unknown>)
        : null;
    if (
      !value ||
      !identityMatches(value, attempt) ||
      value.kind !== 'worker_launch_termination_request' ||
  ...
  if (result === 'already-dead' || result === 'identity-mismatch') {
    return (
      terminalCleanupVerified ||
      (hadExistingValidTerminationRequest &&
        isProcessGroupAbsent(record.process_group_id) &&
        (await publishWorkerLaunchTerminationComplete(
          terminationCompletePath,
          attempt,
          record,
        )))
    );
  ...
  if (
    ...
    if (
      (liveness === 'dead' || liveness === 'mismatch') &&
      (await publishWorkerLaunchTerminationComplete(
        terminationCompletePath,
        attempt,
        record,
...
async function publishWorkerLaunchTerminationComplete(
  ...
  const value =
    existingComplete.kind === 'value'
      ? (existingComplete.value as Partial<WorkerLaunchIdentity> &
          Record<string, unknown>)
      : null;
  return (
    !!value &&
...
async function readValidProviderStarted(
  ...
  const record = started.value as Partial<WorkerLaunchProviderStarted>;
  if (
    record.supervisor_completion_path !== undefined &&
  ...
  return identityMatches(record, attempt) &&
    ...
    (record.containment_nonce === undefined ||
      isExactText(record.containment_nonce)) &&
    typeof record.written_at === 'string' &&
    Number.isFinite(Date.parse(record.written_at))
...
export async function awaitWorkerLaunchProviderStarted(
  ...
  while (Date.now() < deadline) {
    ...
    if (published)
      try {
        const handedOff = await withFileLock(
          lockPathFor(attempt.currentPath),
          async () => {
            if (
              !(await isCurrentLaunchIdentity(attempt.currentPath, attempt)) ||
...
export async function isWorkerLaunchProviderStarted(
  attempt: WorkerLaunchAttempt,
): Promise<boolean> {
  return (await readValidProviderStarted(attempt)) !== null;
}

function isDeterministicTransportPath(
  expectedPath: string,
  candidate: unknown,
  fileName: string,
): candidate is string {
  return (
    isExactText(candidate) &&
...
function isValidBootstrapSpec(
  ...
  return (
    ...
    isDeterministicTransportPath(
      spec.expected_path,
      spec.transport_owner_path,
      'transport-owner.json',
    ) &&
    isDeterministicTransportPath(
      spec.expected_path,
      spec.bootstrap_descriptor_path,
      WORKER_LAUNCH_BOOTSTRAP_DESCRIPTOR_FILE,
    ) &&
    isDeterministicTransportPath(
      spec.expected_path,
    ...
    spec.provider_argv
      .slice(1)
      .every((argument) => typeof argument === 'string') &&
    ...
    spec.authority_digest ===
      canonicalAuthorityDigest({
        identity: spec as WorkerLaunchIdentity,
        providerArgv: spec.provider_argv,
        providerEnv: spec.provider_env,
        cwd: spec.cwd,
        containmentNonce: spec.containment_nonce,
        supervisorSourceSha256: spec.supervisor_source_sha256,
      })
  );
}

async function publishAcknowledgement(
...
async function waitForBootstrapDecision(
  spec: WorkerLaunchBootstrapSpec,
): Promise<'accepted' | 'revoked' | 'timeout'> {
  const deadline = Date.now() + spec.decision_timeout_ms;
  while (Date.now() < deadline) {
    const read = await readJson(spec.decision_path);
    if (
      read.kind === 'value' &&
      identityMatches(read.value, spec) &&
      (read.value as Partial<WorkerLaunchDecision>).kind ===
        'worker_launch_decision'
    ) {
...
function buildWindowsSupervisorInvocation(
  spec: WorkerLaunchBootstrapSpec,
): MaterializedProviderSpawnInvocation {
  const env = Object.fromEntries(
    Object.entries(spec.provider_env).sort(([a], [b]) => a.localeCompare(b)),
  );
  ...
  const systemRoot =
    spec.provider_env.SystemRoot ?? spec.provider_env.SYSTEMROOT;
  ...
  return {
    ...
    args: [
      ...
      '-NonInteractive',
      '-EncodedCommand',
      encodePowerShell(buildWindowsSupervisorSource()),
    ],
...
export function buildProviderSpawnInvocation(
  ...
  if (platform === 'win32') {
    ...
    const batchScript = [
      '@echo off',
      `start "" /b /wait ${providerArgv.map(quoteWindowsCmdArgument).join(' ')}`,
      '',
    ].join('\r\n');
    return {
      command: comSpec,
...
async function awaitExternalTerminationCompletion(
  ...
  if (
    ...
    (request.value as Record<string, unknown>).containment_nonce !==
      spec.containment_nonce ||
    (request.value as Record<string, unknown>).authority_digest !==
      spec.authority_digest
  )
    return false;
  ...
  while (Date.now() < deadline) {
    const completed = await readJson(
      `${spec.started_path}.termination-complete`,
    );
    if (
      completed.kind === 'value' &&
      identityMatches(completed.value as Partial<WorkerLaunchIdentity>, spec) &&
      (completed.value as { cleanup_verified?: unknown }).cleanup_verified ===
        true
    )
      return true;
...
async function isCorrelatedTerminationDead(
  spec: WorkerLaunchBootstrapSpec,
): Promise<boolean> {
  ...
  if (
    ...
    started.kind !== 'value' ||
    !identityMatches(request.value as Partial<WorkerLaunchIdentity>, spec) ||
    !identityMatches(started.value as Partial<WorkerLaunchIdentity>, spec)
  )
    return false;
  ...
  if (
    requestRecord.kind !== 'worker_launch_termination_request' ||
    requestRecord.pid !== startedRecord.pid ||
    requestRecord.process_start_identity !==
...
export async function materializeProviderSpawnInvocation(
  ...
  const superviseProcessTree =
    options.superviseProcessTree ?? options.superviseWindowsTree ?? false;
  ...
  try {
    const completionPath = superviseProcessTree
      ? join(wrapperDir, 'provider-exit.txt')
      : undefined;
    if (invocation.batchScript) {
      ...
      await writeFile(
        wrapperPath,
        `${invocation.batchScript}${completionScript}`,
    ...
    await rm(wrapperDir, { recursive: true, force: true }).catch(
      () => undefined,
    );
    throw error;
  }
...
async function publishProviderStarted(
  ...
  const record: WorkerLaunchProviderStarted = {
    ...
    containment_nonce: spec.containment_nonce,
    ...(supervisorCompletionPath
      ? { supervisor_completion_path: supervisorCompletionPath }
      : {}),
    written_at: new Date().toISOString(),
    authority_digest: spec.authority_digest,
...
export async function runWorkerLaunchBootstrap(
  value: unknown,
): Promise<WorkerLaunchBootstrapResult> {
  ...
  try {
    const launched = await withFileLock(
      lockPathFor(spec.current_path),
      async () => {
        if (
          !(await isCurrentLaunchIdentity(spec.current_path, spec)) ||
          (await readJson(`${spec.decision_path}.retired`)).kind !== 'absent'
        ...
        const invocation =
          process.platform === 'win32'
            ...
            : await materializeProviderSpawnInvocation(
        ...
        const child = spawn(invocation.command, invocation.args, {
          ...
          env: providerEnv,
          stdio:
            process.platform === 'win32' ? ['pipe', 'pipe', 'pipe'] : 'inherit',
          detached: process.platform !== 'win32',
        });
        if (
          process.platform === 'win32' &&
          invocation.stdinPayload &&
        ...
        let terminationResult: Promise<
          Awaited<ReturnType<typeof terminateOwnedProcessGroup>>
        > | null = null;
        ...
        const windowsReady = new Promise<boolean>((resolve) => {
          resolveWindowsReady = resolve;
        });
        const windowsTerminal = new Promise<boolean>((resolve) => {
          resolveWindowsTerminal = resolve;
        });
        if (process.platform === 'win32' && child.stdout) {
          ...
          child.stdout.on('data', (chunk) => {
            ...
            for (;;) {
              ...
              const matches =
                message.protocol === WINDOWS_SUPERVISOR_PROTOCOL &&
              ...
              if (
                ...
                void writeExclusiveAtomic(
                  `${spec.started_path}.termination-complete`,
                  {
                    ...
                    ...(Number.isSafeInteger(message.exit_code)
        ...
        const completion = new Promise<WorkerLaunchBootstrapResult>(
          (resolve) => {
            ...
            child.once('exit', async (exitCode, signal) => {
              ...
              const effectiveSignal =
                supervisedExitCode === null ? signal : null;
              const cleanupVerified =
                process.platform === 'win32'
                  ? (await awaitExternalTerminationCompletion(spec)) ||
                    (await readWorkerLaunchCleanupProof(spec))
                  : terminationResult
                    ? [
                        'terminated',
                        'already-dead',
                        'identity-mismatch',
                      ].includes(await terminationResult)
                    : (await awaitExternalTerminationCompletion(spec)) ||
                      (await readWorkerLaunchCleanupProof(spec)) ||
        ...
        if (process.platform === 'win32') {
          ...
          providerStartIdentity = child.pid
            ? getProcessStartIdentitySync(child.pid)
            : null;
          if (
            !child.pid ||
            !providerStartIdentity ||
          ...
          if (
            !reboundIdentity ||
            reboundIdentity !== providerStartIdentity ||
            !isProcessAlive(child.pid)
          ) {
            if (!(await terminateProvider()))
              return { outcome: 'provider_cleanup_unverified' as const };
        ...
        if (
          ...
          existsSync(invocation.completionPath)
        ) {
          const exitCode = Number(
            await readFile(invocation.completionPath, 'utf8').catch(() => ''),
          );
        ...
        if (invocation.completionPath) {
          ...
          supervisorTimer = setInterval(() => {
            ...
            void readFile(invocation.completionPath!, 'utf8')
              .then(async (raw) => {
        ...
        if (process.platform === 'win32') {
          ...
          supervisorTimer = setInterval(() => {
            ...
            void readJson(`${spec.started_path}.termination-request`)
              .then(async (request) => {
                if (request.kind !== 'value') return;
                const record = request.value as Record<string, unknown>;
                if (
                  !identityMatches(record, spec) ||
                  record.kind !== 'worker_launch_termination_request' ||
                  record.operation !== 'terminate' ||
                  record.pid !== providerPid ||
