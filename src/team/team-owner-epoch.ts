...
export type OwnerFenceCheck =
  | { ok: true; record: OwnerEpochRecord }
  | { ok: false; reason: 'missing' | 'malformed' | 'superseded' | 'mismatch' };
...
function parseRecord(
  ...
  try {
    ...
    if (
      ...
      !isValidProcessStartIdentity(parsed.process_started_at) ||
...
function darwinProcessStartFromKinfo(
  raw: Buffer,
  ...
  if (
    seconds < 946684800n ||
    seconds > BigInt(nowSeconds + 86400) ||
    micros >= 1_000_000n
  )
    return null;
  ...
  try {
    ...
    if (platform === 'darwin') {
      ...
      return started && Number.isFinite(startedAtMs)
        ? `darwin:${Math.floor(startedAtMs / 1000)}:0`
        : null;
    }
    const started = exec('ps', ['-o', 'lstart=', '-p', String(pid)], {
      encoding: 'utf8',
...
export function isValidProcessStartIdentity(
  value: unknown,
  platform: NodeJS.Platform = process.platform,
): value is string {
  ...
  return (
    separator > 0 &&
    value.slice(0, separator) === platform &&
...
function processStartIdentitiesMayMatch(
  recorded: string,
  observed: string,
): boolean {
...
export function isProcessIdentityDead(
  record: Pick<OwnerEpochRecord, 'pid' | 'process_started_at'>,
): boolean {
  if (
  ...
  return (
    isValidProcessStartIdentity(observed) &&
    !processStartIdentitiesMayMatch(record.process_started_at, observed)
  );
}

export function readLatestOwnerEpoch(
  cwd: string,
  teamName: string,
): OwnerEpochRecord | null {
  ...
  const record = parseRecord(
    join(directory, `${latestEpoch}.json`),
    latestEpoch,
  );
...
export function publishOwnerEpoch(
  cwd: string,
  teamName: string,
  epoch: number,
  input: OwnerEpochInput = {},
): OwnerEpochRecord {
  ...
  const start =
    ...
    currentProcessStartIdentity(input.pid ?? process.pid);
...
export function acquireSuccessorOwnerEpoch(
  cwd: string,
  teamName: string,
  input: OwnerEpochInput = {},
): OwnerEpochRecord {
...
export function checkOwnerFence(
  cwd: string,
  teamName: string,
  fence: OwnerFence,
): OwnerFenceCheck {
...
export function requireOwnerFence(
  cwd: string,
  teamName: string,
  fence: OwnerFence,
): OwnerEpochRecord {
...
export function isFreshRecoveryElection(
  config: TeamConfig,
  fence: OwnerFence,
...
export function isSameAttemptSuccessorRebind(
  config: TeamConfig,
  prior: TeamRuntimeOwnerEpoch,
  successor: OwnerFence,
  requestId: string,
  recoveryId: string,
): boolean {
  const active = config.active_recovery;
  return (
    successor.epoch === prior.epoch + 1 &&
...
export function isActiveRecoveryEffect(
  config: TeamConfig,
  fence: OwnerFence,
  requestId: string,
  recoveryId: string,
...
export function isFencedServiceMaintenance(
  config: TeamConfig,
  fence: OwnerFence,
): boolean {
  const marker = (config as TeamConfig & { service_recovery?: OwnerFence })
    .service_recovery;
  return (
    !config.active_recovery &&
    config.runtime_owner_epoch?.epoch === fence.epoch &&
