...
export interface RecoverySagaDependencies {
  ...
  validateCheckpoint: (
    ...
    | { ok: false; error: RecoverDeadWorkerV2Error }
  >;
  requeue: (
  ...
  activatePane: (
    input: RecoverySagaInput,
    paneAttemptId: string,
  ) => Promise<{ ok: true } | { ok: false; error: RecoverDeadWorkerV2Error }>;
...
export async function runRecoverySaga(
  ...
  const rejected = checks.find(
    (check): check is { ok: false; error: RecoverDeadWorkerV2Error } =>
      !check.ok,
  );
  if (rejected)
  ...
  const adoptionTokenHash = createHash('sha256')
    .update(input.adoptionToken)
    .digest('hex');
      ...
      return finalize(
        {
          ...
          outcome: 'commit_unknown',
          message:
            'Replacement was committed but continuation adoption remains pending.',
        },
        'reserved',
        'pending',
      );
