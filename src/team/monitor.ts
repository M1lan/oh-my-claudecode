...
function isWorkerInfo(value: unknown): boolean {
  if (
    ...
    !WORKER_NAME_SAFE_PATTERN.test(value.name) ||
    !isSafeCounter(value.index) ||
    value.index < 1
  )
    return false;
  return (
    (value.role === undefined || typeof value.role === 'string') &&
    (value.assigned_tasks === undefined ||
      isStringArray(value.assigned_tasks)) &&
    (value.worker_cli === undefined ||
      ['claude', 'codex', 'gemini', 'cursor', 'grok', 'antigravity'].includes(
        value.worker_cli as string,
      )) &&
    ...
    (value.worktree_repo_root === undefined ||
      typeof value.worktree_repo_root === 'string') &&
    (value.worktree_path === undefined ||
      typeof value.worktree_path === 'string') &&
    (value.worktree_branch === undefined ||
      typeof value.worktree_branch === 'string') &&
    (value.worktree_detached === undefined ||
      typeof value.worktree_detached === 'boolean') &&
    (value.worktree_created === undefined ||
      typeof value.worktree_created === 'boolean') &&
    (value.team_state_root === undefined ||
      typeof value.team_state_root === 'string') &&
    (value.output_file === undefined ||
      typeof value.output_file === 'string') &&
    (value.recovery_id === undefined || isNonEmptyString(value.recovery_id)) &&
    (value.replacement_generation === undefined ||
      isSafeCounter(value.replacement_generation)) &&
    (value.pane_attempt_id === undefined ||
      isNonEmptyString(value.pane_attempt_id)) &&
    (value.operational_state === undefined ||
      ['starting', 'active', 'dead', 'stopped'].includes(
        value.operational_state as string,
      )) &&
    (value.launch_attempt_id === undefined ||
      isNonEmptyString(value.launch_attempt_id)) &&
    (value.launch_descriptor === undefined ||
      isLaunchDescriptor(value.launch_descriptor))
  );
}
...
function isOwnerEpoch(value: unknown): boolean {
  return (
    ...
    isSafeCounter(value.pid) &&
    value.pid > 0 &&
    isNonEmptyString(value.process_started_at) &&
...
function isRecoveryAttempt(value: unknown): boolean {
  return (
    ...
    isNonEmptyString(value.recovery_id) &&
    isNonEmptyString(value.worker_name) &&
    isSafeCounter(value.owner_epoch) &&
    value.owner_epoch > 0 &&
    isNonEmptyString(value.owner_nonce) &&
    [
      'reserved',
      'requeued',
      'ready',
    ...
    (value.original_pane_id === undefined ||
      typeof value.original_pane_id === 'string') &&
    isSafeCounter(value.state_revision) &&
    isTimestamp(value.created_at) &&
...
function isScaleUpAttempt(value: unknown): boolean {
  return (
    ...
    ['reserved', 'effects', 'committed', 'failed'].includes(
...
function isScaleDownAttempt(value: unknown): boolean {
  return (
    isRecord(value) &&
    isNonEmptyString(value.operation_id) &&
    ['draining', 'effects', 'failed'].includes(value.phase as string) &&
    isSafeCounter(value.pid) &&
    value.pid > 0 &&
    isNonEmptyString(value.process_started_at) &&
    ...
    value.workers.every(
      (worker) =>
        isRecord(worker) &&
        isNonEmptyString(worker.name) &&
        (worker.pane_id === undefined || typeof worker.pane_id === 'string') &&
        (worker.worktree_path === undefined ||
          typeof worker.worktree_path === 'string') &&
        (worker.worktree_created === undefined ||
          typeof worker.worktree_created === 'boolean'),
    ) &&
    isSafeCounter(value.state_revision) &&
    isTimestamp(value.created_at) &&
    isTimestamp(value.updated_at) &&
...
function isServiceDescriptor(value: unknown): boolean {
  return (
    isRecord(value) &&
    value.schema_version === 1 &&
    isSafeCounter(value.service_generation) &&
    isNonEmptyString(value.service_attempt_id) &&
    typeof value.auto_merge_enabled === 'boolean' &&
    isNonEmptyString(value.workspace_root) &&
    (value.leader_branch === undefined ||
      typeof value.leader_branch === 'string') &&
    ['disabled', 'worker-auto-commit-v1'].includes(
      value.cadence_policy as string,
    )
  );
}
...
function isTeamConfig(
  ...
  if (
    ...
    (expectedTeamName !== undefined && value.name !== expectedTeamName) ||
    !isNonEmptyString(value.agent_type) ||
    (value.task !== undefined && typeof value.task !== 'string') ||
    (value.worker_launch_mode !== undefined &&
      !['interactive', 'prompt'].includes(
        value.worker_launch_mode as string,
      )) ||
    !isSafeCounter(value.worker_count) ||
    (value.max_workers !== undefined && !isSafeCounter(value.max_workers)) ||
    !Array.isArray(value.workers) ||
    value.worker_count !== value.workers.length ||
    !value.workers.every(isWorkerInfo) ||
    !hasUniqueWorkerIdentity(value.workers) ||
    !isTimestamp(value.created_at) ||
    !isNonEmptyString(value.tmux_session) ||
    (value.next_task_id !== undefined && !isSafeCounter(value.next_task_id)) ||
    !isOptionalPolicy(value.policy) ||
    !isOptionalGovernance(value.governance) ||
    !isOptionalWorkspaceShape(value) ||
    !isOptionalPaneShape(value) ||
  ...
  return (
    (value.lifecycle_state === undefined ||
      ['active', 'shutting_down', 'stopped'].includes(
        value.lifecycle_state as string,
      )) &&
    (value.runtime_owner_epoch === undefined ||
      isOwnerEpoch(value.runtime_owner_epoch)) &&
    (value.active_recovery === undefined ||
      isRecoveryAttempt(value.active_recovery)) &&
    (value.last_recovery === undefined ||
      isRecoveryAttempt(value.last_recovery)) &&
    (value.active_scale_up === undefined ||
      isScaleUpAttempt(value.active_scale_up)) &&
    (value.active_scale_down === undefined ||
      isScaleDownAttempt(value.active_scale_down)) &&
    (value.service_descriptor === undefined ||
      isServiceDescriptor(value.service_descriptor)) &&
    (value.shutdown_attempt === undefined ||
      isShutdownAttempt(value.shutdown_attempt)) &&
    (value.all_dead_recovery === undefined ||
      isAllDeadRecovery(value.all_dead_recovery)) &&
...
function hasUniqueWorkerIdentity(workers: unknown[]): boolean {
  ...
  return workers.every((worker) => {
    if (
      ...
      !WORKER_NAME_SAFE_PATTERN.test(worker.name) ||
...
function isOptionalPolicy(value: unknown): boolean {
  return (
    ...
    (isRecord(value) &&
      ...
      ['hook_preferred_with_fallback', 'transport_direct'].includes(
        value.dispatch_mode as string,
      ) &&
...
function isOptionalWorkspaceShape(value: Record<string, unknown>): boolean {
  return (
    (value.leader_cwd === undefined || typeof value.leader_cwd === 'string') &&
    (value.team_state_root === undefined ||
      typeof value.team_state_root === 'string') &&
    (value.workspace_mode === undefined ||
      ['single', 'worktree'].includes(value.workspace_mode as string)) &&
    (value.worktree_mode === undefined ||
      ['disabled', 'detached', 'named'].includes(
        value.worktree_mode as string,
      )) &&
    (value.lifecycle_profile === undefined ||
      ['default', 'linked_ralph'].includes(value.lifecycle_profile as string))
  );
}

function isOptionalPaneShape(value: Record<string, unknown>): boolean {
  return (
    (value.leader_pane_id === undefined ||
      value.leader_pane_id === null ||
      typeof value.leader_pane_id === 'string') &&
    (value.hud_pane_id === undefined ||
      value.hud_pane_id === null ||
      typeof value.hud_pane_id === 'string') &&
    (value.resize_hook_name === undefined ||
      value.resize_hook_name === null ||
      typeof value.resize_hook_name === 'string') &&
    (value.resize_hook_target === undefined ||
      value.resize_hook_target === null ||
      typeof value.resize_hook_target === 'string') &&
    (value.next_worker_index === undefined ||
      (isSafeCounter(value.next_worker_index) && value.next_worker_index > 0))
  );
}
...
function isOptionalRouting(value: unknown): boolean {
  ...
  if (
    !isRecord(value) ||
    Object.keys(value).length !== CANONICAL_TEAM_ROLES.length
  )
    return false;
  return CANONICAL_TEAM_ROLES.every((role) => isResolvedRoleRoute(value[role]));
}

function isResolvedRoleRoute(
  value: unknown,
): value is { primary: RoleAssignment; fallback: RoleAssignment } {
  return (
    isRecord(value) &&
    isRoleAssignment(value.primary) &&
    isRoleAssignment(value.fallback)
  );
}
...
function isRoleAssignment(value: unknown): value is RoleAssignment {
  return (
    isRecord(value) &&
    ['claude', 'codex', 'gemini', 'grok', 'cursor', 'antigravity'].includes(
      value.provider as string,
    ) &&
...
function hasMatchingActiveFenceRevisions(
  ...
  return [
    ...
    (fence) =>
      fence === undefined ||
      (isRecord(fence) && fence.state_revision === revision),
  );
}

export function alignActiveFenceRevisions(
  config: TeamConfig,
  revision: number,
): TeamConfig {
  return {
...
export function validateRevisionedTeamConfig(
  value: unknown,
  expectedTeamName?: string,
): TeamConfig | null {
...
export function validateLegacyTeamConfig(
  value: unknown,
  expectedTeamName?: string,
): TeamConfig | null {
...
async function assertPersistedConfigPathBinding(
  teamName: string,
  cwd: string,
  includeManifestWhenAbsent = false,
): Promise<void> {
  const state = await readJsonFileState<TeamConfig>(
  ...
  const manifestState = await readJsonFileState<TeamManifestV2>(
    absPath(cwd, TeamPaths.manifest(teamName)),
  );
  if (manifestState.kind === 'invalid')
    throw new Error('invalid_persisted_state');
  if (
    manifestState.kind === 'value' &&
...
export async function readTeamConfig(
  ...
  const [configState, manifestState] = await Promise.all([
    ...
    readJsonFileState<TeamManifestV2>(
      absPath(cwd, TeamPaths.manifest(teamName)),
    ),
  ]);
  ...
  const manifest =
    manifestState.kind === 'value'
      ? normalizeTeamManifest(manifestState.value)
      : null;
...
export async function readRevisionedTeamConfig(
  teamName: string,
  cwd: string,
...
export function withTeamConfigMutationLock<T>(
  teamName: string,
  cwd: string,
  fn: () => Promise<T> | T,
): Promise<T> {
  ...
  return withTeamConfigMutationLock(teamName, cwd, async () => {
    ...
    if (configState.kind === 'value') {
      ...
      if (legacy) {
        ...
        const revisioned = validateRevisionedTeamConfig(
          configState.value,
          teamName,
        );
        if (!revisioned) throw new Error('invalid_persisted_state');
...
export type FenceReclaimAuthorization = Partial<
  Record<ActiveFenceFamily, true>
>;
  ...
  const checkScaleLike = (
    ...
    if (cur && next) {
      if (sameScaleOwner(cur, next)) {
        ...
        const scaleDownFailedResume =
          family === 'active_scale_down' &&
          cur.phase === 'failed' &&
          next.phase === 'draining';
        if (!scaleDownFailedResume && to < from)
          throw new Error('invalid_persisted_state');
        if (
          family === 'active_scale_up' &&
          cur.phase === 'committed' &&
        ...
        if (
          preserveWorkers &&
          JSON.stringify(cur.workers) !== JSON.stringify(next.workers)
        ) {
          throw new Error('invalid_persisted_state');
        }
        return;
...
export async function saveTeamConfigAtRevision(
  ...
  if (
    !validateRevisionedTeamConfig(
      alignActiveFenceRevisions(config, config.state_revision),
      config.name,
    )
  ) {
    throw new Error('invalid_persisted_state');
...
export async function readTeamManifest(
  ...
  const state = await readJsonFileState<TeamManifestV2>(
    absPath(cwd, TeamPaths.manifest(teamName)),
  );
  if (state.kind === 'invalid') throw new Error('invalid_persisted_state');
...
export async function writeWorkerStatus(
  ...
  const persisted =
    launchAttemptId && !status.launch_attempt_id
...
async function saveTeamConfigUnlocked(
  config: TeamConfig,
  cwd: string,
): Promise<void> {
  ...
  const existingManifest =
    manifestState.kind === 'value' ? manifestState.value : null;
  if (existingManifest) {
...
export async function saveTeamConfig(
  ...
  if (
    !(inputIsRevisioned
      ? validateRevisionedTeamConfig(config, config.name)
      : validateLegacyTeamConfig(config, config.name))
  ) {
    throw new Error('invalid_persisted_state');
  }
  await assertPersistedConfigPathBinding(config.name, cwd);
  await withTeamConfigMutationLock(config.name, cwd, async () => {
    const currentState = await readJsonFileState<TeamConfig>(
      absPath(cwd, TeamPaths.config(config.name)),
    );
    ...
    if (
      current &&
      Object.hasOwn(current, 'state_revision') &&
      !validateRevisionedTeamConfig(current, config.name)
    )
      throw new Error('invalid_persisted_state');
    if (
      current &&
      !Object.hasOwn(current, 'state_revision') &&
      !validateLegacyTeamConfig(current, config.name)
    )
      throw new Error('invalid_persisted_state');
    ...
    if (
      ...
      if (
        ...
        config.state_revision !== expectedRevision
      ) {
...
export async function withScalingLock<T>(
  ...
  return withProcessIdentityFileLock(
    absPath(cwd, TeamPaths.scalingLock(teamName)),
    fn,
    timeoutMs,
  );
}
