...
function isDispatchTransportPreference(
  value: unknown,
): value is TeamDispatchTransportPreference {
  return (
    value === 'hook_preferred_with_fallback' ||
  ...
  while (true) {
    try {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, Math.floor(pollMs * jitter)),
      );
...
async function readDispatchRequestsFromFile(
  teamName: string,
  cwd: string,
    ...
    return parsed
      .map((entry) =>
        normalizeDispatchRequest(
          teamName,
          entry as Partial<TeamDispatchRequest>,
        ),
      )
...
async function writeDispatchRequestsToFile(
  teamName: string,
...
function materializeStrictDispatchRequest(
  ...
  const request: TeamDispatchRequest = {
    ...
    transport_preference:
      raw.transport_preference as TeamDispatchTransportPreference,
  ...
  if ('inbox_correlation_key' in raw)
    request.inbox_correlation_key = raw.inbox_correlation_key as string;
...
function validateStrictDispatchRow(
  ...
  if (
    'inbox_correlation_key' in raw &&
    !isStrictText(raw.inbox_correlation_key)
  ) {
...
async function readStrictDispatchStore(
  teamName: string,
  cwd: string,
): Promise<StrictDispatchStoreResult> {
  const path = absPath(cwd, TeamPaths.dispatchRequests(teamName));
  let parsed: unknown;
  try {
...
function lookupStrictDispatchRequest(
  ...
  const duplicateIndexes = [...indexesByRequestId.values()]
...
async function readStrictDispatchRequestWithIndex(
  ...
  read: StrictDispatchLookupResult;
  store?: StrictValidatedDispatchStore;
}> {
...
export async function enqueueDispatchRequest(
  ...
  if (
    requestInput.kind === 'mailbox' &&
    (!requestInput.message_id || requestInput.message_id.trim() === '')
  ) {
  ...
  return await withDispatchLock(teamName, cwd, async () => {
    ...
    const existing = requests.find((req) =>
      equivalentPendingDispatch(req, requestInput),
    );
