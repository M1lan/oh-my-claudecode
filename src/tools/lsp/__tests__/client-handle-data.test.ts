...
describe('LspClient handleData byte-length fix (#1026)', () => {
  ...
  it('preserves numeric response resolution and rejection without string-ID coercion', () => {
    ...
    (client as any).handleData(
      buildLspMessage(
        JSON.stringify({ jsonrpc: '2.0', id: '1', result: 'wrong' }),
      ),
    );
  ...
  it('releases a queued public request only after its registration error reply is pumped', async () => {
    ...
    const stdin = {
      write: vi.fn((message: string) => {
        ...
        if (outgoing.method === 'initialize') {
          stdoutData!(
            buildLspMessage(
              jsonRpcResponse(outgoing.id as number, { capabilities: {} }),
            ),
          );
          ...
          outgoing.id === 'ts1' &&
          (outgoing.error as { code?: number } | undefined)?.code === -32803
        ) {
          ...
          pumpQueuedWorkspaceResult = () => {
            ...
            stdoutData!(
              buildLspMessage(jsonRpcResponse(outgoing.id as number, [])),
            );
          };
        }
      }),
    ...
    expect(decodeLspMessage(writes[3])).toMatchObject({
      id: 2,
