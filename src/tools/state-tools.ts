...
function clearWorkingDirectoryLocalStateCandidates(
  ...
  let cleared = 0;
  let hadFailure = false;
  const paths = getWorkingDirectoryLocalStateClearCandidates(
    mode,
    root,
    sessionId,
  );
...
export const stateClearTool: ToolDefinition<{
  ...
  name: 'state_clear',
  description:
    'Clear/delete state for a specific mode. Removes the state file and any associated marker files. For merge-readiness, cancels an active gate while preserving the terminal audit record (no deletion).',
  annotations: {
    readOnlyHint: false,
  ...
  handler: async (args) => {
    ...
    try {
      ...
      if (sessionId) {
        validateSessionId(sessionId);
        const requestedSessionOwnedPaths = findSessionOwnedStateFiles(
          mode,
          sessionId,
          root,
        );
      ...
      // Mirrors the session_id path at line ~403. (patch: fix missing cancel signal)
      {
        ...
        for (const sid of listSessionIds(root)) {
          try {
            const sessionSignalPath = resolveSessionStatePath(
              'cancel-signal',
              sid,
              root,
            );
      ...
      for (const sid of sessionIds) {
        if (mode === 'team') {
          collectTeamNamesForCleanup(
            resolveSessionStatePath('team', sid, root),
          );
        }
        if (MODE_CONFIGS[mode as ExecutionMode]) {
          // Only clear if state file exists - avoid false counts for missing files
          const sessionStatePath = getStateFilePath(
            root,
            mode as ExecutionMode,
            sid,
          );
