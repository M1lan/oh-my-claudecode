...
function processGroupIdSync(pid: number): number | null {
  ...
  if (process.platform === 'darwin') {
    try {
      ...
      return result.status === 0 && Number.isInteger(group) && group > 0
        ? group
        : null;
    } catch {
      return null;
...
export function captureOwnedProcessGroup(
  pid: number,
...
> {
  ...
  if (
    identity !== options.expectedStartIdentity ||
...
export function dmtfCreationDateToTicks(dmtf: string): string | null {
  ...
  if (
    ![year, month, day, hour, minute, second, micros, offsetMin].every(
      Number.isFinite,
    )
  )
    return null;
  ...
  const ticks =
    BigInt(wholeUtcMs) * 10000n + BigInt(micros) * 10n + 621355968000000000n;
  return `ticks:${ticks.toString()}`;
}

async function getProcessStartIdentityWindows(
  pid: number,
  deadlineAt?: number,
): Promise<string | null> {
  for (const command of [
    ...
    try {
      ...
      /* try the next exact Windows identity source */
    }
