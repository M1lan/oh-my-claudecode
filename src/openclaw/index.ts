...
export async function wakeOpenClaw(
  ...
  try {
    ...
    const tmuxSession =
      context.tmuxSession ?? getCurrentTmuxSession() ?? undefined;
    ...
    if (
      shouldCollapseOpenClawBurst(event, signal, enrichedContext, tmuxSession)
    ) {
      if (DEBUG) {
        console.error(
    ...
    if (isCommandGateway(gateway)) {
      ...
      result = await wakeCommandGateway(
        gatewayName,
        gateway,
        variables,
        payload,
