...
describe("autoresearch contracts", () => {
  it("slugifies mission names deterministically", () => {
    expect(slugifyMissionName("Missions/My Demo Mission")).toBe(
      "missions-my-demo-mission",
    );
  });
  ...
  it("rejects sandbox contract without frontmatter", () => {
    expect(() => parseSandboxContract("No frontmatter here")).toThrow(
...
`),
  ...
  it("accepts evaluator result with pass and score", () => {
    expect(parseEvaluatorResult('{"pass":false,"score":61}')).toEqual({
      pass: false,
      score: 61,
    });
  });
  ...
  it("rejects evaluator result without pass", () => {
    expect(() => parseEvaluatorResult('{"score":61}')).toThrow(
      /must include boolean pass/i,
    );
  });

  it("rejects evaluator result with non-numeric score", () => {
    expect(() => parseEvaluatorResult('{"pass":true,"score":"high"}')).toThrow(
  ...
  it("loads mission contract from in-repo mission directory", async () => {
    ...
    try {
      ...
      expect(contract.missionRelativeDir.replace(/\\/g, "/")).toBe(
        "missions/demo",
      );
