...
describe('autoresearch runtime', () => {
  it('builds bootstrap instructions with mission, sandbox, and evaluator contract', async () => {
    ...
    try {
      ...
      const instructions = buildAutoresearchInstructions(contract, {
        ...
        candidateFile:
          '.omc/logs/autoresearch/missions-demo-20260314t000000z/candidate.json',
  ...
  it('allows untracked .omc runtime files when checking reset safety', async () => {
    ...
    try {
      ...
      await writeFile(
        join(repo, '.omc', 'logs', 'hooks-2026-03-15.jsonl'),
        '{}\n',
        'utf-8',
      );
      await writeFile(join(repo, '.omc', 'metrics.json'), '{}\n', 'utf-8');
      await writeFile(
        join(repo, '.omc', 'state', 'hud-state.json'),
        '{}\n',
        'utf-8',
      );
  ...
  it('prepares runtime artifacts and persists autoresearch mode state', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      await mkdir(join(repo, 'node_modules', 'fixture-dep'), {
        recursive: true,
      });
      await writeFile(
        join(repo, 'node_modules', 'fixture-dep', 'index.js'),
        'export default 1;\n',
        'utf-8',
      );
      const worktreePath = join(
        ...
        'autoresearch-missions-demo-20260314t000000z',
      );
      execFileSync(
        'git',
        [
      ...
      expect(manifest.branch_name).toBe(
        'autoresearch/missions-demo/20260314t000000z',
      );
      ...
      const ledger = JSON.parse(
        await readFile(runtime.ledgerFile, 'utf-8'),
      ) as Record<string, unknown>;
      expect(Array.isArray(ledger.entries)).toBe(true);
      expect((ledger.entries as unknown[]).length).toBe(1);

      const latestEvaluator = JSON.parse(
        await readFile(runtime.latestEvaluatorFile, 'utf-8'),
      ) as Record<string, unknown>;
      ...
      const worktreeState = readModeState<Record<string, unknown>>(
        'autoresearch',
        worktreePath,
      );
      ...
      expect(state?.mission_spec_file).toBe(
        join(repo, '.omc', 'autoresearch', 'missions-demo', 'mission.md'),
      );
      expect(state?.evaluator_reference_file).toBe(
        join(repo, '.omc', 'autoresearch', 'missions-demo', 'evaluator.json'),
      );
  ...
  it('materializes canonical mission artifacts and markdown decision log paths', async () => {
    ...
    try {
      ...
      const worktreePath = join(
        ...
        '..',
        `${repo.split('/').pop()}.omc-worktrees`,
        'autoresearch-missions-demo-20260314t000500z',
      );
      execFileSync(
        'git',
        [
      ...
      const layout = getAutoresearchMissionArtifactLayout(
        repo,
      ...
      const evaluatorRef = JSON.parse(
        await readFile(layout.evaluatorReferenceFile, 'utf-8'),
      ) as Record<string, unknown>;
  ...
  it('keeps improved candidates and resets discarded candidates back to the last kept commit', async () => {
    ...
    try {
      ...
      const worktreePath = join(
        ...
        '..',
        `${repo.split('/').pop()}.omc-worktrees`,
        'autoresearch-missions-demo-20260314t010000z',
      );
      execFileSync(
        'git',
        [
      ...
      execFileSync('git', ['commit', '-m', 'improve score'], {
        cwd: worktreePath,
        stdio: 'ignore',
      });
      const improvedCommit = execFileSync('git', ['rev-parse', 'HEAD'], {
        cwd: worktreePath,
        encoding: 'utf-8',
      }).trim();
      const initialManifest = await loadAutoresearchRunManifest(
        repo,
        runtime.runId,
      );
      await writeFile(
      ...
      const keepDecision = await processAutoresearchCandidate(
        worktreeContract,
        initialManifest,
        repo,
      );
      expect(keepDecision).toBe('keep');
      const keptManifest = await loadAutoresearchRunManifest(
        repo,
        runtime.runId,
      );
      ...
      const beforeDiscardManifest = await loadAutoresearchRunManifest(
        repo,
        runtime.runId,
      );
      await writeFile(
      ...
      const discardDecision = await processAutoresearchCandidate(
        worktreeContract,
        beforeDiscardManifest,
        repo,
      );
      expect(discardDecision).toBe('discard');
      const headAfterDiscard = execFileSync('git', ['rev-parse', 'HEAD'], {
        cwd: worktreePath,
        encoding: 'utf-8',
      }).trim();
      ...
      expect(instructions).toMatch(
        /"previous_iteration_outcome": "discard:score did not improve"/,
      );
      ...
      const evaluationOne = JSON.parse(
        await readFile(
          join(
            repo,
            '.omc',
...
describe('autoresearch startup exclusivity', () => {
  it('blocks startup when a session-scoped ralph state is active', async () => {
    const repo = await initRepo();
    try {
      expect(writeModeState('ralph', { active: true }, repo, 'session-a')).toBe(
        true,
      );

      await expect(
        assertModeStartAllowed('autoresearch', repo),
      ).rejects.toThrow('Cannot start autoresearch: ralph is already active');
    } finally {
      await rm(repo, { recursive: true, force: true });
    }
  });
