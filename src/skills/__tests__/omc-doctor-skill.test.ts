...
describe('omc-doctor skill (issue #2254)', () => {
  it('documents CLAUDE.md OMC version drift check against cached plugin version', () => {
    ...
    expect(content).toContain(
      'VERSION DRIFT: CLAUDE.md and plugin versions differ',
    );
    expect(content).toContain(
      'VERSION CHECK SKIPPED: missing CLAUDE marker or plugin cache',
    );
    expect(content).toContain(
      'VERSION MATCH: CLAUDE and plugin cache are aligned',
    );
    ...
    expect(content).toContain(
      'If `CLAUDE.md OMC version` != `Latest cached plugin version`: WARN - version drift detected',
    );
  });
});
...
describe('omc-doctor skill package version diagnostic (issue #2981)', () => {
  it('checks the canonical published npm package for latest version', () => {
    ...
    expect(content).toContain('pnpm info oh-my-claude-sisyphus version');
    expect(content).not.toContain('pnpm info oh-my-claudecode version');
  });
});
