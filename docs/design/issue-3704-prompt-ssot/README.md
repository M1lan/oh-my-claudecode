...
| Build gate | `scripts/build-prompt-ssot.ts` (`pnpm run prompt-ssot:build` / `:check`) | Regenerates `generated/prompt-ssot/*.md`; `--check` exits 1 on stale or stray projections |
| Measurement | `scripts/measure-prompt-ssot.ts` (`pnpm run prompt-ssot:measure`) | Emits `measurements.json` acceptance evidence |
...
From `docs/design/issue-3704-prompt-ssot/measurements.json` (reproduce: `pnpm run prompt-ssot:measure`):
...
```
pnpm exec vitest run src/agents/prompt-ssot   # 17 passed
pnpm exec tsc --noEmit                        # clean
pnpm run prompt-ssot:check               # 5 projections fresh
```
