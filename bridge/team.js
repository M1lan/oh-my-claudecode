...
function readWorkspaceMarkerConfig(workspaceRoot) {
  try {
    const raw = readFileSync(
      join2(workspaceRoot, WORKSPACE_MARKER),
      "utf-8"
    ).trim();
    if (!raw) return {};
  ...
  "src/team/state-paths.ts"() {
    ...
    TeamPaths = {
      ...
      recoveryOwnerBootstrapCandidate: (teamName, expectedEpoch, nonce) => {
        if (nonce.length === 0 || nonce.length > 128 || nonce === "." || nonce === ".." || !/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(nonce))
...
function parseRecord(path4, expectedEpoch) {
  try {
    ...
    if (parsed.schema_version !== 1 || !Number.isSafeInteger(parsed.epoch) || parsed.epoch < 1 || expectedEpoch !== void 0 && parsed.epoch !== expectedEpoch || typeof parsed.nonce !== "string" || typeof parsed.pid !== "number" || !isValidProcessStartIdentity(parsed.process_started_at) || typeof parsed.payload_hash !== "string")
      return null;
  ...
  try {
    ...
    if (platform === "darwin") {
      try {
        ...
        const birth = darwinProcessStartFromKinfo(
    ...
    const started = exec3("ps", ["-o", "lstart=", "-p", String(pid)], {
      encoding: "utf8"
    }).trim();
...
function readLatestOwnerEpoch(cwd, teamName) {
  ...
  const record = parseRecord(
    join4(directory, `${latestEpoch}.json`),
    latestEpoch
  );
...
function publishOwnerEpoch(cwd, teamName, epoch, input = {}) {
  ...
  const temp = join4(
    dirname2(target),
    `.${epoch}.${record.nonce}.${randomUUID()}.tmp`
  );
...
async function withProcessIdentityFileLock(lockPath, fn, timeoutMs = 1e4) {
  ...
  writeFileSync3(tempPath, JSON.stringify(owner), {
    encoding: "utf8",
    mode: 384,
    flush: true
  });
      ...
      try {
        ...
        if (existing && isProcessIdentityDead(existing)) {
          try {
            ...
            if (reclaimError.code !== "EEXIST" && reclaimError.code !== "ENOENT")
              throw reclaimError;
          }
        }
        if (Date.now() >= deadline)
...
function canonicalizeWorkers(workers) {
  ...
  for (const worker of workers) {
    ...
    byName.set(name, {
      ...winner,
      name,
      assigned_tasks: mergeAssignedTasks(
        winner.assigned_tasks,
        loser.assigned_tasks
      ),
      ...
      worktree_repo_root: backfillText(
        ...
        loser.worktree_repo_root
      ),
      worktree_path: backfillText(winner.worktree_path, loser.worktree_path),
      worktree_branch: backfillText(
        winner.worktree_branch,
        loser.worktree_branch
      ),
      worktree_detached: backfillBoolean(
        winner.worktree_detached,
        loser.worktree_detached
      ),
      worktree_created: backfillBoolean(
        winner.worktree_created,
        loser.worktree_created
      ),
      team_state_root: backfillText(
        winner.team_state_root,
        loser.team_state_root
      )
    });
  }
  return {
...
function isWorkerInfo(value) {
  if (!isRecord(value) || typeof value.name !== "string" || !WORKER_NAME_SAFE_PATTERN.test(value.name) || !isSafeCounter(value.index) || value.index < 1)
    return false;
  return (value.role === void 0 || typeof value.role === "string") && (value.assigned_tasks === void 0 || isStringArray(value.assigned_tasks)) && (value.worker_cli === void 0 || ["claude", "codex", "gemini", "cursor", "grok", "antigravity"].includes(
    value.worker_cli
  )) && (value.pid === void 0 || isSafeCounter(value.pid) && value.pid > 0) && (value.pane_id === void 0 || typeof value.pane_id === "string") && (value.working_dir === void 0 || typeof value.working_dir === "string") && (value.worktree_repo_root === void 0 || typeof value.worktree_repo_root === "string") && (value.worktree_path === void 0 || typeof value.worktree_path === "string") && (value.worktree_branch === void 0 || typeof value.worktree_branch === "string") && (value.worktree_detached === void 0 || typeof value.worktree_detached === "boolean") && (value.worktree_created === void 0 || typeof value.worktree_created === "boolean") && (value.team_state_root === void 0 || typeof value.team_state_root === "string") && (value.output_file === void 0 || typeof value.output_file === "string") && (value.recovery_id === void 0 || isNonEmptyString(value.recovery_id)) && (value.replacement_generation === void 0 || isSafeCounter(value.replacement_generation)) && (value.pane_attempt_id === void 0 || isNonEmptyString(value.pane_attempt_id)) && (value.operational_state === void 0 || ["starting", "active", "dead", "stopped"].includes(
    value.operational_state
  )) && (value.launch_descriptor === void 0 || isLaunchDescriptor(value.launch_descriptor));
}
function isLaunchDescriptor(value) {
  return isRecord(value) && value.schema_version === 1 && ["claude", "codex", "gemini", "cursor", "grok", "antigravity"].includes(
    value.provider
  ) && (value.model === null || typeof value.model === "string") && isNonEmptyString(value.binary) && isStringArray(value.args);
}
...
function isRecoveryAttempt(value) {
  return isRecord(value) && isNonEmptyString(value.request_id) && isNonEmptyString(value.recovery_id) && isNonEmptyString(value.worker_name) && isSafeCounter(value.owner_epoch) && value.owner_epoch > 0 && isNonEmptyString(value.owner_nonce) && [
    "reserved",
    "requeued",
    "ready",
    "active",
    ...
    "adopted",
    "failed"
  ].includes(value.phase) && (value.original_pane_id === void 0 || typeof value.original_pane_id === "string") && isSafeCounter(value.state_revision) && isTimestamp(value.created_at) && isTimestamp(value.updated_at);
}
...
function isScaleDownAttempt(value) {
  return isRecord(value) && isNonEmptyString(value.operation_id) && ["draining", "effects", "failed"].includes(value.phase) && isSafeCounter(value.pid) && value.pid > 0 && isNonEmptyString(value.process_started_at) && Array.isArray(value.workers) && value.workers.every(
    (worker) => isRecord(worker) && isNonEmptyString(worker.name) && (worker.pane_id === void 0 || typeof worker.pane_id === "string") && (worker.worktree_path === void 0 || typeof worker.worktree_path === "string") && (worker.worktree_created === void 0 || typeof worker.worktree_created === "boolean")
  ) && isSafeCounter(value.state_revision) && isTimestamp(value.created_at) && isTimestamp(value.updated_at) && (value.failure_reason === void 0 || typeof value.failure_reason === "string");
}
function isServiceDescriptor(value) {
  return isRecord(value) && value.schema_version === 1 && isSafeCounter(value.service_generation) && isNonEmptyString(value.service_attempt_id) && typeof value.auto_merge_enabled === "boolean" && isNonEmptyString(value.workspace_root) && (value.leader_branch === void 0 || typeof value.leader_branch === "string") && ["disabled", "worker-auto-commit-v1"].includes(
    value.cadence_policy
  );
}
...
function isOptionalPolicy(value) {
  return value === void 0 || isRecord(value) && ["split_pane", "auto"].includes(value.display_mode) && ["interactive", "prompt"].includes(value.worker_launch_mode) && ["hook_preferred_with_fallback", "transport_direct"].includes(
    value.dispatch_mode
...
function isOptionalWorkspaceShape(value) {
  return (value.leader_cwd === void 0 || typeof value.leader_cwd === "string") && (value.team_state_root === void 0 || typeof value.team_state_root === "string") && (value.workspace_mode === void 0 || ["single", "worktree"].includes(value.workspace_mode)) && (value.worktree_mode === void 0 || ["disabled", "detached", "named"].includes(
    value.worktree_mode
  )) && (value.lifecycle_profile === void 0 || ["default", "linked_ralph"].includes(value.lifecycle_profile));
}
...
function isRoleAssignment(value) {
  return isRecord(value) && ["claude", "codex", "gemini", "grok", "cursor", "antigravity"].includes(
    value.provider
  ) && isNonEmptyString(value.model) && KNOWN_AGENT_NAMES.some((agent) => agent === value.agent);
}
function hasMatchingActiveFenceRevisions(value) {
  ...
  return [
    ...
    value.active_scale_up,
    value.active_scale_down,
    value.shutdown_attempt,
    value.all_dead_recovery
  ].every(
    (fence) => fence === void 0 || isRecord(fence) && fence.state_revision === revision
  );
}
function alignActiveFenceRevisions(config, revision) {
  return {
    ...
    ...config.active_scale_down ? {
      active_scale_down: {
        ...config.active_scale_down,
        state_revision: revision
      }
    } : {},
    ...config.shutdown_attempt ? {
      shutdown_attempt: {
        ...config.shutdown_attempt,
        state_revision: revision
      }
    } : {},
    ...config.all_dead_recovery ? {
      all_dead_recovery: {
        ...config.all_dead_recovery,
        state_revision: revision
      }
...
async function assertPersistedConfigPathBinding(teamName, cwd, includeManifestWhenAbsent = false) {
  const state = await readJsonFileState(
  ...
  const manifestState = await readJsonFileState(
    absPath(cwd, TeamPaths.manifest(teamName))
  );
  if (manifestState.kind === "invalid")
    throw new Error("invalid_persisted_state");
  if (manifestState.kind === "value" && !validateLegacyTeamConfig(
    configFromManifest(normalizeTeamManifest(manifestState.value)),
    teamName
  )) {
    throw new Error("invalid_persisted_state");
  }
...
async function readRevisionedTeamConfig(teamName, cwd) {
  ...
  if (revisioned)
    return {
      config: canonicalizeTeamConfigWorkers(revisioned),
      stateRevision: revisioned.state_revision
    };
...
function withTeamConfigMutationLock(teamName, cwd, fn) {
  return withProcessIdentityFileLock(
    absPath(cwd, TeamPaths.configMutationLock(teamName)),
    fn
  );
}
async function migrateTeamConfigRevision(teamName, cwd) {
  ...
  return withTeamConfigMutationLock(teamName, cwd, async () => {
    const configState = await readJsonFileState(
      absPath(cwd, TeamPaths.config(teamName))
    );
    if (configState.kind === "invalid")
      throw new Error("invalid_persisted_state");
    ...
    if (configState.kind === "value") {
      ...
      if (legacy) {
        ...
        const revisioned2 = validateRevisionedTeamConfig(
          configState.value,
          teamName
        );
        if (!revisioned2) throw new Error("invalid_persisted_state");
        return {
          config: canonicalizeTeamConfigWorkers(revisioned2),
          stateRevision: revisioned2.state_revision
        };
      }
    } else {
      const manifestState = await readJsonFileState(
...
async function saveTeamConfig(config, cwd, expectedRevision) {
  ...
  await withTeamConfigMutationLock(config.name, cwd, async () => {
    const currentState = await readJsonFileState(
      absPath(cwd, TeamPaths.config(config.name))
    );
    ...
    const committed = alignActiveFenceRevisions(
      { ...config, state_revision: nextRevision },
      nextRevision
    );
  ...
  const lock = await deps.withTaskClaimLock(
    ...
    async () => {
      ...
      await deps.writeAtomic(
        deps.taskFilePath(deps.teamName, taskId, deps.cwd),
...
async function transitionTaskStatus(taskId, from, to, claimToken, terminalData, deps) {
  ...
  const lock = await deps.withTaskClaimLock(
    ...
    async () => {
      ...
      if (new Date(v.claim.leased_until) <= /* @__PURE__ */ new Date())
      ...
      if (to === "completed" && requiresDelegationComplianceEvidence(v) && !delegationCompliance) {
        return {
          ok: false,
          error: "missing_delegation_compliance_evidence"
        };
      }
      const updated = {
        ...v,
  ...
  if (to === "completed") {
    ...
    const updated = existing ? {
      ...
      completedEventTaskIds: {
        ...existing.completedEventTaskIds ?? {},
        [taskId]: true
      }
    } : {
      taskStatusById: {},
...
function reservationFromSidecar(sidecar) {
  return {
    recovery_id: sidecar.recovery_id,
    request_id: sidecar.request_id,
    continuation_sequence: sidecar.continuation_sequence,
    checkpoint_path: sidecar.checkpoint_path,
    checkpoint_hash: sidecar.checkpoint_hash,
    replacement_worker: sidecar.replacement_worker,
    replacement_generation: sidecar.replacement_generation,
    adoption_token_hash: sidecar.adoption_token_hash,
    reserved_at: sidecar.created_at
  };
}
...
async function requeueRecoveredTask(input, deps) {
  const lock = await deps.withTaskClaimLock(
    ...
    async () => {
      ...
      const sidecar = await deps.readRecoverySidecar(
        deps.teamName,
        input.recoveryId,
        input.taskId,
        deps.cwd
      ...
      if (sidecar) {
        ...
        if (!sameAttempt)
          return { ok: false, error: "task_requeue_failed" };
        if (task.status === "pending" && task.version === sidecar.old_task_version + 1 && !task.owner && !task.claim && JSON.stringify(task.recovery_reservation) === JSON.stringify(reservation2))
          return { ok: true, task, reservation: reservation2, replayed: true };
        if (task.status !== "in_progress" || task.version !== sidecar.old_task_version || task.owner !== sidecar.old_owner || task.claim?.owner !== sidecar.old_owner || task.claim?.token !== sidecar.old_claim_token || task.claim?.leased_until !== sidecar.old_claim_leased_until)
          return { ok: false, error: "task_requeue_failed" };
        const checkpoint = await deps.readRecoveryCheckpoint(
          sidecar.checkpoint_path
        );
        ...
        const updated2 = {
          ...
          recovery_reservation: reservation2
        };
        await deps.writeAtomic(
          deps.taskFilePath(deps.teamName, input.taskId, deps.cwd),
          JSON.stringify(updated2, null, 2)
        );
        return {
          ok: true,
          task: updated2,
      ...
      if (task.status !== "in_progress" || !task.owner || !task.claim || task.claim.owner !== task.owner || task.recovery_reservation)
        return { ok: false, error: "task_requeue_failed" };
      const selected = await deps.selectRecoveryCheckpoint(
        deps.teamName,
        task,
        deps.cwd
      );
      ...
      const next = {
        ...
        old_claim_token: task.claim.token,
        old_claim_leased_until: task.claim.leased_until,
        continuation_sequence: selected.checkpoint.sequence,
        checkpoint_path: selected.path,
        checkpoint_hash: selected.checkpoint.resume_payload_hash,
        replacement_worker: input.replacementWorker,
        replacement_generation: input.replacementGeneration,
        adoption_token_hash: input.adoptionTokenHash,
        created_at: createdAt
      };
      await deps.writeRecoverySidecar(
        deps.teamName,
        input.recoveryId,
        input.taskId,
        next,
      ...
      async () => {
        ...
        if (!reservation) {
          if (task.status === "in_progress" && task.owner === workerName && task.claim && task.recovery_adoption?.recovery_id === proof.recoveryId && task.recovery_adoption.request_id === proof.requestId && task.recovery_adoption.replacement_generation === proof.replacementGeneration) {
            const checkpoint2 = await deps.readRecoveryCheckpoint(
              task.recovery_adoption.checkpoint_path
            );
            return checkpoint2.ok ? {
              ok: true,
              task,
              ...
              ok: false,
              error: checkpointError(checkpoint2.error)
            };
          }
          return { ok: false, error: "claim_conflict" };
        }
        if (task.status !== "pending" || task.owner || task.claim || reservation.recovery_id !== proof.recoveryId || reservation.request_id !== proof.requestId || reservation.replacement_worker !== workerName || reservation.replacement_generation !== proof.replacementGeneration || !deps.verifyAdoptionToken(
          proof.adoptionToken,
          reservation.adoption_token_hash
        ))
          return { ok: false, error: "claim_conflict" };
        const checkpoint = await deps.readRecoveryCheckpoint(
          reservation.checkpoint_path
        );
        if (!checkpoint.ok || checkpoint.checkpoint.resume_payload_hash !== reservation.checkpoint_hash || checkpoint.checkpoint.sequence !== reservation.continuation_sequence)
          return {
            ok: false,
            error: checkpointError(checkpoint.ok ? "stale" : checkpoint.error)
          };
        const claimToken = randomUUID3();
        ...
        const updated = {
          ...
          version: task.version + 1,
          recovery_reservation: void 0,
          recovery_adoption: {
            recovery_id: reservation.recovery_id,
            request_id: reservation.request_id,
            continuation_sequence: reservation.continuation_sequence,
            checkpoint_path: reservation.checkpoint_path,
            checkpoint_hash: reservation.checkpoint_hash,
            replacement_worker: workerName,
            replacement_generation: reservation.replacement_generation,
...
async function readCheckpoint(path4) {
  ...
  try {
    const checkpoint = parseCheckpoint(
      JSON.parse(await readFile3(path4, "utf8"))
    );
...
async function publishTaskRecoveryCheckpoint(input, cwd, access2) {
  ...
  if (payloadBytes > MAX_TASK_RECOVERY_CHECKPOINT_BYTES)
    return { ok: false, error: "invalid_checkpoint" };
  const lock = await access2.withTaskLock(
    input.teamName,
    input.taskId,
    cwd,
    async () => {
      ...
      const publication = await publishImmutableCheckpoint(
        path4,
...
async function selectTaskRecoveryCheckpoint(teamName, task, cwd) {
  ...
  const root = absPath(
    cwd,
    TeamPaths.checkpoints(
      teamName,
      task.id,
      taskRecoveryClaimTokenHash(task.claim.token)
    )
  );
  ...
  const parsed = await Promise.all(
    paths.map(async (path4) => ({
      path: path4,
      checkpoint: await readCheckpoint(path4)
    }))
  );
  ...
  const matching = valid.filter(
    ({ checkpoint }) => checkpoint.team_name === teamName && checkpoint.task_id === task.id && checkpoint.worker_name === task.owner && checkpoint.task_version === task.version && checkpoint.claim_token === task.claim?.token
  );
  if (matching.length === 0) return { ok: false, error: "stale" };
  const highest = Math.max(
    ...matching.map(({ checkpoint }) => checkpoint.sequence)
  );
  const selected = matching.filter(
    ({ checkpoint }) => checkpoint.sequence === highest
  );
  if (selected.length !== 1) return { ok: false, error: "ambiguous" };
  const otherHighest = valid.filter(
    ({ checkpoint }) => checkpoint.sequence === highest && checkpoint.resume_payload_hash !== selected[0].checkpoint.resume_payload_hash
  );
...
async function withMailboxLock(teamName, workerName, cwd, fn) {
  ...
  throw new Error(
    `Failed to acquire mailbox lock for ${workerName} after ${timeoutMs}ms`
  );
}
...
function mergeTeamConfigSources(config, manifest) {
  ...
  if (!manifest) return config ? canonicalizeTeamConfigWorkers(config) : null;
  if (!config)
    return canonicalizeTeamConfigWorkers(configFromManifest2(manifest));
  return canonicalizeTeamConfigWorkers({
...
async function teamReadConfig(teamName, cwd) {
  ...
  if (!config && existsSync6(configPath))
    throw new Error("invalid_persisted_state");
  if (config && typeof config.state_revision === "number" && Number.isSafeInteger(config.state_revision)) {
    return canonicalizeTeamConfigWorkers(config);
  }
  if (!manifest && existsSync6(manifestPath))
    throw new Error("invalid_persisted_state");
...
async function teamCreateTask(teamName, task, cwd) {
    ...
    const result = await withLock(lockDir, async () => {
      ...
      const taskLock = await withTaskClaimLock(
        ...
        async () => {
          ...
          try {
            if (!await saveTeamConfigAtRevision(
              nextConfig,
  ...
  throw new Error(
    `Failed to acquire task creation lock for team ${teamName} after ${timeoutMs}ms`
  );
}
...
async function teamUpdateTask(teamName, taskId, updates, cwd) {
  ...
  throw new Error(
    `Failed to acquire task update lock for task ${taskId} in team ${teamName} after ${timeoutMs}ms`
  );
}
...
function recoveryTransitionDeps(teamName, cwd) {
  return {
    ...
    readRecoverySidecar: async (tn, recoveryId, tid, c) => {
      ...
      try {
        return JSON.parse(
          await readFile4(path4, "utf8")
        );
      } catch {
        return "malformed";
      }
    },
    writeRecoverySidecar: (tn, recoveryId, tid, sidecar, c) => writeAtomic3(
...
async function teamPublishTaskRecoveryCheckpoint(input, cwd) {
  return publishTaskRecoveryCheckpoint(input, cwd, {
...
async function teamAdoptRecoveryReservations(teamName, cwd, taskIds, workerName, proof) {
  return adoptRecoveryReservations(
    taskIds,
    workerName,
    proof,
...
async function readLegacyMailboxJsonl(teamName, workerName, cwd) {
  const legacyPath = absPath(
    cwd,
    TeamPaths.mailbox(teamName, workerName).replace(/\.json$/i, ".jsonl")
  );
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const normalized = normalizeLegacyMailboxMessage(
        parsed
      );
...
async function teamReadCanonicalMailboxMessageStrict(teamName, workerName, messageId, cwd) {
  ...
  for (let messageIndex2 = 0; messageIndex2 < parsed.messages.length; messageIndex2 += 1) {
    const validated = validateStrictCanonicalMailboxMessage(
      parsed.messages[messageIndex2],
      messageIndex2
    );
  ...
  if (requestedIndexes.length > 1) {
    return {
      ...
      messageId,
      messageIndexes: requestedIndexes
    };
  }
  const duplicate = [...indexesByMessageId.entries()].find(
    ([, indexes]) => indexes.length > 1
  );
  if (duplicate)
    return {
      kind: "duplicate_message_id",
      messageId: duplicate[0],
      messageIndexes: duplicate[1]
    };
...
async function teamBroadcast(teamName, fromWorker, body, cwd) {
  ...
  for (const worker of cfg.workers) {
    ...
    const msg = await teamSendMessage(
      teamName,
      fromWorker,
      worker.name,
      body,
...
async function teamWriteShutdownRequest(teamName, workerName, requestedBy, cwd) {
  ...
  await writeAtomic3(
    p,
    JSON.stringify(
      { requested_at: (/* @__PURE__ */ new Date()).toISOString(), requested_by: requestedBy },
      null,
      2
    )
  );
...
function resolveDispatchLockTimeoutMs(env = process.env) {
  ...
  return Math.max(
    MIN_DISPATCH_LOCK_TIMEOUT_MS,
    Math.min(MAX_DISPATCH_LOCK_TIMEOUT_MS, Math.floor(parsed))
  );
}
async function withDispatchLock(teamName, cwd, fn) {
  ...
  while (true) {
    try {
      ...
      await new Promise(
        (resolve7) => setTimeout(resolve7, Math.floor(pollMs * jitter))
      );
...
async function readDispatchRequestsFromFile(teamName, cwd) {
  ...
  try {
    ...
    return parsed.map(
      (entry) => normalizeDispatchRequest(
        teamName,
        entry
      )
    ).filter((req) => req !== null);
...
async function readDispatchRequestStrict(teamName, requestId, cwd) {
  const { read } = await readStrictDispatchRequestWithIndex(
    teamName,
    requestId,
    cwd
  );
...
async function patchPendingDispatchReason(teamName, requestId, reason, cwd) {
  if (!existsSync8(absPath(cwd, TeamPaths.root(teamName))))
    return { kind: "missing" };
  try {
    return await withDispatchLock(teamName, cwd, async () => {
      const { read, store } = await readStrictDispatchRequestWithIndex(
        teamName,
        requestId,
        cwd
      );
      ...
      try {
        atomicWriteJson(
          absPath(cwd, TeamPaths.dispatchRequests(teamName)),
          nextRows
        );
      } catch {
...
async function enqueueDispatchRequest(teamName, requestInput, cwd) {
  if (!isDispatchKind(requestInput.kind))
    throw new Error(
      `Invalid dispatch request kind: ${String(requestInput.kind)}`
    );
  ...
  return await withDispatchLock(teamName, cwd, async () => {
    ...
    const existing = requests.find(
      (req) => equivalentPendingDispatch(req, requestInput)
    );
...
async function markDispatchRequestNotified(teamName, requestId, patch = {}, cwd) {
  ...
  if (current.status === "notified" || current.status === "delivered")
    return current;
  return await transitionDispatchRequest(
    teamName,
    requestId,
    current.status,
    "notified",
...
function resolveCanonicalTarget(config, recipient) {
  ...
  const worker = canonicalizeWorkers(config.workers).workers.find(
    (candidate) => candidate.name === recipient
  );
...
function evaluateMailboxNotificationGuard(input, state) {
  ...
  if (request.request_id !== input.requestId || request.team_name !== input.teamName || request.to_worker !== input.recipient || request.message_id !== input.messageId || request.trigger_message !== input.triggerMessage) {
    return {
      kind: "suppress",
      reason: "mailbox_request_identity_mismatch",
      safePendingRequest
    };
  }
  const targetResolution = resolveCanonicalTarget(
    state.config,
    input.recipient
  );
  ...
  if (state.mailbox.kind !== "valid") {
    return {
      kind: "suppress",
      reason: mailboxFailureReason(state.mailbox),
      safePendingRequest,
      target
    };
  }
  ...
  if (message.message_id !== input.messageId || message.to_worker !== input.recipient) {
    return {
      kind: "suppress",
      reason: "mailbox_recipient_mismatch",
      safePendingRequest,
      target
    };
  }
...
async function readCurrentMailboxNotificationGuard(input, cwd, dependencies = {}) {
  ...
  const [config, dispatch, mailbox] = await Promise.all([
    ...
    readStrictDispatch(input.teamName, input.requestId, cwd).catch(
...
function resolveRmuxInvocation(env = process.env) {
  if (process.platform === "win32") return null;
  const program = env.TMUX_PROGRAM;
  const tmux = env.TMUX;
  const looksLikeRmux = env.TERM_PROGRAM === "rmux" || typeof program === "string" && program.includes("rmux-shim");
  if (!looksLikeRmux || !program || !tmux) return null;
  const socket = tmux.split(",")[0];
  if (!socket) return null;
  return { bin: program, socketArgs: ["-S", socket] };
}
function multiplexerShellCommandPrefix() {
...
function resolveRmuxBinaryPath() {
  if (process.platform === "win32") return null;
  if (cachedRmuxBinaryPath !== void 0) return cachedRmuxBinaryPath;
  let resolved = null;
  try {
...
function resolveMultiplexerInvocation(args) {
  const rmux = resolveRmuxInvocation();
  if (rmux) {
...
async function rmuxExecAsync(args, opts) {
...
function rmuxShell(command, opts) {
  const { stripTmux: _, ...execOpts } = opts ?? {};
...
async function rmuxShellAsync(command, opts) {
  const { stripTmux: _, timeout, ...rest } = opts ?? {};
...
async function rmuxCmdAsync(args, opts) {
...
function isTmuxCompatibleMultiplexerAvailable() {
  return resolveRmuxBinaryPath() !== null || isTmuxAvailable();
}
function quoteShellArg(value) {
  return `'${value.replace(/'/g, `'"'"'`)}'`;
}
var cachedRmuxBinaryPath;
var init_rmux_utils = __esm({
  "src/cli/rmux-utils.ts"() {
    "use strict";
  }
});

// src/cli/rmux-clipboard.ts
function hasUniversalClipboardTerminalFeature(features) {
  return features.split(/\r?\n|,/).map((feature) => feature.trim()).some(
    (feature) => feature === UNIVERSAL_CLIPBOARD_FEATURE || feature.startsWith(`${UNIVERSAL_CLIPBOARD_FEATURE}:`)
  );
}
function configureTmuxClipboardForSession(sessionName2, opts) {
  rmuxExec(["set-option", "-t", sessionName2, "set-clipboard", "on"], opts);
  let terminalFeatures = "";
  try {
    terminalFeatures = String(
      rmuxExec(
        ["show-options", "-t", sessionName2, "-v", "terminal-features"],
        opts
      ) ?? ""
    );
  } catch {
...
async function configureTmuxClipboardForSessionAsync(sessionName2, opts) {
  await rmuxExecAsync(
    ["set-option", "-t", sessionName2, "set-clipboard", "on"],
    opts
  );
  let terminalFeatures = "";
  try {
...
// src/team/rmux-session.ts
var rmux_session_exports = {};
__export(rmux_session_exports, {
...
async function applyMainVerticalLayout(teamTarget) {
  try {
    await rmuxExecAsync(["select-layout", "-t", teamTarget, "main-vertical"]);
  } catch {
  }
  try {
...
function isCmuxDialectFailure(error) {
  ...
  return /(?:unknown|unrecognized|invalid|unsupported) (?:command|subcommand|option)|no such (?:command|subcommand)|Found argument .*--surface.*wasn't expected|unexpected argument|unexpected option/i.test(
    text
  );
}
function redactCmuxFailureMessage(error, argLists) {
  ...
  const sensitiveArgs = [
    ...new Set(
      argLists.flatMap((args) => args).flatMap((arg) => {
...
async function cmuxExecPrimaryWithLegacyFallback(primaryArgs, legacyArgs) {
  try {
    ...
    if (!isCmuxDialectFailure(primaryError)) {
      const primaryMessage = redactCmuxFailureMessage(primaryError, [
        primaryArgs
      ]);
    ...
    try {
      ...
      const legacyMessage = redactCmuxFailureMessage(legacyError, [
        primaryArgs,
        legacyArgs
      ]);
      throw new Error(
...
async function verifyTeamTargetOwnership(target, dependencies = defaultMailboxTargetOwnershipDependencies) {
  ...
  if (target.provider === "tmux") {
    ...
    try {
      const result = await dependencies.rmuxExec([
        "list-panes",
        "-t",
...
async function invokeDirectMailboxEffect(target, message, dependencies = defaultDirectMailboxEffectDependencies) {
  ...
  try {
    const notified = target.recipientRole === "leader" ? await dependencies.sendLeader(
      target.providerTarget,
      target.paneId,
      message
    ) : await dependencies.sendWorker(
      target.providerTarget,
      target.paneId,
      message
    );
...
function getDefaultShell() {
  ...
  const name = basename5(shell.replace(/\\/g, "/")).replace(
    /\.(exe|cmd|bat)$/i,
    ""
  );
...
function pathCandidateNames(candidatePath) {
  ...
  if (process.platform === "win32") {
    return Array.from(
      /* @__PURE__ */ new Set([`${bare}.exe`, `${bare}.cmd`, `${bare}.bat`, bare])
    );
  }
...
function resolveSupportedShellAffinity(shellPath) {
  ...
  const name = basename5(shellPath.replace(/\\/g, "/")).replace(
    /\.(exe|cmd|bat)$/i,
    ""
  );
...
function redactWorkerStartCommandForLog(command) {
  return command.replace(/\b([A-Za-z_][A-Za-z0-9_]*)='[^']*'/g, "$1='<redacted>'").replace(/set "([A-Za-z_][A-Za-z0-9_]*)=[^"]*"/g, 'set "$1=<redacted>"').replace(
...
function logWorkerSpawnDiagnostic(message) {
  process.stderr.write(`[team/rmux-session] ${message}
`);
}
...
async function waitForShellReady(paneId, opts = {}) {
  ...
  const envTimeout = Number.parseInt(
    process.env.OMC_TEAM_SHELL_READY_TIMEOUT_MS ?? "",
    10
  );
...
function validateTmux(hasTmuxContext = false) {
  ...
  try {
    rmuxShell("-V", { stripTmux: true, timeout: 5e3, stdio: "pipe" });
  } catch {
    throw new Error(
...
function createSession(teamName, workerName, workingDirectory) {
  ...
  rmuxExec(args, { stripTmux: true, stdio: "pipe", timeout: 5e3 });
  try {
    configureTmuxClipboardForSession(name, {
      stripTmux: true,
      stdio: "pipe",
      timeout: 5e3
    });
...
function listActiveSessions(teamName) {
  ...
  try {
    const output2 = rmuxShell("list-sessions -F '#{session_name}'", {
      timeout: 5e3,
      stdio: ["pipe", "pipe", "pipe"]
    });
...
function spawnBridgeInSession(tmuxSession, bridgeScriptPath, configFilePath) {
  ...
  rmuxExec(["send-keys", "-t", tmuxSession, cmd, "Enter"], {
    stripTmux: true,
    stdio: "pipe",
    timeout: 5e3
  });
...
async function splitTeamWorkerPaneWithEvidence(splitTarget, direction, cwd) {
  ...
  try {
    ...
    const splitResult = await rmuxExecAsync([
      "split-window",
      splitType,
      "-t",
      splitTarget,
...
async function createTeamSession(teamName, workerCount, cwd, options = {}) {
  ...
  if (inCmux) {
    ...
    if (!cmuxLeaderSurface) {
      throw new Error(
        "CMUX_SURFACE_ID is required to create a cmux team session"
      );
    }
    ...
    const detachedResult = await rmuxExecAsync(
      [
        "new-session",
        "-d",
        "-P",
  ...
  if (inTmux && envPaneId) {
    try {
      const targetedContextResult = await rmuxExecAsync([
        "display-message",
        "-p",
        "-t",
        envPaneId,
  ...
  if (!sessionAndWindow || !leaderPaneId) {
    const contextResult = await rmuxCmdAsync([
      "display-message",
      "-p",
      "#S:#I #{pane_id}"
    ]);
  ...
  if (useDedicatedWindow) {
    ...
    const newWindowResult = await rmuxExecAsync([
      "new-window",
      "-d",
      "-P",
      "-F",
  ...
  if (workerCount <= 0) {
    if (!inCmux) {
      ...
      if (sessionMode !== "dedicated-window") {
        try {
          await rmuxExecAsync(["select-pane", "-t", leaderPaneId]);
        } catch {
        }
      }
    }
  ...
  for (let i = 0; i < workerCount; i++) {
    ...
    const splitResult = await rmuxCmdAsync([
      "split-window",
      splitType,
      "-t",
      splitTarget,
  ...
  await Promise.all(
    workerPaneIds.map(
      (workerPaneId) => waitForShellReady(workerPaneId, { timeoutMs: 5e3 })
    )
  );
...
async function spawnWorkerInPane(sessionName2, paneId, config) {
  ...
  try {
    const sendResult = await rmuxExecAsync(
  ...
  try {
    const enterResult = await rmuxExecAsync(
...
async function capturePaneAsync(paneId, opts = {}) {
  try {
    ...
    const result = await rmuxExecAsync(args);
    return result.stdout;
  } catch {
    return "";
  }
...
async function sendTeamPaneKey(paneId, key) {
  ...
  await rmuxExecAsync(["send-keys", "-t", paneId, key]);
}
async function killTeamPane(paneId) {
  ...
  await rmuxExecAsync(["kill-pane", "-t", paneId]);
}
function detectPaneTrustPromptKind(captured) {
  const lines = captured.split("\n").map((l) => l.replace(/\r/g, "").trim()).filter((l) => l.length > 0);
  const tail = lines.slice(-12);
  const hasDirectoryQuestion = tail.some(
    (l) => /Do you trust the contents of this directory\?/i.test(l)
  );
  const hasDirectoryChoices = tail.some(
    (l) => /Yes,\s*continue|No,\s*quit|Press enter to continue/i.test(l)
  );
  if (hasDirectoryQuestion && hasDirectoryChoices) return "directory";
  const hasHookReview = tail.some((l) => /Hooks need review/i.test(l));
  const hasHookTrustChoice = tail.some(
    (l) => /Continue without trusting/i.test(l)
  );
  const hasHookConfirm = tail.some(
    (l) => /Press enter to confirm or esc to go back/i.test(l)
  );
...
function paneHasActiveTask(captured) {
  ...
  if (tail.some(
    (l) => /^[·✻]\s+[A-Za-z][A-Za-z0-9''-]*(?:\s+[A-Za-z][A-Za-z0-9''-]*){0,3}(?:…|\.{3})$/u.test(
      l
    )
  ))
    return true;
...
async function waitForPaneReady(paneId, opts = {}) {
  ...
  console.warn(
    `[rmux-session] waitForPaneReady: pane ${paneId} timed out after ${timeoutMs}ms (set OMC_SHELL_READY_TIMEOUT_MS to tune)`
  );
  return false;
}
...
async function sendToWorker(_sessionName, paneId, message) {
  if (message.length > 200) {
    console.warn(
      `[rmux-session] sendToWorker: message rejected (${message.length} chars exceeds 200 char limit)`
    );
    return false;
  }
  try {
    ...
    if (isCmuxSurfaceTarget(paneId)) {
      ...
      await rmuxExecAsync(["send-keys", "-t", paneId, "-l", "--", message]);
    }
    await sleep(150);
  ...
  try {
    ...
    if (paneHasActiveTask(captured)) {
      if (isCmuxSurfaceTarget(leaderPaneId)) {
        ...
        await rmuxExecAsync(["send-keys", "-t", leaderPaneId, "C-c"]);
      }
...
async function resolveSplitPaneWorkerPaneIds(sessionName2, recordedPaneIds, leaderPaneId) {
  ...
  try {
    const paneResult = await rmuxCmdAsync([
      "list-panes",
      "-t",
      sessionName2,
      "-F",
...
async function killTeamSession(sessionName2, workerPaneIds, leaderPaneId, options = {}) {
  ...
  if (sessionMode === "dedicated-window") {
    try {
      await rmuxExecAsync(["kill-window", "-t", sessionName2]);
    } catch {
    }
    return;
  }
  ...
  if (process.env.OMC_TEAM_ALLOW_KILL_CURRENT_SESSION !== "1" && process.env.TMUX) {
    try {
      const current = await rmuxCmdAsync(["display-message", "-p", "#S"]);
  ...
  try {
    await rmuxExecAsync(["kill-session", "-t", sessionTarget]);
  } catch {
  }
}
...
var init_rmux_session = __esm({
  "src/team/rmux-session.ts"() {
    ...
    defaultMailboxTargetOwnershipDependencies = {
      rmuxExec: (args) => rmuxExecAsync(args),
...
async function writeAndVerifyMailboxMarkers(params, deps) {
  ...
  if (!before.dispatchMarked) {
    writes.push(
      deps.markDispatch(params.teamName, params.requestId, params.cwd).catch(() => null)
    );
  }
...
async function runMailboxNotificationAttempt(params, overrides = {}) {
  ...
  try {
    return await deps.withRequestLock(lockPath, async () => {
      ...
      if (!mailboxNotificationSecurityTupleEquals(
...
function getValidAgentRoles() {
  ...
  try {
    ...
    console.error(
      "[prompt-injection] CRITICAL: Could not scan agents/ directory for role discovery:",
      err
    );
    _cachedRoles = [];
  }
...
function hasVertexModelId(modelIds) {
  return modelIds.some(
    (modelId) => modelId.toLowerCase().startsWith("vertex_ai/")
  );
}
...
function isAutopilotWorkflowSequence(stages) {
  return AUTOPILOT_WORKFLOW_SEQUENCES.some(
    (sequence) => stages.length === sequence.length && stages.every(
      (stage, index) => typeof stage === "string" && stage === sequence[index]
    )
  );
}
...
function composeAutopilotWorkflows(config, userConfig, projectConfig) {
  ...
  if (userWorkflows === void 0 && projectWorkflows === void 0)
    return config;
  return {
    ...config,
    autopilot: {
...
var init_loader = __esm({
  "src/config/loader.ts"() {
    ...
    CURSOR_EXECUTOR_TEAM_ROLE_SET = new Set(
      CURSOR_EXECUTOR_TEAM_ROLES
    );
    KNOWN_AGENT_NAME_SET = new Set(KNOWN_AGENT_NAMES);
    TEAM_ROLE_PROVIDERS = /* @__PURE__ */ new Set([
      "claude",
      "codex",
      "gemini",
      "grok",
...
var init_qa_tester = __esm({
  "src/agents/qa-tester.ts"() {
    ...
    qaTesterAgent = {
      name: "qa-tester",
      description: "Interactive CLI testing specialist using rmux (preferred; tmux-compatible) or tmux. Tests CLI applications, background services, and interactive tools. Manages test sessions, sends commands, verifies output, and ensures cleanup.",
    ...
    TRACER_PROMPT_METADATA = {
      ...
      triggers: [
        ...
        },
        {
          ...
          trigger: "Observed output, artifact, or behavior needs ranked explanations"
        },
        {
  ...
  if (isStrict) {
    cachedConfig = {
      ...
      hardMaxIterations: Math.min(
        ...
        typeof fileOverrides.hardMaxIterations === "number" && fileOverrides.hardMaxIterations > 0 ? fileOverrides.hardMaxIterations : base.hardMaxIterations
      )
    };
  } else {
    cachedConfig = {
...
function resolveBinaryPath(binary) {
  ...
  try {
    ...
    const result = spawnSync3(resolver, [binary], {
      timeout: 5e3,
      encoding: "utf8"
    });
...
function generateTriggerMessage(teamName, workerName, teamStateRoot3 = DEFAULT_INSTRUCTION_STATE_ROOT) {
  const inboxPath = buildTeamStateInstructionPath(
    teamName,
    teamStateRoot3,
    "workers",
    workerName,
...
function generateMailboxTriggerMessage(teamName, workerName, count = 1, teamStateRoot3 = DEFAULT_INSTRUCTION_STATE_ROOT) {
  ...
  const mailboxPath = buildTeamStateInstructionPath(
    teamName,
    teamStateRoot3,
    "mailbox",
    `${workerName}.json`
...
function renderRecoveryContinuationInstruction(instruction) {
  const checkpoint = formatOmcCliInvocation(
    `team api write-task-checkpoint --input "{\\"team_name\\":\\"${instruction.teamName}\\",\\"task_id\\":\\"${instruction.taskId}\\",\\"worker\\":\\"${instruction.workerName}\\",\\"claim_token\\":\\"${instruction.claimToken}\\",\\"task_version\\":${instruction.taskVersion},\\"sequence\\":<next_sequence>,\\"resume_payload\\":<safe_boundary_json>}" --json`
  );
  return [
...
function agentTypeGuidance(agentType) {
  ...
  const transitionTaskStatusCommand = formatOmcCliInvocation(
...
function generateWorkerOverlay(params) {
  ...
  const sentinelPath = buildTeamStateInstructionPath(
    teamName,
  ...
  const heartbeatPath = buildTeamStateInstructionPath(
    teamName,
    instructionStateRoot,
    "workers",
    workerName,
  ...
  const statusPath = buildTeamStateInstructionPath(
    teamName,
    instructionStateRoot,
    "workers",
    workerName,
  ...
  const shutdownAckPath = buildTeamStateInstructionPath(
    teamName,
    instructionStateRoot,
    "workers",
    workerName,
  ...
  const claimTaskCommand = formatOmcCliInvocation(
    `team api claim-task --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"worker\\":\\"${workerName}\\"}" --json`
  );
  const sendAckCommand = formatOmcCliInvocation(
    `team api send-message --input "{\\"team_name\\":\\"${teamName}\\",\\"from_worker\\":\\"${workerName}\\",\\"to_worker\\":\\"leader-fixed\\",\\"body\\":\\"ACK: ${workerName} initialized\\"}" --json`
  );
  const completeTaskCommand = formatOmcCliInvocation(
    `team api transition-task-status --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"from\\":\\"in_progress\\",\\"to\\":\\"completed\\",\\"claim_token\\":\\"<claim_token>\\",\\"result\\":\\"Summary: <what changed>\\\\nVerification: <tests/checks run>\\\\nSubagent skip reason: worker protocol forbids nested subagents; completed focused probe in-session\\"}" --json`
  );
  const failTaskCommand = formatOmcCliInvocation(
    `team api transition-task-status --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"from\\":\\"in_progress\\",\\"to\\":\\"failed\\",\\"claim_token\\":\\"<claim_token>\\"}" --json`
  );
  const readTaskCommand = formatOmcCliInvocation(
    `team api read-task --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\"}" --json`
  );
  const releaseClaimCommand = formatOmcCliInvocation(
    `team api release-task-claim --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"claim_token\\":\\"<claim_token>\\",\\"worker\\":\\"${workerName}\\"}" --json`
  );
  const mailboxListCommand = formatOmcCliInvocation(
    `team api mailbox-list --input "{\\"team_name\\":\\"${teamName}\\",\\"worker\\":\\"${workerName}\\"}" --json`
  );
  const mailboxDeliveredCommand = formatOmcCliInvocation(
    `team api mailbox-mark-delivered --input "{\\"team_name\\":\\"${teamName}\\",\\"worker\\":\\"${workerName}\\",\\"message_id\\":\\"<id>\\"}" --json`
  );
  const checkpointTaskCommand = formatOmcCliInvocation(
    `team api write-task-checkpoint --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"worker\\":\\"${workerName}\\",\\"claim_token\\":\\"<claim_token>\\",\\"task_version\\":<current_task_version>,\\"sequence\\":<next_sequence>,\\"resume_payload\\":<safe_boundary_json>}" --json`
  );
  const teamApiCommand = formatOmcCliInvocation("team api");
  const teamCommand2 = formatOmcCliInvocation("team");
  const taskList = sanitizedTasks.length > 0 ? sanitizedTasks.map(
    (t) => `- **Task ${t.id}**: ${t.subject}
...
async function appendToInbox(teamName, workerName, message, cwd) {
  ...
  const inboxPath = join16(
    cwd,
    `.omc/state/team/${safeTeam}/workers/${safeWorker}/inbox.md`
  );
...
function assertCleanLeaderWorktree(repoRoot) {
  ...
  if (status.length > 0) {
    const error = new Error(
      "leader_worktree_dirty: commit, stash, or clean changes before enabling team worktree mode"
    );
...
function isRegisteredWorktreePath(repoRoot, wtPath) {
  try {
    ...
    return output2.split("\n").some(
      (line) => line.startsWith("worktree ") && canonicalPathForComparison(line.slice("worktree ".length).trim()) === resolvedWtPath
    );
  } catch {
    return false;
  }
...
function isDetached(wtPath) {
  try {
    const branch = execFileSync5("git", ["branch", "--show-current"], {
      cwd: wtPath,
      encoding: "utf-8",
      stdio: "pipe",
      windowsHide: true
...
function statusEntryPath(line) {
  ...
  return normalizeStatusPath(
    renameIndex >= 0 ? payload.slice(renameIndex + renameSeparator.length) : payload
  );
}
function isWorktreeDirtyExcept(wtPath, ignoredRootPaths = []) {
  try {
    const ignored = new Set(ignoredRootPaths);
    const entries = execFileSync5("git", ["status", "--porcelain"], {
      cwd: wtPath,
      encoding: "utf-8",
      stdio: "pipe",
      windowsHide: true
    ...
    const relevantEntries = entries.filter(
      (line) => !ignored.has(statusEntryPath(line))
    );
...
function getRootAgentsBackupPath(repoRoot, teamName, workerName) {
  return join19(
    getWorkerStateDir(repoRoot, teamName, workerName),
...
function readRootAgentsBackup(repoRoot, teamName, workerName) {
  ...
  try {
    return JSON.parse(
      readFileSync10(backupPath, "utf-8")
    );
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    process.stderr.write(
      `[omc] warning: worktree root AGENTS backup parse error: ${msg}
`
    );
    const error = new Error(
      `worktree_root_agents_backup_unreadable:${backupPath}:${msg}`
    );
...
function installWorktreeRootAgents(teamName, workerName, repoRoot, worktreePath, overlayContent) {
  ...
  if (previous && currentContent !== void 0 && currentContent !== previous.installedContent) {
    const error = new Error(
      `agents_dirty: preserving modified worktree root AGENTS.md at ${agentsPath}`
    );
...
function readMetadataResult(repoRoot, teamName) {
  const paths = [
    getMetadataPath(repoRoot, teamName),
    getLegacyMetadataPath(repoRoot, teamName)
  ];
...
function listRootAgentsBackupIssues(repoRoot, teamName, entries) {
  ...
  const knownWorkers = new Set(
    entries.map((entry) => sanitizeName(entry.workerName))
  );
  const issues = [];
  for (const workerName of readdirSync5(workersDir)) {
    const backupPath = join19(
      workersDir,
      workerName,
      "worktree-root-agents.json"
    );
    ...
    try {
      ...
      issues.push({
        path: backupPath,
        message: `worktree_root_agents_backup_unreadable:${workerName}:${message}`
      });
      continue;
    }
...
function writeMetadata(repoRoot, teamName, entries) {
  ...
  ensureDirWithMode(
    join19(getOmcRoot(repoRoot), "state", "team", sanitizeName(teamName))
  );
...
function recordMetadata(repoRoot, teamName, info) {
  ...
  withFileLockSync(metaLockPath, () => {
    const existing = readMetadata(repoRoot, teamName).filter(
      (entry) => entry.workerName !== info.workerName
    );
...
function assertCompatibleExistingWorktree(repoRoot, wtPath, expectedBranch, mode) {
  ...
  if (!registeredBranch) {
    const error = new Error(
      `worktree_path_mismatch: existing path is not a registered git worktree: ${wtPath}`
    );
  ...
  if (isWorktreeDirty(wtPath)) {
    const error = new Error(
      `worktree_dirty: preserving dirty worker worktree at ${wtPath}`
    );
  ...
  if (mode === "named" && registeredBranch !== expectedBranch) {
    const error = new Error(
      `worktree_mismatch: expected branch ${expectedBranch} at ${wtPath}, found ${registeredBranch}`
    );
  ...
  if (mode === "detached" && registeredBranch !== "HEAD") {
    const error = new Error(
      `worktree_mismatch: expected detached worktree at ${wtPath}, found ${registeredBranch}`
    );
    error.code = "worktree_mismatch";
    throw error;
  }
...
function checkWorkerWorktreeRemovalSafety(teamName, workerName, repoRoot, worktreePath) {
  ...
  validateWorktreeRemovalTarget({
    ...
    expectedRoots: [
      join19(getOmcRoot(repoRoot), "team", sanitizeName(teamName), "worktrees")
    ],
  ...
  const dirtyCheck = isWorktreeDirtyExcept(
    wtPath,
    ignoreRootAgents ? ["AGENTS.md"] : []
  );
  if (dirtyCheck.dirty) {
    const error = new Error(
      `worktree_dirty: preserving dirty worker worktree at ${wtPath}`
...
function prepareWorkerWorktreeForRemoval(teamName, workerName, repoRoot, worktreePath) {
  ...
  const agentsRestore = restoreWorktreeRootAgents(
    teamName,
    workerName,
    repoRoot,
    wtPath
  ...
  if (agentsRestore.reason === "agents_dirty") {
    const error = new Error(
      `agents_dirty: preserving modified worktree root AGENTS.md at ${join19(wtPath, "AGENTS.md")}`
    );
    error.code = "agents_dirty";
    throw error;
  }
...
function removeWorkerWorktree(teamName, workerName, repoRoot) {
  ...
  withFileLockSync(metaLockPath, () => {
    ...
    try {
      execFileSync5("git", ["worktree", "remove", wtPath], {
        cwd: repoRoot,
        stdio: "pipe",
        windowsHide: true
      });
      ...
      if (wasRegisteredWorktree) {
        ...
        const error = new Error(
          `worktree_remove_failed: preserving metadata for registered worker worktree at ${wtPath}${detail}`
        );
...
function cleanupTeamWorktrees(teamName, repoRoot) {
  ...
  for (const entry of entries) {
    try {
      ...
      process.stderr.write(
        `[omc] warning: preserved worktree ${entry.path}: ${reason}
`
      );
    }
  }
...
var init_stage_router = __esm({
  "src/team/stage-router.ts"() {
    ...
    TIER_SET = /* @__PURE__ */ new Set([
      "HIGH",
      "MEDIUM",
      "LOW"
    ]);
    CURSOR_EXECUTOR_TEAM_ROLE_SET2 = new Set(
      CURSOR_EXECUTOR_TEAM_ROLES
    );
  }
});
...
var init_cli_worker_contract = __esm({
  "src/team/cli-worker-contract.ts"() {
    ...
    VALID_VERDICTS = /* @__PURE__ */ new Set([
      "approve",
      "revise",
      "reject"
    ]);
    VALID_SEVERITIES = /* @__PURE__ */ new Set([
      "critical",
      "major",
      "minor",
      "nit"
...
function registerCadenceOwner(ctx) {
  if (ctx.serviceGeneration === void 0 || ctx.attemptId === void 0)
    return true;
  const current = cadenceOwners.get(ctx.worktreePath);
...
async function uninstallCommitCadence(ctx, io = {
  readFile: readFile10,
  ...
  if (existsSync21(persistedPath)) {
    try {
      ...
      persisted = JSON.parse(
        readFileSync18(persistedPath, "utf-8")
      );
    } catch {
      persisted = { lastShas: {} };
    }
  }
  const service = config.serviceGeneration === void 0 || config.serviceAttemptId === void 0 ? void 0 : {
...
function publishImmutable(target, value) {
  ...
  if (!published || canonicalize2(published) !== bytes)
...
function replaceDerivedIndex(target, value) {
  ...
  if (!repaired || canonicalize2(repaired) !== bytes)
    throw new Error("immutable_recovery_record_verification_failed");
  return repaired;
}
...
function reserveRecoveryRequest(cwd, requestId, payload, recoveryId = randomUUID8()) {
  ...
  try {
    return {
      kind: "created",
      reservation: publishImmutable(
        reservationPath(cwd, requestId),
        reservation
      )
    };
  } catch (error) {
...
function aliasActiveRecoveryRequest(cwd, requestId, payload, active) {
  ...
  const payloadHash = canonicalRecoveryPayloadHash(payload);
  if (active.operation !== payload.operation || active.payload_hash !== payloadHash || active.team_name !== payload.teamName || active.worker_name !== payload.workerName || active.workspace_hash !== payload.workspaceHash)
  ...
  try {
    return {
      kind: "aliased",
      reservation: publishImmutable(reservationPath(cwd, requestId), alias)
    };
  } catch (error) {
    if (error.code !== "EEXIST") throw error;
    const existing = readRecoveryRequestReservation(cwd, requestId);
...
function readRecoveryRequestReservation(cwd, requestId) {
  const reservation = parseCanonical(
    reservationPath(cwd, requestId)
  );
...
function resolveCanonicalRecoveryRequestId(cwd, requestId) {
  ...
  for (let depth = 0; depth < MAX_RECOVERY_ALIAS_DEPTH; depth += 1) {
    ...
    if (!reservation) {
      if (alias || existsSync22(reservationPath(cwd, currentRequestId)))
        return null;
...
function isValidRecoveryPhase(phase, reservation) {
  return phase?.schema_version === 1 && phase.kind === "phase" && hasMatchingRecoveryPhaseTuple(phase, reservation) && [
    "reserved",
    "elected",
    "requeued",
    "ready",
    ...
    "services_pending",
    "adopted"
  ].includes(phase.phase) && ["none", "selected", "reserved", "adopted"].includes(phase.continuation) && ["not_started", "pending", "adopted"].includes(phase.adoption) && ["not_started", "pending", "synced", "repair_required"].includes(
    phase.services
  ) && ["not_started", "synced", "repair_required"].includes(phase.manifest) && typeof phase.updated_at === "string" && Number.isFinite(Date.parse(phase.updated_at)) && (phase.state_revision === void 0 || Number.isSafeInteger(phase.state_revision) && phase.state_revision >= 0);
}
function writeRecoveryPhase(cwd, phase) {
  ...
  return publishImmutable(
    join26(phaseDirectory(cwd, phase.request_id), sequence),
    {
      ...phase,
      schema_version: 1,
      kind: "phase",
      updated_at: phase.updated_at || (/* @__PURE__ */ new Date()).toISOString()
    }
  );
}
function writeRecoveryFinal(cwd, outcome) {
  ...
  const byTeam = absPath(
    cwd,
    TeamPaths.recoveryResultByTeam(
      reservation.workspace_hash,
      outcome.team_name,
      outcome.recovery_id
    )
  );
  const indexed = publishImmutable(byTeam, published);
  if (canonicalize2(indexed) !== canonicalize2(published))
    throw new Error("immutable_recovery_record_verification_failed");
  return published;
}
function latestPhase(cwd, requestId) {
  ...
  try {
    ...
    const phase = parseCanonical(
      join26(directory, candidates[0])
    );
...
function isValidRecoveryResult(value) {
  if (!value || typeof value !== "object") return false;
  const result = value;
  if (typeof result.requestId !== "string" || typeof result.recoveryId !== "string" || typeof result.teamName !== "string" || typeof result.workerName !== "string" || typeof result.updatedAt !== "string" || !Number.isFinite(Date.parse(result.updatedAt)) || typeof result.committed !== "boolean")
    return false;
  if (result.outcome === "recovered" || result.outcome === "already_running") {
    if (result.committed !== true || typeof result.oldPaneId !== "string" && result.oldPaneId !== null || typeof result.newPaneId !== "string" || !result.newPaneId.trim() || result.outcome === "recovered" && (typeof result.oldPaneId !== "string" || !result.oldPaneId.trim()) || !isStringArray2(result.requeuedTaskIds) || !isPlainRecord2(result.continuationSequenceByTask) || !hasExactUniqueKeys(
      result.requeuedTaskIds,
      result.continuationSequenceByTask
    ) || !Object.values(result.continuationSequenceByTask).every(
      (sequence) => typeof sequence === "number" && Number.isSafeInteger(sequence) && sequence > 0
    ) || typeof result.stateRevision !== "number" || !Number.isSafeInteger(result.stateRevision) || result.activation !== "active" && result.activation !== "services_pending" || result.manifestSync !== "synced" && result.manifestSync !== "repair_required" || result.servicesSync !== "synced" && result.servicesSync !== "repair_required" || !isStringArray2(result.warnings) || !result.warnings.every((warning) => RECOVERY_WARNINGS.has(warning)))
      return false;
...
function readRecoveryFinalState(cwd, requestId) {
  ...
  const reservation = readRecoveryRequestReservation(cwd, requestId);
  if (!reservation || reservation.kind !== "reservation" || reservation.recovery_id !== final.recovery_id || reservation.team_name !== final.team_name || reservation.worker_name !== final.worker_name)
    return { kind: "invalid" };
  const byTeam = absPath(
    cwd,
    TeamPaths.recoveryResultByTeam(
  ...
  try {
    ...
    if (!indexed || canonicalize2(indexed) !== expectedBytes) {
      const lockPath = absPath(
        cwd,
        TeamPaths.recoveryFinalIndexLock(
          reservation.workspace_hash,
          final.team_name,
          final.recovery_id
        )
      ...
      withProcessIdentityFileLockSync(lockPath, () => {
        ...
        if (!current || canonicalize2(current) !== expectedBytes)
          replaceDerivedIndex(byTeam, final);
      });
    }
    const verified = parseCanonical(byTeam);
    if (!verified || canonicalize2(verified) !== expectedBytes)
      return { kind: "invalid" };
  } catch {
    return { kind: "invalid" };
  }
...
function isMatchingRecoveryFinal(outcome, expected = {}) {
  ...
  ) || !["not_started", "pending", "adopted"].includes(outcome.adoption) || !["synced", "repair_required", "terminal_degraded"].includes(
    outcome.services
...
function resolveCanonicalReservation(cwd, reservation) {
  ...
  while (current.alias_of_request_id && !seen.has(current.request_id)) {
    ...
    const parent = readRecoveryRequestReservation(
      cwd,
      current.alias_of_request_id
    );
...
function findActiveIdenticalReservation(cwd, payload) {
  ...
  try {
    for (const name of readdirSync8(
      ...
      if (!reservation || reservation.operation !== payload.operation || reservation.payload_hash !== targetHash || reservation.workspace_hash !== payload.workspaceHash || reservation.team_name !== payload.teamName || reservation.worker_name !== payload.workerName)
        continue;
...
function withRecoveryAdmissionLock(cwd, payloadHash, fn) {
  return withProcessIdentityFileLock(
    absPath(cwd, TeamPaths.recoveryAdmissionLock(payloadHash)),
    fn
  );
}
...
function recoveryOwnerBootstrapCandidatePath(cwd, teamName, expectedEpoch, nonce) {
  return absPath(
    cwd,
    TeamPaths.recoveryOwnerBootstrapCandidate(teamName, expectedEpoch, nonce)
  );
}
...
async function publishRecoveryOwnerBootstrapCandidate(input, recoveryId, expectedEpoch, nonce, pid, processStartedAt, predecessor) {
  ...
  const path4 = recoveryOwnerBootstrapCandidatePath(
    input.cwd,
    input.teamName,
    expectedEpoch,
    nonce
...
function hasLiveOrUnknownBootstrapCandidate(input, recoveryId, expectedEpoch, predecessor) {
  const candidateDirectory = dirname21(
    recoveryOwnerBootstrapCandidatePath(
      input.cwd,
      input.teamName,
      expectedEpoch,
      "candidate"
  ...
  const legacyCandidate = join27(
    dirname21(candidateDirectory),
    `${expectedEpoch}.json`
  );
  ...
  for (const entry of entries) {
    ...
    try {
      value = JSON.parse(
        readFileSync14(join27(candidateDirectory, entry), "utf8")
      );
    } catch {
      return true;
    }
...
async function publishIntent(input, recoveryId) {
  const path4 = absPath(
    input.cwd,
    TeamPaths.recoveryIntent(input.teamName, recoveryId)
  );
  try {
    validateRecoveryIntent(
      parseRecoveryIntent(await readFile11(path4, "utf8")),
      input,
      recoveryId
    );
    return;
...
function persistTerminalResult(input, recoveryId, result) {
  writeRecoveryFinal(input.cwd, {
    ...
    result,
    error: {
      code: "error" in result ? result.error : "runtime_owner_unavailable",
...
function isExpectedRecoveryOwnerSuccessor(owner, expectedEpoch, childPid, childProcessStartedAt, fenceOk, expectedNonce) {
  return Boolean(
    owner && childProcessStartedAt && owner.epoch === expectedEpoch && owner.pid === childPid && owner.process_started_at === childProcessStartedAt && (!expectedNonce || owner.nonce === expectedNonce) && fenceOk
  );
}
async function bootstrapPersistentOwner(input, priorEpoch) {
  ...
  const reservation = readRecoveryRequestReservation(
    input.cwd,
    input.requestId
  );
  if (priorEpoch !== (predecessor?.epoch ?? null) || predecessor && !isProcessIdentityDead(predecessor) || !reservation || reservation.kind !== "reservation" || reservation.recovery_id.length === 0 || reservation.team_name !== input.teamName || reservation.worker_name !== input.workerName)
    return false;
  const predecessorEpoch = predecessor?.epoch ?? 0;
  const expectedEpoch = predecessorEpoch + 1;
  if (hasLiveOrUnknownBootstrapCandidate(
    input,
  ...
  while (Date.now() < deadline) {
    ...
    const config = await readRevisionedTeamConfig(
      input.teamName,
      input.cwd
    ).catch(() => null);
    const configBound = config?.config.runtime_owner_epoch;
    const active = config?.config.active_recovery;
    const fenceOk = owner ? checkOwnerFence(input.cwd, input.teamName, {
      epoch: owner.epoch,
      nonce: owner.nonce
    }).ok : false;
    if (isExpectedRecoveryOwnerSuccessor(
      owner,
      expectedEpoch,
      child.pid,
      childProcessStartedAt,
  ...
  return {
    async recoverDeadWorker(input) {
      ...
      const admitted = await withProcessIdentityFileLock(
        absPath(
          normalized.cwd,
          TeamPaths.recoveryLifecycleLock(
            payload.workspaceHash,
            normalized.teamName
          )
        ),
        async () => {
          const admission2 = await withRecoveryAdmissionLock(
            normalized.cwd,
            canonicalRecoveryPayloadHash(payload),
            () => {
              const active = findActiveIdenticalReservation(
                normalized.cwd,
                payload
              );
              return active ? aliasActiveRecoveryRequest(
                normalized.cwd,
                requestId,
                payload,
                active
          ...
          if (admission2.kind === "conflict")
            return { kind: "conflict", admission: admission2 };
          const canonical2 = resolveCanonicalReservation(
            normalized.cwd,
            admission2.reservation
          );
          const prior = readRecoveryOutcome(
            normalized.cwd,
            canonical2.request_id
          );
          const priorFinalState = readRecoveryFinalState(
            normalized.cwd,
            canonical2.request_id
          );
          ...
          const canonicalInput2 = {
            ...normalized,
            requestId: canonical2.request_id
          };
          if (timing.persistentOwnerBootstrap) {
            const state = await teamRecoveryState(
              normalized.cwd,
              normalized.teamName
            );
            if (state !== "v2") {
              const result = terminalResult(
                canonicalInput2,
                canonical2.recovery_id,
                state
              );
              return {
                kind: "final",
                result: persistTerminalResult(
                  canonicalInput2,
                  canonical2.recovery_id,
                  result
                )
          ...
          return {
            ...
            canonicalInput: canonicalInput2
          };
        }
      );
      ...
      if (timing.persistentOwnerBootstrap) {
        const ownerReady = await withProcessIdentityFileLock(
          ...
          async () => {
            ...
            const availability = ownerAvailability(
              normalized.cwd,
              normalized.teamName
            );
            ...
            return (timing.bootstrapOwner ?? bootstrapPersistentOwner)(
              canonicalInput,
              priorEpoch
            );
          }
      ...
      while (Date.now() < deadline) {
        ...
        const finalState = readRecoveryFinalState(
          normalized.cwd,
          outcomeRequestId
        );
        ...
        await new Promise(
          (resolve7) => setTimeout(resolve7, timing.pollIntervalMs ?? 250)
        );
      }
...
async function requestRuntimeOwnerRecovery(input) {
  const dispatch = installedRecoveryOwnerDispatch ?? (async (ownerInput) => {
    const reservation = readRecoveryRequestReservation(
      ownerInput.cwd,
      ownerInput.requestId
    );
    return timeoutResult(ownerInput, reservation?.recovery_id ?? "");
  });
  return createRecoveryOwnerClient(dispatch, {
    persistentOwnerBootstrap: !installedRecoveryOwnerDispatch
  }).recoverDeadWorker(input);
}
...
async function runRecoverySaga(input, deps) {
  ...
  if (liveness === "unknown")
    return finalize(
      failure(input, "worker_liveness_unknown"),
      "none",
      "not_started"
    );
  if (liveness === "alive") {
    ...
    return finalize(
      {
        ...
        committed: true,
        oldPaneId: null,
        newPaneId: input.originalPaneId,
        requeuedTaskIds: [],
  ...
  const tasks = await deps.listOwnedInProgressTasks(
    input.teamName,
    input.workerName
  );
  if (tasks.length === 0) persistPhase("reserved", "none", "not_started");
  const checks = await Promise.all(
    tasks.map((task) => deps.validateCheckpoint(input.teamName, task))
  );
  ...
  if (!pane.ok)
    return finalize(
      failure(input, pane.error),
      tasks.length ? "reserved" : "none",
      "not_started"
    );
  if (!pane.paneId.trim())
    return finalize(
      failure(input, "spawn_failed"),
      tasks.length ? "reserved" : "none",
      "not_started"
    );
  let persisted;
  if (pane.committed) {
    persisted = {
      stateRevision: pane.stateRevision ?? 0,
      manifestSync: pane.manifestSync ?? "repair_required"
    };
  } else {
    try {
    ...
    return finalize(
      {
        ...failure(
          input,
          "config_commit_failed",
          "Replacement config committed but manifest projection requires repair."
        ),
        outcome: "commit_unknown"
      },
      tasks.length ? "reserved" : "none",
      tasks.length ? "pending" : "not_started"
    );
  }
  ...
  if (!activated.ok) {
    return finalize(
      {
        ...failure(input, activated.error),
        outcome: "commit_unknown",
        message: "Replacement was committed but activation remains pending."
      },
      tasks.length ? "reserved" : "none",
      tasks.length ? "pending" : "not_started"
    );
  ...
  let continuations = [];
  if (tasks.length) {
    const adopted = await deps.adoptAll(
      input,
      {
        recoveryId: input.recoveryId,
        requestId: input.requestId,
    ...
    if (!adopted.ok) {
      return finalize(
        {
          ...failure(input, adopted.error),
          outcome: "commit_unknown",
          message: "Replacement was committed but continuation adoption remains pending."
        },
        "reserved",
        "pending"
      );
  ...
  if (services === "synced") {
    ...
    persistPhase(
      "services_pending",
      tasks.length ? "adopted" : "none",
      tasks.length ? "adopted" : "not_started",
      "repair_required"
    );
  }
  const result = {
    outcome: "recovered",
    committed: true,
    oldPaneId: input.originalPaneId,
    newPaneId: pane.paneId,
    requeuedTaskIds: tasks.map((task) => task.id),
    continuationSequenceByTask: sequences,
    stateRevision: persisted.stateRevision,
    activation: services === "synced" ? "active" : "services_pending",
    manifestSync: persisted.manifestSync,
    servicesSync: services,
    warnings: services === "synced" ? [] : ["services_pending"],
    requestId: input.requestId,
    recoveryId: input.recoveryId,
    teamName: input.teamName,
    workerName: input.workerName,
  ...
  while (Date.now() < deadline) {
    try {
      const value = JSON.parse(
        await readFile12(path4, "utf8")
      );
      if (value.recovery_id === expected.recovery_id && value.worker_name === expected.worker_name && value.replacement_generation === expected.replacement_generation && value.pane_attempt_id === expected.pane_attempt_id)
        return true;
    } catch {
    }
...
async function recoverDeadWorkerV2(teamName, cwd, {
  workerName,
  ...
  const client = runtimeOwnerRecoveryClient ?? {
    requestRuntimeOwnerRecovery: (input) => Promise.resolve().then(() => (init_runtime_owner_client(), runtime_owner_client_exports)).then(
      (module) => module.requestRuntimeOwnerRecovery(input)
    )
  };
  const result = await client.requestRuntimeOwnerRecovery({
    requestId,
    cwd,
    teamName,
    workerName,
...
function registerTeamOrchestrator(teamName, handle, service) {
  orchestratorByTeam.set(teamName, {
    handle,
    ...service,
...
async function removeStaleTeamCadence(teamName, expectedContexts) {
  ...
  let converged = true;
  for (const cadence of entry.entries) {
    const expected = expectedContexts.find(
      (context2) => context2.workerName === cadence.workerName
    );
...
async function reconcileCommittedTeamServices(config, cwd) {
  ...
  if (!descriptor || descriptor.schema_version !== 1 || !Number.isSafeInteger(descriptor.service_generation) || descriptor.service_generation < 1 || !descriptor.service_attempt_id || !descriptor.workspace_root)
  ...
  try {
    ...
    const cadenceRemovalsConverged = await removeStaleTeamCadence(
      config.name,
      expectedContexts
    );
    for (const worker of config.workers) {
    ...
    for (const context of expectedContexts) {
      const installed = cadence?.entries.some(
        (candidate) => cadenceContextMatches(candidate, context)
      );
    ...
    const exactCadence = (finalCadence?.entries.length ?? 0) === expectedContexts.length && expectedContexts.every(
      (context) => finalCadence?.entries.some(
        (candidate) => cadenceContextMatches(candidate, context)
      )
    );
    return cadenceRemovalsConverged && !staleOrchestratorRemovalFailed && exactCadence && local.registeredWorkers.size === expectedWorkers.size && [...expectedWorkers].every(
      (workerName) => local.registeredWorkers.has(workerName)
...
function resolveLeaderBranch(cwd) {
  ...
  if (!out) {
    throw new Error(
      "auto-merge requires a non-detached leader branch (git branch --show-current returned empty)"
    );
  }
  return out;
}
function resolveTaskAssignment(task, resolvedRouting, roleRoutingConfig, resolvedBinaryPaths, fallbackAgent) {
  ...
  if (fallbackAgent === "cursor") {
    if (CURSOR_EXECUTOR_TEAM_ROLES.includes(
      canonical
    )) {
  ...
  if (!hasExplicitRole && !hasConfigForRole) {
    if (fallbackAgent === "cursor" && !CURSOR_EXECUTOR_TEAM_ROLES.includes(
      canonical
    )) {
      throw new Error(
...
function sanitizeTeamName(name) {
  ...
  if (!sanitized)
    throw new Error(
      `Invalid team name: "${name}" produces empty slug after sanitization`
    );
  return sanitized;
}
...
async function notifyStartupInbox(sessionName2, paneId, message) {
  ...
  return notified ? { ok: true, transport: "tmux_send_keys", reason: "worker_pane_notified" } : {
    ok: false,
    transport: "tmux_send_keys",
    reason: "worker_notify_failed"
  };
}
...
async function spawnV2Worker(opts) {
  ...
  const injectContract = shouldInjectContract(
    opts.role ?? null,
    opts.agentType
  );
  const outputFile = injectContract && opts.role ? cliWorkerOutputFilePath(
    teamStateRoot(opts.cwd, opts.teamName),
    opts.workerName
  ) : void 0;
  ...
  const inboxTriggerMessage = generateTriggerMessage(
    opts.teamName,
    opts.workerName,
    instructionStateRoot
  );
    ...
    notify: async (_target, triggerMessage) => {
      if (usePromptMode) {
        return {
          ...
          reason: "prompt_mode_launch_args"
        };
      }
      if (opts.agentType === "gemini") {
        ...
        if (!confirmed) {
          return {
            ok: false,
            transport: "tmux_send_keys",
            reason: "worker_notify_failed:trust-confirm"
          };
        }
...
async function recordRecoveryPaneRollbackFailure(input, recoveryId, pending, reason, liveness) {
  ...
  const path4 = absPath(
    input.cwd,
    TeamPaths.recoveryPaneRollbackFailure(
      input.teamName,
      recoveryId,
      pending.paneAttemptId,
      recordedAt
    )
  );
...
async function cleanupRecoveryPaneAttempt(input, recoveryId, pending, reason) {
  const { killTeamPane: killTeamPane2 } = await Promise.resolve().then(() => (init_rmux_session(), rmux_session_exports));
  let liveness = "unknown";
  for (let attempt = 0; attempt < 2; attempt++) {
    await killTeamPane2(pending.paneId).catch(() => void 0);
    liveness = await getWorkerLiveness(pending.paneId).catch(
      () => "unknown"
    );
    if (liveness === "dead") {
...
function buildRecoveryPaneContext(input, sagaInput, config, worker, descriptor, paneId, paneAttemptId) {
  ...
  const gate = {
    ...
    readyPath: absPath(
      input.cwd,
      TeamPaths.recoveryReady(
        input.teamName,
        sagaInput.recoveryId,
        paneAttemptId
      )
    ),
    activatePath: absPath(
      input.cwd,
      TeamPaths.recoveryActivate(
        input.teamName,
        sagaInput.recoveryId,
        paneAttemptId
      )
...
function persistRecoveryFinal(input, recoveryId, result) {
  ...
  writeRecoveryFinal(input.cwd, {
    ...
    result,
    error: failureResult ? {
      code: failureResult.error,
      message: failureResult.message,
      commit_uncertain: failureResult.outcome === "commit_unknown"
    } : void 0,
...
async function finalizeRecoveryOwnerResult(input, recoveryId, result, deps = {
  ...
  if (terminal && active?.recovery_id === recoveryId && active.request_id === input.requestId && active.worker_name === input.workerName && active.owner_epoch === terminal.config.runtime_owner_epoch?.epoch && active.owner_nonce === terminal.config.runtime_owner_epoch?.nonce) {
    ...
    let saved = false;
    try {
      saved = await deps.saveConfigAtRevision(
        finalConfig,
        terminal.stateRevision,
        input.cwd,
        async () => {
          const verified = await deps.readRevisionedConfig(
            input.teamName,
            input.cwd
          );
...
function selectRecoveryReplayTasks(tasks, workerName, recoveryId, committedPaneLiveness) {
  return tasks.filter(
    (task) => task.recovery_reservation?.recovery_id === recoveryId || task.recovery_adoption?.recovery_id === recoveryId || (committedPaneLiveness === null || committedPaneLiveness === "dead") && task.status === "in_progress" && task.owner === workerName
  );
}
async function resolveCommittedRecoveryManifestSync(readManifest, expected) {
  try {
    const manifest = await readManifest();
    const projected = manifest?.workers.find(
      (candidate) => candidate.name === expected.workerName
    );
...
async function readOrCreateRecoveryAttempt(input, recoveryId, replacementGeneration) {
  const path4 = absPath(
    input.cwd,
    TeamPaths.recoveryAttempt(input.teamName, recoveryId)
  );
  try {
    return validateRecoveryAttemptSecret(
      JSON.parse(await readFile13(path4, "utf8")),
      input,
      recoveryId,
...
function waitForBootstrapRecoveryEvidence(delayMs, signal) {
  return new Promise((resolve7, reject) => {
    ...
    const onAbort = () => {
      ...
      signal?.removeEventListener("abort", onAbort);
      reject(
        signal?.reason ?? new Error("bootstrap_recovery_evidence_aborted")
      );
    };
...
async function hasBootstrapRecoveryEvidence(teamName, cwd, input, waitOptions = {}) {
  ...
  if (!reservation || reservation.kind !== "reservation" || reservation.recovery_id !== bootstrap.recoveryId || reservation.team_name !== teamName || reservation.worker_name !== input.workerName)
    return false;
  try {
    const intent = parseRecoveryIntent(
      await readFile13(
        absPath(cwd, TeamPaths.recoveryIntent(teamName, bootstrap.recoveryId)),
        "utf8"
      )
    );
    if (intent.request_id !== input.requestId || intent.recovery_id !== bootstrap.recoveryId || intent.team_name !== teamName || intent.worker_name !== input.workerName)
      return false;
    const now = waitOptions.now ?? Date.now;
    const timeoutMs = waitOptions.timeoutMs === void 0 ? BOOTSTRAP_RECOVERY_EVIDENCE_MAX_WAIT_MS : Number.isFinite(waitOptions.timeoutMs) ? Math.min(
      Math.max(waitOptions.timeoutMs, 0),
    ...
    for (let attempt = 0; attempt <= Math.ceil(timeoutMs / BOOTSTRAP_RECOVERY_EVIDENCE_POLL_MS) && !waitOptions.signal?.aborted; attempt++) {
      const candidate = await readRecoveryOwnerBootstrapCandidate(
        teamName,
        cwd,
        bootstrap.expectedEpoch,
        bootstrap.nonce
      );
      if (candidate && candidateMatchesBootstrap(candidate, input)) return true;
      const owner = readLatestOwnerEpoch(cwd, teamName);
      if (owner && (owner.epoch > bootstrap.expectedEpoch || owner.epoch === bootstrap.expectedEpoch && (owner.pid !== bootstrap.pid || owner.process_started_at !== bootstrap.processStartedAt || owner.nonce !== bootstrap.nonce)))
        return false;
      const remainingMs = deadline - now();
      if (remainingMs <= 0) return false;
      await sleep3(
        Math.min(BOOTSTRAP_RECOVERY_EVIDENCE_POLL_MS, remainingMs),
        waitOptions.signal
      );
    }
    return false;
...
function recoveryOwnerBootstrapCandidatePath2(teamName, expectedEpoch, nonce) {
  return TeamPaths.recoveryOwnerBootstrapCandidate(
    teamName,
    expectedEpoch,
    nonce
  );
...
async function isExactDeadOrphanBootstrapCandidate(teamName, cwd, input, config, orphan) {
  const bootstrap = input.bootstrap;
  if (!bootstrap || !orphan || !isProcessIdentityDead(orphan) || orphan.epoch !== bootstrap.predecessorEpoch || orphan.nonce !== bootstrap.predecessorNonce || orphan.pid !== bootstrap.predecessorPid || orphan.process_started_at !== bootstrap.predecessorProcessStartedAt)
    return false;
...
async function hasBootstrapActiveRecoveryEvidence(teamName, cwd, input, config) {
  ...
  if (active.request_id !== input.requestId || active.recovery_id !== bootstrap.recoveryId || active.worker_name !== input.workerName)
    return false;
  const worker = config.workers.find(
    (candidate) => candidate.name === input.workerName
  );
  ...
  try {
    attempt = validateRecoveryAttemptSecret(
      JSON.parse(
        await readFile13(
          absPath(cwd, TeamPaths.recoveryAttempt(teamName, active.recovery_id)),
          "utf8"
        )
      ),
      input,
  ...
  const continuations = tasks.filter(
    (task) => task.recovery_reservation?.recovery_id === active.recovery_id || task.recovery_adoption?.recovery_id === active.recovery_id
  );
  const untouchedClaims = tasks.filter(
    (task) => task.status === "in_progress" && task.owner === input.workerName && !continuations.some((continuation) => continuation.id === task.id)
  );
...
async function ensureRecoveryOwner(teamName, cwd, input, waitOptions) {
  ...
  if (bootstrap) {
    ...
    if (alreadyPublished) {
      ...
      const retryFromNoOwner = bootstrap.predecessorEpoch === 0 && !current.config.runtime_owner_epoch && (!current.config.active_recovery || await hasBootstrapActiveRecoveryEvidence(
        teamName,
        cwd,
        input,
        current.config
      ));
      const retryFromPredecessor = bootstrap.predecessorEpoch > 0 && current.config.runtime_owner_epoch?.epoch === bootstrap.predecessorEpoch && current.config.runtime_owner_epoch?.nonce === bootstrap.predecessorNonce && current.config.active_recovery?.owner_epoch === bootstrap.predecessorEpoch && current.config.active_recovery?.owner_nonce === bootstrap.predecessorNonce && await hasBootstrapActiveRecoveryEvidence(
        teamName,
        cwd,
        input,
        current.config
      ...
      if (bootstrapFromNoOwner) {
        if (predecessor || current.config.runtime_owner_epoch || current.config.active_recovery && !await hasBootstrapActiveRecoveryEvidence(
          teamName,
          cwd,
          input,
          current.config
  ...
  for (let bindAttempt = 0; bindAttempt < 3 && (current.config.runtime_owner_epoch?.epoch !== owner.epoch || current.config.runtime_owner_epoch?.nonce !== owner.nonce); bindAttempt++) {
    ...
    if (bootstrap && current.config.active_recovery && !await hasBootstrapActiveRecoveryEvidence(
      teamName,
      cwd,
      input,
      current.config
    ...
    const bootstrapWorker = bootstrap ? current.config.workers.find(
      (candidate) => candidate.name === input.workerName
    ) : void 0;
    const next = {
      ...current.config,
  ...
  if (!current) throw new Error("invalid_persisted_state");
  if (current.config.runtime_owner_epoch?.epoch !== owner.epoch || current.config.runtime_owner_epoch?.nonce !== owner.nonce)
    throw new Error("stale_state_revision");
  return {
    fence,
...
async function executeRecoverDeadWorkerV2Owner(input) {
  ...
  try {
    const beforeOwner = await readRevisionedTeamConfig(
      input.teamName,
      input.cwd
    );
    ...
    if (owner.config.lifecycle_state === "shutting_down" || owner.config.lifecycle_state === "stopped") {
      return finalizeBoundRecoveryOwnerTerminal(
        input,
        recoveryId,
        recoveryError(input, recoveryId, "team_shutting_down")
      );
    }
    if (owner.config.active_scale_down || owner.config.active_scale_up)
      return recoveryError(input, recoveryId, "team_mutation_busy");
    const worker = owner.config.workers.find(
      (candidate) => candidate.name === input.workerName
    );
    if (!worker)
      return finalizeBoundRecoveryOwnerTerminal(
        input,
        recoveryId,
        recoveryError(input, recoveryId, "worker_not_found")
      );
    if (!worker.launch_descriptor)
      return finalizeBoundRecoveryOwnerTerminal(
        input,
        recoveryId,
        recoveryError(input, recoveryId, "launch_metadata_incomplete")
      );
    let launchDescriptor;
    try {
      launchDescriptor = validateWorkerLaunchDescriptor(
        worker.launch_descriptor
      );
      ...
      return finalizeBoundRecoveryOwnerTerminal(
        input,
        recoveryId,
        recoveryError(input, recoveryId, "launch_descriptor_unresolvable")
      );
    }
    ...
    const attempt = await readOrCreateRecoveryAttempt(
      input,
      recoveryId,
      replacementGeneration
    );
    ...
    const deps = {
      ...
      getLiveness: async () => {
        ...
        const currentWorker = config.workers.find(
          (candidate) => candidate.name === input.workerName
        );
        ...
        committedReplacementLiveness = await getWorkerPaneLiveness(
      ...
      validateCheckpoint: async (teamName, task) => {
        ...
        if (persisted?.recovery_id === recoveryId) {
          const selected2 = await readTaskRecoveryCheckpoint(
          ...
          return {
            ok: false,
            error: selected2.ok ? "recovery_checkpoint_stale" : `recovery_checkpoint_${selected2.error}`
          };
        }
        const selected = await selectTaskRecoveryCheckpoint(
          teamName,
          { ...task, version: task.version ?? 1 },
          input.cwd
        );
      ...
      requeue: async (sagaInput2, taskId, adoptionTokenHash) => {
        ...
        if (currentTask?.recovery_adoption?.recovery_id === sagaInput2.recoveryId) {
          return {
            ok: true,
            sequence: currentTask.recovery_adoption.continuation_sequence
          };
        }
        const result2 = await teamRequeueRecoveredTask(
          input.teamName,
          input.cwd,
          {
        ...
        return result2.ok ? { ok: true, sequence: result2.reservation.continuation_sequence } : {
          ok: false,
          error: result2.error.startsWith("checkpoint_") ? `recovery_${result2.error}` : "task_requeue_failed"
        };
      },
      spawnGatedPane: async (sagaInput2) => {
        const config = await ensureFence();
        const currentWorker = config.workers.find(
          (candidate) => candidate.name === sagaInput2.workerName
        );
        ...
        if (committedPane) {
          const committedPaneLiveness = await getWorkerPaneLiveness(
            committedPane.paneId
          );
          ...
          if (committedPaneLiveness === "alive") {
            ...
            if (!pending2) {
              try {
                pending2 = buildRecoveryPaneContext(
                  input,
                  sagaInput2,
                  config,
                  currentWorker,
            ...
            const ready = await waitForRecoveryGateRecord(
              pending2.gate.readyPath,
              expected,
              1e3
            );
        ...
        let prepared;
        try {
          prepared = buildRecoveryPaneContext(
            input,
            sagaInput2,
            config,
            currentWorker,
        ...
        for (const candidate of config.workers) {
          if (!candidate.pane_id || candidate.name === sagaInput2.workerName)
            continue;
      ...
      persistActive: async (sagaInput2, paneId) => {
        await ensureFence();
        const current = await readRevisionedTeamConfig(
          input.teamName,
          input.cwd
        );
        if (!current) throw new Error("invalid_persisted_state");
        ...
        const nextWorkers = current.config.workers.map(
          (candidate) => candidate.name === sagaInput2.workerName ? {
            ...candidate,
            pane_id: paneId,
            pane_attempt_id: pending.paneAttemptId,
      ...
      activatePane: async (sagaInput2, paneAttemptId) => {
        ...
        await writeFile8(
          pending.gate.activatePath,
          JSON.stringify(record),
          "utf8"
        );
        const adoptedReady = await waitForRecoveryGateRecord(
          `${pending.gate.readyPath}.adoption-ready`,
          record,
          3e4
        );
      ...
      adoptAll: async (sagaInput2, proof, taskIds) => {
        await ensureFence();
        const results = await teamAdoptRecoveryReservations(
          input.teamName,
          input.cwd,
          taskIds,
        ...
        if (failed && !failed.ok) {
          return {
            ok: false,
            error: failed.error.startsWith("checkpoint_") ? `recovery_${failed.error}` : "worker_activation_failed"
          };
        }
      ...
      writeRun: async (sagaInput2, paneAttemptId, continuations) => {
        ...
        const instruction = continuations.length > 0 ? continuations.map(
          (continuation) => renderRecoveryContinuationInstruction({
            teamName: input.teamName,
            workerName: sagaInput2.workerName,
        ...
        if (!existsSync24(launchedPath)) {
          ...
          const launched = await waitForRecoveryGateRecord(
            launchedPath,
            record,
            3e4
          );
        ...
        if (!pending.promptMode) {
          ...
          const outcome = await queueInboxInstruction({
            ...
            notify: async (_target, triggerMessage) => notifyStartupInbox(
              pending.sessionName,
              pending.paneId,
              triggerMessage
            ),
      ...
      killAttemptPane: async (paneAttemptId) => {
        ...
        const cleaned = await cleanupRecoveryPaneAttempt(
          input,
          recoveryId,
          pending,
          "recovery_saga_rollback"
        );
...
async function rollbackUnpersistedNativeWorktreeStartup(teamName, cwd, cause) {
  ...
  const writeFailureMarker = async (extra = {}) => {
    ...
    await writeFile8(
      join28(teamRoot, "startup-failure.json"),
      JSON.stringify(
        {
...
`
  ...
  await rollbackUnpersistedNativeWorktreeStartup(
    args.teamName,
    args.cwd,
    args.cause
  );
...
async function startTeamV2(config) {
  ...
  if (config.autoMerge) {
    if (!isRuntimeV2Enabled()) {
      throw new Error(
        "auto-merge requires OMC_RUNTIME_V2=1 (this feature is v2-only)."
      );
    }
...
`
  ...
  await mkdir13(absPath(leaderCwd, TeamPaths.tasks(sanitized)), {
    recursive: true
  });
  await mkdir13(absPath(leaderCwd, TeamPaths.workers(sanitized)), {
    recursive: true
  });
  await mkdir13(
    join28(getOmcRoot(leaderCwd), "state", "team", sanitized, "mailbox"),
    { recursive: true }
  );
...
`
    ...
    await appendTeamEvent(
      ...
      leaderCwd
    ).catch(missingBinaryLogFailure);
  }
  for (let i = 0; i < config.tasks.length; i++) {
    const taskId = String(i + 1);
    const taskFilePath = absPath(
      leaderCwd,
      TeamPaths.taskFile(sanitized, taskId)
    );
  ...
  try {
    if (worktreeMode !== "disabled") {
      for (const workerName of workerNames) {
        const worktree = ensureWorkerWorktree(
          sanitized,
          workerName,
          leaderCwd,
          {
  ...
  if (unownedTaskIndices.length > 0) {
    const allocationTasks = unownedTaskIndices.map(
      (idx) => ({
        id: String(idx),
    ...
    const allocationWorkers = workerNames.map(
      (name, i) => ({
        name,
    ...
    for (const r of allocateTasksToWorkers(
      allocationTasks,
      allocationWorkers
    )) {
      startupAllocations.push({
        workerName: r.workerName,
        taskIndex: Number(r.taskId)
      });
    }
  }
  const startupByWorker = new Map(
    startupAllocations.map((item) => [item.workerName, item.taskIndex])
  );
  ...
  for (let i = 0; i < workerNames.length; i++) {
    ...
    const outputFile = taskIndex !== void 0 && assignment.role && shouldInjectContract(assignment.role, assignment.agentType) ? cliWorkerOutputFilePath(
      teamStateRoot(leaderCwd, sanitized),
      workerName
    ) : void 0;
    const outputContract = outputFile && assignment.role ? renderCliWorkerOutputContract(assignment.role, outputFile) : void 0;
    const promptArgs = taskIndex !== void 0 && isPromptModeAgent(assignment.agentType) ? getPromptModeArgs(
    ...
    if (!binary)
      throw new Error(
        `No validated binary available for ${assignment.agentType}`
      );
    const descriptor = buildValidatedWorkerLaunchDescriptor(
      assignment.agentType,
      {
        teamName: sanitized,
        workerName,
  ...
  try {
    await writeFile8(
      absPath(leaderCwd, TeamPaths.manifest(sanitized)),
  ...
  if (config.autoMerge && autoMergeLeaderBranch) {
    try {
      ...
      try {
        ...
        process.stderr.write(
          `[team/runtime-v2] auto-merge recover-from-restart failed: ${recErr}
`
        );
      }
...
`
  ...
  for (const {
    ...
    if (stallReason) {
      ...
      if (stallReason === "missing_dependency") {
        ...
        recommendations.push(
          `Investigate ${w.name}: assigned work but no work-start evidence; pane is idle at prompt`
        );
      } else if (stallReason === "stale_or_missing_worker_reports") {
        recommendations.push(
          `Investigate ${w.name}: pane is idle while status/heartbeat are stale or missing`
        );
      } else {
        recommendations.push(
          `Investigate ${w.name}: no meaningful turn progress and pane is idle at prompt`
        );
      }
    }
  }
  ...
  await writeMonitorSnapshot(
    sanitized,
    {
      ...
      workerLivenessByName: Object.fromEntries(
        workers.map((w) => [w.name, w.liveness])
      ),
...
async function shutdownTeamV2(teamName, cwd, options = {}) {
  ...
  const lifecycleLock = absPath(
    cwd,
    TeamPaths.recoveryLifecycleLock(workspaceHash2, sanitized)
  );
  ...
  let config = await withProcessIdentityFileLock(lifecycleLock, async () => {
    ...
    if (current.config.active_recovery)
      throw new Error(
        `shutdown_blocked:active_recovery:${current.config.active_recovery.recovery_id}`
      );
    if (current.config.active_scale_down)
      throw new Error(
        `shutdown_blocked:active_scale_down:${current.config.active_scale_down.operation_id}`
      );
    if (current.config.active_scale_up) {
      throw new Error(
        `shutdown_blocked:active_scale_up:${current.config.active_scale_up.operation_id}`
      );
    }
  ...
  const revalidateShutdownFence = async () => withProcessIdentityFileLock(lifecycleLock, async () => {
    ...
    if (!current || !["shutting_down", "stopped"].includes(
      current.config.lifecycle_state ?? ""
    ) || current.config.active_recovery || current.config.active_scale_up) {
      throw new Error(
        current?.config.active_recovery ? `shutdown_blocked:active_recovery:${current.config.active_recovery.recovery_id}` : "shutdown_fence_lost"
      );
    }
    return current.config;
  });
  ...
  const rollbackRejectedShutdownFence = async (expected) => withProcessIdentityFileLock(lifecycleLock, async () => {
    const current = await readRevisionedTeamConfig(sanitized, cwd);
    if (!ownedShutdownNonce || !current || current.config.lifecycle_state !== "shutting_down" || current.config.active_recovery || current.config.active_scale_up || current.stateRevision !== expected.state_revision || current.config.shutdown_attempt?.nonce !== ownedShutdownNonce)
      return false;
    const active = {
      ...current.config,
...
`
        ...
        process.stderr.write(
          `[team/runtime-v2] orchestrator drainAndStop: ${err}
`
        );
      } finally {
        await stopTeamCadence(sanitized);
  ...
  if (!config) {
    ...
    if (cleanupSafety.hasEvidence) {
      process.stderr.write(
        "[team/runtime-v2] preserving team state because config is missing and worktree cleanup evidence remains\n"
      );
      return;
    }
...
`;
      ...
      process.stderr.write(
        `[team/runtime-v2] shutdown request failed for ${w.name}: ${err}
`
      );
    }
  }
  ...
  try {
    const {
      killWorkerPanes: killWorkerPanes2,
      killTeamSession: killTeamSession2,
      resolveSplitPaneWorkerPaneIds: resolveSplitPaneWorkerPaneIds2,
      getWorkerLiveness: getWorkerLiveness2
    } = await Promise.resolve().then(() => (init_rmux_session(), rmux_session_exports));
    ...
    const paneById = new Map(
      config.workers.filter(
        (w) => typeof w.pane_id === "string" && w.pane_id.trim().length > 0
      ).map((w) => [w.pane_id, w.name])
    );
    const liveness = await Promise.all(
      workerPaneIds.map(
        async (paneId) => [paneId, await getWorkerLiveness2(paneId)]
      )
    );
...
`);
    if (recordedWorkerPaneIds.length > 0) {
      process.stderr.write(
        "[team/runtime-v2] preserving worktrees/state because tmux cleanup did not prove worker panes exited\n"
      );
      await finalizeAutoMerge();
      return;
    }
...
`);
  ...
  if (preservedWorktrees === 0) {
    ...
    process.stderr.write(
      `[team/runtime-v2] preserved ${preservedWorktrees} worktree(s); keeping team state for follow-up cleanup
`
    );
  }
}
async function resumeTeamV2(teamName, cwd) {
  ...
  try {
    ...
    await rmuxExecAsync(["has-session", "-t", sessionName2.split(":")[0]]);
    return {
      teamName: sanitized,
      sanitizedName: sanitized,
      sessionName: sessionName2,
...
async function monitorTeam(teamName, cwd, workerPaneIds) {
  ...
  try {
    ...
    for (const f of taskFiles.filter((f2) => f2.endsWith(".json"))) {
      const task = await readJsonSafe3(
        join21(root, "tasks", f)
      );
...
async function shutdownTeam(teamName, sessionName2, cwd, timeoutMs = 3e4, workerPaneIds, leaderPaneId, ownsWindow) {
  ...
  const CLI_AGENT_TYPES = /* @__PURE__ */ new Set([
    "claude",
    "codex",
    "gemini",
    "grok",
  ...
  const effectiveWorkerPaneIds = sessionMode === "split-pane" ? await resolveSplitPaneWorkerPaneIds(
    sessionName2,
    workerPaneIds,
    leaderPaneId
  ) : workerPaneIds;
  await killTeamSession(sessionName2, effectiveWorkerPaneIds, leaderPaneId, {
    sessionMode
  });
  try {
...
init_git_worktree();
init_swallowed_error();
var TEAM_UPDATE_TASK_MUTABLE_FIELDS = /* @__PURE__ */ new Set([
  "subject",
  "description",
  "blocked_by",
...
var TEAM_UPDATE_TASK_REQUEST_FIELDS = /* @__PURE__ */ new Set([
  "team_name",
  "task_id",
  "workingDirectory",
  ...TEAM_UPDATE_TASK_MUTABLE_FIELDS
]);
var RECOVER_WORKER_REQUEST_FIELDS = /* @__PURE__ */ new Set([
  "team_name",
  "worker",
  "request_id",
  "timeout_ms"
...
var READ_RECOVERY_RESULT_REQUEST_FIELDS = /* @__PURE__ */ new Set([
  "team_name",
  "request_id"
]);
...
function parseTaskDelegationPlan(value) {
  ...
  if (mode !== "none" && mode !== "optional" && mode !== "auto" && mode !== "required") {
    throw new Error(
      "delegation.mode must be one of: none, optional, auto, required"
    );
  }
  const plan = { mode };
  if ("max_parallel_subtasks" in raw) {
    if (!isFiniteInteger(raw.max_parallel_subtasks) || raw.max_parallel_subtasks < 1) {
      throw new Error(
        "delegation.max_parallel_subtasks must be a positive integer when provided"
      );
    }
  ...
  if ("required_parallel_probe" in raw) {
    if (typeof raw.required_parallel_probe !== "boolean")
      throw new Error(
        "delegation.required_parallel_probe must be a boolean when provided"
      );
  ...
  if ("spawn_before_serial_search_threshold" in raw) {
    if (!isFiniteInteger(raw.spawn_before_serial_search_threshold) || raw.spawn_before_serial_search_threshold < 1) {
      throw new Error(
        "delegation.spawn_before_serial_search_threshold must be a positive integer when provided"
      );
    }
  ...
  if ("child_model_policy" in raw) {
    ...
    if (policy !== "standard" && policy !== "fast" && policy !== "inherit" && policy !== "frontier") {
      throw new Error(
        "delegation.child_model_policy must be one of: standard, fast, inherit, frontier"
      );
    }
  ...
  if ("subtask_candidates" in raw) {
    if (!Array.isArray(raw.subtask_candidates) || !raw.subtask_candidates.every((item) => typeof item === "string")) {
      throw new Error(
        "delegation.subtask_candidates must be an array of strings when provided"
      );
    }
  ...
  if ("child_report_format" in raw) {
    ...
    if (format !== "bullets" && format !== "json")
      throw new Error(
        "delegation.child_report_format must be bullets or json when provided"
      );
  ...
  if ("skip_allowed_reason_required" in raw) {
    if (typeof raw.skip_allowed_reason_required !== "boolean")
      throw new Error(
        "delegation.skip_allowed_reason_required must be a boolean when provided"
      );
...
function assertNoNativeWorktreeCleanupEvidence(teamName, cwd) {
  ...
  throw new Error(
    `cleanup_blocked:worktree_cleanup_evidence_present:${details}`
  );
}
...
function resolveTeamWorkingDirectoryFromMetadata(teamName, candidateCwd, workerContext) {
  ...
  if (workerContext?.teamName === teamName) {
    const workerRoot = readTeamStateRootFromFile(
      join29(teamRoot, "workers", workerContext.workerName, "identity.json")
    );
  ...
  for (const manifestName of ["manifest.json", "manifest.v2.json"]) {
    const fromManifest = readTeamStateRootFromFile(
      join29(teamRoot, manifestName)
    );
...
function resolveTeamWorkingDirectory(teamName, preferredCwd) {
  ...
  if (typeof envTeamStateRoot === "string" && envTeamStateRoot.trim() !== "") {
    const envWorkingDirectory = stateRootToWorkingDirectory(
      envTeamStateRoot.trim()
    );
  ...
  return readCurrentMailboxNotificationGuard(input, cwd, {
    ...
    readStrictDispatchRequest: async (teamName, requestId, requestCwd) => {
      ...
      if (dispatch.kind === "valid" && input.recipient === "leader-fixed" && dispatch.request.to_worker === "leader-fixed" && dispatch.request.pane_id === void 0 && hasExactText2(canonicalLeaderPaneId)) {
        return {
          kind: "valid",
          request: { ...dispatch.request, pane_id: canonicalLeaderPaneId }
        };
      }
      return dispatch;
    },
...
function findWorkerDispatchTarget(teamName, toWorker, cwd) {
  return teamReadConfig(teamName, cwd).then((config) => {
    ...
    const recipient = config?.workers.find(
      (worker) => worker.name === toWorker
    );
    return {
      paneId: recipient?.pane_id,
      workerIndex: recipient?.index,
      instructionStateRoot: resolveInstructionStateRoot(
        recipient?.worktree_path
      )
    };
  });
}
async function findMailboxDispatchRequestId(teamName, workerName, messageId, cwd) {
  ...
  const matching = requests.filter((request) => request.message_id === messageId).sort(
...
async function syncMailboxDispatchNotified(teamName, workerName, messageId, cwd) {
  ...
  const requestId = await findMailboxDispatchRequestId(
    teamName,
    workerName,
    messageId,
    cwd
...
async function executeTeamApiOperation(operation, args, fallbackCwd) {
  try {
    ...
    switch (operation) {
      case "recover-worker": {
        const unsupported = unsupportedFields(
          args,
          RECOVER_WORKER_REQUEST_FIELDS
        );
        if (unsupported.length > 0) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: `recover-worker received unsupported fields: ${unsupported.join(", ")}`
            }
          };
        }
      ...
      case "write-task-checkpoint": {
        const unsupported = unsupportedFields(
          args,
          WRITE_TASK_CHECKPOINT_REQUEST_FIELDS
        );
        if (unsupported.length > 0) {
          return {
            ok: false,
            ...
            error: {
              code: "invalid_input",
              message: `write-task-checkpoint received unsupported fields: ${unsupported.join(", ")}`
            }
          };
        }
        const teamName = requiredString(args, "team_name");
        ...
        if (!teamName || !taskId || !workerName || !claimToken || !Object.hasOwn(args, "resume_payload") || !isFiniteInteger(taskVersion) || taskVersion <= 0 || !isFiniteInteger(sequence) || sequence <= 0) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, task_id, worker, claim_token, positive task_version, positive sequence, and resume_payload are required"
            }
          };
        }
        const workerContext = parseTeamWorkerContextFromEnv();
        if (!workerContext) {
          return {
            ...
            error: {
              ...
              message: "write-task-checkpoint requires OMC_TEAM_WORKER or OMX_TEAM_WORKER authentication"
            }
          };
        }
        if (workerContext.teamName !== teamName || workerContext.workerName !== workerName) {
          return {
            ...
            operation,
            error: {
              code: "worker_auth_mismatch",
              message: "authenticated worker does not match team_name and worker"
            }
          };
        }
        ...
        return result.ok ? { ok: true, operation, data: result } : {
          ok: false,
          operation,
      ...
      case "read-recovery-result": {
        const unsupported = unsupportedFields(
          args,
          READ_RECOVERY_RESULT_REQUEST_FIELDS
        );
        const teamName = requiredString(args, "team_name");
        ...
        return outcome ? { ok: true, operation, data: { outcome } } : {
          ok: false,
          operation,
          error: {
            code: "recovery_result_not_found",
            message: "recovery_result_not_found"
          }
        };
      }
      case "send-message": {
        ...
        if (!fromWorker) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "from_worker is required. You must identify yourself."
            }
          };
        }
        ...
        const notificationOutcome = await queueDirectMailboxMessage({
          ...
          deps: {
            ...
            markMessageNotified: (resolvedTeamName, workerName, messageId, resolvedCwd) => teamMarkMessageNotified(
              resolvedTeamName,
              workerName,
              messageId,
              resolvedCwd
        ...
        return {
          ok: true,
          operation,
          data: { message, notification_outcome: notificationOutcome }
        };
      }
      case "broadcast": {
        const teamName = String(args.team_name || "").trim();
        ...
        if (!teamName || !fromWorker || !body) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, from_worker, body are required"
            }
          };
        }
        const messages = [];
        ...
        const notificationOutcomes = await queueBroadcastMailboxMessage({
          ...
          triggerFor: (workerName) => generateMailboxTriggerMessage(
            ...
            1,
            recipients.find(
              (recipient) => recipient.workerName === workerName
            )?.instructionStateRoot
          ),
          notify: (_target, resolvedTriggerMessage, context) => notifyMailboxTarget({
            teamName,
        ...
        return {
          ...
          data: {
            ...
            messages,
            notification_outcomes: notificationOutcomes
          }
        };
      }
      case "mailbox-list": {
        ...
        if (!teamName || !worker) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name and worker are required"
            }
          };
        }
        ...
        return {
          ok: true,
          operation,
          data: { worker, count: messages.length, messages }
        };
      }
      case "mailbox-mark-delivered": {
        ...
        if (!teamName || !worker || !messageId) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, worker, message_id are required"
            }
          };
        }
        const updated = await teamMarkMessageDelivered(
          teamName,
          worker,
          messageId,
          cwd
      ...
      case "mailbox-mark-notified": {
        ...
        const notified = await teamMarkMessageNotified(
          teamName,
          worker,
          messageId,
          cwd
      ...
      case "create-task": {
        ...
        if (!teamName || !subject || !description) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, subject, description are required"
            }
          };
        }
        ...
        const task = await teamCreateTask(
          teamName,
          {
            subject,
            description,
      ...
      case "read-task": {
        ...
        if (!teamName || !taskId) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name and task_id are required"
            }
          };
        }
        const task = await teamReadTask(teamName, taskId, cwd);
        return task ? { ok: true, operation, data: { task } } : {
          ok: false,
          operation,
      ...
      case "list-tasks": {
        ...
        if (!teamName) {
          return {
            ok: false,
            operation,
            error: { code: "invalid_input", message: "team_name is required" }
          };
        }
        const tasks = await teamListTasks(teamName, cwd);
      ...
      case "update-task": {
        ...
        if (presentLifecycleFields.length > 0) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: `team_update_task cannot mutate lifecycle fields: ${presentLifecycleFields.join(", ")}`
            }
          };
        }
        const unexpectedFields = Object.keys(args).filter(
          (field) => !TEAM_UPDATE_TASK_REQUEST_FIELDS.has(field)
        );
        if (unexpectedFields.length > 0) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: `team_update_task received unsupported fields: ${unexpectedFields.join(", ")}`
            }
          };
        }
        const updates = {};
        if ("subject" in args) {
          if (typeof args.subject !== "string") {
            return {
              ...
              error: {
                code: "invalid_input",
                message: "subject must be a string when provided"
              }
            };
          }
        ...
        if ("requires_code_change" in args) {
          if (typeof args.requires_code_change !== "boolean") {
            return {
              ...
              error: {
                code: "invalid_input",
                message: "requires_code_change must be a boolean when provided"
              }
            };
          }
      ...
      case "claim-task": {
        ...
        if (!teamName || !taskId || !worker) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, task_id, worker are required"
            }
          };
        }
        ...
        if (rawExpectedVersion !== void 0 && (!isFiniteInteger(rawExpectedVersion) || rawExpectedVersion < 1)) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "expected_version must be a positive integer when provided"
            }
          };
        }
      ...
      case "transition-task-status": {
        ...
        if (!teamName || !taskId || !from || !to || !claimToken) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, task_id, from, to, claim_token are required"
            }
          };
        }
        ...
        if (!allowed.has(from) || !allowed.has(to)) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "from and to must be valid task statuses"
            }
          };
        }
        if (transitionResult !== void 0 && typeof transitionResult !== "string") {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "result must be a string when provided"
            }
          };
        }
        if (transitionError !== void 0 && typeof transitionError !== "string") {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "error must be a string when provided"
            }
          };
        }
      ...
      case "release-task-claim": {
        ...
        if (!teamName || !taskId || !claimToken || !worker) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, task_id, claim_token, worker are required"
            }
          };
        }
        const result = await teamReleaseTaskClaim(
          teamName,
          taskId,
          claimToken,
          worker,
      ...
      case "read-config": {
        ...
        const config = await teamReadConfig(teamName, cwd);
        return config ? { ok: true, operation, data: { config } } : {
          ok: false,
          operation,
          error: { code: "team_not_found", message: "team_not_found" }
        };
      }
      case "read-manifest": {
        const teamName = String(args.team_name || "").trim();
        ...
        return manifest ? { ok: true, operation, data: { manifest } } : {
          ok: false,
          operation,
          error: {
      ...
      case "update-worker-heartbeat": {
        ...
        if (!teamName || !worker || typeof pid !== "number" || typeof turnCount !== "number" || typeof alive !== "boolean") {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, worker, pid, turn_count, alive are required"
            }
          };
        }
        await teamUpdateWorkerHeartbeat(
          teamName,
          worker,
          {
            pid,
      ...
      case "write-worker-inbox": {
        ...
        if (!teamName || !worker || !content) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, worker, content are required"
            }
          };
        }
      ...
      case "write-worker-identity": {
        ...
        if (!teamName || !worker || typeof index !== "number" || !role) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, worker, index, role are required"
            }
          };
        }
      ...
      case "append-event": {
        ...
        if (!teamName || !eventType || !worker) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, type, worker are required"
            }
          };
        }
        if (!TEAM_EVENT_TYPES.includes(eventType)) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: `type must be one of: ${TEAM_EVENT_TYPES.join(", ")}`
            }
          };
        }
        const event = await teamAppendEvent(
          teamName,
          {
            type: eventType,
            worker,
      ...
      case "get-summary": {
        ...
        return summary ? { ok: true, operation, data: { summary } } : {
          ok: false,
          operation,
          error: { code: "team_not_found", message: "team_not_found" }
        };
      ...
      case "write-shutdown-request": {
        ...
        if (!teamName || !worker || !requestedBy) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, worker, requested_by are required"
            }
          };
        }
      ...
      case "read-shutdown-ack": {
        ...
        const ack = await teamReadShutdownAck(
          teamName,
          worker,
          cwd,
          args.min_updated_at
      ...
      case "write-monitor-snapshot": {
        ...
        if (!teamName || !snapshot) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name and snapshot are required"
            }
          };
        }
      ...
      case "write-task-approval": {
        ...
        if (!teamName || !taskId || !status || !reviewer || !decisionReason) {
          return {
            ...
            error: {
              code: "invalid_input",
              message: "team_name, task_id, status, reviewer, decision_reason are required"
            }
          };
        }
        if (!TEAM_TASK_APPROVAL_STATUSES.includes(
          status
        )) {
          return {
            ok: false,
            ...
            error: {
              code: "invalid_input",
              message: `status must be one of: ${TEAM_TASK_APPROVAL_STATUSES.join(", ")}`
            }
          };
        }
...
function selectMatchingTestSpecsForPrd(prdPath, testSpecPaths) {
  ...
  return (requiredTimestampedFileName ? testSpecPaths.filter(
    (path4) => basename11(path4) === requiredTimestampedFileName
  ) : testSpecPaths.filter(
    (path4) => legacyTestSpecSlug(path4) === prdArtifact.slug
...
var VALID_CLI_AGENT_TYPES = /* @__PURE__ */ new Set([
  "claude",
  "codex",
  "gemini",
  "cursor",
...
async function assertTeamSpawnAllowed(cwd, env = process.env) {
  ...
  if (workerIdentity) {
    ...
    const governance = normalizeTeamGovernance2(
...
async function resolveCleanupPaneEvidence(job, jobsDir, jobId) {
  const paneArtifact = await readFile14(
    panesArtifactPath(jobsDir, jobId),
    "utf-8"
  ).then((content) => parseJsonSafe(content)).catch(() => null);
  ...
  const configPaneIds = (config.workers ?? []).map((worker) => worker.pane_id).filter(
    (paneId) => typeof paneId === "string" && paneId.trim().length > 0
  );
...
function convergeWithResultArtifact(jobId, job, jobsDir) {
  try {
    const artifactRaw = readFileSync17(
  ...
  if (paneArtifact?.paneIds?.length) {
    const liveness = await Promise.all(
      paneArtifact.paneIds.map(
        async (paneId) => [paneId, await getWorkerLiveness(paneId)]
      )
    );
  ...
  if (runtimeV2.isRuntimeV2Enabled()) {
    ...
    return {
      ...
      workerPaneIds: Array.from(
        new Set(
          (config?.workers ?? []).map((worker) => worker.pane_id).filter(
            (paneId) => typeof paneId === "string" && paneId.trim().length > 0
          )
        )
      ),
...
async function teamShutdownByName(teamName, options = {}) {
  ...
  if (!runtime) {
    if (options.force) {
      await rm5(teamStateRoot2(cwd, teamName), {
        recursive: true,
        force: true
      }).catch(() => void 0);
      return {
...
async function teamCommand(argv) {
  ...
  if (command === "api") {
    ...
    const result = await executeTeamApiOperation2(
      parsed.operation,
      parsed.input,
      parsed.cwd ?? process.cwd()
    );
