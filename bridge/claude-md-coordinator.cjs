...
function publicOperation(operation) {
  return {
    path: operation.path,
    type: operation.type,
    existedBefore: operation.existedBefore
  };
}
...
function captureTransactionRoot(root, fs) {
  ...
  return {
    alias,
    canonical,
    dev: typeof stat.dev === "number" ? stat.dev : void 0,
    ino: typeof stat.ino === "number" ? stat.ino : void 0
  };
}
function verifyCapturedRoot(root, fs, verifyAlias) {
  if (verifyAlias && (0, import_node_path.resolve)(fs.realpathSync(root.alias)) !== root.canonical)
    throw new Error(`Transaction root changed: ${root.alias}`);
  if ((0, import_node_path.resolve)(fs.realpathSync(root.canonical)) !== root.canonical)
    throw new Error(`Transaction root changed: ${root.canonical}`);
  const stat = fs.lstatSync(root.canonical);
  if (stat.isSymbolicLink() || !stat.isDirectory() || root.dev !== void 0 && stat.dev !== root.dev || root.ino !== void 0 && stat.ino !== root.ino)
...
function cleanCanonical(source) {
  ...
  if (markers.state === "corrupt")
    throw new Error(
      `Canonical source has corrupt OMC markers: ${markers.diagnostics.join(", ")}`
    );
  if (markers.state !== "complete" || markers.managedRanges.length !== 1)
    throw new Error(
      "Canonical source missing required OMC markers or does not contain exactly one complete managed block"
    );
...
function renderManaged(canonical, version) {
  const body = cleanCanonical(canonical).replace(
    /<!-- OMC:VERSION:[^\s]*? -->\r?\n?/g,
    ""
  );
...
function cleanedExisting(content) {
  ...
  if (analysis.markers.state === "corrupt")
    throw new Error(
      `Existing CLAUDE.md has corrupt OMC markers: ${analysis.markers.diagnostics.join(", ")}`
    );
  const imports = importRanges(content).filter(
    (range) => analysis.markers.outsideRanges.some(
      (outside) => range.start >= outside.start && range.end <= outside.end
    )
  );
  const ranges = [
    ...analysis.markers.managedRanges,
    ...analysis.exactMatches,
    ...imports,
    ...generatedHeaderRanges(analysis.markers)
  ];
  return {
    content: removeClaudeMdRanges(content, ranges),
    ranges,
    variants: analysis.exactMatches.map((match) => match.variantId)
  };
}
function mergeForOverwrite(existing, canonical, version) {
  ...
  return {
    content: cleaned.content.length === 0 ? managed : `${managed}
<!-- User customizations -->
${cleaned.content}`,
    ranges: cleaned.ranges,
...
function executeClaudeMdTransaction(request) {
  ...
  try {
    ...
    const canonical = decodeClaudeMdUtf82(
    ...
    if (request.mode !== "local" && companionBytes && parseClaudeMdMarkers(decodeClaudeMdUtf82(companionBytes, companion)).state === "corrupt")
    ...
    if (request.mode === "local")
      operations.push({
        path: main2,
        type: "write",
        existedBefore: !!mainBytes,
        bytes: Buffer.from(overwrite.content, "utf8")
      });
      ...
      const mainContent = mainIsAlreadyOwned && mainBytes !== void 0 ? mainBytes : Buffer.from(
        `${preserve.content}${preserve.content.length ? "\n\n" : ""}${CLAUDE_MD_IMPORT_BLOCK}`,
        "utf8"
      );
      operations.push({
        path: companion,
        type: "write",
        existedBefore: !!companionBytes,
        bytes: Buffer.from(renderManaged(canonical, request.version), "utf8")
      });
      operations.push({
        path: main2,
        type: "write",
    ...
    const states = new Map(
      effectiveOperations.map((operation) => [
        operation.path,
        {
          path: operation.path,
          existedBefore: operation.existedBefore,
          bytes: operation.path === main2 ? mainBytes : companionBytes
        }
      ])
    );
    const appliedMainCleanup = effectiveOperations.some(
      (operation) => operation.path === main2
    );
    const result = {
      ok: false,
      exitCode: 0,
      mode: request.mode,
      operations: effectiveOperations.map(publicOperation),
      completedOperations: [],
      backups: [],
      createdPaths: [],
      deletedPaths: [],
      mutatedPaths: [],
      removedRanges: appliedMainCleanup ? request.mode === "global-preserve" ? preserve.ranges : overwrite.ranges : [],
      removedVariants: appliedMainCleanup ? request.mode === "global-preserve" ? preserve.variants : overwrite.variants : [],
      warnings: [],
      rollback: [],
      tempCleanup: []
    };
    ...
    try {
      ...
      result.failedPath = effectiveOperations.find(
        (operation) => !result.completedOperations.some(
          (done) => done.path === operation.path
        )
      )?.path;
...
var COMPILED_SOURCE_SHA256 = true ? "c4697429f7b0853393bbf6e60be3dde93912bb9c84b754970ad0f87b47012360" : "";
function runClaudeMdCoordinatorHandshake() {
  if (!COMPILED_ENGINE_VERSION || !COMPILED_SOURCE_SHA256) {
    return {
      exitCode: 2,
      response: coordinatorError(
        2,
        "Coordinator build handshake is unavailable"
      )
    };
  }
  return {
    ...
    response: {
      ...
      engineVersion: COMPILED_ENGINE_VERSION,
...
function verifiedSource(pluginRootInput, sourceInput) {
  ...
  if ((0, import_node_fs.lstatSync)(pluginRoot).isSymbolicLink())
  ...
  for (const part of suffix) {
    ...
    if ((0, import_node_fs.lstatSync)(component).isSymbolicLink())
...
function runClaudeMdCoordinator(input) {
  try {
    if (!isObject(input))
      return {
        exitCode: 2,
        response: coordinatorError(2, "Request must be an object")
      };
    ...
    if (Object.keys(input).some((key) => !allowed.has(key)))
      return {
        exitCode: 2,
        response: coordinatorError(2, "Unknown request field")
      };
    const { mode } = input;
    if (input.schemaVersion !== CLAUDE_MD_COORDINATOR_SCHEMA_VERSION || input.engineVersion !== COMPILED_ENGINE_VERSION || mode !== "local" && mode !== "global-overwrite" && mode !== "global-preserve" || typeof input.configRoot !== "string" || typeof input.pluginRoot !== "string" || typeof input.sourcePath !== "string" || typeof input.sourceSha256 !== "string" || typeof input.sourceVersion !== "string")
      return {
        exitCode: 2,
        response: coordinatorError(2, "Invalid coordinator request")
      };
    if (!COMPILED_ENGINE_VERSION || !COMPILED_SOURCE_SHA256)
      return {
        exitCode: 2,
    ...
    if (sourceSha256 !== COMPILED_SOURCE_SHA256 || input.sourceSha256 !== COMPILED_SOURCE_SHA256 || input.sourceVersion !== COMPILED_ENGINE_VERSION)
      return {
        exitCode: 2,
        response: coordinatorError(2, "Canonical source handshake mismatch")
      };
    const result = executeClaudeMdTransaction({
      mode,
      root: input.configRoot,
      source: source.sourcePath,
      sourceRoot: source.pluginRoot,
    ...
    return {
      ...
      response: coordinatorError(
        3,
        `Coordinator I/O validation failed: ${error instanceof Error ? error.message : String(error)}`
      )
    };
  }
}
...
`);
  ...
  try {
    parsed = JSON.parse(
      new TextDecoder("utf-8", { fatal: true }).decode((0, import_node_fs.readFileSync)(0))
    );
  } catch {
    process.stdout.write(
      `${JSON.stringify(coordinatorError(2, "Malformed UTF-8 JSON request"))}
`
    );
    process.exitCode = 2;
    return;
