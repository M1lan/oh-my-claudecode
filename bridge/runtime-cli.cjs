...
function resolveRmuxBinaryPath() {
  ...
  let resolved = null;
  try {
    const result = (0, import_child_process.spawnSync)("rmux", ["-V"], {
      timeout: 5e3,
      stdio: "ignore"
    });
...
function rmuxExec(args, opts) {
  ...
  return (0, import_child_process.execFileSync)(invocation.command, invocation.args, {
    encoding: "utf-8",
    ...execOpts,
    env: resolveEnv(opts)
  });
...
function rmuxShell(command, opts) {
  const { stripTmux: _, ...execOpts } = opts ?? {};
  return (0, import_child_process.execSync)(`${multiplexerShellCommandPrefix()} ${command}`, {
    encoding: "utf-8",
    ...execOpts,
    env: resolveEnv(opts)
  });
...
async function rmuxShellAsync(command, opts) {
  const { stripTmux: _, timeout, ...rest } = opts ?? {};
  return (0, import_util.promisify)(import_child_process.exec)(`${multiplexerShellCommandPrefix()} ${command}`, {
    encoding: "utf-8",
    env: resolveEnv(opts),
    ...timeout !== void 0 ? { timeout } : {},
    ...rest
...
function isTmuxAvailable() {
  try {
    ...
    if (process.platform === "win32") {
      const result = (0, import_child_process.spawnSync)(resolvedBinary, ["-V"], {
        timeout: 5e3,
        shell: true
      });
      return result.status === 0;
...
var import_child_process, import_path, import_util, cachedRmuxBinaryPath;
...
function readWorkspaceMarkerConfig(workspaceRoot) {
  try {
    const raw = (0, import_fs5.readFileSync)(
      (0, import_path8.join)(workspaceRoot, WORKSPACE_MARKER),
      "utf-8"
    ).trim();
    if (!raw) return {};
...
function resolveSuperprojectRoot(cwd) {
  ...
  for (let depth = 0; depth < 32; depth++) {
    let superRoot;
    try {
      superRoot = (0, import_child_process3.execFileSync)(
        "git",
...
function getProjectIdentifier(worktreeRoot) {
  ...
  try {
    const commonDir = (0, import_child_process3.execFileSync)(
      "git",
...
function getDefaultShell() {
  ...
  const name = (0, import_path9.basename)(shell.replace(/\\/g, "/")).replace(
    /\.(exe|cmd|bat)$/i,
    ""
  );
...
function resolveSupportedShellAffinity(shellPath) {
  ...
  const name = (0, import_path9.basename)(shellPath.replace(/\\/g, "/")).replace(
    /\.(exe|cmd|bat)$/i,
    ""
  );
...
function listActiveSessions(teamName) {
  ...
  try {
    const output = rmuxShell("list-sessions -F '#{session_name}'", {
      timeout: 5e3,
      stdio: ["pipe", "pipe", "pipe"]
    });
...
async function killWorkerPanes(opts) {
  ...
  const shutdownPath = (0, import_path9.join)(
    getOmcRoot(cwd),
    "state",
    "team",
    teamName,
...
function readLatestOwnerEpoch(cwd, teamName) {
  ...
  const record = parseRecord(
    (0, import_path18.join)(directory, `${latestEpoch}.json`),
    latestEpoch
  );
...
function publishOwnerEpoch(cwd, teamName, epoch, input = {}) {
  ...
  const temp = (0, import_path18.join)(
    (0, import_path18.dirname)(target),
    `.${epoch}.${record.nonce}.${(0, import_crypto4.randomUUID)()}.tmp`
  );
...
function readLock(path4) {
  try {
    const record = JSON.parse(
      (0, import_node_fs3.readFileSync)(path4, "utf8")
    );
...
async function withProcessIdentityFileLock(lockPath, fn, timeoutMs = 1e4) {
  ...
  (0, import_node_fs3.writeFileSync)(tempPath, JSON.stringify(owner), {
    encoding: "utf8",
    mode: 384,
    flush: true
  });
...
async function readJsonFileState(filePath) {
  try {
    return {
      kind: "value",
      value: JSON.parse(await (0, import_promises5.readFile)(filePath, "utf8"))
    };
  } catch (error) {
...
async function readWorkerStatus(teamName, workerName2, cwd) {
  const data = await readJsonSafe2(
    absPath(cwd, TeamPaths.workerStatus(teamName, workerName2))
  );
  return data ?? { state: "unknown", updated_at: "" };
}
async function readWorkerHeartbeat(teamName, workerName2, cwd) {
  return readJsonSafe2(
    absPath(cwd, TeamPaths.heartbeat(teamName, workerName2))
  );
}
...
async function writeShutdownRequest(teamName, workerName2, fromWorker, cwd) {
  ...
  await writeAtomic(
    absPath(cwd, TeamPaths.shutdownRequest(teamName, workerName2)),
    JSON.stringify(data, null, 2)
  );
}
async function readShutdownAck(teamName, workerName2, cwd, requestedAfter) {
  const ack = await readJsonSafe2(absPath(cwd, TeamPaths.shutdownAck(teamName, workerName2)));
  if (!ack) return null;
...
async function listTasksFromFiles(teamName, cwd) {
  ...
  for (const entry of entries) {
    ...
    if (!match) continue;
    const task = await readJsonSafe2(
...
async function writeWorkerInbox(teamName, workerName2, content, cwd) {
  await writeAtomic(
    absPath(cwd, TeamPaths.inbox(teamName, workerName2)),
    content
  );
}
...
function resolveCanonicalRecoveryRequestId(cwd, requestId) {
  ...
  for (let depth = 0; depth < MAX_RECOVERY_ALIAS_DEPTH; depth += 1) {
    ...
    if (!reservation) {
      if (alias || (0, import_fs22.existsSync)(reservationPath(cwd, currentRequestId)))
        return null;
...
function writeRecoveryPhase(cwd, phase) {
  ...
  return publishImmutable(
    (0, import_path24.join)(phaseDirectory(cwd, phase.request_id), sequence),
    {
      ...phase,
      schema_version: 1,
      kind: "phase",
...
function latestPhase(cwd, requestId) {
  ...
  try {
    ...
    const phase = parseCanonical(
      (0, import_path24.join)(directory, candidates[0])
    );
...
var CURSOR_EXECUTOR_TEAM_ROLE_SET = new Set(
  CURSOR_EXECUTOR_TEAM_ROLES
);
var KNOWN_AGENT_NAME_SET = new Set(KNOWN_AGENT_NAMES);
var TEAM_ROLE_PROVIDERS = /* @__PURE__ */ new Set([
  "claude",
  "codex",
  "gemini",
  "grok",
...
function resolveBinaryPath(binary) {
  ...
  try {
    ...
    const result = (0, import_child_process2.spawnSync)(resolver, [binary], {
      timeout: 5e3,
      encoding: "utf8"
    });
...
function generateWorkerOverlay(params) {
  ...
  const claimTaskCommand = formatOmcCliInvocation(
    `team api claim-task --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"worker\\":\\"${workerName2}\\"}" --json`
  );
  const sendAckCommand = formatOmcCliInvocation(
    `team api send-message --input "{\\"team_name\\":\\"${teamName}\\",\\"from_worker\\":\\"${workerName2}\\",\\"to_worker\\":\\"leader-fixed\\",\\"body\\":\\"ACK: ${workerName2} initialized\\"}" --json`
  );
  ...
  const releaseClaimCommand = formatOmcCliInvocation(
    `team api release-task-claim --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"claim_token\\":\\"<claim_token>\\",\\"worker\\":\\"${workerName2}\\"}" --json`
  );
  const mailboxListCommand = formatOmcCliInvocation(
    `team api mailbox-list --input "{\\"team_name\\":\\"${teamName}\\",\\"worker\\":\\"${workerName2}\\"}" --json`
  );
  const mailboxDeliveredCommand = formatOmcCliInvocation(
    `team api mailbox-mark-delivered --input "{\\"team_name\\":\\"${teamName}\\",\\"worker\\":\\"${workerName2}\\",\\"message_id\\":\\"<id>\\"}" --json`
  );
  const checkpointTaskCommand = formatOmcCliInvocation(
    `team api write-task-checkpoint --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"worker\\":\\"${workerName2}\\",\\"claim_token\\":\\"<claim_token>\\",\\"task_version\\":<current_task_version>,\\"sequence\\":<next_sequence>,\\"resume_payload\\":<safe_boundary_json>}" --json`
  );
  const teamApiCommand = formatOmcCliInvocation("team api");
...
async function composeInitialInbox(teamName, workerName2, content, cwd, cliOutputContract) {
  const inboxPath = (0, import_path12.join)(
    cwd,
    `.omc/state/team/${teamName}/workers/${workerName2}/inbox.md`
  );
...
async function ensureWorkerStateDir(teamName, workerName2, cwd) {
  const workerDir = (0, import_path12.join)(
    cwd,
    `.omc/state/team/${teamName}/workers/${workerName2}`
  );
...
async function writeWorkerOverlay(params) {
  ...
  const overlayPath = (0, import_path12.join)(
    cwd,
    `.omc/state/team/${teamName}/workers/${workerName2}/AGENTS.md`
  );
...
function isWorktreeDirtyExcept(wtPath, ignoredRootPaths = []) {
  try {
    ...
    const entries = (0, import_node_child_process.execFileSync)("git", ["status", "--porcelain"], {
      cwd: wtPath,
      encoding: "utf-8",
      stdio: "pipe",
      windowsHide: true
...
function getRootAgentsBackupPath(repoRoot, teamName, workerName2) {
  return (0, import_node_path2.join)(
    getWorkerStateDir(repoRoot, teamName, workerName2),
...
function readRootAgentsBackup(repoRoot, teamName, workerName2) {
  ...
  try {
    return JSON.parse(
      (0, import_node_fs2.readFileSync)(backupPath, "utf-8")
    );
  } catch (err) {
  ...
  for (const metaPath of paths) {
    ...
    try {
      const entries = JSON.parse(
        (0, import_node_fs2.readFileSync)(metaPath, "utf-8")
      );
...
function listRootAgentsBackupIssues(repoRoot, teamName, entries) {
  const workersDir = (0, import_node_path2.join)(
    getOmcRoot(repoRoot),
    "state",
    "team",
    sanitizeName(teamName),
  ...
  for (const workerName2 of (0, import_node_fs2.readdirSync)(workersDir)) {
    const backupPath = (0, import_node_path2.join)(
      workersDir,
      workerName2,
      "worktree-root-agents.json"
    );
    ...
    try {
      ...
      issues.push({
        path: backupPath,
        message: `worktree_root_agents_backup_unreadable:${workerName2}:${message}`
      });
      continue;
    }
...
function writeMetadata(repoRoot, teamName, entries) {
  ...
  ensureDirWithMode(
    (0, import_node_path2.join)(getOmcRoot(repoRoot), "state", "team", sanitizeName(teamName))
  );
...
function forgetMetadataUnlocked(repoRoot, teamName, workerName2) {
  const existing = readMetadata(repoRoot, teamName).filter(
    (entry) => entry.workerName !== workerName2
  );
...
function ensureWorkerWorktree(teamName, workerName2, repoRoot, options = {}) {
  ...
  try {
    (0, import_node_child_process.execFileSync)("git", ["worktree", "prune"], {
      cwd: repoRoot,
      stdio: "pipe",
      windowsHide: true
    });
  ...
  const wtDir = (0, import_node_path2.join)(
    getOmcRoot(repoRoot),
    "team",
    sanitizeName(teamName),
    "worktrees"
  ...
  (0, import_node_child_process.execFileSync)("git", args, {
    cwd: repoRoot,
    stdio: "pipe",
    windowsHide: true
  });
...
function checkWorkerWorktreeRemovalSafety(teamName, workerName2, repoRoot, worktreePath) {
  ...
  validateWorktreeRemovalTarget({
    ...
    expectedRoots: [
      (0, import_node_path2.join)(getOmcRoot(repoRoot), "team", sanitizeName(teamName), "worktrees")
    ],
...
function prepareWorkerWorktreeForRemoval(teamName, workerName2, repoRoot, worktreePath) {
  ...
  if (agentsRestore.reason === "agents_dirty") {
    const error = new Error(
      `agents_dirty: preserving modified worktree root AGENTS.md at ${(0, import_node_path2.join)(wtPath, "AGENTS.md")}`
    );
    error.code = "agents_dirty";
    throw error;
  }
...
function removeWorkerWorktree(teamName, workerName2, repoRoot) {
  ...
  withFileLockSync(metaLockPath, () => {
    ...
    try {
      (0, import_node_child_process.execFileSync)("git", ["worktree", "remove", wtPath], {
        cwd: repoRoot,
        stdio: "pipe",
        windowsHide: true
      });
    ...
    try {
      (0, import_node_child_process.execFileSync)("git", ["branch", "-D", branch], {
        cwd: repoRoot,
        stdio: "pipe",
        windowsHide: true
      });
...
function failureSidecarPath(teamName, taskId, cwd) {
  return (0, import_path14.join)(
    canonicalTasksDir(teamName, cwd),
    `${sanitizeTaskId(taskId)}.failure.json`
  );
}
  ...
  for (let i = 0; i < runtime.config.tasks.length; i++) {
    ...
    if (!task) {
      for (let attempt = 1; attempt < transientReadRetryAttempts; attempt++) {
        await new Promise(
          (resolve8) => setTimeout(resolve8, transientReadRetryDelayMs)
        );
  ...
  try {
    ...
    for (const f of taskFiles.filter((f2) => f2.endsWith(".json"))) {
      const task = await readJsonSafe(
        (0, import_path15.join)(root, "tasks", f)
      );
...
async function withDispatchLock(teamName, cwd, fn) {
  ...
  while (true) {
    try {
      ...
      await new Promise(
        (resolve8) => setTimeout(resolve8, Math.floor(pollMs * jitter))
      );
  ...
  for (const taskId of [...taskIds].sort()) {
    const lock = await deps.withTaskClaimLock(
      ...
      async () => {
        ...
        if (task.status !== "pending" || task.owner || task.claim || reservation.recovery_id !== proof.recoveryId || reservation.request_id !== proof.requestId || reservation.replacement_worker !== workerName2 || reservation.replacement_generation !== proof.replacementGeneration || !deps.verifyAdoptionToken(
          proof.adoptionToken,
        ...
        const updated = {
          ...
          recovery_adoption: {
            ...
            replacement_worker: workerName2,
...
async function readCheckpoint(path4) {
  ...
  try {
    const checkpoint = parseCheckpoint(
      JSON.parse(await (0, import_promises8.readFile)(path4, "utf8"))
    );
...
async function teamReadConfig(teamName, cwd) {
  ...
  if (!config && (0, import_node_fs5.existsSync)(configPath))
    throw new Error("invalid_persisted_state");
  if (config && typeof config.state_revision === "number" && Number.isSafeInteger(config.state_revision)) {
    return canonicalizeTeamConfigWorkers(config);
  }
  if (!manifest && (0, import_node_fs5.existsSync)(manifestPath))
    throw new Error("invalid_persisted_state");
...
function recoveryTransitionDeps(teamName, cwd) {
  return {
    ...
    readRecoverySidecar: async (tn, recoveryId, tid, c) => {
      ...
      try {
        return JSON.parse(
          await (0, import_promises9.readFile)(path4, "utf8")
        );
      } catch {
        return "malformed";
      }
    },
    writeRecoverySidecar: (tn, recoveryId, tid, sidecar, c) => writeAtomic2(
...
var CURSOR_EXECUTOR_TEAM_ROLE_SET2 = new Set(
  CURSOR_EXECUTOR_TEAM_ROLES
);
...
function mergeWorkerBranch(workerBranch, baseBranch, repoRoot) {
  ...
  try {
    ...
    try {
      (0, import_node_child_process3.execFileSync)("git", ["merge", "--abort"], {
        cwd: repoRoot,
        stdio: "pipe",
        windowsHide: true
      });
...
function startFallbackPoller(worktreePath, workerName2, opts) {
  ...
  const watcher = (0, import_fs21.watch)(
    worktreePath,
    { recursive: true },
    (eventType, filename) => {
      if (stopped) return;
...
async function uninstallCommitCadence(ctx, io = {
  readFile: import_promises11.readFile,
  writeFile: import_promises11.writeFile
}) {
...
function gitPath(worktreePath, gitPathName) {
  try {
    const resolved = (0, import_node_child_process4.execFileSync)(
      "git",
  ...
  if ((0, import_node_fs7.existsSync)(persistedPath)) {
    try {
      ...
      persisted = JSON.parse(
        readFileSync16(persistedPath, "utf-8")
      );
    } catch {
      persisted = { lastShas: {} };
    }
  ...
  async function fanOutRebase(triggeringWorker) {
    for (const other of workers.values()) {
      ...
      try {
        ...
        const baseSha = (() => {
          try {
            return (0, import_node_child_process4.execFileSync)(
              "git",
  ...
  while (Date.now() < deadline) {
    try {
      const value = JSON.parse(
        await (0, import_promises13.readFile)(path4, "utf8")
      );
...
async function runWorkerActivationGate(gate) {
  ...
  if (!await waitForRecoveryGateRecord(
    gate.activatePath,
    expected,
    timeoutMs,
    pollIntervalMs
  ...
  await writeAtomic3(`${gate.readyPath}.adoption-ready`, {
    ...expected,
    written_at: (/* @__PURE__ */ new Date()).toISOString()
  });
  if (!await waitForRecoveryGateRecord(
  ...
  const completion = new Promise((resolve8) => {
    child.once(
      "exit",
      (exitCode, signal) => resolve8({ outcome: "ran", exitCode, signal })
    );
...
async function reconcileCommittedTeamServices(config, cwd) {
  ...
  try {
    ...
    return cadenceRemovalsConverged && !staleOrchestratorRemovalFailed && exactCadence && local.registeredWorkers.size === expectedWorkers.size && [...expectedWorkers].every(
      (workerName2) => local.registeredWorkers.has(workerName2)
...
async function hasWorkerTaskClaimEvidence(teamName, workerName2, cwd, taskId) {
  try {
    const raw = await (0, import_promises14.readFile)(
  ...
  const dispatchOutcome = await queueInboxInstruction({
    ...
    notify: async (_target, triggerMessage) => {
      ...
      if (opts.agentType === "gemini") {
        const confirmed = await notifyPaneWithRetry2(
          opts.sessionName,
          paneId,
          "1"
        );
...
function selectRecoveryReplayTasks(tasks, workerName2, recoveryId, committedPaneLiveness) {
  return tasks.filter(
    (task) => task.recovery_reservation?.recovery_id === recoveryId || task.recovery_adoption?.recovery_id === recoveryId || (committedPaneLiveness === null || committedPaneLiveness === "dead") && task.status === "in_progress" && task.owner === workerName2
  );
}
...
async function readOrCreateRecoveryAttempt(input, recoveryId, replacementGeneration) {
  ...
  try {
    return validateRecoveryAttemptSecret(
      JSON.parse(await (0, import_promises14.readFile)(path4, "utf8")),
      input,
      recoveryId,
...
async function hasBootstrapRecoveryEvidence(teamName, cwd, input, waitOptions = {}) {
  ...
  try {
    const intent = parseRecoveryIntent(
      await (0, import_promises14.readFile)(
...
async function rollbackUnpersistedNativeWorktreeStartup(teamName, cwd, cause) {
  ...
  const writeFailureMarker = async (extra = {}) => {
    ...
    await (0, import_promises14.writeFile)(
      (0, import_path25.join)(teamRoot, "startup-failure.json"),
      JSON.stringify(
        {
...
`
  ...
  await (0, import_promises14.mkdir)(absPath(leaderCwd, TeamPaths.tasks(sanitized)), {
    recursive: true
  });
  await (0, import_promises14.mkdir)(absPath(leaderCwd, TeamPaths.workers(sanitized)), {
    recursive: true
  });
  await (0, import_promises14.mkdir)(
    (0, import_path25.join)(getOmcRoot(leaderCwd), "state", "team", sanitized, "mailbox"),
    { recursive: true }
  );
...
async function shutdownTeamV2(teamName, cwd, options = {}) {
  ...
  const lifecycleLock = absPath(
    cwd,
    TeamPaths.recoveryLifecycleLock(workspaceHash, sanitized)
  );
...
async function refreshRuntimeWorkerPaneIds(runtime, teamName, cwd) {
  ...
  if (!current) return null;
  const authoritativePaneIds = current.config.workers.map((worker) => worker.pane_id).filter(
    (paneId) => typeof paneId === "string" && paneId.length > 0
  );
  runtime.workerPaneIds = [
    .../* @__PURE__ */ new Set([...runtime.workerPaneIds, ...authoritativePaneIds])
  ];
  return {
...
function classifyAllDeadRecoveryEvidence(refresh, workers, hasOutstanding) {
  ...
  if (!refresh.allWorkerPaneIdsKnown || refresh.authoritativePaneIds.length === 0 || workers.length !== refresh.authoritativePaneIds.length)
    return "unknown";
...
async function handleRecoverDeadWorkerV2Owner(input, execute = executeRecoverDeadWorkerV2Owner) {
  ...
  const path4 = absPath(
    input.cwd,
    TeamPaths.recoveryIntent(input.teamName, reservation.recovery_id)
  );
  const intent = validateCanonicalRecoveryIntent(
    input.teamName,
    input.cwd,
...
async function processPendingRecoveryIntents(teamName, cwd, execute = handleRecoverDeadWorkerV2Owner) {
  ...
  for (const name of names) {
    ...
    try {
      ...
      process.stderr.write(
        `[runtime-cli/v2] recovery intent ${name} failed: ${error}
`
      );
    }
  }
...
async function updateAllDeadRecoveryGrace(teamName, cwd, evidence, nowMs = Date.now()) {
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    const existingDeadline = Date.parse(
      current.config.all_dead_recovery?.deadline_at ?? ""
    );
    if (evidence === "unknown") {
      return {
        deadlineAt: Number.isFinite(existingDeadline) ? existingDeadline : null,
        expired: false
      };
    }
    ...
    if (await saveTeamConfigAtRevision(nextConfig, current.stateRevision, cwd)) {
      return {
        deadlineAt: evidence === "all_dead" ? deadlineAt : null,
        expired: false
      };
    }
  }
...
function hasVerifiedTerminalRepairForMalformedIntent(teamName, cwd, recoveryId, path4) {
  try {
    ...
    if (raw.team_name !== teamName || raw.recovery_id !== recoveryId || typeof raw.request_id !== "string" || !isSafeRecoveryRequestId(raw.request_id) || typeof raw.worker_name !== "string" || raw.worker_name.length === 0)
      return false;
...
function hasPendingRecoveryIntentBeforeDeadline(teamName, cwd, deadlineAt) {
  ...
  for (const name of names) {
    ...
    try {
      ...
      if (malformedIntentMayPredateDeadline(path4, deadlineAt) && !hasVerifiedTerminalRepairForMalformedIntent(
        teamName,
        cwd,
        recoveryId,
        path4
...
function hasPendingRecoveryAdmissionBeforeDeadline(teamName, cwd, deadlineAt) {
  ...
  for (const name of names) {
    ...
    try {
      ...
      if (reservation.team_name !== teamName || reservation.workspace_hash !== workspaceHash || Date.parse(reservation.created_at) > deadlineAt)
        continue;
...
async function fenceAllDeadRecoveryExpiry(teamName, cwd, deadlineAt) {
  const workspaceHash = (0, import_node_crypto7.createHash)("sha256").update(cwd).digest("hex");
  return withProcessIdentityFileLock(
    absPath(cwd, TeamPaths.recoveryLifecycleLock(workspaceHash, teamName)),
    async () => {
      const current = await readRevisionedTeamConfig(teamName, cwd);
      if (!current || Date.parse(current.config.all_dead_recovery?.deadline_at ?? "") !== deadlineAt || Date.now() < deadlineAt || current.config.lifecycle_state === "shutting_down" || current.config.lifecycle_state === "stopped")
        return false;
      if (hasPendingRecoveryAdmissionBeforeDeadline(teamName, cwd, deadlineAt) || hasPendingRecoveryIntentBeforeDeadline(teamName, cwd, deadlineAt))
        return false;
      const nextRevision = current.stateRevision + 1;
...
function ownsPersistentRecoveryFence(input, fence, expectedEpoch) {
  if (expectedEpoch !== void 0 && fence.epoch !== expectedEpoch)
    return false;
...
async function runPersistentRecoveryOwnerLoop(input, options = {}) {
  ...
  while (options.shouldContinue?.(iteration) ?? true) {
    ...
    try {
      ...
      process.stderr.write(
        `[runtime-cli/v2] recovery owner config maintenance failed: ${error}
`
      );
    ...
    if (current.config.lifecycle_state === "shutting_down") {
      try {
        ...
        process.stderr.write(
          `[runtime-cli/v2] recovery owner terminal cleanup failed: ${error}
`
        );
      }
      iteration += 1;
    ...
    if (bootstrapPending) {
      ...
      try {
        await execute(input);
        const afterBootstrap = await readRevisionedTeamConfig(
          input.teamName,
          input.cwd
        );
        ...
        process.stderr.write(
          `[runtime-cli/v2] recovery owner bootstrap intent failed: ${error}
`
        );
      }
    }
    try {
      ...
      process.stderr.write(
        `[runtime-cli/v2] recovery owner service maintenance failed: ${error}
`
      );
    }
    try {
      ...
      process.stderr.write(
        `[runtime-cli/v2] recovery owner intent maintenance failed: ${error}
`
      );
    }
    let afterIntents;
...
`
    ...
    const refresh = {
      authoritativePaneIds: panes,
      allWorkerPaneIdsKnown: panes.length === afterIntents.config.workers.length
    };
    let snapshot = null;
    try {
      ...
      process.stderr.write(
        `[runtime-cli/v2] recovery owner monitor maintenance failed: ${error}
`
      );
    }
    if (snapshot) {
      const outstanding = snapshot.tasks.pending + snapshot.tasks.in_progress > 0;
      const evidence = classifyAllDeadRecoveryEvidence(
        refresh,
        snapshot.workers,
        outstanding
      );
      try {
        const grace = await updateAllDeadRecoveryGrace(
          input.teamName,
          input.cwd,
          evidence
        );
        if (evidence === "all_dead" && grace.expired && grace.deadlineAt !== null) {
          await fenceAllDeadRecoveryExpiry(
            input.teamName,
            input.cwd,
            grace.deadlineAt
          );
        ...
        process.stderr.write(
          `[runtime-cli/v2] recovery owner all-dead maintenance failed: ${error}
`
        );
      }
    }
...
async function checkWatchdogFailedMarker(stateRoot2, startTime) {
  ...
  if (failedAt >= startTime) {
    return {
      failed: true,
      reason: `Watchdog marked team failed at ${new Date(failedAt).toISOString()}`
    };
  }
  try {
...
async function writeResultArtifact(output, finishedAt, jobId = process.env.OMC_JOB_ID, omcJobsDir = process.env.OMC_JOBS_DIR) {
  ...
  await (0, import_promises15.writeFile)(tmpPath, JSON.stringify({ ...output, finishedAt }), "utf-8");
...
`
  ...
  try {
    ...
    process.stderr.write(
      `[runtime-cli] ${err instanceof Error ? err.message : String(err)}
`
    );
    process.exit(1);
  }
  ...
  async function doShutdown(status) {
    ...
    const output = await finalizeRuntimeShutdown(
      ...
      async () => {
        ...
        try {
          if (useV2) {
            await shutdownTeamV2(runtime.teamName, runtime.cwd, {
              force: true
            });
          } else {
            await shutdownTeam(
...
`);
      ...
      async (publishedOutput) => {
        ...
        try {
          ...
          process.stderr.write(
            `[runtime-cli] Failed to persist result artifact: ${err}
`
          );
        }
      }
...
`);
  ...
  try {
    await writePanesFile(
      jobId,
      runtime.workerPaneIds,
      runtime.leaderPaneId,
      runtime.sessionName,
      Boolean(runtime.ownsWindow)
    );
  } catch (err) {
  ...
  if (useV2) {
    ...
    while (pollActive) {
      ...
      try {
        ...
        process.stderr.write(
          `[runtime-cli/v2] Failed to read authoritative pane evidence: ${err}
`
        );
        continue;
      }
      if (!paneRefresh) {
        process.stderr.write(
          "[runtime-cli/v2] Authoritative pane evidence missing; preserving team state\n"
        );
        continue;
      }
      let snap;
      ...
      const leaderGuidance = deriveTeamLeaderGuidance({
        ...
        workers: {
          ...
          idle: snap.workers.filter(
            (worker) => worker.alive && (worker.status.state === "idle" || worker.status.state === "done")
          ).length,
...
`
      ...
      if (leaderGuidance.nextAction !== "keep-checking-status" && leaderGuidance.reason !== lastLeaderNudgeReason) {
        await appendTeamEvent(
          ...
          cwd
        ).catch(logLeaderNudgeEventFailure);
...
`
      ...
      if (evidence === "all_dead" && grace.expired && grace.deadlineAt !== null && await fenceAllDeadRecoveryExpiry(teamName, cwd, grace.deadlineAt)) {
        process.stderr.write(
          "[runtime-cli/v2] All-worker recovery grace expired\n"
        );
        await doShutdown("failed");
        return;
      }
...
`);
    ...
    try {
      ...
      process.stderr.write(
        `[runtime-cli] Failed to persist pane IDs: ${err}
`
      );
    }
    process.stderr.write(
...
`
    ...
    if (allDeadWithWork) {
      ...
      if (Date.now() - allDeadSince >= 3e5) {
        process.stderr.write(
          "[runtime-cli] All-worker recovery grace expired\n"
        );
        exitWithoutShutdown("failed");
        return;
      }
...
async function runRecoveryOwnerFromEnvironment() {
  ...
  if (typeof input.teamName !== "string" || typeof input.cwd !== "string" || typeof input.workerName !== "string" || typeof input.requestId !== "string")
  ...
  const predecessorEpoch = Number(
    process.env.OMC_RECOVERY_OWNER_PREDECESSOR_EPOCH
  );
