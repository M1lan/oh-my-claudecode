...
function readWorkspaceMarkerConfig(workspaceRoot) {
  try {
    const raw = (0, import_fs2.readFileSync)(
      (0, import_path3.join)(workspaceRoot, WORKSPACE_MARKER),
      "utf-8"
    ).trim();
    if (!raw) return {};
...
function resolveTaskPathForRead(teamName, taskId, cwd) {
  const canonical = (0, import_path7.join)(
    canonicalTasksDir(teamName, cwd),
    `${sanitizeTaskId(taskId)}.json`
  );
  if ((0, import_fs5.existsSync)(canonical)) return canonical;
  const legacy = (0, import_path7.join)(
    legacyTasksDir(teamName),
    `${sanitizeTaskId(taskId)}.json`
  );
...
function updateTask(teamName, taskId, updates, opts) {
  ...
  if (!handle) {
    throw new Error(
      `Cannot acquire lock for task ${taskId}: another process holds the lock`
    );
  }
  try {
    doUpdate();
...
async function findNextTask(teamName, workerName, opts) {
  ...
  for (const id of taskIds) {
    ...
    const handle = acquireTaskLock(teamName, id, {
      workerName,
      cwd: opts?.cwd
    });
    if (!handle) continue;
...
function listTaskIds(teamName, opts) {
  const scanDir = (dir) => {
    ...
    try {
      return (0, import_fs5.readdirSync)(dir).filter(
        (f) => f.endsWith(".json") && !f.includes(".tmp.") && !f.includes(".failure.") && !f.endsWith(".lock")
...
function configPath(teamName) {
  const result = (0, import_path9.join)(
    getClaudeConfigDir(),
    "teams",
...
function peekRecentOutboxMessages(teamName, workerName, maxMessages = 10) {
  ...
  const outboxPath2 = (0, import_path13.join)(
    getClaudeConfigDir(),
    "teams",
    safeName,
    "outbox",
...
function getTeamStatus(teamName, workingDirectory, heartbeatMaxAgeMs = 3e4, options) {
  ...
  const workers = mcpWorkers.map((w) => {
    ...
    const failed = workerTasks.filter(
      (t) => t.status === "failed" || t.status === "completed" && t.metadata?.permanentlyFailed === true
    ).length;
    const completedClean = workerTasks.filter(
...
function main() {
  ...
  if (!validateConfigPath(configPath2, home, claudeConfigDir)) {
    console.error(
      `Config path must be under ~/ with ${claudeConfigDir} or ~/.omc/ subpath: ${configPath2}`
    );
    process.exit(1);
  }
  let config;
